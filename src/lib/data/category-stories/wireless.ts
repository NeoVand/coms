...
export const wirelessStory: CategoryStory = {
	categoryId: 'wireless',
	tagline:
		'Bits through the air — from ALOHAnet to {{wifi-8|Wi-Fi 8}}, from a 1994 Ericsson headset cable to billions of AirPods',
	sections: [
		{
			type: 'narrative',
...
Two technologies broke through. **[[wifi|Wi-Fi]]** ({{ieee-802-15-4|IEEE}} [[wifi|802.11]], 1997) took [[ethernet|Ethernet]]'s shared-medium model — {{csma-cd|CSMA/CD}} on coax — and adapted it for radio: {{csma-ca|CSMA/CA}} (Collision *Avoidance*, because radios can't detect collisions while transmitting), {{rts-cts|RTS/CTS}} handshakes for hidden terminals, {{encryption|encryption}} built in because the air can't be physically secured. Today it carries most consumer traffic.
...
Together — Wi-Fi for local broadband, Bluetooth for personal-area, Cellular for wide-area — the three cover every wireless surface from streaming 4K video to a hearing aid sipping power from a coin cell to a phone connecting to a {{starlink|Starlink}} satellite 600 km overhead.`
		},
		{
			...
			people: [
				...
				},
				{
					...
					contribution:
						'Chaired the {{ieee-802-15-4|IEEE}} [[wifi|802.11]] working group from 1990 to 2002, shepherding the wireless {{lan|LAN}} standard from concept to global adoption. Known as the "Father of [[wifi|Wi-Fi]]" for his persistence in driving consensus across a fractious vendor ecosystem.'
				},
				{
					name: 'Jaap Haartsen',
					years: '1963–',
				...
				},
				{
					...
					org: 'Intel',
					contribution:
						'Proposed the name "[[bluetooth|Bluetooth]]" at a 1997 {{sig|SIG}} meeting after Harald "Blåtand" Gormsson, the 10th-century Danish king who united Denmark and Norway — analogous to the {{sig|SIG}}\'s goal of uniting Ericsson, {{ibm|IBM}}, {{intel|Intel}}, Nokia, and Toshiba behind one short-range wireless standard. The name was supposed to be a placeholder. It stuck.'
				}
			]
		},
		{
			...
			entries: [
				...
				},
				{
					...
					description:
						"Radiolinja launches the world's first commercial {{gsm|GSM}} network in Finland — the 2G digital standard that European carriers built and that became the foundation of [[cellular|cellular]] worldwide. The {{cdma|CDMA}}-vs-{{gsm|GSM}} standards war won't resolve until {{wcdma|WCDMA}} inside {{umts|UMTS}} in the early 2000s.",
					protocolId: 'cellular'
				},
				{
					year: 1994,
				...
				},
				{
					...
					description:
						'The original [[wifi|802.11]] standard ratifies 2 Mbps wireless {{lan|LAN}}. Slow and expensive — but proves wireless networking is viable. The Wi-Fi Alliance forms two years later to certify interoperability.',
					protocolId: 'wifi'
				},
				{
					year: 1998,
					...
					description:
						'Ericsson, {{ibm|IBM}}, {{intel|Intel}}, Nokia, and Toshiba sign the [[bluetooth|Bluetooth]] Special Interest Group charter in May 1998 to standardise short-range wireless. The first commercial product (a hands-free headset) ships at COMDEX 1999.',
					protocolId: 'bluetooth'
				},
				{
					year: 2009,
				...
				},
				{
					...
					description:
						"{{3gpp|3GPP}} freezes Release 8 in December 2008. **Long Term Evolution** abandons {{wcdma|WCDMA}}'s spreading codes for an {{ofdma|OFDMA}} + SC-FDMA air interface — the clean-sheet radio design that scales linearly with {{spectrum|spectrum}} width. The architectural substrate every {{5g-nr|5G NR}} design choice is later evolved from.",
					protocolId: 'cellular'
				},
				{
					year: 2018,
				...
				},
				{
					...
					description:
						'Adopted 3 September 2024. Phase-based + RTT distance measurement on a new {{le-audio|LE}} 2M 2BT {{phy|PHY}} — centimetre-class accuracy up to ~150 m. The protocol-level answer to [[uwb|UWB]] for digital keys, anti-stalking, and finder applications.',
					protocolId: 'bluetooth'
				},
				{
					year: 2025,
					...
					description:
						'{{ieee-802-15-4|IEEE}} 802.11be ratified 22 July 2025: 320-MHz channels in 6 GHz, 4096-{{qam|QAM}}, Multi-Link Operation, preamble puncturing. [[wifi|Wi-Fi]] 8 (802.11bn / Ultra High Reliability) is in draft for 2028 — not faster, but 25% better at the 95th percentile.',
					protocolId: 'wifi'
				},
				{
					year: 2026,
					...
					description:
						'On 28 January 2026 Frankfurt Airport became the first airport to {{broadcast|broadcast}} all gate announcements over **{{auracast|Auracast}}** — {{lc3|LC3}}-based one-to-many {{le-audio|LE Audio}}. The first real-world replacement for analog hearing loops.',
					protocolId: 'bluetooth'
				},
				{
					year: 2025,
		...
		},
		{
			...
			text: `Three are already in this category, four more are queued for the next research wave:\n\n1. **Wi-Fi** ([[wifi|802.11]]) — universal local broadband. *Shipped.*\n2. **Bluetooth** ({{br-edr|BR/EDR}} + {{ble|BLE}}) — personal-area + IoT commissioning. *Shipped.*\n3. **Cellular** (4G {{lte|LTE}} + {{5g-nr|5G NR}}) — wide-area, licensed, mobile. ~9B subs. *Shipped.*\n4. **[[nfc|NFC]]** ({{iso-iec|ISO/IEC}} 18092) — contactless payment + {{transit|transit}} cards. *Queued.*\n5. **[[uwb|UWB]]** ({{ieee-802-15-4|IEEE}} 802.15.4z) — sub-decimetre ranging, AirTag Precision Finding, {{ccc-digital-key|CCC Digital Key}}. *Queued.*\n6. **{{thread|Thread}}** ({{ieee-802-15-4|IEEE 802.15.4}} + 6LoWPAN) — the IPv6-native smart-home mesh under {{matter|Matter}}. *Coming with {{matter|Matter}}+{{thread|Thread}} bundle.*\n7. **LoRaWAN** — sub-GHz {{lpwan|LPWAN}} for metering, agriculture, smart cities; 125M+ devices deployed by end-2025. *Queued.*\n8. **Zigbee** ({{sig-bluetooth-acronym|CSA}}) — the legacy mesh that still runs the Philips Hue installed base. *Queued.*\n\n**Sidebars** rather than full pages: NB-IoT / {{lte|LTE}}-M (inside the Cellular page), Z-Wave (inside Zigbee/{{thread|Thread}}), GNSS / NMEA 0183 (positioning, not a network). **Callouts only**: DECT NR+, WirelessHART/ISA100.11a, {{broadcast|broadcast}} (AM/FM/DAB+/ATSC 3.0), passive {{rfid|RFID}}, IrDA.`
		},
		{
			type: 'callout',
			title: 'Spectrum at a glance',
			text: `Every wireless protocol picks a band, and the band picks the trade-offs:\n\n- **Sub-GHz (433/868/915 MHz)** — long range, low data rate. Unlicensed in most regions. *LoRaWAN, Sigfox, Z-Wave, NB-IoT in 700 MHz.*\n- **2.4 GHz {{ism-band|ISM}}** — global, unlicensed, crowded. *Wi-Fi b/g/n/ax, Bluetooth {{br-edr|BR/EDR}} + {{ble|BLE}}, Zigbee, {{thread|Thread}}, microwave-oven leakage, baby monitors.* Modern combo chips ({{apple|Apple}} H-series, {{broadcom|Broadcom}}, Qualcomm) do time-division arbitration to keep them all alive on one antenna.\n- **5 GHz / 6 GHz** — fast, less crowded. *Wi-Fi a/n/ac/ax/be (6E and 7).* The 6 GHz expansion in the US ({{fcc|FCC}} 2020) and EU (CEPT 2021) added 1200 MHz of unlicensed {{spectrum|spectrum}} — the biggest single {{bandwidth|bandwidth}} grant in 20 years.\n- **24–52 GHz {{mmwave|mmWave}}** — line-of-sight, gigabits. *Wi-Fi WiGig, {{5g-nr|5G NR}} FR2.* Loses 20 dB on a wet leaf; deployed mostly in stadiums and dense urban hotspots.\n- **Licensed cellular (600 MHz – 3.7 GHz mid-band)** — predictable QoS, carrier-operated. *4G {{lte|LTE}} FR1, {{5g-nr|5G NR}} FR1.* {{spectrum|Spectrum}} auctioned for billions.\n- **[[uwb|UWB]] (6–8.5 GHz)** — wide {{bandwidth|bandwidth}}, ultra-low power, sub-decimetre ranging. *{{fira|FiRa Consortium}}, {{ieee-802-15-4|IEEE}} 802.15.4z.*\n- **Satellite L/S/Ka band** — global coverage. *{{starlink|Starlink}} {{direct-to-cell|Direct-to-Cell}} on n25/n26, AST SpaceMobile, {{apple|Apple}} Globalstar Emergency SOS.*`
		},
		{
			type: 'narrative',
			title: 'Cutting the Cord — How Wi-Fi Works',
			text: `[[wifi|Wi-Fi]] brought [[ethernet|Ethernet]]'s shared-medium model to the airwaves, but radio introduced challenges that cables never had. The wireless medium is shared — you can't run a dedicated cable to each device — so [[wifi|Wi-Fi]] uses **{{csma-ca|CSMA/CA}}** (Collision *Avoidance*) instead of {{csma-cd|CSMA/CD}}: devices announce intent to transmit and wait for clear {{airtime|airtime}} rather than detecting collisions after the fact.

An [[wifi|802.11]] frame carries three or four {{mac-address|MAC}} addresses (receiver, transmitter, destination, and sometimes source) compared to [[ethernet|Ethernet]]'s two. The {{access-point|access point}} bridges between worlds: it receives encrypted [[wifi|Wi-Fi]] frames from wireless clients, decrypts and strips the [[wifi|802.11]] header, then wraps the {{payload|payload}} in a standard [[ethernet|Ethernet]] frame for the wired network. This seamless bridging is why your laptop doesn't care whether it's plugged in or on [[wifi|Wi-Fi]] — [[ip|IP]] works the same either way.`
		},
		{
			type: 'diagram',
  ...
  subgraph EthFrame["[[ethernet|Ethernet]] Frame (Layer 2 — Wired)"]
    E1["Dst {{mac-address|MAC}} — 6 bytes"]
    E2["Src {{mac-address|MAC}} — 6 bytes"]
  ...
  EthFrame ~~~ WiFiFrame`,
			caption:
				'[[ethernet|Ethernet]] frames use two {{mac-address|MAC}} addresses and are sent in the clear. [[wifi|Wi-Fi]] frames need three or four addresses (receiver, transmitter, destination, and optionally source) and encrypt the {{payload|payload}} — reflecting the complexity of shared airwaves vs dedicated cables.'
		},
		{
			type: 'narrative',
			title: 'Bluetooth — Personal Area, Personal Power Budget',
			text: `Where [[wifi|Wi-Fi]] reaches across rooms at hundreds of megabits, [[bluetooth|Bluetooth]] is *personal*. Its design lives within 10 metres and microamps. **{{br-edr|BR/EDR}}** ("Classic") hops 1,600 times per second across 79 × 1 MHz channels in the 2.4 GHz {{ism-band|ISM band}}, dodging interference from microwave ovens and Wi-Fi by design — the cost is a slightly chaotic radio that's hard to capture cleanly. It still carries the A2DP audio in every set of wireless headphones.
...
The future of [[bluetooth|Bluetooth]] in 2026 is **{{channel-sounding|Channel Sounding}}** (centimetre-class distance ranging for digital car keys and anti-stalking) and **{{auracast|Auracast}}** ({{broadcast|broadcast}} {{le-audio|LE Audio}} replacing analog hearing loops). Both shipped in the last 24 months. The 1994 cable-replacement project is now load-bearing for keyless entry, hearing accessibility, and the smart-home commissioning bootstrap that hands Wi-Fi credentials to every new IoT device.`
		},
		{
			...
			text: 'Both [[wifi|Wi-Fi]] and [[bluetooth|Bluetooth]] live in the 2.4 GHz **{{ism-band|ISM}} (Industrial, Scientific, Medical)** band — globally unlicensed, free for any device to transmit on. The crowding is real: a microwave oven leaks 2.4 GHz energy strong enough to mute nearby [[bluetooth|Bluetooth]] earbuds. Modern coexistence in combo chips (e.g. {{broadcom|Broadcom}}, Qualcomm, {{apple|Apple}} H1/H2) uses **time-division arbitration** at the silicon level — [[bluetooth|Bluetooth]] gives up {{airtime|airtime}} when [[wifi|Wi-Fi]] needs it, and vice versa. [[wifi|Wi-Fi]] 6E and 7 escape to 6 GHz; [[bluetooth|Bluetooth]] does not — it stays at 2.4 GHz where every consumer device with a battery already lives.'
		},
		{
			type: 'image',
			...
			caption:
				'The [[bluetooth|Bluetooth]] logo is a bind-rune combining **Hagall** (ᚼ) and **Bjarkan** (ᛒ) — the initials of *Harald Blåtand*, the 10th-century Danish king who united Denmark and Norway in Younger Futhark. Jim Kardach proposed the name as a placeholder during a 1997 {{sig|SIG}} meeting. It was never supposed to ship.',
