		...
		steps: [
			{
				...
				description:
					'Before your browser can reach {{google|google}}.com, it needs an actual address — like needing a street address before you can mail a letter. Your device sends a [[dns|DNS]] query to a {{recursive-resolver|recursive resolver}}, which walks the [[dns|DNS]] hierarchy: root servers, then .com TLD servers, then {{google|google}}.com\'s {{authoritative-nameserver|authoritative nameserver}}. The answer (e.g., 142.250.80.46) is cached locally so future requests skip this entire chain. Without [[dns|DNS]], you would have to memorize raw [[ip|IP]] addresses for every website you visit.',
		...
		steps: [
			{
				...
				description:
					'[[coap|CoAP]] (Constrained Application Protocol) brings [[rest|REST]]-like semantics to devices too small and power-limited for HTTP. Running over [[udp|UDP]] with compact binary headers (just 4 bytes fixed), it supports GET, PUT, POST, and DELETE on resources identified by URIs — familiar patterns for web developers, but sized for microcontrollers with 10KB of RAM. Its "{{coap-observe|Observe}}" option lets a client register for notifications when a resource changes (like a temperature sensor), avoiding the cost of repeated polling. [[coap|CoAP]] also supports {{multicast|multicast}} for discovering devices on a network and {{dtls|DTLS}} for encrypted communication. Think of it as HTTP shrunk down for a world where every byte and milliwatt counts.',
				transition:
		...
		steps: [
			{
				...
				description:
					'[[rest|REST]] (Representational State Transfer) models every piece of data as a resource with a unique URL. Clients interact using standard HTTP methods: GET to read, POST to create, PUT to replace, DELETE to remove. [[rest|REST]] APIs are {{stateless|stateless}} — every request contains all the information needed to process it, making them easy to cache and scale. The trade-off is rigidity: clients get fixed data structures, often leading to {{over-fetching|over-fetching}} (getting fields they do not need) or under-fetching (needing multiple requests to assemble a view). [[rest|REST]] dominates the web because of its simplicity, but complex UIs with deeply nested data often outgrow it.',
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				description:
					'While [[rest|REST]] and [[grpc|gRPC]] dominated mainstream APIs, a quieter revolution was happening in infrastructure. [[json-rpc|JSON-RPC]] 2.0 — a one-page spec for calling methods by name over {{json|JSON}} — became the backbone of Ethereum\'s blockchain API, Bitcoin Core, and {{microsoft|Microsoft}}\'s {{lsp|Language Server Protocol}}. Its appeal was radical simplicity: no URL routing, no [[http1|HTTP]] verb semantics, no schema compilation. Just a method name, params, and an ID. The spec was so simple and transport-agnostic ([[http1|HTTP]], [[websockets|WebSocket]], {{stdio|stdio}}, [[tcp|TCP]]) that it became the natural foundation for the next generation of protocols.',
		...
		steps: [
			{
				...
				description:
					"The car's BLE radio broadcasts `{{adv-ind|ADV_IND}}` on advertising channels 37 (2402 MHz), 38 (2426 MHz), and 39 (2480 MHz) every 100 ms or so — three channels carefully chosen to avoid [[wifi|Wi-Fi]]'s 1/6/11. The advert carries the {{ccc-digital-key|CCC Digital Key}} service UUID and an ephemeral identifier. Your iPhone (or Galaxy, or {{android|Android}} phone with {{aliro|Aliro}}) scans those channels continuously. When the phone sees the car's UUID, it sends `{{connect-ind|CONNECT_IND}}` — switching both radios from advertising channels to one of the 37 data channels — and the BLE link is up.",
		...
		steps: [
			{
				...
				description:
					"The new bulb powers on, has no parent yet, and broadcasts an {{ieee-802-15-4|IEEE 802.15.4}} MAC Command `0x07` (**Beacon Request**) on its channel — picked from {15, 20, 25, 26} to dodge [[wifi|Wi-Fi]] 1/6/11 at 2412/2437/2462 MHz. Every {{zigbee-router|Zigbee router}} with permit-joining enabled responds with a **Beacon**: the {{pan-id|PAN ID}}, the Coordinator's {{short-address|short address}} (0x0000), the Stack Profile (Zigbee PRO), and the Permit-Joining flag. The joiner picks the best parent by {{rssi|RSSI}} + LQI. This is the slowest step — beacon scanning can take 2–4 seconds across all candidate channels.",
			...
			},
			{
				...
				description:
					"The bulb sends MAC Command `0x01` (**{{association-request|Association Request}}**) with its Capability byte (`0x8E` = FFD, mains-powered, security-capable, allocate-short). The Coordinator allocates a unique 16-bit {{short-address|short address}} (e.g. `0x3F4E`) and replies with MAC Command `0x02` (**{{association-response|Association Response}}**). The bulb is now associated with the network at the MAC layer — but it doesn't have the **{{network-key|network key}}** yet, which means it can't decrypt or {{encryption|encrypt}} any actual Zigbee frames. The critical security step is next.",
