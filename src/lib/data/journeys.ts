...
export const journeys: Journey[] = [
	// ── Global Journeys ────────────────────────────────────────────────
	{
		...
		steps: [
			...
			},
			{
				...
				title: 'TCP Handshake',
				description:
					'The internet is inherently unreliable — packets can vanish, arrive out of order, or show up twice. [[tcp|TCP]] solves this with a {{three-way-handshake|three-way handshake}} (SYN, SYN-ACK, ACK) that synchronizes sequence numbers between your browser and the server, creating a reliable ordered channel. This {{handshake|handshake}} also negotiates window sizes for {{flow-control|flow control}}, ensuring neither side overwhelms the other. It costs one round trip of {{latency|latency}}, but without it every application would need to implement its own reliability logic.',
		...
		steps: [
			{
				...
				transition: 'The [[ethernet|Ethernet]] frame needs a destination {{mac-address|MAC address}} — but your application only knows an {{ip-address|IP address}}. Something has to bridge the gap between Layer 3 (IP) and Layer 2 ([[ethernet|Ethernet]])...'
			},
			{
				protocolId: 'arp',
				title: 'ARP Resolution',
				description:
					'[[arp|ARP]] is the glue between IP addresses and physical hardware. When your machine needs to reach 192.168.1.1 but only knows its IP, it broadcasts an [[arp|ARP]] request to every device on the LAN: "Who has 192.168.1.1? Tell me your {{mac-address|MAC address}}." The target replies directly with its MAC, and the result is cached in an [[arp|ARP]] table so future packets skip the {{broadcast|broadcast}}. This is why the first packet to a new host on your LAN is slightly slower — ARP has to resolve the address first.',
				transition: 'With the destination MAC resolved and the [[ethernet|Ethernet]] frame ready, the packet can now be addressed for its journey beyond the local network. The IP layer takes over to handle global addressing and routing...'
			},
			{
				protocolId: 'ip',
				title: 'IP Routing',
				description:
					'IP is the postal service of the internet — it stamps each packet with a source and destination address, then forwards it hop by hop toward its target. Each router along the path examines the destination IP, consults its {{routing-table|routing table}}, decrements the TTL (Time To Live) by one, and forwards the packet to the next hop. If TTL reaches zero, the packet is discarded and an [[icmp|ICMP]] "Time Exceeded" is sent back — this is how traceroute works. IP makes no guarantees about delivery order or reliability; it simply does its best to get each packet to the right machine.',
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				description:
					'[[tcp|TCP]] transforms the unreliable internet into a dependable byte stream. It establishes connections with a {{three-way-handshake|three-way handshake}}, assigns sequence numbers to every byte, requires acknowledgments for received data, and retransmits anything that goes missing. Its {{congestion-control|congestion control}} algorithms (Reno, {{cubic|CUBIC}}, BBR) actively probe the network to find the maximum safe sending rate without causing collapse. TCP has been the backbone of the internet for over 40 years — HTTP, email, [[ssh|SSH]], and file transfer all depend on it. The tradeoff is {{latency|latency}}: the {{handshake|handshake}}, acknowledgment delays, and {{head-of-line-blocking|head-of-line blocking}} (one lost packet stalls everything behind it) add up.',
		...
		steps: [
			{
				...
				transition: 'The [[ethernet|Ethernet]] frame is ready to send, but there is a chicken-and-egg problem: your application knows the destination {{ip-address|IP address}} (192.168.1.100), not the destination {{mac-address|MAC address}}. You cannot build an Ethernet frame without a MAC. The network needs a way to translate between these two addressing systems...'
			},
			{
				protocolId: 'arp',
				title: 'ARP Resolution',
				description:
					'[[arp|ARP]] solves the IP-to-MAC translation with an elegant {{broadcast|broadcast}} mechanism. Your machine sends an ARP request to the broadcast address (FF:FF:FF:FF:FF:FF) asking "Who has 192.168.1.100? Tell 192.168.1.1." Every device on the LAN segment hears this, but only the owner of that IP replies with its {{mac-address|MAC address}}. The mapping is cached in your ARP table (typically for 20 minutes) so subsequent packets skip the broadcast entirely. You can see your own ARP cache by running "arp -a" in a terminal. ARP {{spoofing|spoofing}} — where an attacker sends fake ARP replies to redirect traffic — is why network security often relies on higher-layer {{encryption|encryption}}.',
				transition: 'With the destination MAC resolved, the [[ethernet|Ethernet]] frame can be properly addressed for the local segment. Now the IP layer needs to make a critical decision: is this packet destined for a machine on the same local network, or does it need to be forwarded to the {{gateway|default gateway}} for routing across the internet?'
			},
			{
				protocolId: 'ip',
				title: 'IP Addressing & Routing',
				description:
					'The IP layer makes the key routing decision. It compares the destination IP against the {{subnet|subnet}} mask to determine if the target is local or remote. If local, [[arp|ARP]] resolves the target\'s MAC directly. If remote, the packet is sent to the {{gateway|default gateway}} (your router), which consults its {{routing-table|routing table}} and forwards it toward the destination, hop by hop. Each router decrements the TTL (Time To Live, typically starting at 64) by one — when it reaches zero, the packet is discarded to prevent infinite routing loops. The IP header also includes a header {{checksum|checksum}}, protocol field (6 for [[tcp|TCP]], 17 for [[udp|UDP]]), and {{fragmentation|fragmentation}} controls for packets that exceed a link\'s MTU.',
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				description:
					'[[bgp|BGP]] (Border Gateway Protocol) is the protocol that literally holds the internet together. Each {{autonomous-system|autonomous system}} uses [[bgp|BGP]] to announce which IP prefixes it owns and which paths it can reach. [[bgp|BGP]] routers at network borders {{exchange|exchange}} these announcements with their peers, building a global map of reachability. Path selection is policy-driven — an ISP might prefer cheaper {{transit|transit}} providers, avoid routes through certain countries, or favor shorter AS paths. When a [[bgp|BGP]] misconfiguration happens (like Pakistan accidentally hijacking YouTube\'s prefix in 2008), large portions of the internet can go dark. Despite carrying the {{routing-table|routing table}} for the entire internet (nearly 1 million [[ip|IPv4]] prefixes), BGP runs on surprisingly modest hardware and converges within minutes after topology changes.'
			}
		]
	},
		...
		steps: [
			{
				...
				description:
					'[[tcp|TCP]] has been the workhorse of reliable internet communication since 1981 — it guarantees that every byte arrives, in order, without duplication. Underneath, it uses sequence numbers, acknowledgments, {{retransmission|retransmission}} timers, and sophisticated {{congestion-control|congestion control}} (algorithms like Reno, {{cubic|CUBIC}}, and BBR that probe the network to find the optimal sending rate). But TCP has a fundamental limitation: it provides a single ordered byte stream. When [[http2|HTTP/2]] multiplexes dozens of requests over one TCP connection, a single lost packet blocks ALL streams until it is retransmitted. This {{head-of-line-blocking|head-of-line blocking}} problem becomes increasingly painful as connections carry more concurrent data.',
			...
			},
			{
				...
				description:
					'Multipath [[tcp|TCP]] extends standard TCP to use multiple network interfaces simultaneously. Your phone can send data over both WiFi and cellular at the same time, aggregating their {{bandwidth|bandwidth}}. If you walk out of WiFi range, the cellular subflow keeps going seamlessly — no connection drop, no reconnection delay. [[mptcp|MPTCP]] works by establishing multiple TCP subflows and distributing data across them using a coupled {{congestion-control|congestion control}} algorithm that balances load fairly. Apple uses [[mptcp|MPTCP]] in iOS for Siri and Maps, and it powers the seamless WiFi-to-cellular transitions you experience daily without noticing. The tradeoff is complexity: schedulers must decide which path gets which data, and reordering at the receiver adds {{latency|latency}}.',
			...
			},
			{
				...
				description:
					'[[quic|QUIC]] represents the culmination of everything learned from [[tcp|TCP]], [[sctp|SCTP]], and [[mptcp|MPTCP]]. Built on [[udp|UDP]] to bypass ossified middleboxes, it implements its own reliability and {{congestion-control|congestion control}} in userspace (making it updatable without OS kernel changes). From SCTP it borrows independent streams without {{head-of-line-blocking|head-of-line blocking}}. It integrates [[tls|TLS]] 1.3 directly into the {{handshake|handshake}}, achieving a secure connection in just one round trip (or zero for repeat visits). QUIC connections are identified by a variable-length Connection ID rather than the IP/port tuple, enabling {{connection-migration|connection migration}} — switch from WiFi to cellular and the connection survives. Google developed it, the IETF standardized it, and it now powers [[http3|HTTP/3]] for billions of users.'
			}
		]
	},
		...
		steps: [
			{
				...
				description:
					'To load a web page over HTTPS, a browser traditionally needs three sequential round trips before any page data flows: one for the [[tcp|TCP]] {{handshake|handshake}} (SYN, SYN-ACK, ACK), one or two more for [[tls|TLS]] (exchanging cipher suites, certificates, and key material). On a connection with 100ms {{latency|latency}}, that is 200-300ms of pure handshake overhead before a single byte of HTML arrives. Even worse, [[http2|HTTP/2]] multiplexes all its streams over a single TCP connection, so when one packet is lost, TCP\'s {{head-of-line-blocking|head-of-line blocking}} stalls every request — even though the lost data might belong to an unrelated resource like a tiny favicon.',
			...
			},
			{
				...
				description:
					'The key insight was that [[udp|UDP]] passes through virtually every middlebox on the internet. NATs translate UDP ports, firewalls allow it, and ISP equipment does not inspect it. By layering a new protocol on top of UDP, Google could deploy revolutionary transport features without waiting for routers and operating systems to be updated — a process that historically takes decades. UDP itself adds almost nothing (just 8 bytes of header with ports and a {{checksum|checksum}}), giving [[quic|QUIC]] a blank canvas to build its own reliability, {{encryption|encryption}}, and stream management entirely in userspace. This also means QUIC can be updated with a browser release, not an OS kernel update.',
			...
			},
			{
				...
				transition: 'With [[quic|QUIC]] providing fast, encrypted, multiplexed transport, the final piece was adapting HTTP to take advantage of it. [[http3|HTTP/3]] is not just "[[http2|HTTP/2]] on QUIC" — it had to be redesigned because QUIC\'s streams replaced the stream {{multiplexing|multiplexing}} that HTTP/2 implemented over [[tcp|TCP]]...'
			},
			{
				protocolId: 'http3',
				title: 'HTTP/3: The Payoff',
				description:
					'[[http3|HTTP/3]] maps one HTTP {{request-response|request-response}} to one [[quic|QUIC]] stream, giving each request independent {{flow-control|flow control}} and loss recovery. A lost packet carrying image data does not block the CSS or JavaScript streams. QPACK replaces {{hpack|HPACK}} for header compression (adapted for QUIC\'s out-of-order delivery), and {{zero-rtt|0-RTT}} resumption lets returning visitors send their first HTTP request in the very first packet — zero round-trip {{latency|latency}} for the initial data. On lossy mobile networks, HTTP/3 delivers pages measurably faster than [[http2|HTTP/2]] over [[tcp|TCP]]. As of 2024, over 30% of global web traffic runs on HTTP/3, and adoption is accelerating as CDNs and cloud providers enable it by default.'
			}
		]
	},
	...
	// ── Web / API ──────────────────────────────────────────────────────
	{
		...
		steps: [
			...
			},
			{
				...
				transition: '[[http2|HTTP/2]] eliminated the HTTP-layer {{head-of-line-blocking|head-of-line blocking}}, but it exposed a deeper problem: all those multiplexed streams shared a single [[tcp|TCP]] connection. When one TCP packet was lost, TCP\'s ordered delivery guarantee stalled ALL streams until the {{retransmission|retransmission}} arrived. The protocol needed a new transport layer that would not punish unrelated streams for one stream\'s lost packet...'
			},
			{
				protocolId: 'http3',
				title: 'HTTP/3: QUIC-Powered',
				description:
					'[[http3|HTTP/3]] (2022) runs on [[quic|QUIC]] instead of [[tcp|TCP]], finally eliminating {{head-of-line-blocking|head-of-line blocking}} at every layer. Each HTTP stream maps to an independent QUIC stream with its own loss recovery — a dropped packet on one stream cannot block any other. QPACK replaces {{hpack|HPACK}} for header compression, adapted for QUIC\'s potentially out-of-order stream delivery. The integrated [[tls|TLS]] 1.3 {{handshake|handshake}} means a new connection is ready in one round trip, and {{zero-rtt|0-RTT}} resumption for returning visitors means the browser can send its first HTTP request instantly. {{connection-migration|Connection migration}} (via QUIC\'s Connection ID) means a phone switching from WiFi to cellular does not drop the page load. The three decades of HTTP evolution culminate here: a fast, encrypted, multiplexed protocol that works well even on lossy mobile networks.'
			}
		]
	},
	{
		...
		steps: [
			...
			},
			{
				...
				description:
					'WebSockets upgrade an HTTP connection into a persistent, {{full-duplex|full-duplex}} channel where both client and server can send messages independently at any time — no request/response pairing required. After the initial HTTP upgrade {{handshake|handshake}}, the protocol switches to a lightweight {{binary-framing|binary framing}} format with minimal overhead (as little as 2 bytes per frame). This makes WebSockets ideal for chat applications (Slack, Discord), multiplayer gaming (real-time position updates), collaborative editing (Google Docs cursors), and financial trading (live order book updates). The tradeoff is that WebSockets are raw pipes — they give you a message channel but impose no structure on what flows through it. You must design your own message formats, error handling, and connection management.',
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				transition:
					'[[mqtt|MQTT]] reliably moves IoT data from the edge to central systems, but enterprise applications — web dashboards, analytics platforms, legacy systems — often cannot speak MQTT directly. You need a protocol that bridges the gap, one that is simple enough for a web developer to integrate in an afternoon...'
			},
			{
				protocolId: 'stomp',
		...
		steps: [
			{
				...
				description:
					'[[rest|REST]] (Representational State Transfer) models every piece of data as a resource with a unique URL. Clients interact using standard HTTP methods: GET to read, POST to create, PUT to replace, DELETE to remove. REST APIs are {{stateless|stateless}} — every request contains all the information needed to process it, making them easy to cache and scale. The trade-off is rigidity: clients get fixed data structures, often leading to over-fetching (getting fields they do not need) or under-fetching (needing multiple requests to assemble a view). REST dominates the web because of its simplicity, but complex UIs with deeply nested data often outgrow it.',
			...
			},
			{
				...
				description:
					'[[graphql|GraphQL]] lets clients send a query describing exactly the shape of the data they want, and the server returns only that — no more, no less. A single query can traverse relationships (user → posts → comments) that would require multiple [[rest|REST]] endpoints. The schema is typed and introspectable, making APIs self-documenting. Mutations handle writes, and subscriptions push real-time updates over WebSockets. The trade-off: the server must resolve arbitrary query shapes, making caching harder (no URL-based caching), rate-limiting more complex (a single query can be expensive), and N+1 database problems common without careful DataLoader usage.',
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				description:
					'[[tls|TLS]] protects every [[oauth2|OAuth]] token, every API key, and every password in {{transit|transit}}. During the {{handshake|handshake}}, the server proves its identity with a {{certificate|certificate}} signed by a trusted {{certificate-authority|Certificate Authority}}, and both sides negotiate {{encryption|encryption}} parameters. The key {{exchange|exchange}} (ECDHE) generates ephemeral session keys that provide {{forward-secrecy|forward secrecy}} — even if the server\'s {{private-key|private key}} is later compromised, past sessions remain secure. TLS 1.3 streamlined this to a single round trip and removed all insecure legacy algorithms. Without TLS, OAuth tokens would be visible to anyone on the network path.',
		...
		steps: [
			{
				...
				description:
					'When something goes wrong on a network, [[icmp|ICMP]] is the first tool you reach for. The ping command sends an ICMP Echo Request to a host and measures the time until the Echo Reply comes back — giving you {{rtt|round-trip time}}, packet loss percentage, and basic reachability in one command. Traceroute exploits ICMP in a clever way: it sends packets with incrementally increasing TTL (Time to Live) values. Each router along the path decrements the TTL, and when it hits zero, that router sends back an ICMP Time Exceeded message — revealing its identity and distance. This traces the exact path packets take across the internet, exposing where {{latency|latency}} spikes or failures occur.',
			...
			},
			{
				...
				description:
					'[[dns|DNS]] problems masquerade as total network failures — everything seems down, but the real issue is name resolution. The dig command queries DNS servers directly, showing you exactly which records are returned, which nameserver answered, the TTL remaining on cached entries, and the full query chain. You can query specific record types (A, AAAA, MX, CNAME, NS) and specific DNS servers to isolate whether the problem is your local resolver, the authoritative server, or caching. Common issues include stale cached records (wait for TTL expiry or flush the cache), misconfigured NS delegations, missing or incorrect A/AAAA records, and {{dnssec|DNSSEC}} validation failures.',
	...
	},
	{
		...
		description:
			'How transport protocols evolved to handle the reality of mobile networks — where connections drop, IP addresses change, and {{bandwidth|bandwidth}} varies.',
		color: '#10B981',
		scope: 'transport',
		steps: [
			{
				...
				description:
					'[[tcp|TCP]] was designed for fixed networks where connections are stable and endpoints do not move. It binds a connection to a 4-tuple: source IP, source port, destination IP, destination port. This works perfectly on desktops and servers, but on mobile devices it creates a fragile assumption — the moment your phone switches from Wi-Fi to cellular (or between cell towers), your {{ip-address|IP address}} changes, and every TCP connection silently breaks. The application must detect the failure, re-establish the TCP {{handshake|handshake}}, re-negotiate [[tls|TLS]], and re-authenticate. For a video call or file download, this means a visible interruption.',
				transition: '[[tcp|TCP]] ties connections to IP addresses, making them fragile on mobile networks. What if a single connection could span multiple network interfaces simultaneously, surviving handoffs without the application even noticing?'
			},
			{
				protocolId: 'mptcp',
				title: 'MPTCP: Multiple Paths',
				description:
					'Multipath [[tcp|TCP]] extends TCP to use multiple network paths simultaneously. A phone can send data over both Wi-Fi and cellular at the same time, seamlessly shifting traffic when one path degrades. When you walk out of Wi-Fi range, [[mptcp|MPTCP]] gracefully migrates the connection to cellular without dropping a single byte — the application sees one uninterrupted TCP stream. Apple has used MPTCP in iOS since 2013 (for Siri and Apple Maps) and it is enabled system-wide in iOS 17+. MPTCP is backward-compatible: it falls back to regular TCP when the other endpoint does not support it. The trade-off is complexity — middleboxes (firewalls, NATs) sometimes strip the MPTCP options they do not understand.',
				transition: '[[mptcp|MPTCP]] adds resilience to [[tcp|TCP]], but it inherits TCP\'s fundamental limitations: {{head-of-line-blocking|head-of-line blocking}}, ossified middleboxes, and a kernel-level implementation that is slow to deploy. A ground-up redesign built on [[udp|UDP]] avoids all of these constraints...'
			},
			{
				protocolId: 'quic',
				title: 'QUIC: Connection Migration',
				description:
					'[[quic|QUIC]] was designed from the start for mobile networks. Instead of identifying connections by IP addresses and ports, QUIC uses a variable-length Connection ID — a token that both endpoints recognize regardless of the underlying network path. When your phone switches from Wi-Fi to cellular, the {{ip-address|IP address}} changes but the Connection ID stays the same, so the QUIC connection continues without interruption — no re-{{handshake|handshake}}, no re-authentication, no lost data. QUIC also eliminates {{head-of-line-blocking|head-of-line blocking}} (a lost packet only affects its own stream, not all streams), and its {{one-rtt|1-RTT}} handshake integrates transport and {{encryption|encryption}} setup into a single round trip. Google reports that QUIC reduces video rebuffering by 18% on mobile networks compared to [[tcp|TCP]].'
			}
		]
	},
	{
