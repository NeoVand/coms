...
export const journeys: Journey[] = [
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				title: 'IP Routing',
				description:
					'[[ip|IP]] is the postal service of the internet — it stamps each packet with a source and destination address, then forwards it hop by hop toward its target. Each router along the path examines the destination [[ip|IP]], consults its {{routing-table|routing table}}, decrements the TTL (Time To Live) by one, and forwards the packet to the next hop. If TTL reaches zero, the packet is discarded and an [[icmp|ICMP]] "Time Exceeded" is sent back — this is how traceroute works. [[ip|IP]] makes no guarantees about delivery order or reliability; it simply does its best to get each packet to the right machine.',
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				description:
					'[[tcp|TCP]] transforms the unreliable internet into a dependable byte stream. It establishes connections with a {{three-way-handshake|three-way handshake}}, assigns sequence numbers to every byte, requires acknowledgments for received data, and retransmits anything that goes missing. Its {{congestion-control|congestion control}} algorithms (Reno, {{cubic|CUBIC}}, BBR) actively probe the network to find the maximum safe sending rate without causing collapse. [[tcp|TCP]] has been the backbone of the internet for over 40 years — HTTP, email, [[ssh|SSH]], and file transfer all depend on it. The tradeoff is {{latency|latency}}: the {{handshake|handshake}}, acknowledgment delays, and {{head-of-line-blocking|head-of-line blocking}} (one lost packet stalls everything behind it) add up.',
	...
	// ── Network Foundations ─────────────────────────────────────────────
	{
		...
		steps: [
			...
			},
			{
				...
				description:
					'The [[ip|IP]] layer makes the key routing decision. It compares the destination [[ip|IP]] against the {{subnet|subnet}} mask to determine if the target is local or remote. If local, [[arp|ARP]] resolves the target\'s MAC directly. If remote, the packet is sent to the {{gateway|default gateway}} (your router), which consults its {{routing-table|routing table}} and forwards it toward the destination, hop by hop. Each router decrements the TTL (Time To Live, typically starting at 64) by one — when it reaches zero, the packet is discarded to prevent infinite routing loops. The [[ip|IP]] header also includes a header {{checksum|checksum}}, protocol field (6 for [[tcp|TCP]], 17 for [[udp|UDP]]), and {{fragmentation|fragmentation}} controls for packets that exceed a link\'s MTU.',
		...
		steps: [
			{
				...
				description:
					'[[tcp|TCP]] has been the workhorse of reliable internet communication since 1981 — it guarantees that every byte arrives, in order, without duplication. Underneath, it uses sequence numbers, acknowledgments, {{retransmission|retransmission}} timers, and sophisticated {{congestion-control|congestion control}} (algorithms like Reno, {{cubic|CUBIC}}, and BBR that probe the network to find the optimal sending rate). But [[tcp|TCP]] has a fundamental limitation: it provides a single ordered byte stream. When [[http2|HTTP/2]] multiplexes dozens of requests over one [[tcp|TCP]] connection, a single lost packet blocks ALL streams until it is retransmitted. This {{head-of-line-blocking|head-of-line blocking}} problem becomes increasingly painful as connections carry more concurrent data.',
	...
	// ── Web / API ──────────────────────────────────────────────────────
	{
		...
		description: 'Three decades of HTTP evolution — from Tim Berners-Lee\'s hypertext to [[quic|QUIC]]-powered streams.',
		color: '#00D4FF',
		scope: 'web-api',
		steps: [
			{
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				description:
					'[[dns|DNS]] problems masquerade as total network failures — everything seems down, but the real issue is name resolution. The dig command queries [[dns|DNS]] servers directly, showing you exactly which records are returned, which nameserver answered, the TTL remaining on cached entries, and the full query chain. You can query specific record types (A, AAAA, MX, CNAME, NS) and specific [[dns|DNS]] servers to isolate whether the problem is your local resolver, the authoritative server, or caching. Common issues include stale cached records (wait for TTL expiry or flush the cache), misconfigured NS delegations, missing or incorrect A/AAAA records, and {{dnssec|DNSSEC}} validation failures.',
