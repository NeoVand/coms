...
export const journeys: Journey[] = [
	// ── Global Journeys ────────────────────────────────────────────────
	{
		...
		description:
			'Follow a request from your browser to a server and back — through [[dns|DNS]], [[tcp|TCP]], [[tls|TLS]], and {{http-method|HTTP}}.',
		color: '#00D4FF',
		scope: 'global',
		steps: [
			{
				...
				transition:
					'The browser now knows WHERE the server lives — but packets on the internet can be lost, reordered, or duplicated. Before sending any real data, it needs to negotiate a reliable channel...'
			},
			{
				protocolId: 'tcp',
				title: 'TCP Handshake',
				description:
					'The internet is inherently unreliable — packets can vanish, arrive out of order, or show up twice. [[tcp|TCP]] solves this with a {{three-way-handshake|three-way handshake}} ({{syn-cookies|SYN}}, {{syn-cookies|SYN}}-{{ack|ACK}}, {{ack|ACK}}) that synchronizes sequence numbers between your browser and the server, creating a reliable ordered channel. This {{handshake|handshake}} also negotiates window sizes for {{flow-control|flow control}}, ensuring neither side overwhelms the other. It costs one round trip of {{latency|latency}}, but without it every application would need to implement its own reliability logic.',
				transition:
					'A reliable pipe now connects your browser to the server — but anyone sitting on the network path (your {{isp|ISP}}, a coffee shop router, a government {{firewall|firewall}}) can read every byte in plaintext. The data needs {{encryption|encryption}}...'
			},
			{
				protocolId: 'tls',
				...
				transition:
					'The connection is now both reliable and encrypted — no eavesdropper can read or tamper with the data. Everything is ready to speak the language of the web...'
			},
			{
				protocolId: 'http1',
				title: 'HTTP Request & Response',
				description:
					'With the secure channel established, the browser finally speaks {{http-method|HTTP}}. It sends a GET request with headers describing what it accepts (content types, encodings, languages) and any cookies for the domain. The server processes the request and responds with a status code (200 OK), response headers (caching rules, content type), and the {{html|HTML}} document body. This single {{request-response|request-response}} cycle is the fundamental unit of the web — and a modern page will trigger hundreds more for {{css|CSS}}, JavaScript, images, and fonts.'
			}
		]
	},
	{
		...
		steps: [
			{
				...
				description:
					'Every piece of data on a local network travels as an [[ethernet|Ethernet]] frame — a precisely structured envelope containing source and destination {{mac-address|MAC}} addresses (48-bit hardware identifiers burned into every network card), a type field that indicates what protocol lives inside ([[ip|IPv4]], [[ipv6|IPv6]], [[arp|ARP]]), and a Frame Check Sequence for error detection. The frame is the physical currency of LANs: switches read the destination {{mac-address|MAC}} to forward it to the correct port. Without this framing, raw electrical signals on the wire would be meaningless noise.',
				transition:
					'The [[ethernet|Ethernet]] frame needs a destination {{mac-address|MAC address}} — but your application only knows an {{ip-address|IP address}}. Something has to bridge the gap between Layer 3 ([[ip|IP]]) and Layer 2 ([[ethernet|Ethernet]])...'
			},
			{
				protocolId: 'arp',
				title: 'ARP Resolution',
				description:
					'[[arp|ARP]] is the glue between [[ip|IP]] addresses and physical hardware. When your machine needs to reach 192.168.1.1 but only knows its [[ip|IP]], it broadcasts an [[arp|ARP]] request to every device on the {{lan|LAN}}: "Who has 192.168.1.1? Tell me your {{mac-address|MAC address}}." The target replies directly with its {{mac-address|MAC}}, and the result is cached in an [[arp|ARP]] table so future packets skip the {{broadcast|broadcast}}. This is why the first packet to a new host on your {{lan|LAN}} is slightly slower — [[arp|ARP]] has to resolve the address first.',
				transition:
					'With the destination {{mac-address|MAC}} resolved and the [[ethernet|Ethernet]] frame ready, the packet can now be addressed for its journey beyond the local network. The [[ip|IP]] layer takes over to handle global addressing and routing...'
			},
			{
				protocolId: 'ip',
				title: 'IP Routing',
				description:
					'[[ip|IP]] is the postal service of the internet — it stamps each packet with a source and destination address, then forwards it hop by hop toward its target. Each router along the path examines the destination [[ip|IP]], consults its {{routing-table|routing table}}, decrements the {{ttl|TTL}} ({{ttl|Time To Live}}) by one, and forwards the packet to the next hop. If {{ttl|TTL}} reaches zero, the packet is discarded and an [[icmp|ICMP]] "{{time-exceeded|Time Exceeded}}" is sent back — this is how {{traceroute|traceroute}} works. [[ip|IP]] makes no guarantees about delivery order or reliability; it simply does its best to get each packet to the right machine.',
				transition:
					'[[ip|IP]] delivered the packet to the correct machine — but a server might be running dozens of applications simultaneously. A web server on port 80, an [[ssh|SSH]] daemon on port 22, a database on port 5432. Something has to deliver the data to the right process...'
			},
			{
				protocolId: 'tcp',
				title: 'TCP Delivery',
				...
				transition:
					'The data has been reliably delivered to the correct process on the correct machine. Now the application can finally interpret the bytes using its own protocol semantics...'
			},
			{
				protocolId: 'http1',
				title: 'HTTP Application',
				description:
					'At the very top of the stack, {{http-method|HTTP}} gives meaning to the raw bytes. It defines a structured conversation: the client sends a request with a method (GET, POST, PUT), a path (/index.html), and headers describing its capabilities. The server responds with a status code (200 OK, 404 Not Found), its own headers (content type, caching directives), and the actual content — {{html|HTML}}, {{json|JSON}}, images. This is the layer that developers interact with directly, but it only works because every layer below carried the data faithfully.'
			}
		]
	},
	{
		...
		description:
			'From raw datagrams to reliable streams — the foundational transport protocols and their evolution.',
		color: '#39FF14',
		scope: 'global',
		steps: [
			{
				...
				transition:
					'[[udp|UDP]] gives you raw speed, but many applications cannot tolerate missing or reordered data. A web page with a missing {{css|CSS}} file, a bank transfer with lost bytes, or a file download with gaps — these need every byte in the right order. The internet needed a more disciplined transport...'
			},
			{
				protocolId: 'tcp',
				title: 'TCP: Reliable Streams',
				description:
					'[[tcp|TCP]] transforms the unreliable internet into a dependable byte stream. It establishes connections with a {{three-way-handshake|three-way handshake}}, assigns sequence numbers to every byte, requires acknowledgments for received data, and retransmits anything that goes missing. Its {{congestion-control|congestion control}} algorithms (Reno, {{cubic|CUBIC}}, {{bbr|BBR}}) actively probe the network to find the maximum safe sending rate without causing collapse. [[tcp|TCP]] has been the backbone of the internet for over 40 years — {{http-method|HTTP}}, email, [[ssh|SSH]], and file transfer all depend on it. The tradeoff is {{latency|latency}}: the {{handshake|handshake}}, acknowledgment delays, and {{head-of-line-blocking|head-of-line blocking}} (one lost packet stalls everything behind it) add up.',
				transition:
					'[[tcp|TCP]] proved that reliability works, but its single-stream design means one lost packet blocks all data behind it — even unrelated requests. And its {{handshake|handshake}} adds a full round trip before any data flows. Engineers began asking: can we keep the reliability but eliminate these bottlenecks?'
			},
			{
				protocolId: 'quic',
		...
		steps: [
			{
				...
				transition:
					'[[tls|TLS]] secures client-to-server connections beautifully, but system administrators need more than encrypted web traffic — they need to log into remote servers, transfer files, and tunnel network connections, all securely. A different protocol emerged for this exact purpose...'
			},
			{
				protocolId: 'ssh',
				title: 'SSH: Secure Shell',
				description:
					'Before [[ssh|SSH]], administrators used Telnet to manage remote servers — sending passwords and commands in cleartext. [[ssh|SSH]] replaced it with a fully encrypted channel that supports public-key authentication (no passwords to steal), secure file transfer ({{scp-copy|SCP}} and {{sftp|SFTP}}), and {{port-forwarding|port forwarding}} that can tunnel any [[tcp|TCP]] connection through the encrypted link. [[ssh|SSH]] uses its own key {{exchange|exchange}} and {{encryption|encryption}} layer independent of [[tls|TLS]], and its agent forwarding feature lets you chain [[ssh|SSH]] connections through jump {{hosts-bare|hosts}} without exposing your {{private-key|private key}}. It became the universal tool for server management, Git operations, and secure automation.',
				transition:
					'With [[tls|TLS]] protecting web connections and [[ssh|SSH]] securing server access, the foundations of internet security were in place. But there was still a problem: {{http-method|HTTP}} {{encryption|encryption}} was optional, and most sites did not bother. The web needed a forcing function to make {{encryption|encryption}} the default, not the exception...'
			},
			{
				protocolId: 'http2',
				title: 'HTTPS Everywhere',
				description:
					'[[http2|HTTP/2]] was the tipping point. While the specification technically allows plaintext [[http2|HTTP/2]], every major browser only implements it over [[tls|TLS]] — making {{encryption|encryption}} a de facto requirement for the modern web. Combined with free {{certificate|certificate}} authorities like Let\'s Encrypt and browser warnings on {{http-method|HTTP}} sites ("Not Secure"), the web went from roughly 40% encrypted in 2015 to over 95% today. [[http2|HTTP/2]] also introduced {{binary-framing|binary framing}}, multiplexed streams, and {{hpack|HPACK}} header compression, but its most lasting impact may be normalizing the idea that all web traffic should be encrypted by default.'
			}
		]
	},
	...
	// ── Network Foundations ─────────────────────────────────────────────
	{
		...
		description:
			'Follow a single packet from creation to delivery on a local network, including diagnostics.',
		color: '#F472B6',
		...
		steps: [
			{
				...
				description:
					"Every packet's journey begins at the network interface card ({{nic|NIC}}), which constructs an [[ethernet|Ethernet]] frame. The {{nic|NIC}} stamps on its own 48-bit {{mac-address|MAC address}} as the source, adds the destination {{mac-address|MAC}}, a type field (0x0800 for [[ip|IPv4]], 0x0806 for [[arp|ARP]]), the {{payload|payload}}, and a 4-byte Frame Check Sequence ({{crc|CRC}}-32) that lets the receiver detect bit errors caused by electrical interference. If any bits are corrupted in {{transit|transit}}, the {{fcs|FCS}} check fails and the frame is silently discarded — no correction, just detection. This is the foundation of all local network communication.",
				transition:
					'The [[ethernet|Ethernet]] frame is ready to send, but there is a chicken-and-egg problem: your application knows the destination {{ip-address|IP address}} (192.168.1.100), not the destination {{mac-address|MAC address}}. You cannot build an [[ethernet|Ethernet]] frame without a {{mac-address|MAC}}. The network needs a way to translate between these two addressing systems...'
			},
			{
				protocolId: 'arp',
				title: 'ARP Resolution',
				description:
					'[[arp|ARP]] solves the [[ip|IP]]-to-{{mac-address|MAC}} translation with an elegant {{broadcast|broadcast}} mechanism. Your machine sends an [[arp|ARP]] request to the {{broadcast|broadcast}} address (FF:FF:FF:FF:FF:FF) asking "Who has 192.168.1.100? Tell 192.168.1.1." Every device on the {{lan|LAN}} segment hears this, but only the owner of that [[ip|IP]] replies with its {{mac-address|MAC address}}. The mapping is cached in your [[arp|ARP]] table (typically for 20 minutes) so subsequent packets skip the {{broadcast|broadcast}} entirely. You can see your own [[arp|ARP]] cache by running "arp -a" in a terminal. [[arp|ARP]] {{spoofing|spoofing}} — where an attacker sends fake [[arp|ARP]] replies to redirect traffic — is why network security often relies on higher-layer {{encryption|encryption}}.',
				transition:
					'With the destination {{mac-address|MAC}} resolved, the [[ethernet|Ethernet]] frame can be properly addressed for the local segment. Now the [[ip|IP]] layer needs to make a critical decision: is this packet destined for a machine on the same local network, or does it need to be forwarded to the {{gateway|default gateway}} for routing across the internet?'
			},
			{
				protocolId: 'ip',
				title: 'IP Addressing & Routing',
				description:
					"The [[ip|IP]] layer makes the key routing decision. It compares the destination [[ip|IP]] against the {{subnet|subnet}} mask to determine if the target is local or remote. If local, [[arp|ARP]] resolves the target's {{mac-address|MAC}} directly. If remote, the packet is sent to the {{gateway|default gateway}} (your router), which consults its {{routing-table|routing table}} and forwards it toward the destination, hop by hop. Each router decrements the {{ttl|TTL}} ({{ttl|Time To Live}}, typically starting at 64) by one — when it reaches zero, the packet is discarded to prevent infinite routing loops. The [[ip|IP]] header also includes a header {{checksum|checksum}}, protocol field (6 for [[tcp|TCP]], 17 for [[udp|UDP]]), and {{fragmentation|fragmentation}} controls for packets that exceed a link's {{mtu|MTU}}.",
				transition:
					'The packet has been delivered successfully — but networks are not always healthy. Links go down, routes change, {{hosts-bare|hosts}} become unreachable, and packets get dropped. How does a network administrator diagnose problems and verify that everything is working correctly?'
			},
			{
				protocolId: 'icmp',
				title: 'ICMP Diagnostics',
				description:
					'[[icmp|ICMP]] is the internet\'s diagnostic nervous system — it does not carry user data but instead reports on the health and status of the network itself. The "{{ping|ping}}" command sends [[icmp|ICMP]] Echo Requests and measures round-trip times to verify reachability. "{{traceroute|Traceroute}}" cleverly sends packets with incrementing {{ttl|TTL}} values (1, 2, 3...), causing each successive router to reply with a "{{time-exceeded|Time Exceeded}}" message, mapping the entire path to a destination. [[icmp|ICMP]] also carries critical error messages: "Destination Unreachable" (with subcodes for network, host, port, and {{fragmentation|fragmentation}} failures), "Redirect" (telling a host to use a better route), and "Source Quench" (legacy congestion {{signaling|signaling}}). Without [[icmp|ICMP]], debugging network problems would be nearly impossible.'
			}
		]
	},
	{
		...
		steps: [
			{
				...
				transition:
					'{{iana|IANA}} allocated the last [[ip|IPv4]] address blocks in 2011. The stopgap of {{nat|NAT}} created a fragile internet where devices could not directly reach each other. A fundamental redesign had been in the works since the 1990s, and the world is finally adopting it...'
			},
			{
				protocolId: 'ipv6',
		...
		steps: [
			{
				...
				transition:
					"[[tcp|TCP]]'s single-stream design meant that loss in one logical conversation blocked every other conversation sharing the same connection. The telecom industry, which needed to carry multiple independent {{signaling|signaling}} messages simultaneously, developed a different approach..."
			},
			{
				protocolId: 'sctp',
				...
				transition:
					'[[sctp|SCTP]] proved that independent streams eliminate {{head-of-line-blocking|head-of-line blocking}}, and {{multi-homing|multi-homing}} provides resilience. But what if you want to go further — not just failing over between network paths, but actively using multiple paths simultaneously to aggregate {{bandwidth|bandwidth}}?'
			},
			{
				protocolId: 'mptcp',
				title: 'MPTCP: Multiple Paths',
				description:
					'{{multipath|Multipath}} [[tcp|TCP]] extends standard [[tcp|TCP]] to use multiple network interfaces simultaneously. Your phone can send data over both [[wifi|WiFi]] and cellular at the same time, aggregating their {{bandwidth|bandwidth}}. If you walk out of [[wifi|WiFi]] range, the cellular {{subflow|subflow}} keeps going seamlessly — no connection drop, no reconnection delay. [[mptcp|MPTCP]] works by establishing multiple [[tcp|TCP]] subflows and distributing data across them using a coupled {{congestion-control|congestion control}} algorithm that balances load fairly. {{apple|Apple}} uses [[mptcp|MPTCP]] in iOS for Siri and Maps, and it powers the seamless [[wifi|WiFi]]-to-cellular transitions you experience daily without noticing. The tradeoff is complexity: schedulers must decide which path gets which data, and reordering at the receiver adds {{latency|latency}}.',
				transition:
					"[[mptcp|MPTCP]] showed the power of using multiple paths, but it still inherits [[tcp|TCP]]'s fundamental constraints — the {{three-way-handshake|three-way handshake}}, the kernel implementation that is hard to {{bgp-update|update}}, and middlebox interference. A completely new transport protocol, designed with all these lessons in mind, would soon arrive..."
			},
			{
				protocolId: 'quic',
				title: 'QUIC: The Modern Synthesis',
				description:
					'[[quic|QUIC]] represents the culmination of everything learned from [[tcp|TCP]], [[sctp|SCTP]], and [[mptcp|MPTCP]]. Built on [[udp|UDP]] to bypass ossified middleboxes, it implements its own reliability and {{congestion-control|congestion control}} in userspace (making it updatable without {{os|OS}} kernel changes). From [[sctp|SCTP]] it borrows independent streams without {{head-of-line-blocking|head-of-line blocking}}. It integrates [[tls|TLS]] 1.3 directly into the {{handshake|handshake}}, achieving a secure connection in just one round trip (or zero for repeat visits). [[quic|QUIC]] connections are identified by a variable-length Connection {{id-identifier|ID}} rather than the [[ip|IP]]/port tuple, enabling {{connection-migration|connection migration}} — switch from [[wifi|WiFi]] to cellular and the connection survives. {{google|Google}} developed it, the {{ietf|IETF}} standardized it, and it now powers [[http3|HTTP/3]] for billions of users.'
			}
		]
	},

	{
		...
		steps: [
			...
			},
			{
				...
				transition:
					'With [[quic|QUIC]] providing fast, encrypted, multiplexed transport, the final piece was adapting {{http-method|HTTP}} to take advantage of it. [[http3|HTTP/3]] is not just "[[http2|HTTP/2]] on [[quic|QUIC]]" — it had to be redesigned because [[quic|QUIC]]\'s streams replaced the stream {{multiplexing|multiplexing}} that [[http2|HTTP/2]] implemented over [[tcp|TCP]]...'
			},
			{
				protocolId: 'http3',
				title: 'HTTP/3: The Payoff',
				description:
					"[[http3|HTTP/3]] maps one {{http-method|HTTP}} {{request-response|request-response}} to one [[quic|QUIC]] stream, giving each request independent {{flow-control|flow control}} and loss recovery. A lost packet carrying image data does not block the {{css|CSS}} or JavaScript streams. {{qpack|QPACK}} replaces {{hpack|HPACK}} for header compression (adapted for [[quic|QUIC]]'s out-of-order delivery), and {{zero-rtt|0-RTT}} resumption lets returning visitors send their first {{http-method|HTTP}} request in the very first packet — zero round-trip {{latency|latency}} for the initial data. On lossy mobile networks, [[http3|HTTP/3]] delivers pages measurably faster than [[http2|HTTP/2]] over [[tcp|TCP]]. As of 2024, over 30% of global web traffic runs on [[http3|HTTP/3]], and adoption is accelerating as CDNs and cloud providers enable it by default."
			}
		]
	},
	...
	// ── Web / API ──────────────────────────────────────────────────────
	{
		...
		description:
			"Three decades of {{http-method|HTTP}} evolution — from [[pioneer:tim-berners-lee|Tim Berners-Lee]]'s hypertext to [[quic|QUIC]]-powered streams.",
		color: '#00D4FF',
		scope: 'web-api',
		steps: [
			{
	...
	},
	{
		...
		description:
			'From simple {{request-response|request-response}} to persistent bidirectional communication.',
		color: '#00D4FF',
		scope: 'web-api',
		steps: [
			{
		...
		steps: [
			{
				...
				description:
					'[[mqtt|MQTT]] was designed for the harshest conditions: sensors on oil rigs with satellite uplinks, medical devices with intermittent cellular, smart home gadgets on flaky [[wifi|WiFi]]. Its binary protocol is extraordinarily compact — a minimal {{mqtt-publish|publish}} message is just 2 bytes of overhead. Clients {{mqtt-publish|publish}} to hierarchical topics (like "home/kitchen/temperature") and {{mqtt-subscribe|subscribe}} with wildcards ("home/+/temperature" for all rooms). Three QoS levels let you choose between {{fire-and-forget|fire-and-forget}} (QoS 0), acknowledged delivery (QoS 1), and {{exactly-once-delivery|exactly-once delivery}} (QoS 2). The broker handles all routing: publishers and subscribers never need to know about each other, creating clean decoupling. A "{{last-will|Last Will}}" message is even sent automatically if a device disconnects unexpectedly.',
				transition:
					'[[mqtt|MQTT]] excels at getting small messages from constrained devices to a broker, but enterprise systems need more sophisticated message routing. What if you need to route messages based on content, fan out to multiple queues, implement priority ordering, or guarantee transactional processing with dead-letter handling?'
			},
			{
				protocolId: 'amqp',
				...
				transition:
					'[[amqp|AMQP]] handles complex enterprise routing brilliantly, but what happens when you need to process millions of events per second, replay historical data, and scale horizontally across dozens of servers? Enterprise message brokers were not designed for internet-scale event streaming...'
			},
			{
				protocolId: 'kafka',
				title: 'Kafka: Event Streaming',
				description:
					'[[kafka|Kafka]] reimagined messaging as a distributed commit log — an append-only, immutable sequence of events that can be replayed from any point in time. Topics are split into partitions distributed across a cluster, with each {{partition|partition}} replicated for fault tolerance. Consumer groups enable parallel processing: each consumer in a group reads from different partitions, providing horizontal scalability. Unlike traditional message queues that delete messages after consumption, [[kafka|Kafka]] retains events for a configurable period (days, weeks, or forever), enabling new consumers to reprocess the entire history. This makes it ideal for event sourcing, change data capture, {{stream-processing|stream processing}} pipelines, and building real-time data platforms. {{linkedin|LinkedIn}}, where [[kafka|Kafka]] was born, processes over 7 trillion messages per day through it.'
			}
		]
	},
	{
		...
		description: 'From constrained sensors to enterprise integrations — messaging at every scale.',
		color: '#C084FC',
		scope: 'async-iot',
		steps: [
			{
				...
				description:
					'[[coap|CoAP]] (Constrained Application Protocol) brings [[rest|REST]]-like semantics to devices too small and power-limited for {{http-method|HTTP}}. Running over [[udp|UDP]] with compact binary headers (just 4 bytes fixed), it supports GET, PUT, POST, and DELETE on resources identified by URIs — familiar patterns for web developers, but sized for microcontrollers with 10KB of {{ram|RAM}}. Its "{{coap-observe|Observe}}" option lets a client register for notifications when a resource changes (like a temperature sensor), avoiding the cost of repeated polling. [[coap|CoAP]] also supports {{multicast|multicast}} for discovering devices on a network and {{dtls|DTLS}} for encrypted communication. Think of it as {{http-method|HTTP}} shrunk down for a world where every byte and milliwatt counts.',
				transition:
	...
	// ── Real-Time A/V ──────────────────────────────────────────────────
	{
		...
		description: 'The protocols that make real-time video communication possible in the browser.',
		color: '#FF9F67',
		scope: 'realtime-av',
		steps: [
			{
			...
			},
			{
				...
				description:
					'[[webrtc|WebRTC]] is the browser-native framework that makes {{peer-to-peer|peer-to-peer}} video calls possible without plugins. It orchestrates an entire stack: {{ice|ICE}} (Interactive Connectivity Establishment) punches through NATs by testing multiple connection candidates (local addresses, server-reflexive via {{stun|STUN}}, relay via {{turn|TURN}}) and selecting the best path. {{dtls|DTLS}} (Datagram [[tls|TLS]]) encrypts the connection, and {{srtp|SRTP}} (Secure [[rtp|RTP]]) encrypts the media streams. The getUserMedia {{api|API}} accesses cameras and microphones, RTCPeerConnection manages the connection lifecycle, and RTCDataChannel provides a reliable or unreliable channel for arbitrary data (file sharing, game state, text chat) alongside the media. All of this happens {{peer-to-peer|peer-to-peer}} — video data flows directly between browsers without passing through a server, reducing {{latency|latency}} and server costs.'
			}
		]
	},
	{
		...
		steps: [
			{
				...
				transition:
					"[[rtmp|RTMP]]'s dependence on Flash and specialized servers was its downfall. The insight that changed everything was simple: what if you broke video into small files and served them over plain {{http-method|HTTP}}? Suddenly any web server, any {{cdn|CDN}}, and any {{http-method|HTTP}} cache in the world could deliver video without special software..."
			},
			{
				protocolId: 'hls',
				title: 'HLS: Adaptive HTTP',
				description:
					'[[hls|HLS]] ({{http-method|HTTP}} Live Streaming), created by {{apple|Apple}} in 2009, broke video streaming wide open. The encoder splits the video into small segments (typically 6-10 seconds each), encodes each segment at multiple quality levels (360p, 720p, 1080p, 4K), and generates a playlist file (.m3u8) listing the available segments and qualities. The player downloads the playlist, starts fetching segments, and continuously monitors download speed — if {{bandwidth|bandwidth}} drops, it seamlessly switches to a lower quality; when {{bandwidth|bandwidth}} recovers, it ramps back up. Because everything is plain {{http-method|HTTP}}, it works through any {{cdn|CDN}}, proxy, or cache, making it massively scalable. [[hls|HLS]] is natively supported in Safari and iOS, and nearly every streaming platform uses it as their primary delivery format.',
				transition:
					'[[hls|HLS]] transformed video delivery but is an {{apple|Apple}}-developed technology. The internet standards community wanted an open, vendor-neutral alternative that could offer the same adaptive streaming benefits while supporting a wider range of codecs and {{drm|DRM}} systems without patent encumbrances...'
			},
			{
				protocolId: 'dash',
				title: 'DASH: Open Standard',
				description:
					'[[dash|DASH]] (Dynamic Adaptive Streaming over {{http-method|HTTP}}) is the {{iso|ISO}}-standardized answer to [[hls|HLS]]. Instead of an {{apple|Apple}}-specific playlist format, [[dash|DASH]] uses an {{xml|XML}}-based Media Presentation Description ({{mpd|MPD}}) that describes available representations — each with its {{codec|codec}}, resolution, bitrate, and segment URLs. Being {{codec|codec}}-agnostic, [[dash|DASH]] supports H.264, H.265/{{hevc|HEVC}}, {{vp9|VP9}}, {{av1|AV1}}, and any future {{codec|codec}} without protocol changes. It offers more flexible segment addressing (template-based URLs, byte-range requests, timeline-based indexing) and supports features like multi-period presentations (inserting ads as separate periods) and content protection descriptors for {{drm|DRM}} integration. Netflix, YouTube, and most major streaming services use [[dash|DASH]] for non-{{apple|Apple}} devices, often running both [[dash|DASH]] and [[hls|HLS]] in parallel to cover the entire device ecosystem.'
			}
		]
	},
		...
		steps: [
			{
				...
				description:
					'[[dns|DNS]] is the first step in almost every internet interaction — and also one of the most vulnerable. Traditional [[dns|DNS]] queries travel in plaintext over [[udp|UDP]], meaning your {{isp|ISP}} (or anyone on the network path) can see every domain you visit. Modern hardening includes {{dnssec|DNSSEC}} (cryptographic signatures that prevent spoofed [[dns|DNS]] responses), [[dns|DNS]]-over-HTTPS (DoH) and [[dns|DNS]]-over-[[tls|TLS]] (DoT) which encrypt queries so observers cannot read them, and [[dns|DNS]]-based authentication like DANE which allows domain owners to {{mqtt-publish|publish}} their [[tls|TLS]] {{certificate|certificate}} fingerprints in [[dns|DNS]] itself. Securing [[dns|DNS]] is critical because a compromised [[dns|DNS]] response can redirect you to a phishing site that perfectly mimics your bank — and no amount of [[tls|TLS]] will help if you {{mqtt-connect|connect}} to the wrong server.',
				transition:
					'[[dns|DNS]] has told you WHERE to {{mqtt-connect|connect}}, but the connection itself is still vulnerable — anyone on the network path can read, modify, or inject data. The next step is wrapping the connection in {{encryption|encryption}} that guarantees confidentiality, integrity, and authenticity...'
			},
			{
				protocolId: 'tls',
				title: 'TLS: Encryption',
				description:
					"[[tls|TLS]] is the {{encryption|encryption}} layer that makes secure internet communication possible. During the {{handshake|handshake}}, the server presents a {{certificate|certificate}} signed by a trusted {{certificate-authority|Certificate Authority}}, proving it is who it claims to be (authentication). Both sides negotiate a {{cipher-suite|cipher suite}} and perform a key {{exchange|exchange}} (typically {{ecdhe|ECDHE}} — Elliptic Curve Diffie-Hellman Ephemeral) to create shared session keys that provide {{forward-secrecy|forward secrecy}}: even if the server's long-term {{private-key|private key}} is later compromised, past recorded sessions cannot be decrypted. Every byte of application data is then encrypted (confidentiality) and authenticated with a {{mac-address|MAC}} (integrity). [[tls|TLS]] 1.3 simplified the protocol dramatically, removing insecure legacy algorithms, reducing the {{handshake|handshake}} to one round trip, and making {{zero-rtt|0-RTT}} resumption possible for repeat connections.",
				transition:
			...
			},
			{
				...
				description:
					'[[ssh|SSH]] replaced the dangerously insecure Telnet and rlogin protocols, becoming the universal standard for secure remote server management. It supports multiple authentication methods — password, public-key (Ed25519 or {{rsa|RSA}}), {{certificate|certificate}}-based, and multi-factor — with public-key being the gold standard (no password to intercept or guess). Beyond remote shells, [[ssh|SSH]] provides {{scp-copy|SCP}} and {{sftp|SFTP}} for encrypted file transfer, local and remote {{port-forwarding|port forwarding}} for tunneling any [[tcp|TCP]] connection through the encrypted channel, and agent forwarding for securely chaining through jump {{hosts-bare|hosts}}. Its config file (~/.ssh/config) lets you define per-host settings, jump proxies, and identity files. [[ssh|SSH]] is also the transport for Git operations, making it foundational to modern software development workflows.'
			}
		]
	},
	{
		...
		description: 'The chain of protocols that runs every time a device connects to a network.',
		color: '#2DD4BF',
		scope: 'utilities',
		steps: [
			{
			...
			},
			{
				...
				description:
					'With a working network connection and the [[dns|DNS]] server address provided by [[dhcp|DHCP]], your device can finally translate domain names into [[ip|IP]] addresses. The {{recursive-resolver|recursive resolver}} (usually run by your {{isp|ISP}} or a public service like 8.8.8.8 or 1.1.1.1) does the heavy lifting: it queries root servers, {{tld|TLD}} servers, and authoritative nameservers on your behalf, caching results at each level. The first [[dns|DNS]] query after connecting might take 50-100ms as the resolver walks the hierarchy, but subsequent queries for the same domain hit the cache and resolve in under 1ms. This is also where content delivery networks work their magic — [[dns|DNS]] can return different [[ip|IP]] addresses based on your geographic location, directing you to the nearest server.',
				transition:
			...
			},
			{
				...
				description:
					'With the full network stack operational, your email client can {{mqtt-connect|connect}} to the mail server and send your first message. [[smtp|SMTP]] (Simple Mail Transfer Protocol) uses a text-based command dialogue that has barely changed since 1982: {{ehlo|EHLO}} introduces your client, {{smtp-auth|AUTH}} {{login-auth|LOGIN}} authenticates you, {{smtp-mail-from|MAIL FROM}} and {{smtp-rcpt-to|RCPT TO}} specify the envelope addresses, and {{smtp-data|DATA}} begins the message body (headers, {{mime|MIME}} parts, attachments). Modern [[smtp|SMTP]] uses {{starttls|STARTTLS}} to upgrade the connection to encrypted, and {{spf|SPF}}/{{dkim|DKIM}}/{{dmarc|DMARC}} records in [[dns|DNS]] authenticate the sender to prevent {{spoofing|spoofing}}. This first successful email is proof that every layer of the network stack — from physical [[ethernet|Ethernet]] to application protocols — is functioning correctly.'
			}
		]
	},
	{
		...
		steps: [
			{
				...
				description:
					'[[rest|REST]] (Representational State Transfer) models every piece of data as a resource with a unique {{url|URL}}. Clients interact using standard {{http-method|HTTP}} methods: GET to read, POST to create, PUT to replace, DELETE to remove. [[rest|REST]] APIs are {{stateless|stateless}} — every request contains all the information needed to process it, making them easy to cache and scale. The trade-off is rigidity: clients get fixed data structures, often leading to {{over-fetching|over-fetching}} (getting fields they do not need) or under-fetching (needing multiple requests to assemble a view). [[rest|REST]] dominates the web because of its simplicity, but complex UIs with deeply nested data often outgrow it.',
				transition:
					'[[rest|REST]] APIs work well for simple {{crud|CRUD}} operations, but mobile and single-page apps often need data from many related resources in a single screen. What if the client could specify exactly the data it needs in one request?'
			},
			{
				protocolId: 'graphql',
		...
		steps: [
			{
				...
				description:
					'[[oauth2|OAuth]] 2.0 solves a fundamental problem: how can a third-party app access your data without knowing your password? Instead of sharing credentials, [[oauth2|OAuth]] uses an authorization server as an intermediary. You authenticate directly with the provider ({{google|Google}}, GitHub), approve specific permissions (scopes), and the provider issues a time-limited {{access-token|access token}} to the app. The Authorization Code flow with {{pkce|PKCE}} is the standard for web and mobile apps — it uses a code verifier to prevent interception attacks. Refresh tokens allow silent re-authentication without user interaction. [[oauth2|OAuth]] is authorization (what you can access), not authentication (who you are) — OpenID {{mqtt-connect|Connect}} adds the identity layer on top.',
				transition:
					'[[oauth2|OAuth]] tokens travel over the network as Bearer tokens in {{http-method|HTTP}} headers. But how are those {{http-method|HTTP}} connections themselves protected from eavesdroppers? The answer is the {{encryption|encryption}} layer that secures virtually all internet traffic...'
			},
			{
				protocolId: 'tls',
				title: 'TLS: Encrypted Channels',
				description:
					"[[tls|TLS]] protects every [[oauth2|OAuth]] token, every {{api|API}} key, and every password in {{transit|transit}}. During the {{handshake|handshake}}, the server proves its identity with a {{certificate|certificate}} signed by a trusted {{certificate-authority|Certificate Authority}}, and both sides negotiate {{encryption|encryption}} parameters. The key {{exchange|exchange}} ({{ecdhe|ECDHE}}) generates ephemeral session keys that provide {{forward-secrecy|forward secrecy}} — even if the server's {{private-key|private key}} is later compromised, past sessions remain secure. [[tls|TLS]] 1.3 streamlined this to a single round trip and removed all insecure legacy algorithms. Without [[tls|TLS]], [[oauth2|OAuth]] tokens would be visible to anyone on the network path.",
				transition:
					'[[tls|TLS]] secures data in {{transit|transit}} over the network. But system administrators need a different kind of secure access — interactive shell sessions, file transfers, and tunnels to remote machines. A purpose-built protocol provides this with a different trust model...'
			},
			{
				protocolId: 'ssh',
				title: 'SSH: Key-Based Machine Access',
				description:
					'[[ssh|SSH]] replaces the trust model entirely: instead of {{certificate|Certificate}} Authorities, it uses a "trust on first use" approach where you verify the server\'s host key fingerprint on first connection and your client remembers it for future sessions. Authentication typically uses Ed25519 key pairs — your {{private-key|private key}} stays on your machine, your {{public-key|public key}} is placed on every server you need access to. No passwords traverse the network. [[ssh|SSH]] also provides encrypted tunnels ({{port-forwarding|port forwarding}}) that can wrap any [[tcp|TCP]] protocol in {{encryption|encryption}}, {{sftp|SFTP}} for secure file transfer, and agent forwarding for chaining through bastion {{hosts-bare|hosts}}. It is the backbone of DevOps: git pushes, Ansible deployments, and remote debugging all flow through [[ssh|SSH]].'
			}
		]
	},
	{
		...
		description:
			'The diagnostic toolkit every engineer reaches for — from pinging {{hosts-bare|hosts}} to tracing routes to resolving addresses.',
		color: '#EF4444',
		scope: 'global',
		steps: [
			{
				...
				description:
					'When something goes wrong on a network, [[icmp|ICMP]] is the first tool you reach for. The {{ping|ping}} command sends an [[icmp|ICMP]] {{echo-request|Echo Request}} to a host and measures the time until the {{echo-reply|Echo Reply}} comes back — giving you {{rtt|round-trip time}}, packet loss percentage, and basic reachability in one command. {{traceroute|Traceroute}} exploits [[icmp|ICMP]] in a clever way: it sends packets with incrementally increasing {{ttl|TTL}} ({{ttl|Time to Live}}) values. Each router along the path decrements the {{ttl|TTL}}, and when it hits zero, that router sends back an [[icmp|ICMP]] {{time-exceeded|Time Exceeded}} message — revealing its identity and distance. This traces the exact path packets take across the internet, exposing where {{latency|latency}} spikes or failures occur.',
				transition:
					'[[icmp|ICMP]] told you whether a host is reachable and traced the network path to get there. But what if the problem is not routing — what if the hostname itself is not resolving to the right {{ip-address|IP address}}? [[dns|DNS]] issues are one of the most common causes of "the internet is broken"...'
			},
			{
				protocolId: 'dns',
				title: 'DNS: Is It Resolving?',
				description:
					'[[dns|DNS]] problems masquerade as total network failures — everything seems down, but the real issue is name resolution. The dig command queries [[dns|DNS]] servers directly, showing you exactly which records are returned, which {{nameserver|nameserver}} answered, the {{ttl|TTL}} remaining on cached entries, and the full query chain. You can query specific record types (A, {{aaaa-record|AAAA}}, {{mx-record|MX}}, {{cname-record|CNAME}}, {{ns-record|NS}}) and specific [[dns|DNS]] servers to isolate whether the problem is your local resolver, the authoritative server, or caching. Common issues include stale cached records (wait for {{ttl|TTL}} expiry or flush the cache), misconfigured {{ns-record|NS}} delegations, missing or incorrect A/{{aaaa-record|AAAA}} records, and {{dnssec|DNSSEC}} validation failures.',
				transition:
					'[[dns|DNS]] resolved the hostname to an {{ip-address|IP address}}, and [[icmp|ICMP]] confirmed the host is reachable at the network layer. But on a local network, there is one more address translation that happens invisibly — if this layer breaks, devices on the same {{subnet|subnet}} cannot communicate at all...'
			},
			{
				protocolId: 'arp',
				title: 'ARP: Is Layer 2 Working?',
				description:
					"[[arp|ARP]] operates at the boundary between [[ip|IP]] addresses and physical hardware. When your machine knows the [[ip|IP]] of a target on the same {{subnet|subnet}}, it still needs the target's {{mac-address|MAC address}} to build an [[ethernet|Ethernet]] frame. [[arp|ARP]] broadcasts a request to the entire {{lan|LAN}}, and the target responds with its {{mac-address|MAC address}}. The arp command shows the local [[arp|ARP]] cache — mapping IPs to MACs — and can reveal subtle issues: stale entries pointing to decommissioned hardware, duplicate [[ip|IP]] addresses (two MACs responding for the same [[ip|IP]]), or [[arp|ARP]] poisoning attacks where a malicious device claims to be the gateway. On modern networks, [[arp|ARP]] issues are rare but devastating when they occur, often causing intermittent connectivity that is maddeningly difficult to diagnose without checking the [[arp|ARP]] table directly."
			}
		]
	},
	{
		...
		steps: [
			{
				...
				transition:
					'[[tcp|TCP]] ties connections to [[ip|IP]] addresses, making them fragile on mobile networks. What if a single connection could span multiple network interfaces simultaneously, surviving handoffs without the application even noticing?'
			},
			{
				protocolId: 'mptcp',
				title: 'MPTCP: Multiple Paths',
				...
				transition:
					"[[mptcp|MPTCP]] adds resilience to [[tcp|TCP]], but it inherits [[tcp|TCP]]'s fundamental limitations: {{head-of-line-blocking|head-of-line blocking}}, ossified middleboxes, and a kernel-level implementation that is slow to deploy. A ground-up redesign built on [[udp|UDP]] avoids all of these constraints..."
			},
			{
				protocolId: 'quic',
				title: 'QUIC: Connection Migration',
				description:
					'[[quic|QUIC]] was designed from the start for mobile networks. Instead of identifying connections by [[ip|IP]] addresses and ports, [[quic|QUIC]] uses a variable-length Connection {{id-identifier|ID}} — a token that both endpoints recognize regardless of the underlying network path. When your phone switches from [[wifi|Wi-Fi]] to cellular, the {{ip-address|IP address}} changes but the Connection {{id-identifier|ID}} stays the same, so the [[quic|QUIC]] connection continues without interruption — no re-{{handshake|handshake}}, no re-authentication, no lost data. [[quic|QUIC]] also eliminates {{head-of-line-blocking|head-of-line blocking}} (a lost packet only affects its own stream, not all streams), and its {{one-rtt|1-RTT}} {{handshake|handshake}} integrates transport and {{encryption|encryption}} setup into a single round trip. {{google|Google}} reports that [[quic|QUIC]] reduces video rebuffering by 18% on mobile networks compared to [[tcp|TCP]].'
			}
		]
	},
	{
		...
		steps: [
			{
				...
				description:
					"When you send an email to user@example.com, your mail server does not just look up example.com's {{ip-address|IP address}} — it queries [[dns|DNS]] for {{mx-record|MX}} (Mail {{exchange|Exchange}}) records, a special record type that specifies which servers accept email for that domain and their priority ordering. A domain might have multiple {{mx-record|MX}} records (mx1.example.com at priority 10, mx2.example.com at priority 20) for redundancy — if the primary mail server is down, the sender automatically tries the backup. Your server also checks {{spf|SPF}} records (which IPs are authorized to send for that domain), {{dkim|DKIM}} records (public keys for verifying message signatures), and {{dmarc|DMARC}} records (the domain's policy for handling authentication failures). This [[dns|DNS]] step is where email security begins.",
				transition:
					"The [[dns|DNS]] lookup revealed exactly which servers accept mail for the recipient's domain, along with their priority ordering. Now your mail server opens a [[tcp|TCP]] connection to the highest-priority {{mx-record|MX}} server and begins the [[smtp|SMTP]] dialogue to deliver the message..."
			},
			{
				protocolId: 'smtp',
				title: 'SMTP: Send the Email',
				description:
					"[[smtp|SMTP]] is one of the oldest internet protocols still in active use — its text-based command dialogue has remained remarkably stable since 1982. Your mail server connects to the recipient's {{mx-record|MX}} server and they {{exchange|exchange}} greetings ({{ehlo|EHLO}}, which also advertises supported extensions like {{starttls|STARTTLS}} and SIZE). After upgrading to an encrypted connection via {{starttls|STARTTLS}}, the envelope is defined: {{smtp-mail-from|MAIL FROM}} specifies the bounce address, {{smtp-rcpt-to|RCPT TO}} identifies the recipient(s). Then the {{smtp-data|DATA}} command signals that the message body follows — headers (From, To, Subject, Date, Message-{{id-identifier|ID}}), {{mime|MIME}} parts (text/plain, text/html), and base64-encoded attachments, terminated by a lone period on a line. The receiving server may relay the message through additional hops (forwarding, mailing lists) before final delivery to the recipient's mailbox.",
				transition:
...

	{
		...
		description:
			'Follow an {{ai|AI}} agent from tool discovery to multi-agent collaboration — through [[json-rpc|JSON-RPC]], [[mcp|MCP]], and [[a2a|A2A]].',
		color: '#00D4FF',
		scope: 'web-api',
		steps: [
			{
				...
				description:
					'Before {{ai|AI}} protocols existed, [[json-rpc|JSON-RPC]] 2.0 was already the wire format of choice for infrastructure — Ethereum nodes, Bitcoin Core, and the {{lsp|Language Server Protocol}} in VS Code all spoke [[json-rpc|JSON-RPC]]. Its appeal was radical simplicity: send a {{json|JSON}} object with a method name, params, and an {{id-identifier|ID}}; get back a result or error with the same {{id-identifier|ID}}. No schema files, no code generation, no binary encoding. When {{anthropic|Anthropic}} and {{google|Google}} independently designed their {{ai|AI}} protocols, both chose [[json-rpc|JSON-RPC]] 2.0 as the foundation — not because it was trendy, but because it was the simplest thing that could possibly work.',
				transition:
					'[[json-rpc|JSON-RPC]] provides the wire framing, but it says nothing about what methods should exist, what parameters they should take, or how {{ai|AI}} applications should discover tools. A higher-level protocol was needed to define the semantics of {{ai|AI}} tool use...'
			},
			{
				protocolId: 'mcp',
				...
				transition:
					'[[mcp|MCP]] beautifully connects a single agent to its tools. But what happens when the task requires multiple specialized agents — a travel agent, a research agent, a booking agent — each with their own tools and expertise? A different protocol handles that layer of coordination...'
			},
			{
				protocolId: 'a2a',
	...
	},
	{
		...
		description:
			'The evolution of web APIs — from resource-oriented [[rest|REST]] to {{ai|AI}}-native protocols.',
		color: '#00D4FF',
		scope: 'global',
		steps: [
			{
				...
				description:
					"[[pioneer:roy-fielding|Roy Fielding]]'s 2000 dissertation defined [[rest|REST]] as an architectural style: use [[http1|HTTP]] verbs for operations, URLs for resources, and make everything {{stateless|stateless}}. [[rest|REST]] dominated the {{api|API}} landscape for two decades because of its simplicity — any language with an [[http1|HTTP]] client could call a [[rest|REST]] {{api|API}}. But [[rest|REST]] was designed for human developers building web applications, not for {{ai|AI}} systems that need to dynamically discover and invoke capabilities. {{over-fetching|Over-fetching}}, under-fetching, and the lack of machine-readable schemas were annoyances for developers but blockers for autonomous agents.",
				transition:
					'[[rest|REST]] served the web brilliantly for human-to-machine communication. But as systems grew more complex, developers needed richer patterns — typed contracts, efficient {{serialization|serialization}}, and flexible data fetching...'
			},
			{
				protocolId: 'grpc',
				title: 'gRPC: Typed, Efficient RPC',
				description:
					"{{google|Google}}'s internal {{rpc|RPC}} system, Stubby, handled billions of requests per day. When they open-sourced it as [[grpc|gRPC]] in 2015, it brought {{protocol-buffers|protocol buffers}} (binary {{serialization|serialization}} 3-10x smaller than {{json|JSON}}), [[http2|HTTP/2]] {{multiplexing|multiplexing}}, and streaming to the developer community. For the first time, {{api|API}} contracts were machine-enforced at compile time via .proto files. The tradeoff was complexity — code generation, [[http2|HTTP/2]] requirements, and binary payloads that couldn't be debugged with curl. But for service-to-service communication at scale, [[grpc|gRPC]] was a revelation.",
				transition:
					"[[grpc|gRPC]] optimized for machine-to-machine efficiency, but it required schema compilation and couldn't easily serve browser clients. What if there was something in between — human-readable like [[rest|REST]] but with typed schemas and flexible querying?"
			},
			{
				protocolId: 'json-rpc',
				title: 'JSON-RPC: The Minimal Wire Format',
				description:
					"While [[rest|REST]] and [[grpc|gRPC]] dominated mainstream APIs, a quieter revolution was happening in infrastructure. [[json-rpc|JSON-RPC]] 2.0 — a one-page spec for calling methods by name over {{json|JSON}} — became the backbone of Ethereum's blockchain {{api|API}}, Bitcoin Core, and {{microsoft|Microsoft}}'s {{lsp|Language Server Protocol}}. Its appeal was radical simplicity: no {{url|URL}} routing, no [[http1|HTTP]] verb semantics, no schema compilation. Just a method name, params, and an {{id-identifier|ID}}. The spec was so simple and transport-agnostic ([[http1|HTTP]], [[websockets|WebSocket]], {{stdio|stdio}}, [[tcp|TCP]]) that it became the natural foundation for the next generation of protocols.",
				transition:
					'[[json-rpc|JSON-RPC]] proved that method-oriented {{rpc|RPC}} over {{json|JSON}} was powerful enough for critical infrastructure. When the {{ai|AI}} revolution demanded new protocols for tool use and agent collaboration, [[json-rpc|JSON-RPC]] was the obvious wire format...'
			},
			{
				protocolId: 'mcp',
				title: 'MCP: AI-Native Tool Access',
				description:
					"In November 2024, {{anthropic|Anthropic}} released the Model Context Protocol — the first protocol designed specifically for {{ai|AI}} applications. Built on [[json-rpc|JSON-RPC]] 2.0, [[mcp|MCP]] provides dynamic tool discovery (tools/list returns {{json|JSON}} Schema definitions that LLMs can understand), resource access (files, database rows, {{api|API}} responses), and prompt templates. The key insight was that {{ai|AI}} applications don't just need APIs — they need self-describing APIs that an {{llm|LLM}} can discover, understand, and invoke autonomously. Within a year, Claude, ChatGPT, Copilot, Cursor, and VS Code all spoke [[mcp|MCP]], with over 10,000 [[mcp|MCP]] servers in production.",
				transition:
					'[[mcp|MCP]] connected {{ai|AI}} agents to tools, solving the integration problem. But the next challenge was bigger: how do you get multiple {{ai|AI}} agents — potentially from different vendors, built with different frameworks — to collaborate on complex tasks?'
			},
			{
				protocolId: 'a2a',
				title: 'A2A: AI-Native Agent Collaboration',
				description:
					"{{google|Google}}'s Agent-to-Agent Protocol, announced in April 2025, completed the picture. Where [[mcp|MCP]] connects agents to tools, [[a2a|A2A]] connects agents to each other. An agent publishes its skills in an {{agent-card|Agent Card}}, and other agents discover it, delegate tasks, and receive structured results — all without knowing anything about the internal implementation. The task lifecycle (submitted → working → completed) with [[sse|SSE]] streaming provides the coordination primitives that multi-agent systems need. Together, [[mcp|MCP]] and [[a2a|A2A]] form the two-protocol foundation of the agentic {{ai|AI}} era — donated to the {{linux|Linux}} Foundation as open industry standards."
			}
		]
	},
	...
	// ── Wireless ───────────────────────────────────────────────────────
	{
		...
		description:
			'A contactless {{emv-cryptogram|EMV}} payment is the most-deployed cryptographic protocol on Earth. Follow a tap from antenna power-on to issuer approval — through [[nfc|NFC]], {{emv-cryptogram|EMV}}, the eSE, and back through the [[cellular|cellular]] network to the bank.',
		color: '#FBBF24',
		scope: 'wireless',
		steps: [
			{
				...
				description:
					"The terminal's antenna is radiating a 13.56 MHz magnetic field continuously. When you bring your phone within ~4 cm, the phone's [[nfc|NFC]] controller and {{ese|embedded Secure Element}} harvest microwatts directly from the field — no battery contribution needed on the {{se-secure-element|SE}} side. This is the {{inductive-coupling|inductive coupling}} that makes [[nfc|NFC]] work: magnetic field falls off as 1/r³, which is exactly why 10 cm is a feature, not a bug. The biometric release (Face {{id-identifier|ID}} / Touch {{id-identifier|ID}}) had to happen *before* this point — the eSE applet is now armed and waiting.",
				transition:
					'The phone is powered and the terminal needs to discover what kind of card is in the field. {{iso|ISO}} 14443-3 defines a tight {{anti-collision|anti-collision}} sequence — {{reqa|REQA}} / {{atqa|ATQA}} / {{sel-iso|SEL}} / {{sak|SAK}} — that completes in under 20 ms...'
			},
			{
				protocolId: 'nfc',
				title: 'Anti-collision + RATS/ATS — negotiate framing',
				description:
					'The terminal sends a 7-bit `0x26` {{reqa|REQA}}. The eSE replies with **{{atqa|ATQA}}** declaring a 4-byte {{uid|UID}} and standard {{anti-collision|anti-collision}}. The terminal then runs the bit-frame {{anti-collision|anti-collision}} loop (`{{sel-iso|SEL}}=0x93`, `NVB=0x20` → {{uid|UID}} → `NVB=0x70`) to converge on the {{frame|frame}} {{uid|UID}}, ending in **{{sak|SAK}}** = `0x28` — bit 6 set means *I speak {{iso|ISO}} 14443-4*. The terminal sends **{{rats|RATS}}**, the eSE replies with **{{ats-nfc|ATS}}** declaring its frame-size budget (FSCI=5 = 64 bytes max). Both ends now know how to {{packet|packet}}ise the {{emv-cryptogram|EMV}} {{exchange|exchange}}.',
				transition:
					'With 14443-4 framing established, the terminal switches into {{iso|ISO}} 7816-4 {{apdu|APDU}} mode and asks the canonical {{emv-cryptogram|EMV}} opening question: *which payment networks do you support?*'
			},
			{
				protocolId: 'nfc',
				title: 'SELECT PPSE → SELECT AID — enumerate payment apps',
				description:
					'The terminal sends `00 A4 04 00 0E 32 50 41 59 2E 53 59 53 2E 44 44 46 30 31` — {{imap-select|SELECT}} **{{ppse|PPSE}}** (Proximity Payment System Environment, the magic {{aid|AID}} `2PAY.SYS.DDF01`). The eSE returns an FCI Template listing every payment {{aid|AID}} it supports in priority order — Mastercard `A0000000041010`, Visa `A0000000031010`, etc. The terminal picks the highest-priority one and SELECTs it; the card returns its **{{pdol|PDOL}}** — the list of {{emv-cryptogram|EMV}} tags the card needs filled in to compute the cryptogram (amount, currency, country, terminal type, Unpredictable Number).',
				transition:
					'The card now knows what payment network it is on; the terminal knows what data the card wants. Time to bind the transaction: amount, currency, a fresh random {{nonce|nonce}}.'
			},
			{
				protocolId: 'nfc',
				title: 'GENERATE AC — the cryptogram',
				description:
					'After GET PROCESSING OPTIONS (returning AIP+{{afl|AFL}}) and {{read-record|READ RECORD}} ×N (pulling the **{{dpan|DPAN}}**, expiry, {{certificate-chain|certificate chain}}), the terminal sends `80 AE 80 00` {{generate-ac|GENERATE AC}} with the filled CDOL1 data. The eSE composes the inputs (amount, currency, country, TVR, {{atc|ATC}}, Unpredictable Number, AIP) and runs them through {{aes|AES}}-{{mac-address|MAC}} under the per-{{dpan|DPAN}} key, producing the **{{emv-cryptogram|Application Cryptogram (ARQC)}}**. The {{anti-replay|ATC}} (Application Transaction Counter) has already incremented by 1; the cryptogram is unforgeable without the eSE key.',
				transition:
					"The cryptogram is now in the terminal's hand. But the eSE has no online connection — it can't authorise the transaction itself. The terminal needs to send the {{arqc|ARQC}} to the issuer bank, which is somewhere on the other end of the carrier network..."
			},
			{
				protocolId: 'cellular',
				title: 'Backhaul over LTE / 5G to the acquirer',
				description:
					"Modern wireless POS terminals (Square, Stripe, Verifone Engage) send the {{arqc|ARQC}} to the acquirer over [[cellular|cellular]] data — {{lte|LTE}} Cat-1 or 5G in 2026. The terminal opens a [[tls|TLS]] connection to the acquirer's {{api|API}} endpoint, posts a {{json|JSON}} body containing the cryptogram + transaction details. The carrier core wraps the [[ip|IP]] traffic in **{{gtp-u|GTP-U}} over [[ipsec|IPsec ESP]]** between the gNB and {{upf|UPF}} — per {{3gpp|3GPP}} {{ts-3gpp|TS}} 33.501, every backhaul {{hop|hop}} is IPsec-wrapped. The single largest enterprise [[ipsec|IPsec]] deployment on Earth lives inside this layer.",
				transition:
					"The cryptogram reaches the acquirer; the acquirer routes it through the payment network (Mastercard / Visa) to the issuing bank's {{hsm|HSM}}..."
			},
			{
				protocolId: 'tls',
				title: 'Issuer verification + ARPC return',
				description:
					"The **issuer bank's {{hsm|HSM}}** decrypts the cryptogram inputs using the per-{{dpan|DPAN}} key it minted at tokenisation time, re-derives the {{arqc|ARQC}}, and compares. Match + sufficient balance + no fraud flag = **APPROVED**. The issuer returns an **ARPC** (Authorisation Response Cryptogram) over its own [[tls|TLS]]-protected connection back through the payment network to the acquirer, then back over [[cellular|cellular]] to the terminal. Total time including the round-trip: typically **300–800 ms**. The terminal beeps green; the phone vibrates with the {{apple|Apple}} Pay success animation. Total [[nfc|NFC]] {{airtime|airtime}} in the field was less than half a second — but the cryptographic chain stretched from the eSE through [[ipsec|IPsec]], cellular, [[tls|TLS]], and back."
			}
		]
	},

	{
		...
		description:
			'The 2022 Tesla {{ble|BLE}} relay made it clear that {{rssi|RSSI}} proximity is fundamentally broken. {{ccc-digital-key|CCC Digital Key}} 3.0 is the industry response: [[bluetooth|BLE]] for discovery and credential {{exchange|exchange}}, [[uwb|UWB]] for the cryptographic distance bound that the speed of light cannot lie about.',
		color: '#FBBF24',
		scope: 'wireless',
		steps: [
			{
				...
				description:
					"The car's {{ble|BLE}} radio broadcasts `{{adv-ind|ADV_IND}}` on advertising channels 37 (2402 MHz), 38 (2426 MHz), and 39 (2480 MHz) every 100 ms or so — three channels carefully chosen to avoid [[wifi|Wi-Fi]]'s 1/6/11. The advert carries the {{ccc-digital-key|CCC Digital Key}} service {{uuid|UUID}} and an ephemeral identifier. Your iPhone (or Galaxy, or {{android|Android}} phone with {{aliro|Aliro}}) scans those channels continuously. When the phone sees the car's {{uuid|UUID}}, it sends `{{connect-ind|CONNECT_IND}}` — switching both radios from advertising channels to one of the 37 data channels — and the {{ble|BLE}} link is up.",
				transition:
					'{{ble|BLE}} is now connected, but {{ble|BLE}} alone proved insecure in 2022 — Sultan Qasim Khan unlocked a parked Tesla from 25 m using $50 of dev boards by relaying the {{ble|BLE}} signals. {{ccc-digital-key|CCC Digital Key}} 3.0 fixes this with a layered cryptographic + physical defence...'
			},
			{
				protocolId: 'bluetooth',
				title: 'GATT pairing + SPAKE2+ authentication',
				description:
					'Over the {{ble|BLE}} encrypted channel, the car and the phone run **{{spake2|SPAKE2}}+ / {{pake|PAKE}}** authentication — the car proves it has the right vehicle key, the phone proves it has the right Digital Key applet in its {{ese|embedded Secure Element}}. {{apdu|APDUs}} flow over {{gatt|GATT}} carrying the {{emv-cryptogram|EMV}}-style {{certificate-chain|certificate chain}}. Both sides now share session keys. The next critical message: the car sends the phone the **STS_KEY** — a 128-bit {{aes|AES}} key for the upcoming [[uwb|UWB]] ranging session — over the now-encrypted {{ble|BLE}} channel. [[uwb|UWB]] has no power-efficient discovery of its own; {{ble|BLE}} provides the on-ramp.',
				transition:
					'The phone has the STS_KEY. The car has the STS_KEY. Both fire up their [[uwb|UWB]] radios for the ranging round that proves *the phone is actually here*, not relayed from across the parking lot...'
			},
			{
				protocolId: 'uwb',
				title: 'UWB DS-TWR — three messages, six timestamps, cm-class distance',
				description:
					'The phone transmits a **Poll** RFRAME on [[uwb|UWB]] Channel 9 (7987.2 MHz, 499.2 MHz {{bandwidth|bandwidth}}, {{bprf|BPRF}} mode, 6.81 Mbps). The frame carries a 32-chip **{{sts|STS}}** (Scrambled Timestamp Sequence) generated by `{{aes-128|AES-128}}-CTR(STS_KEY, {{nonce|nonce}})` — the {{aes|AES}}-keyed pulse pattern an attacker without STS_KEY cannot predict. The phone records `t1 = TX timestamp` at the {{sfd|SFD}} with ~15 ps resolution. The car (multiple anchors typically) RX-timestamps at t2, delays by `T_reply1` (~200 µs), transmits **Response** carrying t2 + t3. Phone records t4 = RX. Phone delays by T_reply2, transmits **Final** carrying t1, t4, t5. Car records t6 = RX. All six timestamps now exist.',
				transition:
					'The car can now compute {{tof-ranging|time-of-flight}}. The cross-product formula cancels relative {{clock-drift|clock drift}} to first order — {{ds-twr|DS-TWR}} is insensitive to 20 ppm crystal offsets. Multiply ToF by the speed of light and you have distance...'
			},
			{
				protocolId: 'uwb',
				title: 'Distance check — and the unlock decision',
				description:
					"Using the **{{twr|DS-TWR}} cross-product** `ToF = (T_round1·T_round2 − T_reply1·T_reply2) / (T_round1+T_round2+T_reply1+T_reply2)`, the car computes {{tof-ranging|time-of-flight}} to 1 ns precision — about 30 cm. Multiple anchors give an x/y/z position around the car. If `distance ≤ threshold` *and* the {{ble|BLE}} credential is valid *and* the {{sts|STS}} validation passed (no Ghost Peak-style injection detected), the car unlocks the side you're approaching, lights the welcome animation, and adjusts your seat. The key property: **{{tof-ranging|time-of-flight}} cannot be shortened by a relay** — the speed of light is the hard upper bound. The 2022 {{ble|BLE}} relay attack does not work against [[uwb|UWB]] ranging. [[nfc|NFC]] remains the fallback for when your phone's battery is dead."
			}
		]
	},

	{
		...
		description:
			'You unbox a new Philips Hue bulb, plug it in, and the Hue app on your phone walks it through Zigbee {{trust-center|Trust Center}} commissioning. Behind that simple {{ux|UX}} is a four-message join sequence with one critical secret transfer.',
		color: '#FBBF24',
		scope: 'wireless',
		steps: [
			{
				...
				description:
					"The new bulb powers on, has no parent yet, and broadcasts an {{ieee-802-15-4|IEEE 802.15.4}} {{mac-address|MAC}} Command `0x07` (**Beacon Request**) on its channel — picked from {15, 20, 25, 26} to dodge [[wifi|Wi-Fi]] 1/6/11 at 2412/2437/2462 MHz. Every {{zigbee-router|Zigbee router}} with permit-joining enabled responds with a **Beacon**: the {{pan-id|PAN ID}}, the Coordinator's {{short-address|short address}} (0x0000), the Stack Profile (Zigbee PRO), and the Permit-Joining flag. The joiner picks the best parent by {{rssi|RSSI}} + LQI. This is the slowest step — beacon scanning can take 2–4 seconds across all candidate channels.",
				transition:
					'The bulb has picked a parent. Now it needs a {{short-address|short address}} — a 16-bit local identifier much cheaper than its 64-bit {{eui-64|EUI-64}} on every frame for the rest of its life on this network...'
			},
			{
				protocolId: 'zigbee',
				title: 'Association Request + Response — get a short address',
				description:
					"The bulb sends {{mac-address|MAC}} Command `0x01` (**{{association-request|Association Request}}**) with its Capability byte (`0x8E` = {{ffd|FFD}}, mains-powered, security-capable, allocate-short). The Coordinator allocates a unique 16-bit {{short-address|short address}} (e.g. `0x3F4E`) and replies with {{mac-address|MAC}} Command `0x02` (**{{association-response|Association Response}}**). The bulb is now associated with the network at the {{mac-address|MAC}} layer — but it doesn't have the **{{network-key|network key}}** yet, which means it can't decrypt or {{encryption|encrypt}} any actual Zigbee frames. The critical security step is next.",
				transition:
					'Without the {{network-key|network key}} the bulb is useless. The Coordinator (acting as {{trust-center|Trust Center}}) needs to securely deliver it — and how it does that is the single most important security decision in the whole Zigbee architecture...'
			},
			{
				protocolId: 'zigbee',
				title: 'APS Transport-Key — the network key, encrypted at the application layer',
				description:
					"The Coordinator sends an **{{aps-layer|APS}} Transport-Key** command (cmd `0x05`) containing the 128-bit {{aes|AES}} {{network-key|network key}}, **{{encryption|encrypted}} under the joiner's pre-configured link key**. With a per-device {{install-code|install code}} (printed on the Hue bulb's box as a QR code), that link key is unique and an eavesdropper at join cannot decrypt this frame. With the default *ZigBeeAlliance09* link key (universally known: `5A:69:67:42:65:65:41:6C:6C:69:61:6E:63:65:30:39`), an eavesdropper *can* — this is the canonical Zigbee sniffer-at-join attack. Zigbee R23's **{{dynamic-link-key|Dynamic Link Key}}** with SPEKE-over-{{curve25519|Curve25519}} removes the question entirely.",
				transition:
					'The bulb now has both a {{short-address|short address}} and the {{network-key|network key}}. It can encrypt frames and join the mesh as a router for its neighbours. Time to announce its arrival...'
			},
			{
				protocolId: 'zigbee',
				title: 'Device Announce + first ZCL command',
				description:
					'The bulb {{zigbee-nwk|NWK}}-broadcasts a **Device Announce** {{zigbee-zdo|ZDO}} message (cluster `0x0013`): *I am 0x3F4E, {{eui-64|EUI-64}} = …, capability = mains-powered router*. Every router on the mesh adds the new bulb to its {{routing-table|routing}} and binding tables. The Hue app now appears to discover the bulb. Tapping the on/off toggle sends a single **{{zcl|ZCL}} OnOff.Toggle** command — {{aps-layer|APS}} profile `0x0104` (Home Automation), cluster `0x0006` (OnOff), command `0x02` (Toggle). The whole on-the-wire {{payload|payload}}, including all 802.15.4 + {{zigbee-nwk|NWK}} + {{aps-layer|APS}} + {{zcl|ZCL}} {{header|headers}} + {{aes|AES}}-{{ccm-mode|CCM}}* {{mic|MIC}}, fits in ~40 bytes. The bulb turns on. From boot to first command: ~4 seconds.'
			}
		]
	}
];
