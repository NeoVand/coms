...
export const journeys: Journey[] = [
	// ── Global Journeys ────────────────────────────────────────────────
	{
		...
		steps: [
			...
			},
			{
				...
				transition: 'A reliable pipe now connects your browser to the server — but anyone sitting on the network path (your {{isp|ISP}}, a coffee shop router, a government {{firewall|firewall}}) can read every byte in plaintext. The data needs {{encryption|encryption}}...'
			},
			{
				protocolId: 'tls',
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				transition: 'With the destination {{mac-address|MAC}} resolved and the [[ethernet|Ethernet]] frame ready, the packet can now be addressed for its journey beyond the local network. The [[ip|IP]] layer takes over to handle global addressing and routing...'
			},
			{
				protocolId: 'ip',
				title: 'IP Routing',
		...
		steps: [
			{
				...
				transition: '[[udp|UDP]] gives you raw speed, but many applications cannot tolerate missing or reordered data. A web page with a missing {{css|CSS}} file, a bank transfer with lost bytes, or a file download with gaps — these need every byte in the right order. The internet needed a more disciplined transport...'
			},
			{
				protocolId: 'tcp',
	...
	},
	{
		...
		steps: [
			...
			},
			{
				...
				transition: 'With [[tls|TLS]] protecting web connections and [[ssh|SSH]] securing server access, the foundations of internet security were in place. But there was still a problem: {{http-method|HTTP}} {{encryption|encryption}} was optional, and most sites did not bother. The web needed a forcing function to make {{encryption|encryption}} the default, not the exception...'
			},
			{
				protocolId: 'http2',
		...
		steps: [
			{
				...
				transition: 'The [[ethernet|Ethernet]] frame is ready to send, but there is a chicken-and-egg problem: your application knows the destination {{ip-address|IP address}} (192.168.1.100), not the destination {{mac-address|MAC address}}. You cannot build an [[ethernet|Ethernet]] frame without a {{mac-address|MAC}}. The network needs a way to translate between these two addressing systems...'
			},
			{
				protocolId: 'arp',
				title: 'ARP Resolution',
				...
				transition: 'With the destination {{mac-address|MAC}} resolved, the [[ethernet|Ethernet]] frame can be properly addressed for the local segment. Now the [[ip|IP]] layer needs to make a critical decision: is this packet destined for a machine on the same local network, or does it need to be forwarded to the {{gateway|default gateway}} for routing across the internet?'
			},
			{
				protocolId: 'ip',
				...
				transition: 'The packet has been delivered successfully — but networks are not always healthy. Links go down, routes change, {{hosts-bare|hosts}} become unreachable, and packets get dropped. How does a network administrator diagnose problems and verify that everything is working correctly?'
			},
			{
				protocolId: 'icmp',
...

	{
		...
		steps: [
			...
			},
			{
				...
				transition: 'With [[quic|QUIC]] providing fast, encrypted, multiplexed transport, the final piece was adapting {{http-method|HTTP}} to take advantage of it. [[http3|HTTP/3]] is not just "[[http2|HTTP/2]] on [[quic|QUIC]]" — it had to be redesigned because [[quic|QUIC]]\'s streams replaced the stream {{multiplexing|multiplexing}} that [[http2|HTTP/2]] implemented over [[tcp|TCP]]...'
			},
			{
				protocolId: 'http3',
	...
	// ── Web / API ──────────────────────────────────────────────────────
	{
		...
		description: 'Three decades of {{http-method|HTTP}} evolution — from [[pioneer:tim-berners-lee|Tim Berners-Lee]]\'s hypertext to [[quic|QUIC]]-powered streams.',
		color: '#00D4FF',
		scope: 'web-api',
		steps: [
			{
		...
		steps: [
			{
				...
				transition: '[[rest|REST]] APIs work well for simple {{crud|CRUD}} operations, but mobile and single-page apps often need data from many related resources in a single screen. What if the client could specify exactly the data it needs in one request?'
			},
			{
				protocolId: 'graphql',
		...
		steps: [
			{
				...
				transition: '[[oauth2|OAuth]] tokens travel over the network as Bearer tokens in {{http-method|HTTP}} headers. But how are those {{http-method|HTTP}} connections themselves protected from eavesdroppers? The answer is the {{encryption|encryption}} layer that secures virtually all internet traffic...'
			},
			{
				protocolId: 'tls',
		...
		steps: [
			{
				...
				transition: '[[json-rpc|JSON-RPC]] provides the wire framing, but it says nothing about what methods should exist, what parameters they should take, or how {{ai|AI}} applications should discover tools. A higher-level protocol was needed to define the semantics of {{ai|AI}} tool use...'
			},
			{
				protocolId: 'mcp',
	...
	},
	{
		...
		description: 'The evolution of web APIs — from resource-oriented [[rest|REST]] to {{ai|AI}}-native protocols.',
		color: '#00D4FF',
		scope: 'global',
		steps: [
			{
			...
			},
			{
				...
				transition: '[[json-rpc|JSON-RPC]] proved that method-oriented {{rpc|RPC}} over {{json|JSON}} was powerful enough for critical infrastructure. When the {{ai|AI}} revolution demanded new protocols for tool use and agent collaboration, [[json-rpc|JSON-RPC]] was the obvious wire format...'
			},
			{
				protocolId: 'mcp',
				...
				transition: '[[mcp|MCP]] connected {{ai|AI}} agents to tools, solving the integration problem. But the next challenge was bigger: how do you get multiple {{ai|AI}} agents — potentially from different vendors, built with different frameworks — to collaborate on complex tasks?'
			},
			{
				protocolId: 'a2a',
		...
		steps: [
			{
				...
				transition: 'The phone is powered and the terminal needs to discover what kind of card is in the field. {{iso|ISO}} 14443-3 defines a tight {{anti-collision|anti-collision}} sequence — {{reqa|REQA}} / {{atqa|ATQA}} / {{sel-iso|SEL}} / {{sak|SAK}} — that completes in under 20 ms...'
			},
			{
				protocolId: 'nfc',
				...
				transition: "With 14443-4 framing established, the terminal switches into {{iso|ISO}} 7816-4 {{apdu|APDU}} mode and asks the canonical {{emv-cryptogram|EMV}} opening question: *which payment networks do you support?*"
			},
			{
				protocolId: 'nfc',
			...
			},
			{
				...
				transition: "The cryptogram is now in the terminal's hand. But the eSE has no online connection — it can't authorise the transaction itself. The terminal needs to send the {{arqc|ARQC}} to the issuer bank, which is somewhere on the other end of the carrier network..."
			},
			{
				protocolId: 'cellular',
				...
				transition: "The cryptogram reaches the acquirer; the acquirer routes it through the payment network (Mastercard / Visa) to the issuing bank's {{hsm|HSM}}..."
			},
			{
				protocolId: 'tls',
		...
		steps: [
			{
				...
				transition: '{{ble|BLE}} is now connected, but {{ble|BLE}} alone proved insecure in 2022 — Sultan Qasim Khan unlocked a parked Tesla from 25 m using $50 of dev boards by relaying the {{ble|BLE}} signals. {{ccc-digital-key|CCC Digital Key}} 3.0 fixes this with a layered cryptographic + physical defence...'
			},
			{
				protocolId: 'bluetooth',
			...
			},
			{
				...
				transition: 'The car can now compute {{tof-ranging|time-of-flight}}. The cross-product formula cancels relative {{clock-drift|clock drift}} to first order — {{ds-twr|DS-TWR}} is insensitive to 20 ppm crystal offsets. Multiply ToF by the speed of light and you have distance...'
			},
			{
				protocolId: 'uwb',
		...
		steps: [
			{
				...
				transition: 'The bulb has picked a parent. Now it needs a {{short-address|short address}} — a 16-bit local identifier much cheaper than its 64-bit {{eui-64|EUI-64}} on every frame for the rest of its life on this network...'
			},
			{
				protocolId: 'zigbee',
				...
				transition: 'Without the {{network-key|network key}} the bulb is useless. The Coordinator (acting as {{trust-center|Trust Center}}) needs to securely deliver it — and how it does that is the single most important security decision in the whole Zigbee architecture...'
			},
			{
				protocolId: 'zigbee',
				...
				transition: "The bulb now has both a {{short-address|short address}} and the {{network-key|network key}}. It can encrypt frames and join the mesh as a router for its neighbours. Time to announce its arrival..."
			},
			{
				protocolId: 'zigbee',
