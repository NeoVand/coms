		...
		sections: [
			{
				...
				text: `A {{protocol|protocol}} is a set of rules that two systems agree to follow so they can exchange information. The dictionary definition would tell you it is a sequence of moves; the engineering one is sharper. A protocol is a **shared specification** — a document that any third party can read and implement, such that two implementations written by people who have never met can interoperate on the first try.
...
Together: [[dns|DNS]] resolves the hostname → [[ip|IP]] routes the packet to the host → [[arp|ARP]] (or [[ipv6|IPv6 NDP]]) resolves the next-hop [[ip|IP]] to a MAC → the port delivers the payload to the right application.`
			},
			{
				...
				definition: `graph TD
  DNS["Hostname<br/><b>google.com</b><br/>for humans"]
  IP["IP address<br/><b>142.250.80.46</b><br/>routes across the internet"]
  MAC["MAC address<br/><b>f4:5c:89:9c:1a:30</b><br/>delivers on the local segment"]
...
A {{packet|packet}} is a **self-contained unit** with a **header** (control information — addresses, length, checksum) and a **payload** (the bytes the application actually sent). Routers along the path read only the header; they never need to look at the payload, and they certainly never need to remember anything about previous packets in the same conversation. That statelessness is what lets the internet route trillions of packets per second through equipment that costs less than a car.`
			},
			{
				type: 'diagram',
  ...
  S["<b>TCP Segment</b><br/>+ src/dst port, seq#"]
  P["<b>IP Packet</b><br/>+ src/dst IP, TTL"]
  F["<b>Ethernet Frame</b><br/>+ src/dst MAC, CRC"]
  D -->|"TCP wraps"| S
  S -->|"IP wraps"| P
  P -->|"Ethernet wraps"| F`,
...
**Federated systems** (Mastodon, ActivityPub, [[smtp|SMTP]] email) are client-server within each instance but **peer-to-peer at the server level** — your Mastodon server federates with thousands of others to expose a global social graph without a central operator.
...
[[tls|TLS]] (the protocol behind HTTPS) provides three things, all of them precisely defined: **confidentiality** — nobody on the path can read your bytes; **integrity** — nobody can modify your bytes without you noticing; **authenticity** — you can verify the server is who its certificate claims it is. It does not provide non-repudiation, end-to-end encryption beyond the channel, or any guarantee about how the server stores or processes your data after decryption. Knowing exactly what [[tls|TLS]] gives you is the first step in not over-trusting it.`
			},
			{
				...
				text: `Two families of encryption exist, with opposite trade-offs.
...
**{{asymmetric-encryption|Asymmetric encryption}}** uses **two keys** that are mathematically paired: a {{public-key|public key}} you give out freely, and a {{private-key|private key}} you keep on your server. Anything encrypted with the public key can only be decrypted with the private key, and vice versa. RSA-2048, X25519 (elliptic curve), and (post-quantum) ML-KEM-768 are the modern examples. The strength of asymmetric crypto is that **two strangers can establish trust** — I send you my public key over an open channel; you encrypt your secret with it; only I can read what you sent. The cost: asymmetric encryption is roughly **1000× slower** than symmetric for the same security level.
...
The combination is what makes the modern web tractable. You use slow asymmetric crypto **once** at the start of the connection to safely agree on a fast symmetric key, then use the symmetric key for all the bulk data. The slow operation is amortised across the whole conversation.`
  ...
  Handshake -->|"shared secret"| Data`,
			...
			},
			{
				...
				text: `Asymmetric encryption alone is not enough. If a server hands you "here is my public key, encrypt to me," nothing prevents an attacker from intercepting the connection and handing you their own public key — a classic **man-in-the-middle attack**. You would encrypt to the attacker, who would decrypt, re-encrypt to the real server, and read everything in transit.
...
For a brand-new protocol layer where adoption is the existential risk, picking the format that already works in every language and every developer's mental model is the right move. [[mcp|MCP]] and [[a2a|A2A]] could have invented a binary protocol with schemas; instead they let the message shape be a transport-level concern and put their innovation in **what** the messages mean.`
			},
			{
				...
				text: 'AI protocols follow patterns that look very familiar. **Capability discovery** like [[dns|DNS]]: a server lists what it can do; the client queries the list. **Request/response messaging** like [[http1|HTTP]]: each call has a method, parameters, a result. **Stateful sessions** like [[tcp|TCP]]: a connection has an initialisation handshake; subsequent calls share that context. **Streaming** like [[sse|SSE]]: long-running operations stream progress events. The engineering vocabulary is the same; the **payload domain** — describing tools, agents, capabilities, prompts, resources — is what is new.'
			},
			{
				type: 'narrative',
