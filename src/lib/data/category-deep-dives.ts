...
export const categoryDeepDives: CategoryDeepDive[] = [
	{
		...
		tagline:
			'Advanced Layer 2-3 topics — VLANs, spanning tree, IPv6 migration, {{arp-cache|ARP}} security, and enterprise switching.',
		sections: [
			{
				type: 'narrative',
				title: 'VLANs and Network Segmentation',
				text: `In a flat Layer 2 network, every device can see every other device's {{broadcast|broadcast}} traffic. VLANs (Virtual LANs, {{ieee-802-15-4|IEEE}} 802.1Q) solve this by logically partitioning a physical switch into separate {{broadcast|broadcast}} domains. A single 48-port switch can operate as if it were multiple independent switches.

{{vlan|VLAN}} tagging inserts a 4-byte header into {{frame|Ethernet frames}}, containing a 12-bit {{vlan|VLAN}} {{id-identifier|ID}} (1-4094). Trunk ports carry frames from multiple VLANs between switches; access ports strip the tag and {{mqtt-connect|connect}} to end devices. Inter-{{vlan|VLAN}} {{routing-table|routing}} requires a Layer 3 device -- either a router on a stick or a Layer 3 switch.
...
VLANs are essential for security (isolating guest [[wifi|WiFi]] from the corporate network), performance (reducing {{broadcast|broadcast}} storms), and compliance (PCI DSS requires cardholder data on its own {{subnet|subnet}}).`
    ...
    A_AP2 --- A_TRUNK
    A_AP3 --- A_TRUNK
    A_TRUNK -- "Tagged Frames<br/>({{vlan|VLAN}} {{id-identifier|ID}} in 802.1Q header)" --- B_TRUNK
    B_TRUNK --- B_AP1
    B_TRUNK --- B_AP2
    A_TRUNK --- L3
    SVI10 -.- SVI20
    SVI20 -.- SVI30`
			},
			{
				...
				text: `Redundant links between [[ethernet|switches]] are essential for reliability -- but they create loops that cause {{broadcast|broadcast}} storms. Spanning Tree Protocol ({{ieee-802-15-4|IEEE}} 802.1D) prevents loops by electing a root bridge, calculating the shortest path from every switch to the root, and blocking redundant {{port|ports}}.

Classic STP converges in 30-50 seconds after a topology change -- an eternity for modern networks. Rapid Spanning Tree Protocol (RSTP, {{ieee-802-15-4|IEEE}} 802.1w) reduces this to 1-2 seconds with proposal/agreement {{handshake|handshakes}}. Multiple Spanning Tree Protocol (MSTP, 802.1s) maps multiple VLANs to fewer spanning tree instances for efficiency.
...
Dynamic [[arp|ARP]] Inspection (DAI) is the primary defense. It intercepts [[arp|ARP]] {{packet|packets}} on untrusted ports and validates them against a [[dhcp|DHCP]] snooping binding table. If the [[ip|IP]]-to-{{mac-address|MAC}} mapping doesn't match a legitimate [[dhcp|DHCP]] {{lease|lease}}, the [[arp|ARP]] packet is dropped.
...
[[ipv6|IPv6]] replaces [[arp|ARP]] entirely with {{ndp|Neighbor Discovery Protocol}} ({{ndp|NDP}}), which runs over [[icmp|ICMPv6]]. While {{ndp|NDP}} has its own {{spoofing|spoofing}} risks, Secure Neighbor Discovery (SEND) uses cryptographic addressing to authenticate neighbor advertisements.`
			},
			{
				...
				text: `When you walk from one room to another, your device must seamlessly switch from one {{access-point|access point}} to another -- this is [[wifi|Wi-Fi]] roaming. Basic roaming is slow: the client must re-authenticate with each {{access-point|AP}}. {{ieee-802-15-4|IEEE}} 802.11r (Fast {{bss-coloring|BSS}} Transition) pre-authenticates with target APs, reducing handoff to under 50ms.

802.11k (Radio Resource Management) helps clients discover nearby APs without scanning every channel. 802.11v ({{bss-coloring|BSS}} Transition Management) lets APs steer clients toward less-congested access points. Together, 802.11r/k/v enable enterprise-grade seamless roaming.

[[wifi|Wi-Fi]] mesh networks (802.11s) take this further -- APs {{mqtt-connect|connect}} to each other wirelessly, forming a self-healing fabric. Each {{access-point|AP}} acts as both an {{access-point|access point}} and a relay, automatically routing traffic along the best path. Consumer mesh systems (Eero, {{google|Google}} [[wifi|WiFi]]) use proprietary versions of this concept.`
			},
			{
				type: 'narrative',
...
**Local preference** controls outbound traffic by assigning weights to routes -- higher preference means "prefer this path." **{{med|MED}} (Multi-Exit Discriminator)** hints to neighboring ASes which entry point to use. **{{autonomous-system|AS}}-path prepending** makes a path look longer to discourage inbound traffic through that link.

[[bgp|BGP]] communities tag routes with metadata that triggers policies in other networks. Standard communities (like "no-export") prevent route propagation. Large communities ({{rfc-doc|RFC}} 8092) enable fine-grained traffic engineering across multi-provider networks.`
			}
		]
	},
	{
		...
		tagline:
			'{{congestion-control|Congestion control}} algorithms, {{flow-control|flow control}} mechanics, and the subtle engineering that keeps the internet stable.',
		sections: [
			{
				type: 'narrative',
...
**{{bbr|BBR}}** (2016, {{google|Google}}) took a fundamentally different approach. Instead of reacting to loss, {{bbr|BBR}} actively measures the bottleneck {{bandwidth|bandwidth}} and minimum {{rtt|RTT}}, then paces {{segment|packets}} to match the path capacity. It performs dramatically better on long-distance, high-{{bandwidth|bandwidth}} links.`
			},
			{
				...
				definition: `graph TD
    START["Connection Start<br/>cwnd = 1 {{mss|MSS}}"] --> SS["{{slow-start|Slow Start}} Phase<br/>cwnd doubles each RTT"]
    ...
    RENO_HALF --> RENO_CA["{{congestion-avoidance|Congestion Avoidance}}<br/>Linear growth +1 {{mss|MSS}}/RTT"]
    ...
    THRESH -- "{{bbr|BBR}} (no loss trigger)" --> BBR_PROBE["Probe {{bandwidth|Bandwidth}}<br/>Measure BtlBw + RTprop"]
...
If the receiver's window shrinks to zero, the sender must stop. To avoid deadlock (the sender waiting for a window {{bgp-update|update}} that gets lost), [[tcp|TCP]] uses persist timers -- small probe {{packet|packets}} sent periodically to check if the window has reopened.
...
{{nagle|Nagle's algorithm}} ({{rfc-doc|RFC}} 896) coalesces small writes into larger {{segment|segments}} to reduce overhead -- but it interacts badly with delayed ACKs, causing 200ms {{latency|latency}} spikes. Most interactive applications disable it with TCP_NODELAY.`
			},
			{
				type: 'callout',
			...
			},
			{
				type: 'narrative',
				title: 'TCP Fast Open',
				text: `Standard [[tcp|TCP]] requires a full {{rtt|RTT}} for the three-way {{handshake|handshake}} before any data can flow. [[tcp|TCP]] Fast Open (TFO, {{rfc-doc|RFC}} 7413) allows data in the {{syn-cookies|SYN}} {{packet|packet}} itself.

On the first {{connection-oriented|connection}}, the server sends a {{cookie|cookie}} in the {{syn-cookies|SYN}}-{{ack|ACK}}. On subsequent connections, the client includes this {{cookie|cookie}} and application data in the {{syn-cookies|SYN}}. The server can process the data immediately, saving one full {{rtt|RTT}}.

TFO is widely deployed ({{linux|Linux}}, macOS, iOS) but faces challenges: middleboxes sometimes strip the TFO option, and {{idempotent|idempotency}} is required (the {{syn-cookies|SYN}} data might be delivered twice). Despite this, it provides measurable {{latency|latency}} improvements for short-lived connections like web requests.`
			}
		]
	},
	{
		categoryId: 'web-api',
		tagline:
			'{{http-method|HTTP}} caching, {{cors|CORS}}, header compression, {{json|JSON}}-{{rpc|RPC}} internals, {{mcp-tool|MCP}} architecture, {{task-lifecycle-a2a|A2A}} discovery, and the mechanics beneath modern web and {{ai|AI}} APIs.',
		sections: [
			{
				type: 'narrative',
...
**stale-while-revalidate** ({{rfc-doc|RFC}} 5861) is a modern gem: serve the stale cached version immediately while asynchronously checking for updates. The user gets instant responses; the cache stays fresh. Combined with CDNs, this pattern enables sub-10ms {{latency|response times}} globally.`
			},
			{
				type: 'narrative',
...
This matters because [[http2|HTTP/2]] over [[tcp|TCP]] still suffers from transport-level {{head-of-line-blocking|head-of-line blocking}} -- a single lost {{packet|packet}} stalls all {{multiplexing|multiplexed}} streams. [[http3|HTTP/3]] over [[quic|QUIC]] solves this at the transport layer, and {{qpack|QPACK}} ensures header decompression doesn't reintroduce it.`
			},
			{
				type: 'narrative',
				title: 'JSON-RPC 2.0 — The Universal Wire Format',
				text: `[[json-rpc|JSON-RPC]] 2.0 is deceptively simple — four message types (request, response, error, {{notification|notification}}) and a handful of rules — but this simplicity is its power. It has become the wire format of choice for systems that need structured {{rpc|RPC}} without the overhead of [[rest|REST]]'s resource-oriented model or [[grpc|gRPC]]'s binary {{serialization|serialization}}.
...
Transport is pluggable: **{{stdio|stdio}}** for local processes (the server is a subprocess communicating over stdin/stdout), **Streamable {{http-method|HTTP}}** for remote servers (a single {{http-method|HTTP}} endpoint that handles both {{request-response|request-response}} and server-initiated events via [[sse|SSE]] streams). The protocol is transport-agnostic by design — the same [[json-rpc|JSON-RPC]] messages work identically over either transport.`
			},
			{
				type: 'diagram',
...
Discovery centers on the **{{agent-card|Agent Card}}** — a {{json|JSON}} document served at \`/.well-known/agent.json\`. It declares the agent's name, description, supported skills, authentication requirements, and the [[a2a|A2A]] endpoint {{url|URL}}. A client agent fetches the card, inspects the skills array, and decides whether this remote agent can help with the task at hand. This is analogous to how [[dns|DNS]] {{txt-record|TXT}} records or [[oauth2|OAuth]] discovery documents work — a well-known {{url|URL}} that bootstraps trust.
...
The key architectural insight is that [[mcp|MCP]] and [[a2a|A2A]] operate at different levels of abstraction. [[mcp|MCP]] is like a function call — "execute this tool with these parameters and return the result." [[a2a|A2A]] is like a work order — "here's what I need done; figure out how to do it and get back to me." This distinction maps cleanly to the difference between deterministic tool execution and autonomous agent reasoning.`
	...
	},
	{
		...
		tagline:
			'QoS semantics, broker architectures, {{exactly-once-delivery|exactly-once delivery}}, and event sourcing patterns at scale.',
		sections: [
			{
				type: 'narrative',
...
**QoS 0 (At Most Once)**: Fire and forget. The publisher sends the message once and doesn't wait for confirmation. Fastest, lowest overhead, but messages can be lost. Suitable for sensor readings where the next {{bgp-update|update}} obsoletes the previous one.
...
**{{idempotent|Idempotent}} producers** ([[kafka|Kafka]]): each message gets a {{sequence-number|sequence number}}. The {{broker|broker}} deduplicates based on producer {{id-identifier|ID}} + sequence, discarding {{retransmission|retransmissions}}.
...
**{{dead-letter-queue|Dead-letter queues}}** catch messages that fail after max retries -- preventing poison messages from blocking the entire queue while preserving them for manual inspection.`
	...
	},
	{
		...
		tagline:
			'{{jitter|Jitter}} buffers, error correction, adaptive bitrate, {{codec|codec}} negotiation, and the engineering behind real-time media.',
		sections: [
			{
				type: 'narrative',
...
**Static {{jitter|jitter}} buffers** use a fixed delay (e.g., 60ms). Simple but suboptimal -- too small and you get dropouts; too large and the {{latency|latency}} is noticeable. **Adaptive {{jitter|jitter}} buffers** dynamically adjust their size based on observed network conditions, growing during high {{jitter|jitter}} and shrinking when the network is stable. [[webrtc|WebRTC]]'s NetEQ is a sophisticated adaptive {{jitter-buffer|jitter buffer}} that also handles packet loss concealment.`
			},
			{
				...
				text: `When [[udp|UDP]] {{datagram|packets}} are lost, the data is gone -- there's no {{retransmission|retransmission}} mechanism. For real-time media, waiting for a retransmit is worse than skipping the lost data. Forward Error Correction ({{forward-error-correction|FEC}}) adds redundant data so the receiver can reconstruct lost packets without {{retransmission|retransmission}}.

**XOR-based {{forward-error-correction|FEC}}**: Send N data packets plus 1 parity packet (XOR of all N). If any single packet is lost, it can be reconstructed from the others. Simple, low overhead (1/N+1), handles single-packet losses.

**Reed-Solomon codes**: More sophisticated -- can recover from multiple simultaneous losses. Higher {{cpu|CPU}} cost but configurable redundancy. Used in professional {{broadcast|broadcast}} systems.

**Opus {{forward-error-correction|FEC}}**: The Opus audio {{codec|codec}} includes built-in {{forward-error-correction|FEC}} -- each packet contains a low-bitrate encoding of the **previous** packet's audio. If a packet is lost, the next packet contains enough information to reconstruct a reasonable approximation. Nearly zero overhead for the common case of isolated packet losses.`
			},
			{
				type: 'diagram',
    ...
    subgraph Encode["Server-Side Encoding"]
        ...
        SEG1 --> {{man-acr|MAN}}["{{manifest|Manifest}} / Playlist<br/>(M3U8 or {{mpd|MPD}})"]
        SEG2 --> {{man-acr|MAN}}
        SEG3 --> MAN
        SEG4 --> MAN
    end

    subgraph Player["Client-Side {{abr-segment-switch|ABR}} Algorithm"]
        MAN --> {{imap-fetch|FETCH}}["{{imap-fetch|Fetch}} {{manifest|manifest}}"]
        {{imap-fetch|FETCH}} --> MON["Monitor:<br/>Download speed<br/>Buffer level<br/>RTT"]
        MON --> ALG{"{{abr-segment-switch|ABR}} Decision<br/>(BBA / Throughput / {{abr-bola|BOLA}})"}
        ...
        ALG -- "Medium" --> {{med|MED}}["Request 720p segment"]
        HIGH --> MON
        LOW --> MON
        {{med|MED}} --> MON
    end`
			},
			{
				...
				text: `Adaptive bitrate ({{abr-segment-switch|ABR}}) streaming dynamically adjusts video quality based on network conditions. [[hls|HLS]] and [[dash|DASH]] encode each video at multiple quality levels (representations), each segmented into short chunks (2-10 seconds).

The {{abr-segment-switch|ABR}} algorithm runs in the client's player. It monitors download speed, buffer level, and sometimes {{rtt|RTT}} to decide which quality to request next. Key algorithms:
...
**Hybrid ({{abr-bola|BOLA}}, MPC)**: Combines buffer and throughput signals with mathematical optimization. Netflix's MPC (Model Predictive Control) looks multiple segments ahead to minimize rebuffering while maximizing quality. These achieve near-optimal quality with minimal stalls.`
	...
	},
	{
		...
		tagline:
			'{{pki|PKI}} internals, {{tls-handshake|TLS}} 1.3 {{handshake|handshake}} walkthrough, cryptographic primitives, and {{dns-resolution|DNS resolution}} mechanics.',
		sections: [
			{
				type: 'narrative',
...
The browser verifies this by following the {{certificate-chain|certificate chain}}: the server cert is signed by an intermediate {{certificate-authority|CA}}, which is signed by a root {{certificate-authority|CA}}. Root CAs are pre-installed in your browser/{{os|OS}} (~150 of them). If the chain validates, the padlock icon appears.
...
{{ocsp-crl|OCSP}} (Online {{certificate|Certificate}} Status Protocol) and CRL ({{certificate|Certificate}} Revocation Lists) handle revoked certificates. {{ocsp-crl|OCSP}} Stapling (where the server includes a signed "not revoked" proof in the [[tls|TLS]] {{tls-handshake|handshake}}) avoids the privacy and performance problems of clients querying CAs directly.`
			},
			{
				type: 'diagram',
    ...
    subgraph RTT1["{{one-rtt|1-RTT}} {{handshake|Handshake}}"]
        CH["Client Hello<br/>Supported cipher suites<br/>Random {{nonce|nonce}}<br/>Key shares ({{x25519|X25519}}, P-256)"] -- "Flight 1" --> SH["Server Hello<br/>Chosen {{cipher-suite|cipher suite}}<br/>Server {{key-share|key share}}"]
        ...
        CV --> SF["Server Finished<br/>{{mac-address|MAC}} over {{handshake|handshake}}"]
    ...
    subgraph ZERO["{{zero-rtt|0-RTT}} Resumption"]
        CH0["Client Hello +<br/>Early Data (encrypted)<br/>with {{psk|PSK}} from prior session"] -- "Immediate" --> S0["Server processes<br/>early data"]
...
**Client Hello**: The client sends supported {{cipher-suite|cipher suites}}, a random {{nonce|nonce}}, and **key shares** for all supported key {{exchange|exchange}} algorithms (usually {{x25519|X25519}} and P-256). By sending keys upfront, the client gambles that the server will accept one -- eliminating the extra round trip that [[tls|TLS]] 1.2 needed.
...
**Stub resolver** (your {{os|OS}}): Checks the local cache, the {{hosts-bare|hosts}} file, then forwards the query to a configured {{recursive-resolver|recursive resolver}} (usually your {{isp|ISP}}'s or a public one like 8.8.8.8).

**{{recursive-resolver|Recursive resolver}}**: Does the hard work. If not cached, it starts at the root -- querying one of the 13 root server clusters for ".com." The root points to the .com {{tld|TLD}} servers. The {{tld|TLD}} server points to example.com's authoritative nameservers. The authoritative server returns the actual {{ip-address|IP address}}.

**Caching**: Every response includes a {{ttl|TTL}}. The {{recursive-resolver|recursive resolver}} caches results for that duration. A {{ttl|TTL}} of 300 means the record is fresh for 5 minutes -- reducing load on authoritative servers while allowing reasonably fast [[dns|DNS]] changes.
...
{{dnssec|DNSSEC}} adds cryptographic signatures to [[dns|DNS]] responses, preventing {{spoofing|spoofing}}. [[dns|DNS]]-over-HTTPS (DoH) and [[dns|DNS]]-over-[[tls|TLS]] (DoT) encrypt the query itself using [[tls|TLS]], preventing ISPs and network operators from snooping on which domains you're looking up.`
	...
	},
	{
		categoryId: 'wireless',
		tagline:
			'The power–range–throughput triangle, {{csma-ca|CSMA/CA}} versus {{csma-cd|CSMA/CD}}, the 2.4 GHz coexistence dance, the {{ble|BLE}}-bootstrap pattern, and the cross-cutting security history of the wireless family.',
		sections: [
			{
				type: 'narrative',
...
The numbers below are typical 2026 production values; spec maxima are higher, real-world is usually lower. **Edholm's law of {{bandwidth|bandwidth}}** — wireless data rates double roughly every 18 months — is what keeps the table moving.`
    ...
    subgraph Personal["Personal-area (≤10 m)"]
        ...
        {{ble|BLE}}["[[bluetooth|BLE]]<br/>10 m, 1-2 Mbit/s<br/>1-100 mW"]
    ...
    [[nfc|NFC]] -.->|tap-to-pair bootstrap| {{ble|BLE}}
    [[nfc|NFC]] -.->|tap-to-pair bootstrap| WIFI
    {{ble|BLE}} -.->|session-key bootstrap| [[uwb|UWB]]
    {{ble|BLE}} -.->|commissioning| ZIGBEE
    WIFI -.->|offload| CELL`
			},
			{
				type: 'narrative',
...
**Zigbee dodges Wi-Fi.** Zigbee channels 11–14 sit under Wi-Fi 1; 15, 20, 25, and 26 fit in the gaps between Wi-Fi 1/6/11. The single most common cause of unreliable Zigbee is a coordinator dongle plugged directly into a Wi-Fi router's {{usb|USB}} port — the router's switched-mode PSU radiates broadband 2.4 GHz noise.

**{{ble|BLE}} picks its advertising channels carefully.** Channels 37/38/39 — the three primary advertising channels — sit at 2402, 2426, and 2480 MHz, deliberately outside Wi-Fi 1/6/11. The 37 data channels (0–36) overlap and rely on adaptive frequency hopping to dodge active Wi-Fi APs.
...
**[[uwb|UWB]] ranging** never starts without [[bluetooth|BLE]] first. The lock or car advertises a service {{uuid|UUID}} over {{ble|BLE}}; the phone connects, runs {{spake2|SPAKE2}}+/{{pake|PAKE}} authentication, transfers the **STS_KEY** for the [[uwb|UWB]] session over the {{ble|BLE}} encrypted channel, and only then powers up its [[uwb|UWB]] radio for a three-message {{ds-twr|DS-TWR}} ranging round. [[uwb|UWB]] has no power-efficient discovery mechanism of its own — {{ble|BLE}} provides it. ({{ccc-digital-key|CCC Digital Key 3.0}}, {{aliro|Aliro 1.0}}, every {{apple|Apple}} AirTag Precision Finding round.)

**[[bluetooth|Bluetooth]] / [[wifi|Wi-Fi]] handover** is bootstrapped by [[nfc|NFC]]. The [[nfc|NFC]] Forum Connection Handover spec defines {{ndef|NDEF}} records carrying the {{ble|BLE}} {{mac-address|MAC}} + {{smp-pairing|SMP}} {{oob|OOB}} key or the Wi-Fi {{ssid|SSID}} + {{wpa2|WPA2}} key. A single 4 cm tap replaces the entire discovery + pairing dialog on Bluetooth speakers, printers, and {{matter|Matter}} device commissioning.

**[[cellular|Cellular]] data** falls back to [[wifi|Wi-Fi]] calling when the carrier signal is weak — [[ipsec|IPsec]] ePDG tunnel from the {{ue|UE}} to the carrier core over any {{ip-address|IP}} link. The reverse is now true too: **{{wifi-8|Wi-Fi 8}} and the {{3gpp|3GPP}} "Wi-Fi {{ran|RAN}}" work** is exploring Wi-Fi as a fully {{3gpp|3GPP}}-managed access network so the phone never has to know which radio it's on.

**Zigbee + {{thread|Thread}}** are commissioned over {{ble|BLE}} (Zigbee Direct, R23 mandatory in Coordinators) or Wi-Fi ({{matter|Matter}} setup). Once commissioned they run their own mesh; BLE is just the on-ramp.
...
**Tesla Model 3 {{ble|BLE}} relay — May 2022** (Sultan Qasim Khan, NCC Group, CVSS 6.8). ~$50 of dev boards, ~8 ms relay {{latency|latency}}, below Tesla's ~30 ms {{gatt|GATT}} threshold. {{rssi|RSSI}} proximity is fundamentally broken when an attacker can relay. The industry response: [[uwb|UWB]] cryptographic distance bounds ({{ccc-digital-key|CCC Digital Key}} 3.0) — the speed of light is the hard upper bound that no relay can shorten.

**Ghost Peak — {{usenix-conf|USENIX}} Security 2022** (Leu, Camurati et al.). Even [[uwb|UWB]]'s {{sts|STS}} distance commitment was attackable at ~4 % success with a $65 device — random {{sts|STS}}-like signal injection biased the correlation peak earlier. Motivated **{{ieee-802-15-4|IEEE}} 802.15.4ab** (Draft D03 Sept 2025) with {{nba-mms|NBA-MMS}} narrowband-assist and a redesigned receiver.

**{{ss7|SS7}} / {{diameter|Diameter}} abuse — ongoing** (Citizen Lab 2024–25, CISA testimony to {{fcc|FCC}} 2024). The cellular signalling plane was designed in 1975 with implicit trust between carriers. Modern surveillance actors (STA1, STA2) exploit {{ss7|SS7}} / {{diameter|Diameter}} routing to silently track mobile users worldwide. The fix is a multi-year migration to authenticated SBI and signature-checked roaming — partially complete in 5GC but the {{ss7|SS7}} layer below is still everywhere.

**The pattern:** every spec that depends on a *secret algorithm* or a *trust assumption between operators* eventually gets broken in public. Every spec that depends on *cryptographic primitives + open analysis* ({{wpa3|WPA3}}, CCMP-256, {{emv-cryptogram|EMV}} cryptograms, {{ieee-802-15-4|IEEE}} 802.15.4z {{sts|STS}} as redesigned) survives the next attack.`
			},
			{
				type: 'callout',
