...
**Idempotency keys** make retries safe — a request with the same key, sent twice, has the effect of being processed once. Stripe pioneered this for payments in 2015; it is now standard in [[rest|REST]] APIs, [[kafka|Kafka]] producers, and any system that needs at-least-once semantics without duplicate side effects.`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			title: 'Failure Modes',
			synopsis:
				'{{bufferbloat|Bufferbloat}}, ossification, head-of-line, microloops, MTU black holes — the bestiary every operator learns by being burned.',
			slots: [
				{
					kind: 'pull-quote',
...
**[[bgp|BGP]] hijacks** — an AS announces a prefix it does not own. Examples: [[outage:as-7007-1997|AS 7007 1997]], [[outage:pakistan-youtube-2008|Pakistan / YouTube 2008]], [[outage:china-telecom-2010|China Telecom 2010]]. The fix in flight: [[frontier:rpki-rov-50-percent|RPKI/ROV]].
...
**Cascading failures** — one failure increases load on healthy components, which then fail too. Examples: [[outage:facebook-2021|Facebook 2021]], [[outage:rogers-2022|Rogers 2022]]. Cure: rate-limiting and circuit-breakers at every layer; the SRE pattern is "fail fast, fail isolated."`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			synopsis: 'Tahoe → Reno → {{cubic|CUBIC}} → BBR → L4S, in one sitting.',
			slots: [
				{
					kind: 'pull-quote',
...
[[pioneer:van-jacobson|Van Jacobson]] and Mike Karels at Berkeley spent six months instrumenting and reading the BSD source. Their 1988 SIGCOMM paper, *"{{congestion-avoidance|Congestion Avoidance}} and Control,"* was the inflection point. Six algorithms in one paper. Saved the internet.`
						},
						{
							...
							text: `**Tahoe (1988)**, by [[pioneer:van-jacobson|Van Jacobson]] and Mike Karels — the original. {{slow-start|Slow start}} (double cwnd every RTT), {{aimd|AIMD}} {{congestion-avoidance|congestion avoidance}} (additive increase, multiplicative decrease), fast retransmit on three duplicate ACKs, exponential RTO backoff. Shipped in 4.3BSD-Tahoe.

**Reno (1990)** — added fast recovery: when fast retransmit fires, halve the {{congestion-window|congestion window}} instead of dropping it to 1 MSS. Less brutal on the sender; faster to recover.
...
**Compound (Microsoft, 2007)** — combined loss-based and delay-based components. Used in Windows. Withdrawn in newer versions.`
						},
						{
							...
							text: `**BBR (Google, 2016)** — fundamentally different from everything before. Model the bottleneck {{bandwidth|bandwidth}} and minimum RTT directly, then send at a paced rate just below the model. Robust to the random-loss problem (where {{cubic|CUBIC}} over-reacts). Deployed by default for google.com and YouTube outbound traffic from 2016. **BBRv3** ([[frontier:bbrv3-default|currently shipping default in production]] from 2024) addresses fairness issues v1 had with non-BBR flows on shared bottlenecks.
...
The result: **sub-millisecond queuing {{latency|latency}}** even at 100% link utilisation, for flows that participate. Comcast launched L4S in production in January 2025 across six US metros, with Apple, NVIDIA GeForce NOW, Meta, and Valve as launch partners. Apple shipped L4S support in iOS 17 / macOS Sonoma in 2023, default for [[quic|QUIC]] in newer releases.
