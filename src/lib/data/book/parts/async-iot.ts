...
The internal name was the "Argo Lightweight On The Wire Protocol." Version 2 (later 1999) was renamed **MQ Integrator Pervasive Device Protocol (MQIpdp)** before settling on MQTT in v3 (2000). The "MQ" prefix is a **vestige of IBM MQSeries** ("Message Queue") even though MQTT does not implement queues. OASIS formally treats "MQTT" as the protocol name itself (not an acronym) since 2013.
...
The constraint was brutal: kilobits-per-second satellite links priced per byte, devices that ran for years on a single battery, network drops measured in hours. **The Fixed Header is 2 bytes**; the smallest possible Control Packet (e.g. PINGREQ) is 2 bytes total — the per-byte thrift that makes MQTT viable on satellite/cellular IoT.`
						},
						{
							...
							text: `MQTT defines three quality-of-service levels (at-most-once, at-least-once, exactly-once) that let the application trade reliability for bandwidth. A persistent **session** survives disconnections — when a sleeping sensor wakes up an hour later and reconnects, the broker delivers any messages it missed.
...
MQTT is also the historical template for two patterns now considered universal: **Last Will & Testament** (a message the broker sends on the device's behalf when it dies) and **retained messages** (the latest value on a topic, kept by the broker for late subscribers). Both were invented in 1999.
...
The first mission-critical AMQP deployment (mid-2006) "served 2,000 users and processes 300 million messages per day" — published in O'Hara's 2007 ACM Queue paper (DOI 10.1145/1255421.1255424).`
						},
						{
							type: 'narrative',
							title: 'AMQP 0-9-1 Versus AMQP 1.0 — Two Different Protocols',
							text: `**AMQP 0-9-1 and AMQP 1.0 are completely different protocols sharing a name.** 0-9-1 prescribes exchanges/queues/bindings. 1.0 is a symmetric peer-to-peer transfer protocol with **zero queues defined in the spec**.

AMQP 0-9-1 has **exchanges** that route messages to **queues** based on routing keys (direct), patterns (topic), header values (headers), or fanout broadcast. **Durable queues** survive broker restarts. **Channels** multiplex multiple logical sessions over a single TCP connection. The wire format uses a literal **\`0xCE\` frame-end sentinel** byte (not a checksum).
					...
					sections: [
						{
							...
							text: `[[kafka|Apache Kafka]] was built at LinkedIn around 2010 by **Jay Kreps, Neha Narkhede, and Jun Rao**; named after **Franz Kafka** because, per Kreps on Quora, "it's a system optimized for writing" and he liked the writer. Open-sourced 2011; graduated from Apache Incubator **23 October 2012**.
...
A Kafka **topic** is a partitioned log. Producers append records; consumers read them at their own pace, tracking their position by offset. The log is **persistent** — records are not deleted when consumed, they age out by retention policy (often days or weeks). That persistence enables **event sourcing**, **stream processing**, **replay**, and **multiple independent consumer groups** reading the same log for different purposes.`
						},
						{
							...
							text: `**Kafka 4.0 (18 March 2025) removed ZooKeeper entirely** — the end of a 10-year migration that began with KIP-500 in 2019. **KRaft** (Kafka Raft metadata) is now the only metadata mode. KIP-848 (new consumer rebalance protocol) GA shipped in the same release.
...
**LinkedIn runs the largest publicly disclosed deployment**: 100+ clusters, 4,000+ brokers, ~7M partitions, **>7 trillion messages/day** (2019 LinkedIn engineering blog; floor figure, never updated downward).`
						...
						},
						{
							...
							text: `**Confluent acquired WarpStream on 9 September 2024**. **IBM agreed to acquire Confluent for $11B at $31/share on 8 December 2025**, deal closed **17 March 2026**. Apache Kafka itself remains independent at the ASF.
...
2024-2025 CVE wave: **CVE-2024-56128** (SCRAM skipped server-nonce check, fixed 3.7.2/3.8.1/3.9.0); **CVE-2025-27817** (SASL/OAUTHBEARER arbitrary file read/SSRF, fixed 3.9.1/4.0.0); the older **CVE-2023-25194** JndiLoginModule RCE in Connect REST API was the field's Log4Shell moment.
...
CoAP is what you build when you want REST semantics on a 32 kB microcontroller talking over a 50 kbps mesh radio. It looks like [[http1|HTTP]] from a distance — GET, POST, PUT, DELETE, status codes, URIs — but the wire format is binary and it runs over [[udp|UDP]] instead of [[tcp|TCP]].

**The fixed header is 4 bytes**; a minimal Empty CoAP message is 4 bytes total; a typical GET sits at 10-20 bytes — designed for IEEE 802.15.4 frames that fragment IPv6 packets into ~80-byte chunks. CoAP defines **four message types** (CON/NON/ACK/RST) and a **2-byte Message ID** for de-duplication, with **0-8 byte Tokens** for matching responses across messages. Default UDP port is **5683**, CoAPS over DTLS is **5684**.`
						},
						{
							type: 'callout',
						...
						},
						{
							...
							text: `The "everyone gets this wrong" fact: **Matter does NOT use CoAP** for its main payloads. Matter has its own Message Reliability Protocol on UDP/5540. CoAP is only used in **Thread network management** (the "CoAP-TMF" Wireshark dissector). Older blog posts saying "Thread uses CoAP" are imprecise.

**The QLC Chain surprise**: NETSCOUT's January 2019 scan found **388,344 publicly-reachable CoAP endpoints, 81% in China**, but most were *not* IoT devices — they were **Chinese smartphones running the QLC Chain peer-to-peer crypto stack**. CoAP's largest-deployed-at-scale use case for years was, embarrassingly, P2P crypto on phones.
...
**CoAP reflection/amplification DDoS** (Jan-Mar 2019) had a measured **34× amplification factor** (21-byte \`GET /.well-known/core\` → ~720-byte response). The often-cited "3,300×" figure is *not* CoAP — that was memcached in Feb 2018. The CoAP number is real but smaller than the press coverage suggested.`
						},
						{
							...
							text: `**\`draft-ietf-core-coap-pubsub\`** advanced from -15 (Oct 2024) to -19 (Mar 2026) — a CoAP publish/subscribe broker is now stable and approaching RFC publication, the WG's belated answer to MQTT brokers. CoAP catching up to MQTT's pattern shows how late the constrained-IoT ecosystem moves.
