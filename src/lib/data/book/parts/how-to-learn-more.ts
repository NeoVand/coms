...
**[[rfc:9293|RFC 9293]] — TCP (2022 update of RFC 793)**. The single most-implemented protocol on the internet. Read sections 3.4 (sequence numbers and reliability), 3.6 (connection close and TIME_WAIT), and 3.8 (flow control). The state machine in 3.3.2 is worth memorising.

**[[rfc:8446|RFC 8446]] — TLS 1.3**. Modern crypto-engineering at its best. Eric Rescorla's prose is the model for how to write a security spec. Read the entire handshake section; the formal-methods analysis appendices are optional.

**[[rfc:9000|RFC 9000]] — QUIC**. The first transport designed in user space, and the place to see how the lessons of [[tcp|TCP]]'s 40 years got applied. Sections on streams (2.1), connection migration (9), and 0-RTT are essential.

**[[rfc:1035|RFC 1035]] — DNS**. Forty years old, still the canonical reference. Read it with [[rfc:1034|RFC 1034]] (concepts) for the architecture; together they are 100 pages and they explain a system that has scaled to a billion hostnames.
...
**[[rfc:5681|RFC 5681]] — TCP Congestion Control**. Codifies slow start, congestion avoidance, fast retransmit, and fast recovery — the four algorithms [[pioneer:van-jacobson|Van Jacobson]] introduced in 1988, formalised over decades.
...
**[[rfc:9110|RFC 9110]] — HTTP Semantics**. The clean separation of "what HTTP means" (verbs, headers, status codes, content negotiation) from "how it's encoded on the wire." Reading this once makes [[http1|HTTP/1.1]], [[http2|HTTP/2]], and [[http3|HTTP/3]] all click.`
						}
					]
				},
...
**Ilya Grigorik, "High Performance Browser Networking" (O'Reilly, 2013).** The book on what actually matters for web performance. TCP, TLS, HTTP/1.1, HTTP/2 (no HTTP/3 — it predates the spec), WebRTC, browser networking APIs. The single most useful book for full-stack web developers who want to debug latency.
					...
					sections: [
						{
							...
							text: `**Stanford CS144 — Introduction to Computer Networking**. The single most useful course on this topic. The labs walk you through implementing a working TCP/IP stack from scratch — your code goes from sending raw IP datagrams to running a full reliable transport with congestion control. Materials are public; the labs run on Linux and produce a stack that interoperates with the real internet.
...
**Berkeley CS168 — Introduction to the Internet**. Newer than CS144 and more focused on operational reality (BGP, CDN architecture, DNS, real-world security incidents). Materials online; the projects involve building a small BGP simulator and a CDN.
					...
					sections: [
						{
							...
							text: `**Cloudflare blog** (blog.cloudflare.com) — the most consistently good source of operationally-grounded networking content on the internet. They run a global anycast network, see real production traffic, and write about both the engineering and the security incidents in detail. If you read one networking blog, read this one.

**Meta Engineering blog** (engineering.fb.com) — periodic deep-dives on the protocols and infrastructure powering Facebook/Instagram/WhatsApp. Strong on QUIC deployment, BGP operations, datacenter networking.
...
**Daniel Stenberg's blog** (daniel.haxx.se) — the maintainer of curl. If you want to understand HTTP at the level where bugs actually live, read his QUIC and HTTP/3 deployment posts.
...
**curl** (curl.se) — the command-line HTTP client and Swiss-army knife of the protocol world. \`curl -v\` shows you the entire request and response with headers; \`--http3\` forces QUIC; \`--resolve\` overrides DNS for one request; \`-w\` formats timing information.
...
**FRRouting** (frrouting.org) — open-source routing stack (BGP, OSPF, IS-IS, RIP, etc.) that runs on Linux. Used in containerlab simulations and in some production small-scale ISPs.
...
**RIPE Atlas** (atlas.ripe.net) — a global mesh of probes you can use to measure reachability, latency, and routing from arbitrary vantage points. Free for non-commercial research.

**bgp.tools** — the modern BGP looking glass. Type any AS or prefix; see the global routing table from multiple vantage points. The successor to the older route-views.org infrastructure.

**iperf3** — bandwidth measurement between two endpoints. The standard for "how fast can these two hosts actually talk to each other."
