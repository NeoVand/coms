...
**[[rfc:9293|RFC 9293]] — [[tcp|TCP]] (2022 {{bgp-update|update}} of [[rfc:793|RFC 793]])**. The single most-implemented protocol on the internet. Read sections 3.4 (sequence numbers and reliability), 3.6 (connection close and {{time-wait|TIME_WAIT}}), and 3.8 ({{flow-control|flow control}}). The state machine in 3.3.2 is worth memorising.
...
**{{rfc-doc|RFC}} 1958 — Architectural Principles of the Internet**. Brian Carpenter's 7-page summary of **why** the internet is the way it is. End-to-end principle, layering, robustness — all in one short document.

**[[rfc:9110|RFC 9110]] — {{http-method|HTTP}} Semantics**. The clean separation of "what {{http-method|HTTP}} means" (verbs, headers, status codes, {{content-negotiation|content negotiation}}) from "how it's encoded on the wire." Reading this once makes [[http1|HTTP/1.1]], [[http2|HTTP/2]], and [[http3|HTTP/3]] all click.`
						},
						{
							type: 'image',
							...
							caption:
								'The {{arpanet|ARPANET}} in 1974 — the small research network the first few hundred **RFCs** documented. By 2026 the {{ietf|IETF}} {{rfc-doc|RFC}} series exceeds **9,800 documents**. The seven RFCs in the reading list above are the ones that most pay back the time investment for an engineer trying to be conversant with the modern stack.',
...
For specialised areas: **Olivier Bonaventure, "Computer Networking: Principles, Protocols and Practice"** (free online) — a lighter alternative to Tanenbaum. **[[pioneer:radia-perlman|Radia Perlman]], "Interconnections" (2nd ed., 2000)** — the routing/switching deep-dive by the inventor of spanning tree. **Socolofsky & Kale, "{{rfc-doc|RFC}} 1180: A [[tcp|TCP]]/[[ip|IP]] Tutorial" (1991)** — 28 pages, free, 35 years old, still the cleanest introduction to the basics.`
						},
						{
							...
							caption:
								'A library stack. The five books in the list above plus the four follow-ups are enough to make any engineer genuinely fluent in this field — a few months of evenings spent on **Stevens** and **{{cs144-course|CS144}}** is the highest-leverage networking education you can give yourself.',
					...
					sections: [
						{
							...
							text: `**Stanford {{cs144-course|CS144}} — Introduction to Computer Networking**. The single most useful course on this {{topic|topic}}. The labs walk you through implementing a working [[tcp|TCP]]/[[ip|IP]] stack from scratch — your code goes from sending raw [[ip|IP]] datagrams to running a full reliable transport with {{congestion-control|congestion control}}. Materials are public; the labs run on {{linux|Linux}} and produce a stack that interoperates with the real internet.

**{{mit|MIT}} 6.829 — Computer Networks**. Graduate-level, with a focus on the research-paper canon — Jacobson 1988, Cardwell {{bbr|BBR}} 2016, Cerf & Kahn 1974. Strong on routing and {{congestion-control|congestion control}}. Materials and lecture videos public.

**Berkeley CS168 — Introduction to the Internet**. Newer than {{cs144-course|CS144}} and more focused on operational reality ([[bgp|BGP]], {{cdn|CDN}} architecture, [[dns|DNS]], real-world security incidents). Materials online; the projects involve building a small [[bgp|BGP]] simulator and a {{cdn|CDN}}.
...
For a self-paced path: read **{{rfc-doc|RFC}} 1180** (28 pages), then take **{{cs144-course|CS144}}**, then read **Stevens [[tcp|TCP]]/[[ip|IP]] Illustrated Vol 1**. Three months of evenings will make you genuinely fluent in the field.`
						},
						{
							type: 'image',
							...
							caption:
								'**Stanford University** — home of **{{cs144-course|CS144}}**, the single most useful networking course on this {{topic|topic}}. The labs walk you through implementing a working [[tcp|TCP]]/[[ip|IP]] stack from scratch — your code goes from sending raw [[ip|IP]] datagrams to running a full reliable transport with {{congestion-control|congestion control}}. Materials are public; the labs run on {{linux|Linux}} and produce a stack that interoperates with the real internet.',
							credit: 'Photo: Wikimedia Commons / CC BY-SA'
						}
					]
				}
...
**{{apnic|APNIC}} Labs** (blog.{{apnic|apnic}}.net) — Geoff Huston's writing, in particular, is the best long-form analysis of internet measurement and policy out there. Long, dense, worth the time.
...
**Daniel Stenberg's blog** (daniel.haxx.se) — the maintainer of curl. If you want to understand {{http-method|HTTP}} at the level where bugs actually live, read his [[quic|QUIC]] and [[http3|HTTP/3]] deployment posts.
...
**The {{ietf|IETF}} mailing lists themselves** — datatracker.{{ietf|ietf}}.org. If you want to see protocols being designed in real time, {{mqtt-subscribe|subscribe}} to a working group whose work interests you. Be ready for high traffic.`
						},
						{
							type: 'image',
		...
		},
		{
			id: 'tools',
			title: 'Tools',
			synopsis: '{{wireshark|Wireshark}}, scapy, FRRouting, Containerlab, {{ripe-ncc|RIPE}} Atlas.',
			slots: [
				{
					kind: 'prose',
					sections: [
...
**curl** (curl.se) — the command-line {{http-method|HTTP}} client and Swiss-army knife of the protocol world. \`curl -v\` shows you the entire request and response with headers; \`--http3\` forces [[quic|QUIC]]; \`--resolve\` overrides [[dns|DNS]] for one request; \`-w\` formats timing information.
...
**FRRouting** (frrouting.org) — open-source routing stack ([[bgp|BGP]], [[ospf|OSPF]], {{isis|IS-IS}}, RIP, etc.) that runs on {{linux|Linux}}. Used in containerlab simulations and in some production small-scale ISPs.
...
**{{ripe-ncc|RIPE}} Atlas** (atlas.ripe.net) — a global mesh of probes you can use to measure reachability, {{latency|latency}}, and routing from arbitrary vantage points. Free for non-commercial research.
...
**iperf3** — {{bandwidth|bandwidth}} measurement between two endpoints. The standard for "how fast can these two {{hosts-bare|hosts}} actually talk to each other."

**ngrep** — like grep for live packet captures. \`ngrep -d any "GET /api"\` shows every {{http-method|HTTP}} GET on the wire matching the pattern.`
						},
						{
							type: 'image',
