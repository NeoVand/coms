...
**Edholm's law of {{bandwidth|bandwidth}}** — wireless data rates double roughly every 18 months — is what keeps the table moving. Every protocol in this Part has shipped two or three generation upgrades since 2010 to keep its corner of the triangle in tension.`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			slots: [
				{
					kind: 'pull-quote',
					text: 'Wi-Fi 6\'s nominal 9.6 Gbit/s shows up as 1–2 Gbit/s of real throughput in a crowded room, because more than half of the {{airtime|airtime}} is spent on DIFS gaps, ACK frames, beacons, and back-off. The headline number is a physics fact; the delivered number is an {{airtime|airtime}} budget.',
					attribution: 'Author'
				},
				{
					kind: 'prose',
...
The story did not end there. Vanhoef's group at KU Leuven has broken Wi-Fi roughly every two years since: **Dragonblood** (April 2019, side-channels in {{wpa3|WPA3}}-SAE), **FragAttacks** (May 2021, {{fragmentation|fragmentation}}/aggregation bugs in [[wifi|802.11]] design itself), **Framing Frames** (March 2023), **{{ssid|SSID}} Confusion / CVE-2023-52424** (May 2024 — the [[wifi|802.11]] standard does NOT require the SSID to enter PMK or session-key derivation in many config paths). The cadence is so reliable that the field plans security audits around his summer talks.
...
That genericity is why Wi-Fi has had to evolve so much at the **PHY** and **MAC** layers and so little above them. The IP stack on top is the same one [[ethernet|Ethernet]] uses — the [[wifi|802.11]] driver bridges [[wifi|802.11]] frames to 802.3 frames before [[ip|IP]] ever sees them. Every architectural choice in this chapter ({{ofdma|OFDMA}}, {{mlo|MLO}}, {{target-wake-time|TWT}}, {{wpa3|WPA3}}) is in the service of carrying *somebody else's traffic* across a hostile shared medium without those upper layers having to know.
...
The cross-industry alliance came next. IBM (Adalio Sanchez) and Ericsson (Rydbeck) agreed the radio should be an open standard so neither company could be locked out. Sanchez recruited Stephen Nachtsheim at {{intel|Intel}}; {{intel|Intel}} pulled in Toshiba and Nokia. On a windy Toronto pub crawl, [[pioneer:jim-kardach|Jim Kardach]] of {{intel|Intel}} and Mattisson talked Viking history — Mattisson had been reading *The Longships*, Kardach had brought *The Vikings*. Kardach pitched the name **"Bluetooth"** as a code name after Harald "Blåtand" Gormsson, the 10th-century Danish king who united Denmark and Norway. It was supposed to be a placeholder. It stuck. The **Bluetooth Special Interest Group** launched on 20 May 1998 with five Promoter members: Ericsson, IBM, {{intel|Intel}}, Nokia, Toshiba. The logo is a bind-rune combining Hagall (ᚼ = H) and Bjarkan (ᛒ = B) — Harald's initials in Younger Futhark.
...
Production silicon hides the seam. {{apple|Apple}}'s H1/H2 chips, {{broadcom|Broadcom}}'s combo dies, Qualcomm's QCAxxxx — all run BR/EDR + BLE + Wi-Fi on the same package and time-slice between them in firmware. Your AirPods Pro charging case implements BR/EDR for the audio (A2DP) and BLE for the *find-my* network, both from the same antenna. You do not notice because you are not supposed to.`
						},
						{
							type: 'callout',
...
**{{ccc-digital-key|CCC Digital Key 3.0}}** uses BLE for proximity and then bootstraps a [[uwb|UWB]] session — the phone advertises a service UUID over BLE, the car authenticates, the encrypted {{gatt|GATT}} channel transfers the STS_KEY for the [[uwb|UWB]] ranging round, and only then does the [[uwb|UWB]] radio power on for the three-message {{ds-twr|DS-TWR}} {{exchange|exchange}}. **{{aliro|Aliro}} 1.0**, the access-control credential standard the CSA finalised on 26 February 2026, follows the same pattern for doors.
...
The 2022 attack is the canonical case study for why every credential standard since — {{ccc-digital-key|CCC Digital Key 3.0}}, {{aliro|Aliro}}, Bluetooth 6.0 ranging — explicitly mandates physics-based proximity, not radio-strength heuristics. {{rssi|RSSI}} was a workable shortcut for a decade. Then it stopped being one.`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			slots: [
				{
					kind: 'pull-quote',
					text: 'The control plane of every modern carrier on Earth is now an [[http2|HTTP/2]] microservice fabric — and every backhaul hop is wrapped in [[ipsec|IPsec]] ESP per {{3gpp|3GPP}} TS 33.501. The single largest enterprise [[ipsec|IPsec]] deployment on Earth runs inside this layer.',
					attribution: 'Author'
				},
				{
					kind: 'prose',
...
Read that sentence again. **The control plane of every 5G carrier on Earth is now an [[http2|HTTP/2]] {{service-mesh|microservice fabric}}.** Cloud-native engineers can read the {{3gpp|3GPP}} TS 23.501 architecture diagrams and recognise their own world — service registry (NRF), API gateway (NEF), policy engine (PCF), token authentication (AUSF). The plumbing under your phone call uses the same patterns as a Kubernetes deployment.`
						},
						{
							...
							text: 'Every interface between the [[cellular|cellular]] {{ran|radio access network}} and the core (N2/N3 in 5G, S1 in LTE) is wrapped in [[ipsec|IPsec ESP]] per {{3gpp|3GPP}} TS 33.501. With ~9 billion subscribers and tens of millions of base stations worldwide, the [[cellular|cellular]] backhaul is the single largest production [[ipsec|IPsec]] deployment that exists. [[pioneer:andreas-steffen|Andreas Steffen]]\'s strongSwan, {{cisco|Cisco}} IOS, and Juniper Junos run more [[ipsec|IPsec]] tunnels inside one [[cellular|cellular]] operator than the entire enterprise VPN market combined.'
						},
						{
							type: 'narrative',
...
**Wi-Fi Calling** is the same IMS stack tunnelled to the carrier over [[ipsec|IPsec]] from any IP network. Your phone in the basement, on the hotel Wi-Fi, places calls through the carrier's *ePDG* (Evolved Packet Data Gateway) — an [[ipsec|IPsec]] head-end the size of a small data centre that terminates millions of IKEv2 tunnels and feeds the inner IMS traffic into the EPC. From the network's perspective, your basement phone looks like any other LTE phone; it just happened to attach through Wi-Fi instead of an eNodeB.`
						},
						{
							...
							text: `The architectural trick that makes [[cellular|cellular]] look like a normal IP network from the application's perspective is **{{gtp-u|GTP-U}}** — the GPRS Tunnelling Protocol — User plane. Every {{pdu-session|PDU session}} has a 32-bit **Tunnel Endpoint Identifier (TEID)**. User-plane packets travel from the gNB to the User Plane Function over [[udp|UDP]]/2152, wrapped in {{gtp-u|GTP-U}} headers that preserve the UE's inner [[ip|IP]] address regardless of which base station the UE is camping on.
...
**NAS** (Non-Access Stratum) signalling runs end-to-end between the UE and the core through the gNB transparently. **Registration Request** with **{{suci|SUCI}}** — the {{public-key|public-key-encrypted}} {{supi|SUPI}} ({{sim-usim|Subscription Permanent Identifier}}) — starts the AKA {{handshake|handshake}} after the device powers on. The UDM decrypts to SUPI, generates an authentication vector, and the USIM verifies AUTN and computes RES*. After **Security Mode Command**, all subsequent NAS messages are integrity-protected and ciphered with K_NASint / K_NASenc. The {{sim-usim|SIM}} is the long-term key K; everything else is derived from it for the device's lifetime.`
						},
						{
							type: 'callout',
...
The {{lpwan|LPWAN}} story is broader than cellular — **LoRaWAN** (sub-GHz unlicensed, 125M+ devices by end-2025) and **Sigfox** (slow-modulation 100 bit/s ultra-narrowband) own much of the metering and agriculture market. But cellular's NB-IoT/LTE-M can run on existing carrier infrastructure, which makes deployment trivial for operators and competitive for everyone else.`
						},
						{
							...
							text: `In **January 2025**, T-Mobile + SpaceX {{starlink|Starlink}} launched the first commercial **{{direct-to-cell|Direct-to-Cell}}** service: SMS and emergency messaging from ordinary smartphones, with the satellite acting as a base station in standard [[cellular|cellular]] bands n255/n256. {{apple|Apple}}'s Globalstar-based Emergency SOS, AT&T's AST SpaceMobile partnership, and Iridium's NTN-ready successors follow similar patterns.
...
The shape rhymes with the rest of this Part. Every wireless protocol's cryptography has held up. Every wireless protocol's *negotiation logic*, *roaming model*, or *trust assumptions between operators* has been broken at the protocol level by now. The 1990s and 2000s patched the cryptography; the 2020s are patching the architecture around it.`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			synopsis:
				'[[nfc|13.56 MHz inductive coupling]], ISO 18092, EMV contactless, {{apple|Apple}} Pay, {{transit|transit}} cards, {{ccc-digital-key|CCC Digital Key}}, {{aliro|Aliro}} 1.0, and the [[pioneer:charles-walton|Charles Walton]]-to-{{apple|Apple}}-Pay arc that took 31 years.',
			slots: [
				{
					kind: 'pull-quote',
...
Seven APDUs, ~300 ms, and the latte is paid for.`
						},
						{
							...
							text: `{{apple|Apple}}'s 9 September 2014 launch was not just "we added an [[nfc|NFC]] chip to the iPhone." It was a *tokenisation revolution* baked into the architecture. The cardholder's real **Funding PAN (FPAN)** never reaches the device. The bank issues a **Device PAN ({{dpan|DPAN}})** instead, provisioned via the Token Service Provider (Visa Token Service or Mastercard Digital Enablement Service) into the {{ese|embedded Secure Element}} or the {{hce|HCE}} keystore.
...
The **TNF** (Type Name Format) picks the namespace: 1 = Well-Known (URI, Text, Smart Poster), 2 = MIME, 3 = Absolute URI, 4 = External, 5 = Unknown. The URI Well-Known record uses a single-byte prefix shorthand — \`0x03\` for \`https://\` saves eight bytes per record on tags as small as 48 bytes. This is why a tap on a museum exhibit's [[nfc|NFC]] plaque can fit a URL, a description, and a localised string into ~100 bytes of tag memory.
...
**{{matter|Matter}} 1.3+** adds [[nfc|NFC]] as one of the three permitted commissioning paths alongside QR code and BLE. Place a {{matter|Matter}}-NFC-capable device near your phone, and it onboards onto your home {{thread|Thread}}/Wi-Fi mesh without ever opening an app. **{{ccc-digital-key|CCC Digital Key 3.0/4.0}}** uses [[nfc|NFC]] to bootstrap a credential into a phone, then BLE for proximity, then [[uwb|UWB]] for centimetre-accurate ranging. **{{aliro|Aliro 1.0}}** likewise spans [[nfc|NFC]] tap-to-access + BLE proximity + BLE/UWB ranged — three transports, one credential. The Connection Handover pattern is now the *default* on-ramp to nearly every other consumer wireless protocol.
...
**Access control** — corporate building entry, hotel keys, residential and multi-family doors — used to run on cloneable LF and 13.56 MHz proximity cards. The new **{{aliro|Aliro 1.0}}** standard (CSA, finalised 26 February 2026) is "{{matter|Matter}} for doors": {{ecdsa|ECDSA}} mutual authentication; [[nfc|NFC]] tap-to-access, BLE proximity, and BLE+[[uwb|UWB]] ranged hands-free, all under one credential. 220+ companies including {{apple|Apple}}, {{google|Google}}, Samsung, ASSA ABLOY, HID, Allegion, Kwikset, Nuki are behind it.`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			synopsis:
				'[[uwb|IEEE 802.15.4z]], FiRa, {{ccc-digital-key|CCC Digital Key}} 3.0, {{apple|Apple}} U1 / U2, the 2022 Tesla [[bluetooth|BLE]] relay attack that motivated the move to [[uwb|UWB]], and the Ghost Peak STS residual.',
			slots: [
				{
					kind: 'pull-quote',
...
The consumer story begins on **10 September 2019**, when {{apple|Apple}} shipped the **U1 chip** in the iPhone 11. It went mass-market on **30 April 2021** with the $29 **AirTag**, whose Precision Finding feature swept the world's awareness of [[uwb|UWB]] into one product. Samsung followed with the **Galaxy SmartTag+** in April 2021. **{{ccc-digital-key|CCC Digital Key 3.0}}** (July 2021) made [[uwb|UWB]] the fine-ranging leg of phone-as-a-key for vehicles — BMW iX shipped first; Mercedes EQS, Hyundai/Kia, and VW ID.7 followed.

[[uwb|UWB]] silicon has consolidated to five suppliers: **{{apple|Apple}} captive** (U1 16 nm, **U2** 7 nm from iPhone 15), **Qorvo** (acquired Decawave's DW1000/DW3000 line), **NXP** (Trimension SR040/SR150/SR250), **Samsung captive** (Exynos Connect U100), and a long tail (STMicroelectronics, Microchip, Spark Microsystems, Infineon's 2024-acquired 3db Access). ABI projects UWB phone penetration rising from **27% in 2025 to 52% by 2030** — UWB is on the same adoption curve [[nfc|NFC]] followed a decade earlier.`
						},
						{
							type: 'callout',
...
The STS block is the **distance commitment** that defeats the entire {{ble|BLE}}-{{rssi|RSSI}} {{replay-attack|relay}} class of attack. {{ccc-digital-key|CCC Digital Key 3.0}}, {{aliro|Aliro 1.0}}, AirTag Precision Finding, and every modern [[uwb|UWB]] product depend on it.`
						},
						{
							type: 'callout',
...
**(1)** BLE GAP advertising with the application's service UUID (the lock advertises; the phone scans). **(2)** {{gatt|GATT}} {{service-discovery|service discovery}} + authentication — SPAKE2+/{{pake|PAKE}} for Digital Key, OOB pairing for AirTag. **(3)** Session-key transport over the BLE encrypted channel — phone and reader {{exchange|exchange}} the **STS_KEY** and ranging parameters via {{apdu|APDU-over-GATT}}. **(4)** BLE-signalled [[uwb|UWB]] ranging start at a scheduled time slot — both radios power on their [[uwb|UWB]] stacks. **(5)** Ranging happens (Poll/Response/Final); results returned over BLE for application-layer policy enforcement.
...
The Schlage FCC waiver (ET Docket 22-248, granted 2023) describes this exact sequence in regulator-vetted form. It is also why BLE {{channel-sounding|Channel Sounding}} — Bluetooth 6.0's cm-class ranging built into the BLE link itself — is interesting: it might collapse the five-step dance into one radio.`
						},
						{
							...
							text: `BMW iX shipped the first {{ccc-digital-key|CCC Digital Key 3.0}} vehicle in early 2022. The vehicle has multiple **[[uwb|UWB]] anchors** (typically one in each B-pillar and one per door handle) plus a BLE radio. The phone has {{apple|Apple}} U1/U2 or NXP/Qorvo [[uwb|UWB]] silicon plus its own BLE.
...
**{{ccc-digital-key|CCC Digital Key 4.0}}** (announced July 2025, tested at the 13th Plugfest hosted by {{apple|Apple}}) adds cross-platform key sharing — you can send an {{android|Android}} friend a Digital Key to your BMW from an iPhone, and the friend's phone can unlock the car. **115 vehicle / module products were CCC-certified in 2025**: BMW (first, late 2024 4.0), Mercedes, Hyundai/Kia, Audi (2025), Volvo, Porsche, GM, Ford, plus Chinese OEMs (NIO, XPENG, Geely group).`
						},
						{
							...
							text: `FCC Part 15.519 caps average power-spectral-density at **−41.3 dBm/MHz** across 3.1–10.6 GHz. ETSI EN 302 065 in Europe is similar but with stricter Detect-and-Avoid requirements in some sub-bands. **Japan** applies a different mask with restrictions in 7.25–7.75 GHz that overlap Channel 9 — {{apple|Apple}}'s iPhone reduces or disables [[uwb|UWB]] features (Precision Finding, Find People) in Japan and a handful of other countries.
...
At commissioning time, a new device joins by exchanging a 16-byte AES-128 **pre-configured link key** with the {{trust-center|Trust Center}}. The {{trust-center|Trust Center}} then sends the **network key** in an APS Transport-Key command encrypted under that link key. So far so good — except that for fifteen years the default link key was a publicly known sixteen-byte string: \`5A:69:67:42:65:65:41:6C:6C:69:61:6E:63:65:30:39\` = ASCII **ZigBeeAlliance09**. Generations of {{wireshark|Wireshark}} users memorised the hex; any attacker sniffing a join with that default key could read the network key in plaintext.

**{{install-code|Install codes}}** — a per-device 128-bit secret printed on the device's packaging or QR-encoded on the box, mandatory in Zigbee 3.0 commissioning — closed that hole. **{{dynamic-link-key|Dynamic Link Key}}** in **Zigbee PRO 2023 (R23)** went further: a SPEKE-over-{{curve25519|Curve25519}} {{handshake|handshake}} that derives a unique link key per device with no pre-shared secret at all. Zigbee's equivalent of the {{wpa3|WPA3}}/{{sae|SAE}} replacement of {{wpa2|WPA2}}-PSK.`
						},
						{
							type: 'narrative',
...
The single most common cause of unreliable Zigbee in any forum {{thread|thread}} you will ever read: **the coordinator dongle is plugged directly into the back of the Wi-Fi router**. Move the dongle to a USB extension cable, three feet away. Half the complaints disappear.`
						},
						{
							type: 'narrative',
					...
					sections: [
						{
							...
							text: `Every wireless protocol in this Part has been broken on stage in public, multiple times, by the same handful of researchers, with strikingly similar shapes of bug. **It is never the cryptography itself.** AES-128, {{aes-gcm|AES-GCM}}, {{ecdsa|ECDSA}}, ChaCha20, {{curve25519|Curve25519}} — the primitives hold up. What gets broken is the **negotiation logic**, the **state machine**, the **roaming model**, the **trust assumptions between operators** — the *protocol surface around* the cryptography.
...
Every BR/EDR device shipped before mid-2024 is affected by at least one of these. {{ble|BLE Secure Connections}} uses different cryptography ({{ecdh|ECDH}} on Curve P-256) and a different pairing flow, and has held up better — but its own pairing-method downgrade (Just Works ↔ Numeric Comparison) has had its share of disclosures. The pattern matches Wi-Fi exactly: **AES is fine; the {{handshake|handshake}} around it is where the bugs live**.`
						},
						{
							type: 'callout',
...
The industry conclusion was structural: **proximity-by-radio-signal-strength is unfixable**. A relay with enough TX power and enough patience defeats any {{rssi|RSSI}} threshold. The only way to verify physical proximity is to measure **{{tof-ranging|time-of-flight}}**, because *the speed of light is the hard upper bound that no relay can shorten*. That insight pushed every credential standard since to mandate physics-based proximity: {{ccc-digital-key|CCC Digital Key 3.0}} ([[uwb|UWB]] ranging), {{aliro|Aliro 1.0}} ([[uwb|UWB]] or {{channel-sounding|Bluetooth Channel Sounding}}), and the Bluetooth 6.0 ranging mode that uses much tighter RTT than legacy BLE.
...
The attack injected STS-like-but-random noise into the [[uwb|UWB]] channel. With enough trials, some of that noise happened to correlate with the legitimate STS sequence well enough to bias the receiver's correlation peak earlier — falsely shortening the measured distance into the "unlock allowed" zone. The fix is **IEEE 802.15.4ab** (Draft D03, September 2025, ratification expected early 2026), which adds **NBA-MMS** (narrowband-assisted multi-millisecond ranging) and a redesigned STS receiver that is much harder to bias.
...
The shape rhymes with every other story in this chapter. Wi-Fi: {{krack|KRACK}} in 2017, fix shipping by 2020. Bluetooth: KNOB/BIAS/BLUFFS, fixes in 5.4/6.0. [[nfc|NFC]]: Crypto1 in 2007, migration to AES by 2020s. [[uwb|UWB]]: Ghost Peak in 2022, 802.15.4ab in 2026. Cellular: SS7 exploitation public since the early 2010s, 5GC SBI partial fix arriving in the late 2020s. Every wireless protocol's cryptography has held; every wireless protocol's *architecture around the cryptography* has been broken and patched and broken and patched.`
						},
						{
							...
							text: 'Across every wireless protocol in this Part: the **AES primitives, {{ecdh|ECDH}} key exchanges, and signature algorithms have all held up under twenty years of public attack**. What gets broken on stage every other year is the *negotiation, state machine, roaming model, or trust assumption between operators* — the *protocol surface around* the cryptography. The fix is always the same: **make every negotiation step cryptographically authenticated, every key derivation forward-secret, every proximity claim physics-based, and every inter-operator interface mutually authenticated.** {{wpa3|WPA3}} did it for Wi-Fi; Bluetooth 5.4/6.0 is doing it for BR/EDR; {{ccc-digital-key|CCC Digital Key}} 3.0 did it for proximity; 5GC SBA is doing it for carrier interconnect. The job is half-finished, on a thirty-year clock.'
						},
						{
							type: 'narrative',
...
**Audit the state machine, not the cipher.** The crypto in your protocol is almost certainly fine — AES-128, {{aes-gcm|AES-GCM}}, {{chacha20-poly1305|ChaCha20-Poly1305}}, {{curve25519|Curve25519}}, NIST P-256 have all survived two decades of attack. The bugs live in *what your protocol does around the crypto* — key reinstallation, entropy negotiation, role switching, {{session-resumption|session resumption}}, downgrade handling. Spend most of your security review there.
...
Every chapter in this Part has security stories. This chapter ties them together as one arc — and the arc is **roughly the same arc as every other security-critical protocol family in this book**. The pattern is not unique to wireless; it is just *most visible* there, because the medium itself is adversarial.`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			synopsis:
				'The non-technical layer that decides where every wireless protocol may live — 6 GHz [[wifi|Wi-Fi]], {{mmwave|mmWave}}, {{cbrs|CBRS}}, WRC-27, [[wifi|Wi-Fi]] 8, {{ambient-iot|Ambient IoT}}, 6G targets, and {{starlink|Starlink}} {{direct-to-cell|Direct-to-Cell}}. The wireless frontier through 2030.',
			slots: [
				{
					kind: 'pull-quote',
...
The result is that *the same physical radio* is legal in some countries, restricted in others, and banned outright in a few. **{{apple|Apple}}'s iPhone disables [[uwb|UWB]] Precision Finding in Japan, Indonesia, parts of the Middle East, and a few smaller markets** because of regional power-mask differences. The {{cbrs|CBRS}} band that powers Private 5G in the United States does not exist in Europe. The 6 GHz upper band that the FCC opened for Wi-Fi in 2020 may yet be reassigned to mobile carriers in the EU. Every protocol chapter has a wire-format story; this chapter is about the *policy* story underneath all of them.`
						},
						{
							...
							text: `**On 23 April 2020, the {{fcc|FCC}} freed 1,200 MHz of 6 GHz {{spectrum|spectrum}}** (5.925–7.125 GHz) for unlicensed use — the largest single {{bandwidth|bandwidth}} grant the United States has authorised in 20 years. Wi-Fi 6E and Wi-Fi 7 were the immediate consumers. The decision essentially **tripled the unlicensed {{spectrum|spectrum}} available for [[wifi|Wi-Fi]]**, easing the 2.4 GHz and 5 GHz crowding that had bedevilled apartment buildings and convention centres for two decades.
...
**Sub-second live media** is the [[wifi|Wi-Fi]] and [[cellular|cellular]] side of the [[rtp|RTP]]-over-[[quic|QUIC]] / MoQ-Transport story. The [[cellular|cellular]] network has had ~99.999% link reliability via {{harq|HARQ}} for fifteen years; the application layer is finally building on top of it. {{cloudflare|Cloudflare}} deployed MoQ relays across 330+ cities in 2025; Twitch's *Warp* QUIC-based live-streaming pilot is the canonical use case. The 2026 Super Bowl, AFC Wild Card, and Paris 2024 Olympics measurements all show OTT streaming still 40–80 seconds behind {{broadcast|broadcast}}. The next five years collapse most of that gap.
