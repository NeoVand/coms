...
The pattern is simple: BLE has the **right discovery + power profile** to be the always-on radio. The actual session moves to whichever radio has the right property for the workload — Wi-Fi for throughput, UWB for ranging, {{thread|Thread}} for mesh.`
						},
						{
							type: 'narrative',
...
Whether {{channel-sounding|Channel Sounding}} actually displaces UWB for digital-key applications is still an open question in 2026. UWB has a five-year head start in {{ccc-digital-key|CCC Digital Key}}, a tighter timing precision (~30 cm at the silicon level versus ~5–10 cm cm-claimed for {{channel-sounding|Channel Sounding}} under good conditions), and dedicated cryptography ({{sts|STS}}) that was designed for adversarial environments from day one. {{channel-sounding|Channel Sounding}} has the deployment advantage: it ships on every new Bluetooth 6.0 chip, which means every new smartphone. The next two years of car-key product announcements will tell.`
						},
						{
							type: 'callout',
...
The {{spectrum|spectrum}} legacy: **the FCC's First Report and Order on Valentine's Day 2002** authorised UWB under Part 15.519 at **−41.3 dBm/MHz** across **3.1–10.6 GHz** — an *unlicensed underlay* on top of every other service in those bands. The trick is the noise floor. UWB transmits at such a low average power that no incumbent licensee notices, but it spreads its energy across so much {{bandwidth|bandwidth}} that the cumulative pulse energy is detectable at the receiver. The two channels everyone actually uses are **Channel 5** (6489.6 MHz) and **Channel 9** (7987.2 MHz), each 499.2 MHz wide.`
						},
						{
							type: 'narrative',
...
The Schlage FCC waiver (ET Docket 22-248, granted 2023) describes this exact sequence in regulator-vetted form. It is also why BLE {{channel-sounding|Channel Sounding}} — Bluetooth 6.0's cm-class ranging built into the BLE link itself — is interesting: it might collapse the five-step dance into one radio.`
						},
						{
							...
							text: `BMW iX shipped the first {{ccc-digital-key|CCC Digital Key 3.0}} vehicle in early 2022. The vehicle has multiple **[[uwb|UWB]] anchors** (typically one in each B-pillar and one per door handle) plus a BLE radio. The phone has {{apple|Apple}} U1/U2 or NXP/Qorvo UWB silicon plus its own BLE.
