...
Wired networking is a problem with a known solution. Wireless networking is three intertwined problems — sharing, fading, and {{multipath|multipath}} — that the entire stack from {{phy|PHY}} to application has to compensate for at every layer.`
						},
						{
							type: 'narrative',
...
[[zigbee|Zigbee]] and [[zigbee|Thread]] use a similar unslotted CSMA-CA on {{ieee-802-15-4|IEEE 802.15.4}}. [[bluetooth|Bluetooth]] dodges the problem by **{{frequency-hopping|frequency-hopping}}** — 1,600 hops/sec on {{br-edr|BR/EDR}} — so collisions on any single channel are statistically rare. [[cellular|Cellular]] does not contend at all on the downlink; the base station schedules every slot.`
						},
						{
							type: 'callout',
						...
						},
						{
							...
							text: `Four protocol families share the unlicensed 2.4 GHz **{{ism-band|ISM band}}**: [[wifi|Wi-Fi]] (20 MHz channels centred at 2412 / 2437 / 2462 MHz — the canonical 1/6/11 trio), [[bluetooth|Bluetooth]] {{br-edr|BR/EDR}} (79 × 1 MHz channels hopping 1,600/sec), [[bluetooth|BLE]] (40 × 2 MHz channels), and [[zigbee|Zigbee]] / {{thread|Thread}} on {{ieee-802-15-4|IEEE 802.15.4}} (16 × 2 MHz channels at 11–26). Plus microwave ovens, baby monitors, cordless phones, {{usb|USB}}-3 hubs, and every other device any regulator ever granted Part 15 to.

They coexist by a series of small accommodations. **Modern combo chips** ({{apple|Apple}} H-series, {{broadcom|Broadcom}}, Qualcomm) put Wi-Fi and Bluetooth radios on the same die and arbitrate {{airtime|airtime}} in firmware, time-slicing so one starves the other only briefly. **Zigbee dodges Wi-Fi**: channels 15, 20, 25, and 26 sit in the gaps between Wi-Fi 1/6/11, and the single most common cause of unreliable Zigbee is a coordinator dongle plugged into a Wi-Fi router's {{usb|USB}} port whose switched-mode PSU radiates broadband 2.4 GHz noise. **{{ble|BLE}} picks its advertising channels carefully** — 37/38/39 sit at 2402, 2426, and 2480 MHz, deliberately outside Wi-Fi 1/6/11; the 37 data channels rely on adaptive frequency hopping to dodge active access points. **The 5/6 GHz escape valve** is where Wi-Fi 5, 6, 6E, 7, and 8 increasingly live, leaving 2.4 GHz to IoT.

[[cellular|Cellular]] bands are **licensed**, which is why your phone's 4G/5G radio does not fight with your Wi-Fi even in the same physical space — different {{spectrum|spectrum}} entirely. The price of that predictability is the billions paid at every national {{spectrum|spectrum}} auction.`
						},
						{
							type: 'image',
...
[[nfc|NFC]] picks **low power + low range** — passive cards harvest microwatts from the reader's field at ≤10 cm and trade everything for a 13.56 MHz carrier that physics caps at ~424 kbit/s. [[bluetooth|BLE]] picks **low power + medium throughput** — coin-cell devices at 1–2 Mbit/s over 10 m. [[wifi|Wi-Fi]] picks **high throughput + medium range** — gigabit speeds at 30 m, but only because the {{ap-access-point|AP}} burns hundreds of milliwatts of TX power and runs off mains electricity. [[cellular|Cellular]] picks **range + throughput** at the cost of power and licensed {{spectrum|spectrum}} — 50 km from the right base station, ~1–10 Gbit/s in FR1, but you do not run a base station on a coin cell. [[uwb|UWB]] sits in a corner of its own: **wide {{bandwidth|bandwidth}} + low average power** by trading {{tof-ranging|time-of-flight}} precision for any meaningful data rate. It is a clock, not a data radio.

**Edholm's law of {{bandwidth|bandwidth}}** — wireless data rates double roughly every 18 months — is what keeps the table moving. Every protocol in this Part has shipped two or three generation upgrades since 2010 to keep its corner of the triangle in tension.`
						},
						{
							type: 'callout',
							title: 'The bootstrap pattern: no wireless protocol works alone',
							text: 'Almost every consumer wireless interaction in 2026 chains *multiple* radios. **[[uwb|UWB]] ranging** never starts without [[bluetooth|BLE]] first — the lock or car advertises a service {{uuid|UUID}} over {{ble|BLE}}, the phone runs {{spake2|SPAKE2}}+ authentication and ships the STS_KEY over the encrypted {{gatt|GATT}} channel, only then does [[uwb|UWB]] power on for a three-message ranging round. **[[bluetooth|Bluetooth]] / [[wifi|Wi-Fi]] handover** is bootstrapped by [[nfc|NFC]] — a 4 cm tap carries the {{ble|BLE}} {{mac-address|MAC}} + {{smp-pairing|SMP}} {{oob|OOB}} key or the Wi-Fi {{ssid|SSID}} + WPA key in an {{ndef|NDEF}} record. **[[zigbee|Zigbee]] + {{thread|Thread}}** are commissioned over {{ble|BLE}} (Zigbee Direct) or Wi-Fi ({{matter|Matter}} setup); once commissioned they run their own mesh. **[[cellular|Cellular]] data** falls back to [[wifi|Wi-Fi]] Calling when the carrier signal is weak — [[ipsec|IPsec]] ePDG tunnel from the {{ue|UE}} to the carrier core over any {{ip-address|IP}} link, including the airport Wi-Fi you just joined. The radio with the **best discovery + power profile** does the bootstrap; the radio with the **right property for the workload** (range, throughput, precision, security) does the actual session. Each chapter that follows is one corner of that bigger picture.'
						},
						{
							type: 'narrative',
							title: 'A note on chapter 2 — Wi-Fi appears twice',
							text: `[[wifi|Wi-Fi]] gets a chapter in Part III (Layer 2–3) and another one here. That is on purpose. Part III's chapter is about Wi-Fi as a **layer-2 fabric** — the bit that bridges [[wifi|802.11]] frames to [[ethernet|Ethernet]] and lets [[ip|IP]] forget the medium underneath. This Part's chapter is about Wi-Fi as a **radio** — {{csma-ca|CSMA/CA}}, {{mlo|MLO}}, the 6 GHz politics, the {{krack|KRACK}} → Dragonblood → {{ssid|SSID}}-Confusion lineage of attacks that only make sense once you understand the shared-medium problem above. Two halves of the same protocol, one from the cable's perspective and one from the airwaves'.`
						}
					]
				},
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			synopsis:
				'The radio side of [[wifi|802.11]] — {{csma-ca|CSMA/CA}} in practice, MIMO → {{ofdma|OFDMA}} → {{mlo|MLO}}, and the {{krack|KRACK}}-to-{{wpa3|WPA3}} arc that finally fixed the wireless {{handshake|handshake}}.',
			slots: [
				{
					kind: 'pull-quote',
					...
					sections: [
						{
							...
							text: `Part III's chapter on Wi-Fi treated [[wifi|802.11]] as a Layer-2 fabric — the bit that bridges wireless frames to [[ethernet|Ethernet]], the regulatory big bang that opened the {{ism-band|ISM}} bands, and the 6 GHz / {{wifi-7|Wi-Fi 7}} / {{wifi-8|Wi-Fi 8}} generational arc. That framing is correct for someone reading bottom-up from the cable.

This chapter reads top-down from the *air*. The frame format and history we leave to Part III; here we look at what actually goes on inside the {{csma-ca|CSMA/CA}} loop, how MIMO and {{ofdma|OFDMA}} turned the original physics problem ({{multipath|multipath}}) into the {{bandwidth|bandwidth}}, what the headline "{{wifi-7|Wi-Fi 7}} multi-link" feature really means in shipping silicon, and why every {{wpa3|WPA3}} {{access-point|access point}} in 2026 exists because Mathy Vanhoef sent three packets in 2017.
...
The visible cost is **{{airtime|airtime}} overhead**. Add up DIFS + SIFS + {{ack|ACK}} frames + beacons + management traffic and you spend ~50% of channel time on protocol housekeeping at typical loads. Real engineers measure Wi-Fi performance in {{airtime|airtime}}, not bits per second — two clients with the *same* throughput target consume very different {{airtime|airtime}} if one is on a slow MCS at cell edge. Modern access points implement **{{airtime|airtime}} fairness** so a slow client cannot starve fast ones; classic [[wifi|802.11]] was famous for letting one device at the edge of the cell drag the entire network down to its rate.`
						},
						{
							type: 'callout',
...
The shipping-silicon story is subtler. Most {{wifi-7|Wi-Fi 7}} client chips implement **eMLSR** (Enhanced Multi-Link Single Radio): one RF chain time-sliced across bands. True simultaneous transmit-receive across bands (STR-{{mlo|MLO}}) requires multiple full RF chains and shows up only in high-end {{access-point|AP}} hardware. **Throughput does NOT add across bands** for eMLSR clients — what they get is **{{latency|latency}} consistency**, not raw aggregate throughput. Marketing material that talks about 46 Gbit/s aggregated across three bands is talking about an {{ap-access-point|AP}}-side capability that almost no consumer client can use.

The real headline feature of [[wifi|Wi-Fi]] 8 / 802.11bn is more honest about this. **{{wifi-8|Wi-Fi 8}} is not a peak-speed upgrade**: same bands as {{wifi-7|Wi-Fi 7}}, same 320 MHz max, same ~46 Gbit/s {{phy|PHY}} peak. The PAR objectives are **+25% throughput at given SINR, −25% 95th-percentile {{latency|latency}}, −25% MPDU loss across {{bss-coloring|BSS}} transitions**. The trend across the last two generations is the same — squeeze the existing speed budget for {{tail-latency|tail latency}} and reliability, not the headline marketing number.`
						},
						{
							type: 'callout',
						...
						},
						{
							...
							text: `{{wpa2|WPA2}}'s four-way {{handshake|handshake}}, specified in 2004, looked solid until 16 October 2017. On that morning, Mathy Vanhoef and Frank Piessens released **{{krack|KRACK}}** — Key Reinstallation AttaCK. The flaw was conceptually small: by replaying message 3 of the {{handshake|handshake}}, an attacker could trick a client into reinstalling an already-used session key, resetting the per-packet {{iv|nonce}} counter and defeating CCMP integrity. **Every {{wpa2|WPA2}} client on Earth needed firmware updates.**

{{krack|KRACK}} forced the {{wpa3|WPA3}} replacement that had been waiting in the wings. {{wpa3|WPA3}}-Personal uses **{{sae|SAE}}** (Simultaneous Authentication of Equals) — a *dragonfly* {{handshake|handshake}} from {{rfc-doc|RFC}} 7664 where both sides prove knowledge of the passphrase without ever sending anything an eavesdropper could grind against a dictionary. Each session derives a fresh Pairwise Master Key with {{pfs|forward secrecy}}. {{wpa3|WPA3}} was announced January 2018 and made mandatory for Wi-Fi CERTIFIED 6 products from July 2020.

The story did not end there. Vanhoef's group at KU Leuven has broken Wi-Fi roughly every two years since: **Dragonblood** (April 2019, side-channels in {{wpa3|WPA3}}-{{sae|SAE}}), **FragAttacks** (May 2021, {{fragmentation|fragmentation}}/aggregation bugs in [[wifi|802.11]] design itself), **Framing Frames** (March 2023), **{{ssid|SSID}} Confusion / {{cve|CVE}}-2023-52424** (May 2024 — the [[wifi|802.11]] standard does NOT require the {{ssid|SSID}} to enter {{pmk|PMK}} or session-key derivation in many config paths). The cadence is so reliable that the field plans security audits around his summer talks.

What Wi-Fi gets right after eight years of public attack is that **the attacks are *not* against the cryptographic primitives** — {{aes|AES}}-CCMP and {{aes|AES}}-GCMP have held up cleanly. The attacks are against the *protocol mechanics* of negotiation, key reinstallation, and middleware downgrade. The same shape recurs in [[bluetooth|Bluetooth]] ({{knob-attack|KNOB/BIAS/BLUFFS}}) and [[cellular|cellular]] ({{ss7|SS7}}/{{diameter|Diameter}} abuse): the cryptographic engine is sound; the **state machine around it** is where the bugs live.`
						},
						{
							type: 'narrative',
							title: 'Wi-Fi as the radio that does not know what it is for',
							text: `Most internet protocols are designed for a workload. [[grpc|gRPC]] is service-to-service {{rpc|RPC}}; [[mqtt|MQTT]] is sensor telemetry; [[rtp|RTP]] is conversational media. [[wifi|Wi-Fi]] is none of those — it is a *generic radio* underneath everything. The same 2.4 GHz {{frame|frame}} carries a [[tcp|TCP]] segment for an [[ssh|SSH]] session, a [[udp|UDP]] datagram for a video call, a [[mdns-dns-sd|Bonjour]] {{multicast|multicast}} for AirPlay discovery, and a {{matter|Matter}} commissioning message.

That genericity is why Wi-Fi has had to evolve so much at the **{{phy|PHY}}** and **{{mac-address|MAC}}** layers and so little above them. The {{ip-address|IP}} stack on top is the same one [[ethernet|Ethernet]] uses — the [[wifi|802.11]] driver bridges [[wifi|802.11]] frames to 802.3 frames before [[ip|IP]] ever sees them. Every architectural choice in this chapter ({{ofdma|OFDMA}}, {{mlo|MLO}}, {{target-wake-time|TWT}}, {{wpa3|WPA3}}) is in the service of carrying *somebody else's traffic* across a hostile shared medium without those upper layers having to know.

The single thing the upper layers do notice is **{{tail-latency|tail latency}}** — the 99th-percentile delay that makes video calls stutter and games lag. From Wi-Fi 4 (2009) to {{wifi-8|Wi-Fi 8}} (2028 target), the protocol's main job has been bending the {{tail-latency|tail latency}} distribution back toward the median while the headline throughput tripled three times.`
						},
						{
							type: 'image',
...
The cross-industry alliance came next. {{ibm|IBM}} (Adalio Sanchez) and Ericsson (Rydbeck) agreed the radio should be an open standard so neither company could be locked out. Sanchez recruited Stephen Nachtsheim at {{intel|Intel}}; {{intel|Intel}} pulled in Toshiba and Nokia. On a windy Toronto pub crawl, [[pioneer:jim-kardach|Jim Kardach]] of {{intel|Intel}} and Mattisson talked Viking history — Mattisson had been reading *The Longships*, Kardach had brought *The Vikings*. Kardach pitched the name **"Bluetooth"** as a code name after Harald "Blåtand" Gormsson, the 10th-century Danish king who united Denmark and Norway. It was supposed to be a placeholder. It stuck. The **Bluetooth Special Interest Group** launched on 20 May 1998 with five Promoter members: Ericsson, {{ibm|IBM}}, {{intel|Intel}}, Nokia, Toshiba. The logo is a bind-rune combining Hagall (ᚼ = H) and Bjarkan (ᛒ = B) — Harald's initials in Younger Futhark.
...
The first commercial product — a hands-free headset — shipped in 1999. The first phone was the Ericsson T39 in June 2001.`
						},
						{
							...
							text: `[[bluetooth|Bluetooth]] in 2026 is **two protocols sharing a logo**. **{{br-edr|BR/EDR}}** ("Classic") is the original 1999 master/{{piconet|peripheral}} wire-replacement system — 79 × 1 MHz channels, 1,600 hops per second, GFSK + DPSK modulation. It still carries A2DP audio, HFP voice, HID (every wireless keyboard and mouse), and RFCOMM serial emulation. {{ble|**BLE (Bluetooth Low Energy)**}} was added in Core 4.0 (December 2009), derived from Nokia Research's *Wibree* project. **Different radio** (40 × 2 MHz channels), **different link layer**, **different framing** ({{l2cap|L2CAP}}), **different security** ({{smp-pairing|SMP}}), **different application protocol** ({{gatt|GATT}}). Both share the 2.4 GHz {{ism-band|ISM}} band and a {{sig|SIG}}, but they share **no bits over the air** — a dual-mode chip runs both stacks side by side.

The split is why every {{ble|BLE}} primer warns readers not to read 2000s-era Bluetooth tutorials: master/slave became Central/Peripheral, A2DP became {{le-audio|LE Audio}}, {{gatt|GATT}} replaced [[sdp|SDP]], and connection setup got an order of magnitude faster. The two protocols are at the *brand-level* one thing and at the *implementation-level* two completely different stacks.

Production silicon hides the seam. {{apple|Apple}}'s H1/H2 chips, {{broadcom|Broadcom}}'s combo dies, Qualcomm's QCAxxxx — all run {{br-edr|BR/EDR}} + {{ble|BLE}} + Wi-Fi on the same package and time-slice between them in firmware. Your AirPods Pro charging case implements {{br-edr|BR/EDR}} for the audio (A2DP) and {{ble|BLE}} for the *find-my* network, both from the same antenna. You do not notice because you are not supposed to.`
						},
						{
							type: 'callout',
							title: 'GATT is a tiny REST API embedded in every BLE peripheral',
							text: 'A {{gatt|GATT}} server (the Peripheral) exposes a tree of **services → characteristics → descriptors**, each with a 16- or 128-bit {{uuid|UUID}} and a numeric handle. A {{gatt|GATT}} client (the Central) discovers them, reads/writes/subscribes-to-notifications. It is, structurally, a tiny [[rest|REST]] {{api|API}} embedded in every {{ble|BLE}} device — services are endpoints, characteristics are the actual values. Every fitness sensor, smart lock, hearing aid, AirTag, electric toothbrush, and smart light bulb speaks this same shape. The 2010 design that makes a wearable battery last a week.'
						},
						{
							type: 'narrative',
							title: 'Why frequency hopping was the right answer',
							text: `[[bluetooth|Bluetooth]]'s **{{frequency-hopping|frequency-hopping spread spectrum}}** is one of those design choices that aged spectacularly well. {{br-edr|BR/EDR}} hops 1,600 times per second across 79 × 1 MHz channels under a pseudo-random sequence keyed to the {{piconet|piconet}} master's clock and BD_ADDR. **AFH** (Adaptive Frequency Hopping, introduced in Core 1.2 in 2003) extends the trick: noisy channels get blacklisted from the hop set, so a microwave oven on its own narrow leakage frequency only knocks out a handful of hops instead of the whole link.

The architectural consequences run deep. **Collision avoidance is statistical** — two piconets in the same room would have to land on the same channel at the same hop instant to collide, and they almost never do. **Sniffing is hard** — to capture a Bluetooth conversation, an attacker must follow the hop pattern, which is keyed to the master's clock and address; off-the-shelf SDRs can do it, but it is much harder than capturing Wi-Fi where everyone sits on a single channel. **Interference resistance is automatic** — even before AFH, a 5% busy channel only drops 5% of hops; {{forward-error-correction|FEC}} handles the rest.
...
{{ble|BLE}} kept the hopping idea but simplified the channel plan. **40 × 2 MHz channels**. Three of them — 37, 38, 39 — sit at 2402, 2426, and 2480 MHz, deliberately placed between [[wifi|Wi-Fi]] channels 1/6/11, and serve as the **primary advertising channels**. The remaining 37 (numbered 0–36) carry data inside an established connection, hopping once per connection event.`
						},
						{
							...
							text: `If [[wifi|Wi-Fi]] is the radio that does not know what it is for, **{{ble|BLE}} is the radio that knows it is a bootstrap**. Almost every consumer wireless interaction in 2026 chains *multiple* radios, and {{ble|BLE}} is the one that does the discovery.

**{{matter|Matter}} commissioning** uses {{ble|BLE}} to hand Wi-Fi or [[zigbee|Thread]] credentials to a new device — the QR code on the side of a smart bulb encodes a {{ble|BLE}} pairing {{payload|payload}}, the phone runs the {{spake2|SPAKE2}}+ pairing dance over {{ble|BLE}}, and only then does the device join the Wi-Fi or {{thread|Thread}} network it will actually operate on.

**{{ccc-digital-key|CCC Digital Key 3.0}}** uses {{ble|BLE}} for proximity and then bootstraps a [[uwb|UWB]] session — the phone advertises a service {{uuid|UUID}} over BLE, the car authenticates, the encrypted {{gatt|GATT}} channel transfers the STS_KEY for the [[uwb|UWB]] ranging round, and only then does the [[uwb|UWB]] radio power on for the three-message {{ds-twr|DS-TWR}} {{exchange|exchange}}. **{{aliro|Aliro}} 1.0**, the access-control credential standard the {{sig-bluetooth-acronym|CSA}} finalised on 26 February 2026, follows the same pattern for doors.

**{{apple|Apple}} AirTags** advertise BLE packets every couple of seconds; nearby iPhones report them through the *Find My* network so the AirTag's owner can locate it without any cellular hardware. **Hue bulbs** (and almost every smart light shipped after 2020) accept Wi-Fi/{{thread|Thread}} credentials over BLE during commissioning. **Hearing aids** use BLE for the control plane and {{le-audio|LE Audio}} for the stream.

The pattern is simple: BLE has the **right discovery + power profile** to be the always-on radio. The actual session moves to whichever radio has the right property for the workload — Wi-Fi for throughput, [[uwb|UWB]] for ranging, {{thread|Thread}} for mesh.`
						},
						{
							type: 'narrative',
...
The {{auracast|Auracast}} accessibility story is the killer app, not "free wireless audio in the airport." For hard-of-hearing listeners, {{auracast|Auracast}} turns every public-address system into something their hearing aids can listen to directly — no analog loop, no battery-eating Bluetooth pair with a wall-mounted transmitter, just a scan-and-{{imap-select|select}}. The {{sig|SIG}} positions it as the *largest accessibility upgrade in consumer audio history*, and the deployment math (every hearing-aid manufacturer ships {{le-audio|LE Audio}} in 2026 hardware) backs that up.`
						},
						{
							type: 'narrative',
							title: 'Channel Sounding — taking the fight to UWB',
							text: `For the last five years the secure-distance-measurement niche has belonged to [[uwb|UWB]]: BMW, Mercedes, and {{apple|Apple}} use [[uwb|UWB]] for cm-class digital car keys precisely *because* {{ble|BLE}}'s {{rssi|RSSI}}-based proximity is broken under relay attacks. **{{channel-sounding|Channel Sounding}}**, added in **Bluetooth 6.0** (adopted 3 September 2024), is the {{sig|SIG}}'s reply.

Two devices in a normal LL connection schedule {{channel-sounding|Channel Sounding}} events on a new **{{le-audio|LE}} 2M 2BT {{phy|PHY}}** specifically designed for ranging. They measure both signal **phase** across many frequencies (Phase-Based Ranging) and **{{rtt|round-trip time}}** of timestamped packets; the combination yields **centimetre-class distance accuracy up to ~150 m**. The intended use: digital car keys, smart locks, anti-stalking tags, and proximity-aware payment terminals — all of which need to know if the {{peer|peer}} is actually *here* and not relayed through a radio.

Whether {{channel-sounding|Channel Sounding}} actually displaces [[uwb|UWB]] for digital-key applications is still an open question in 2026. [[uwb|UWB]] has a five-year head start in {{ccc-digital-key|CCC Digital Key}}, a tighter timing precision (~30 cm at the silicon level versus ~5–10 cm cm-claimed for {{channel-sounding|Channel Sounding}} under good conditions), and dedicated cryptography ({{sts|STS}}) that was designed for adversarial environments from day one. {{channel-sounding|Channel Sounding}} has the deployment advantage: it ships on every new Bluetooth 6.0 chip, which means every new smartphone. The next two years of car-key product announcements will tell.`
						},
						{
							type: 'callout',
							title: 'The KNOB / BIAS / BLUFFS lineage',
							text: 'Three {{br-edr|BR/EDR}} session-security breaks by the same author (Daniele Antonioli) in five years: **{{knob-attack|KNOB}}** ({{cve|CVE}}-2019-9506) downgraded the entropy of the negotiated session key to 1 byte. **{{bias-attack|BIAS}}** ({{cve|CVE}}-2020-10135) impersonated a previously-bonded {{peer|peer}} by abusing role-switch in Legacy Secure Connections. **{{bluffs-attack|BLUFFS}}** ({{cve|CVE}}-2023-24023) broke {{forward-secrecy|forward secrecy}} by forcing reuse of a session-key derivation across reconnections. Every {{br-edr|BR/EDR}} device shipped before mid-2024 is affected; the Core 5.4 / 6.0 patches add explicit minimum-entropy and key-diversification checks. Each attack hit a different part of the *state machine* around the cryptography — the {{aes|AES}} core was fine; the negotiation logic around it was the bug. **The same pattern as {{krack|KRACK}} in Wi-Fi and {{ss7|SS7}}/{{diameter|Diameter}} abuse in cellular.** Every wireless protocol in this Part has now had its negotiation logic publicly broken at least once.'
						},
						{
							type: 'narrative',
							title: 'The 2022 Tesla relay attack — and why physics is the only fix',
							text: `On 15 May 2022, Sultan Qasim Khan at NCC Group disclosed a **link-layer relay attack** against Tesla Model 3 phone-as-a-key. Two ~$25 dev boards (one near the phone, one near the car), a few hundred metres of cellular link between them, and ~8 ms of added {{latency|latency}} was enough to make the Tesla believe the phone was in proximity when it was actually at the supermarket. The attack worked because **{{ble|BLE}} {{rssi|RSSI}} is fundamentally untrustworthy** — signal strength can be amplified arbitrarily by a relay, and the link-layer round-trip-time check in classic {{ble|BLE}} was too coarse (~30 ms) to catch an 8 ms relay.

The industry conclusion: **proximity-by-radio-signal-strength is unfixable.** A relay with enough TX power and enough patience defeats any {{rssi|RSSI}} threshold. The only way to verify physical proximity is to measure **{{tof-ranging|time-of-flight}}**, because *the speed of light is the hard upper bound that no relay can shorten*. That insight pushed the secure-access industry toward [[uwb|UWB]] (where {{tof-ranging|ToF}} is the entire point) and motivated Bluetooth {{channel-sounding|Channel Sounding}}'s RTT mode to use much tighter timing than legacy {{ble|BLE}}.

The 2022 attack is the canonical case study for why every credential standard since — {{ccc-digital-key|CCC Digital Key 3.0}}, {{aliro|Aliro}}, Bluetooth 6.0 ranging — explicitly mandates physics-based proximity, not radio-strength heuristics. {{rssi|RSSI}} was a workable shortcut for a decade. Then it stopped being one.`
						},
						{
							type: 'image',
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			synopsis:
				'One chapter for [[cellular|the radio (LTE-Uu, NR-Uu)]] and the core ({{epc|EPC}} → 5GC {{sba|SBA}}) because the {{3gpp|3GPP}} release calendar is one calendar. {{volte|VoLTE}} / [[wifi|Wi-Fi]] calling, NB-IoT / {{lte|LTE}}-M, satellite {{direct-to-cell|direct-to-cell}}, and the {{ss7|SS7}} / {{diameter|Diameter}} trust holdover.',
			slots: [
				{
					kind: 'pull-quote',
					text: 'The control plane of every modern carrier on Earth is now an [[http2|HTTP/2]] microservice fabric — and every backhaul hop is wrapped in [[ipsec|IPsec]] {{esp|ESP}} per {{3gpp|3GPP}} {{ts-3gpp|TS}} 33.501. The single largest enterprise [[ipsec|IPsec]] deployment on Earth runs inside this layer.',
					attribution: 'Author'
				},
				{
					kind: 'prose',
...
That call started the **53-year arc** from Cooper's prototype to the ~9 billion [[cellular|cellular]] subscriptions running 4G {{lte|LTE}} and {{5g-nr|5G NR}} in 2026. Three generations of analog (1G AMPS / TACS / NMT, ~1983–2000), three generations of digital (2G {{gsm|GSM}} / IS-95, ~1991; 3G {{wcdma|WCDMA}} / CDMA2000, ~2001; 4G {{lte|LTE}}, ~2010), and now {{5g-nr|5G NR}} (~2018) — each one redrawing both the air interface and the core network on roughly a decade's cadence.

The transition that defined the modern era was 2G to 3G. **{{gsm|GSM}}**, launched commercially in Finland by Radiolinja in 1991, was the European digital standard that became the global one through sheer deployment momentum. **{{cdma|CDMA}}** (Andrew Viterbi at Qualcomm) was the technically-superior US challenger that won the underlying maths and lost the deployment war: when 3G ratified {{wcdma|WCDMA}} inside {{umts|UMTS}}, Viterbi's {{cdma|CDMA}} principles were what made the *radio* work, but the *system* was built on the {{gsm|GSM}} operator ecosystem. The pattern is one we have seen elsewhere — [[sctp|SCTP]] being technically superior to [[tcp|TCP]], {{osi-model|OSI}} to {{tcp-ip-model|TCP/IP}} — the better protocol that lost on deployment economics.`
						},
						{
							type: 'narrative',
							title: 'Why one chapter for "4G + 5G"',
							text: `Cellular is the only protocol in this book that gets a single chapter for what looks like *two* protocols. The reason is that {{3gpp|3GPP}}, the standards body, ships both on a single Release schedule, and a 5G phone in 2026 is also an {{lte|LTE}} phone falling back the moment 5G coverage drops. **{{lte|LTE}}** is {{lte|3GPP Release 8}}, frozen December 2008. **{{5g-nr|5G NR}}** is {{5g-nr|3GPP Release 15}}, frozen June 2018. Both share the same air-interface design philosophy ({{ofdma|OFDMA}} + flexible numerology + {{harq|HARQ}}), the mandatory [[ipsec|IPsec]] envelope on every backhaul link, and an [[ipv6|IPv6]] mandate that has quietly migrated every major carrier's user-plane to IPv6-only since ~2020.

The 5G *air interface* extended {{lte|LTE}}'s {{ofdma|OFDMA}} with scalable numerology — five subcarrier spacings (15, 30, 60, 120, 240 kHz) that let the same protocol address sub-6 GHz mid-band (FR1) and {{mmwave|mmWave}} 24–52 GHz (FR2). The 5G *core network* threw out 4G's {{epc|EPC}} zoo of monolithic boxes ({{mme|MME}}, {{sgw-acr|SGW}}, {{pgw|PGW}}, HSS, PCRF) glued by GTP and {{diameter|Diameter}}, and replaced it with **{{sba|Service-Based Architecture}}**: dozens of named network functions ({{aaa|AMF}}, {{smf|SMF}}, {{upf|UPF}}, {{ausf|AUSF}}, {{udm|UDM}}, PCF, NRF, NEF, NSSF, AF) talking to each other over **[[http2|HTTP/2]] with {{json|JSON}} payloads protected by [[tls|TLS]]**.

Read that sentence again. **The control plane of every 5G carrier on Earth is now an [[http2|HTTP/2]] {{service-mesh|microservice fabric}}.** Cloud-native engineers can read the {{3gpp|3GPP}} {{ts-3gpp|TS}} 23.501 architecture diagrams and recognise their own world — service registry (NRF), {{api|API}} gateway (NEF), policy engine (PCF), token authentication ({{ausf|AUSF}}). The plumbing under your phone call uses the same patterns as a Kubernetes deployment.`
						},
						{
							type: 'callout',
						...
						},
						{
							...
							text: `Every 5G phone runs a five-layer stack inside the modem chip. **{{phy|PHY}}** ({{3gpp|3GPP}} {{ts-3gpp|TS}} 38.211–214) carries {{ofdma|OFDMA}} with five numerologies. **{{mac-address|MAC}}** does **{{harq|hybrid ARQ}}** — combining forward error correction with {{retransmission|retransmission}}, where the receiver stores soft-decoded log-likelihood ratios from failed transmissions and combines them with the retransmitted copy. Eight parallel stop-and-wait {{harq|HARQ}} processes per {{ue|UE}} keep the pipe full. **RLC** handles {{fragmentation|segmentation and reassembly}} across 10- or 16-bit {{sequence-number|sequence numbers}}. **PDCP** above it does ROHC {{header|header compression}} (squashing the 40-byte [[ipv6|IPv6]]+{{tcp-rst|TCP}}/UDP header to 1–4 bytes), {{aes|AES}}-CTR {{encryption|ciphering}}, and 32-bit {{anti-replay|anti-replay}}. **{{rrc|RRC}}** drives the connection state machine — \`RRC_IDLE → CONNECTED → INACTIVE\` for 5G — and **{{nas|NAS}}** carries mobility, {{handshake|authentication}}, and session management end-to-end between the {{ue|UE}} and the core.

Above all that, the user plane is just [[ip|IP]] — almost always [[ipv6|IPv6]] now. Above *that*, the application runs whatever ordinary internet applications run. {{harq|HARQ}} is the reason [[cellular|cellular]] reaches ~99.999% link reliability without [[tcp|TCP]]'s retransmit cost on the air. ROHC is the reason a 56-kbps narrowband IoT bearer can carry an [[ipv6|IPv6]] + [[tls|TLS]] 1.3 connection without spending all its bytes on headers.`
						},
						{
							type: 'narrative',
							title: 'Voice as packets — VoLTE, VoNR, Wi-Fi Calling',
							text: `Until the late 2010s, "voice" on a phone meant a circuit-switched call routed through 2G/3G fallback even on an {{lte|LTE}} device. **{{volte|VoLTE}}** (Voice over {{lte|LTE}}, mass deployments from ~2014) finally packetised carrier voice — every call is a [[sip|SIP]] {{sip-invite|INVITE}} inside an {{ims|IMS}} bearer, with audio carried over [[rtp|RTP]] using AMR-WB or EVS {{codec|codecs}}. {{volte|**VoNR**}} (Voice over New Radio, mass deployments from ~2022) does the same over 5G.

{{gsma|GSMA}} reports **310+ commercial {{volte|VoLTE}} operators in 140+ countries** and **45+ commercial {{vonr|VoNR}} networks** by 2025. Every modern carrier voice call is now a [[sip|SIP]] {{sip-invite|INVITE}} — the largest [[sip|SIP]] deployment on Earth runs inside the [[cellular|cellular]] {{ims|IMS}} stack.

**Wi-Fi Calling** is the same {{ims|IMS}} stack tunnelled to the carrier over [[ipsec|IPsec]] from any {{ip-address|IP}} network. Your phone in the basement, on the hotel Wi-Fi, places calls through the carrier's *ePDG* (Evolved Packet Data Gateway) — an [[ipsec|IPsec]] head-end the size of a small data centre that terminates millions of {{ikev2|IKEv2}} tunnels and feeds the inner {{ims|IMS}} traffic into the {{epc|EPC}}. From the network's perspective, your basement phone looks like any other {{lte|LTE}} phone; it just happened to attach through Wi-Fi instead of an eNodeB.`
						},
						{
							type: 'narrative',
							title: 'Tunnels and addresses — how your IP follows you',
							text: `The architectural trick that makes [[cellular|cellular]] look like a normal {{ip-address|IP}} network from the application's perspective is **{{gtp-u|GTP-U}}** — the GPRS Tunnelling Protocol — User plane. Every {{pdu-session|PDU session}} has a 32-bit **Tunnel Endpoint Identifier (TEID)**. User-plane packets travel from the gNB to the User Plane Function over [[udp|UDP]]/2152, wrapped in {{gtp-u|GTP-U}} headers that preserve the {{ue|UE}}'s inner [[ip|IP]] address regardless of which base station the {{ue|UE}} is camping on.

This is the mechanism behind one of [[cellular|cellular]]'s most under-appreciated features: **your phone keeps its [[ip|IP]] address across handovers**. As you drive from one cell tower's coverage to the next, the {{gtp-u|GTP-U}} tunnel terminates at a different base station, but the same {{upf|UPF}} anchors the same [[ipv6|IPv6]] prefix to your device. Your YouTube stream does not have to renegotiate; your video call does not drop. The reason {{tcp-rst|TCP}}-on-cellular works at all is that the network hides the radio handover from the {{ip-address|IP}} layer.

**{{nas|NAS}}** (Non-Access Stratum) signalling runs end-to-end between the {{ue|UE}} and the core through the gNB transparently. **Registration Request** with **{{suci|SUCI}}** — the {{public-key|public-key-encrypted}} {{supi|SUPI}} ({{sim-usim|Subscription Permanent Identifier}}) — starts the AKA {{handshake|handshake}} after the device powers on. The {{udm|UDM}} decrypts to {{supi|SUPI}}, generates an authentication vector, and the USIM verifies AUTN and computes RES*. After **Security Mode Command**, all subsequent {{nas|NAS}} messages are integrity-protected and ciphered with K_NASint / K_NASenc. The {{sim-usim|SIM}} is the long-term key K; everything else is derived from it for the device's lifetime.`
						},
						{
							type: 'callout',
							title: 'The 2024 AT&T outage — 125 million devices, 25,000 failed 911 calls',
							text: 'On **22 February 2024 at 03:30 ET**, AT&T Mobility customers across the United States lost service for up to twelve hours. A routine network upgrade was misconfigured and pushed simultaneously across the production wireless core. **An estimated 25,000 emergency calls were not connected.** AT&T offered affected accounts a $5 service credit. The outage itself drew no FCC penalty — the $13 million AT&T settlement often cited alongside it (September 2024) was for an unrelated 2023 vendor data breach. The structural lesson — **never push a config change to the entire fleet at once** — is the same one the industry was supposed to have learned from [[outage:rogers-2022|Rogers 2022]]. Canary deployments are now post-incident table stakes in carrier networks.'
						},
						{
							type: 'narrative',
							title: 'IoT cellular — NB-IoT, LTE-M, RedCap',
							text: `Most [[cellular|cellular]] subscriptions on Earth are phones. The growing minority is **IoT modules** — asset trackers, fleet telematics, smart meters, agricultural sensors, {{gps|GPS}} pet collars. These devices are battery-powered, send a few kilobytes a day, and need years of operation per charge. Standard {{5g-nr|5G NR}} is wildly overkill for them; the {{3gpp|3GPP}} answer is a family of *cat-down* radio profiles inside {{lte|LTE}}/NR {{spectrum|spectrum}}.

**NB-IoT** (Cat-NB1, Release 13, 2016) gives ~250 kbit/s peak in a single 200 kHz channel — narrower than a Wi-Fi block, broader than nothing. **{{lte|LTE}}-M / Cat-M1** (Release 13, 2016) gives ~1 Mbit/s in 1.4 MHz, enough for voice over {{lte|LTE}} for connected wearables. **RedCap** (Reduced Capability NR, Release 17, 2022) is the 5G equivalent — {{bandwidth|bandwidth}}-constrained {{5g-nr|5G NR}} for wearables and industrial sensors. **{{ambient-iot|Ambient IoT}}** ({{ambient-iot|study items in Release 19/20}}) is the next step: battery-less or near-battery-less cellular devices that harvest RF or motion and transmit tiny payloads.

The {{lpwan|LPWAN}} story is broader than cellular — **LoRaWAN** (sub-GHz unlicensed, 125M+ devices by end-2025) and **Sigfox** (slow-modulation 100 bit/s ultra-narrowband) own much of the metering and agriculture market. But cellular's NB-IoT/{{lte|LTE}}-M can run on existing carrier infrastructure, which makes deployment trivial for operators and competitive for everyone else.`
						},
						{
							type: 'narrative',
							title: 'Direct-to-cell — when the satellite is the tower',
							text: `In **January 2025**, T-Mobile + SpaceX {{starlink|Starlink}} launched the first commercial **{{direct-to-cell|Direct-to-Cell}}** service: {{sms|SMS}} and emergency messaging from ordinary smartphones, with the satellite acting as a base station in standard [[cellular|cellular]] bands n255/n256. {{apple|Apple}}'s Globalstar-based Emergency SOS, AT&T's AST SpaceMobile partnership, and Iridium's {{ntn|NTN}}-ready successors follow similar patterns.

The {{3gpp|3GPP}} framing is **{{ntn|Non-Terrestrial Networks}}** — added in Release 17 (March 2022) as a first-class radio access type, split into NB-IoT {{ntn|NTN}}, NR {{ntn|NTN}}, and the {{direct-to-cell|Direct-to-Cell}} profile. The phone does not need a special radio; it does need to be in line-of-sight of a satellite, which means open sky.
...
What changes about the [[cellular|cellular]] mental model is the word "coverage." For 50 years, "no signal" meant *no signal*. For most of the next 50, "no signal" will mean *no terrestrial signal — try walking outside*. The implications for emergency services, maritime communications, and the half of the planet that has never had reliable mobile coverage are still being worked out.`
						},
						{
							...
							text: `Modern [[cellular|cellular]] security inside one carrier's network is strong. The cryptography is sound, the air interface is encrypted, the {{ims|IMS}} signalling runs over [[tls|TLS]]. But the **interconnect layer between carriers** — how a roaming visitor authenticates, how {{sms|SMS}} routes globally, how location is queried for billing — runs on **{{ss7|SS7}}** (designed 1975) and **{{diameter|Diameter}}** ({{rfc-doc|RFC}} 6733, 2012), both designed in an era of implicit trust between carrier peers.

Modern surveillance actors exploit this trust. **{{ss7|SS7}} routing** can silently track mobile users worldwide — Citizen Lab's 2024–25 disclosures and CISA's 2024 testimony to the {{fcc|FCC}} document active commercial-grade surveillance using exactly this vector. {{diameter|Diameter}} abuse (DoS, location-tracking, {{sms|SMS}} interception) by malicious peers remains a real-world problem. The 5G SBI authenticated interconnect ({{3gpp|3GPP}} {{ts-3gpp|TS}} 33.521) is the long-term fix; it is partially deployed and still has decades of {{ss7|SS7}}-shaped tail to migrate.
...
The shape rhymes with the rest of this Part. Every wireless protocol's cryptography has held up. Every wireless protocol's *negotiation logic*, *roaming model*, or *trust assumptions between operators* has been broken at the protocol level by now. The 1990s and 2000s patched the cryptography; the 2020s are patching the architecture around it.`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			synopsis:
				'[[nfc|13.56 MHz inductive coupling]], {{iso|ISO}} 18092, {{emv-cryptogram|EMV}} contactless, {{apple|Apple}} Pay, {{transit|transit}} cards, {{ccc-digital-key|CCC Digital Key}}, {{aliro|Aliro}} 1.0, and the [[pioneer:charles-walton|Charles Walton]]-to-{{apple|Apple}}-Pay arc that took 31 years.',
			slots: [
				{
					kind: 'pull-quote',
					text: 'The protocol that runs the British contactless economy was published in 2000. The protocol your phone uses to pay your barista was published as {{iso-iec|ISO/IEC}} 18092 in December 2003 — three years before the iPhone existed. [[nfc|NFC]] is one of the rare wireless stacks where the wire format has not changed in 20 years. Every iteration has been at the certification, security, and application-layer level.',
					attribution: 'Author'
				},
				{
					kind: 'prose',
					sections: [
						{
							...
							text: `In 1973, an {{ibm|IBM}} engineer named [[pioneer:charles-walton|Charles Walton]] filed **US Patent 3,752,960** — *"Electronic Identification & Recognition System"*. It described a passive transponder that responded to an interrogator's RF field by modulating its load impedance — the foundational mechanism behind {{rfid|RFID}} and, eventually, every contactless payment terminal on Earth. Walton would file 50 more patents over his career; the 1973 patent is the one historians point to as the parent of [[nfc|NFC]].

Walton received small but steady royalties for thirty years. Then his patent expired in 1993 — eleven years *before* the [[nfc|NFC]] Forum was founded and seventeen years before contactless payments became a consumer phenomenon. The arc from Walton's 1973 patent to **{{apple|Apple}} Pay's launch on 9 September 2014** is one of the longest deployment lags in computing: 41 years from the underlying physics to the consumer product that finally made it ubiquitous.

The middle of that arc is the **{{nxp|NXP}} / Philips / Sony alliance**. Philips Semiconductors (later NXP) and Sony's FeliCa team co-developed the technology that became **{{iso-iec|ISO/IEC}} 18092** in December 2003. **[[pioneer:franz-amtmann|Franz Amtmann]]** at NXP and **[[pioneer:philippe-maugars|Philippe Maugars]]** are the names attached to the {{ieee-802-15-4|IEEE}}-historian record as co-inventors. The **[[nfc|NFC]] Forum** was founded in 2004 by Sony, Philips, and Nokia — Nokia, in particular, championed [[nfc|NFC]] as a feature for its phones a decade before anyone else cared.`
						},
						{
							type: 'narrative',
							title: 'Why 13.56 MHz and 4 cm — physics by design',
							text: `Every [[nfc|NFC]] variant operates at a carrier of **13.56 MHz ± 7 kHz**, the unlicensed {{ism-band|ISM}} allocation that {{iso|ISO}} 14443 inherited. The coupling is **{{inductive-coupling|inductive (magnetic)}}**, not radiative — two loop antennas brought close together share their *near field*, and the magnetic component of that field falls off as **1/r³**, vs the 1/r² of normal radio. The {{cubic|cubic}} falloff is why [[nfc|NFC]]'s ≤10 cm range is a *feature*, not a bug: it makes physical proximity an inherent security property.

The reader (the **{{pcd|PCD}}** — Proximity Coupling Device) energises its loop antenna and transmits the 13.56 MHz carrier. The card (the **{{picc|PICC}}** — Proximity Integrated Circuit Card) is passive: it harvests power from the field — microwatts is enough — and communicates back via **{{load-modulation|load modulation}}**, switching a resistor on its own antenna at an 847.5 kHz subcarrier (which is 13.56 MHz / 16). The reader perceives this as small amplitude or phase changes in its *own* resonant loop. The {{picc|PICC}} has no battery, no transmitter, and no clock of its own — it borrows all three from the reader.
...
Modern phones use **active {{load-modulation|load modulation}}** instead, generating a small reflected carrier from a battery-powered transmitter. This is why an iPhone can be read across a metal-backed case where a plain plastic card cannot — the active emission has enough headroom to overcome the metal's eddy-current attenuation.`
						},
						{
							...
							text: '**NFC-A** ({{iso|ISO}} 14443-A): {{pcd|PCD}}→{{picc|PICC}} is 100% {{ask-modulation|ASK}} modified-Miller; {{picc|PICC}}→{{pcd|PCD}} is OOK Manchester on the 847.5 kHz subcarrier. Base rate 106 kbit/s, scaling to 848. **NFC-B** ({{iso|ISO}} 14443-B): {{pcd|PCD}}→{{picc|PICC}} is 10% {{ask-modulation|ASK}} NRZ-L; {{picc|PICC}}→{{pcd|PCD}} is {{bpsk|BPSK}} on the subcarrier — used by some {{emv-cryptogram|EMV}} cards and many {{icao|ICAO}} e-passports. **{{nfc-f|NFC-F}}** (FeliCa / JIS X 6319-4): 212/424 kbit/s Manchester-coded ASK, *no subcarrier* — dominant in Japan {{transit|transit}} (Suica, PASMO) and Hong Kong (Octopus). **{{nfc-v|NFC-V}}** ({{iso-iec|ISO/IEC}} 15693) is a fourth, longer-range vicinity-coupling mode at lower data rates — used in industrial tagging, library books, and the {{apple|Apple}} Vision Pro Light Seal [[nfc|NFC]] tag. All four coexist in the same reader silicon.'
						},
						{
							type: 'narrative',
							title: 'The contactless payment dance, in seven APDUs',
							text: `Once an [[nfc|NFC]] {{picc|PICC}} enters the reader's field, the {{anti-collision|anti-collision}} dance begins. The reader broadcasts a 7-bit **{{reqa|REQA}}** (0x26) or **WUPA** (0x52) short frame; the card responds with **{{atqa|ATQA}}** (2 bytes) declaring its {{uid|UID}} size (4/7/10 bytes). The reader then runs the **bit-frame {{anti-collision|anti-collision}}** loop with {{sel-iso|SEL}}+NVB frames, converging on each byte of the {{uid|UID}} one bit at a time when multiple cards are in the field. When the {{uid|UID}} is complete, the card answers with **{{sak|SAK}}**; if bit 6 of {{sak|SAK}} is set, the card supports {{iso|ISO}} 14443-4 and the reader proceeds with {{rats|RATS}} → {{ats-nfc|ATS}} to negotiate frame size and timing.

Now the reader speaks **{{apdu|APDUs}}** ({{iso|ISO}} 7816-4) — the same command/response unit every smart card has used since the 1980s. The first command is **{{imap-select|SELECT}} {{ppse|PPSE}}** — the Proximity Payment System Environment {{aid|AID}} \`2PAY.SYS.DDF01\` — and the card returns an FCI listing all supported payment {{aid|AIDs}} in priority order. The reader picks one (e.g. Mastercard \`A0000000041010\`), SELECTs it, gets back a {{pdol|PDOL}} listing the parameters the card needs (amount, currency, country, terminal type, unpredictable number), then sends **GET PROCESSING OPTIONS** with those parameters. The card returns AIP+{{afl|AFL}} telling the reader which files to read; {{read-record|READ}} RECORDs pull the {{pan-id|PAN}}, expiry, and public-key {{certificate-chain|certificate chain}}. Finally **{{generate-ac|GENERATE AC}}** with CDOL1 data asks the card for an **{{emv-cryptogram|Application Cryptogram}}** — either an {{arqc|ARQC}} (online) or TC (offline). The cryptogram is signed in the {{ese|Secure Element}} or {{hce|HCE}} app, and is what proves the transaction to the issuer.
...
The same tokenisation mechanism powers {{google|Google}} Wallet and Samsung Pay. The hardware varies — {{apple|Apple}} uses {{ese|eSE}} exclusively for payment; {{google|Google}} uses a mix of {{hce|HCE}} and {{android|Android}} StrongBox; Samsung uses Knox-isolated payment paths. The cryptography and the protocol on the wire are the same EMVCo specification.`
						},
						{
							type: 'narrative',
							title: 'NDEF — the small data format for everything that is not a payment',
							text: `Not every [[nfc|NFC]] interaction is a payment. The other big use is **{{ndef|NDEF}}** — [[nfc|NFC]] Data {{exchange|Exchange}} Format — the binary record container that lives in passive tags and rides over {{llcp|LLCP}}/{{snep|SNEP}}. Each record begins with a 1-byte header (MB/ME/CF/SR/IL + 3-bit TNF) plus variable type/id/{{payload|payload}}-length fields and a {{payload|payload}}.

The **TNF** (Type Name Format) picks the namespace: 1 = Well-Known ({{uri|URI}}, Text, Smart Poster), 2 = {{mime|MIME}}, 3 = Absolute {{uri|URI}}, 4 = External, 5 = Unknown. The {{uri|URI}} Well-Known record uses a single-byte prefix shorthand — \`0x03\` for \`https://\` saves eight bytes per record on tags as small as 48 bytes. This is why a tap on a museum exhibit's [[nfc|NFC]] plaque can fit a {{url|URL}}, a description, and a localised string into ~100 bytes of tag memory.

**{{ndef|NDEF}} was formally adopted as an IEC standard in March 2026**, alongside the [[nfc|NFC]] Forum Wireless Charging (WLC) extension. The data format is now an international standard 23 years after the underlying radio protocol was standardised.`
						},
						{
							type: 'narrative',
							title: 'Three transports, one tap — bootstrap to BLE / Wi-Fi / Matter',
							text: `For higher-throughput sessions [[nfc|NFC]] is almost always a *bootstrap*. The **Connection Handover** spec (v1.5) defines {{ndef|NDEF}} records of TNF = 0x02 with {{mime|MIME}} \`application/vnd.bluetooth.le.oob\` carrying the [[bluetooth|Bluetooth]] {{mac-address|MAC address}}, name, and Security Manager {{oob|OOB}} key — a single tap replaces a discovery/pairing dialog on speakers, headphones, printers, and AirPods-class earbuds. The parallel Wi-Fi handover record carries {{ssid|SSID}}/key/security mode — used for tap-to-join on printers and some smart-plug commissioning.

**{{matter|Matter}} 1.3+** adds [[nfc|NFC]] as one of the three permitted commissioning paths alongside QR code and {{ble|BLE}}. Place a {{matter|Matter}}-NFC-capable device near your phone, and it onboards onto your home {{thread|Thread}}/Wi-Fi mesh without ever opening an app. **{{ccc-digital-key|CCC Digital Key 3.0/4.0}}** uses [[nfc|NFC]] to bootstrap a credential into a phone, then {{ble|BLE}} for proximity, then [[uwb|UWB]] for centimetre-accurate ranging. **{{aliro|Aliro 1.0}}** likewise spans [[nfc|NFC]] tap-to-access + {{ble|BLE}} proximity + {{ble|BLE}}/UWB ranged — three transports, one credential. The Connection Handover pattern is now the *default* on-ramp to nearly every other consumer wireless protocol.

This is why [[nfc|NFC]] survives despite its painful slow data rate. Nobody uses [[nfc|NFC]] to *carry* a session; everyone uses it to *start* one. The 4 cm physical-presence requirement is exactly the security property the bootstrap needs — you cannot accidentally pair with a device you cannot physically touch.`
						},
						{
							type: 'callout',
							title: 'MIFARE Crypto1 — the security-by-obscurity object lesson',
							text: '**[[pioneer:karsten-nohl|Karsten Nohl]]** and **[[pioneer:henryk-plotz|Henryk Plötz]]** ("Starbug") presented at 24C3 in December 2007: Philips\'s proprietary 48-bit Crypto1 stream cipher on {{mifare|MIFARE}} Classic cards — "secure" by virtue of being secret — had been dismantled by **decapping a chip and photographing ~10,000 gates with an optical microscope**. The cipher proved weak on inspection; attacks reduced effective security to seconds of cloning time. **The Dutch OV-chipkaart kept shipping affected cards until 2024.** The lesson — *security by obscurity does not scale* — became canonical in wireless silicon design. Every [[nfc|NFC]] standard since has used open, {{peer|peer}}-reviewed cryptography ({{aes-128|AES-128}} on {{mifare|MIFARE}} DESFire, {{ecdsa|ECDSA}} on {{ccc-digital-key|CCC Digital Key}}, post-quantum primitives in the {{aliro|Aliro}} roadmap).'
						},
						{
							type: 'narrative',
							title: 'Transit, identity, and the niches NFC quietly owns',
							text: `Most of the world's daily [[nfc|NFC]] interactions are not at coffee shops. **{{transit|Transit}} fare media** alone — Suica, PASMO, ICOCA, Octopus, TfL contactless, Korean T-money, Brazilian Bilhete Único — process well over **2 billion taps per year on TfL contactless alone**. Japan's Suica/PASMO system has been running on FeliCa ({{nfc-f|NFC-F}}) since 2001. Most of these systems run on stored-value cards or open-loop {{emv-cryptogram|EMV}} contactless — the line between "{{transit|transit}} card" and "payment card" has blurred almost completely in the last five years.

**Electronic passports (eMRTDs)** under **{{icao|ICAO}} Doc 9303 Part 11** put roughly **a billion of them in circulation**. The chip embedded in the cover holds your biographical data, your photograph, fingerprints (optional), and a digital signature chain rooted at the issuing country's {{public-key|Public Key}} Directory at {{icao|ICAO}}. The reader at passport control speaks {{apdu|APDUs}} over [[nfc|NFC]]-A or NFC-B to authenticate the chip via BAC/PACE before reading anything — your data is encrypted at rest on the chip, decrypted with a key derived from the {{mrz|MRZ}} line that the reader scans optically.

**Access control** — corporate building entry, hotel keys, residential and multi-family doors — used to run on cloneable LF and 13.56 MHz proximity cards. The new **{{aliro|Aliro 1.0}}** standard ({{sig-bluetooth-acronym|CSA}}, finalised 26 February 2026) is "{{matter|Matter}} for doors": {{ecdsa|ECDSA}} mutual authentication; [[nfc|NFC]] tap-to-access, {{ble|BLE}} proximity, and {{ble|BLE}}+[[uwb|UWB]] ranged hands-free, all under one credential. 220+ companies including {{apple|Apple}}, {{google|Google}}, Samsung, ASSA ABLOY, HID, Allegion, Kwikset, Nuki are behind it.`
						},
						{
							type: 'image',
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			synopsis:
				'[[uwb|IEEE 802.15.4z]], FiRa, {{ccc-digital-key|CCC Digital Key}} 3.0, {{apple|Apple}} {{u1-chip|U1}} / {{u2-chip|U2}}, the 2022 Tesla [[bluetooth|BLE]] relay attack that motivated the move to [[uwb|UWB]], and the Ghost Peak {{sts|STS}} residual.',
			slots: [
				{
					kind: 'pull-quote',
					text: '[[uwb|UWB]] is not a data radio — it is a clock. Modern [[uwb|UWB]] transmits sub-nanosecond impulses across ≥500 MHz of {{spectrum|spectrum}} so two devices can measure the {{tof-ranging|time-of-flight}} of a radio pulse with 10–30 cm accuracy. The point in 2026 is the *security* of the measurement, not just the precision.',
					attribution: 'Author'
				},
				{
					kind: 'prose',
...
The {{bandwidth|bandwidth}} is the whole game. **A 1 ns timing error is 30 cm of range error.** [[uwb|UWB]] chips routinely timestamp pulse arrival to 15–60 picoseconds, which the rest of the radio {{bandwidth|bandwidth}} turns into single-digit centimetre accuracy. [[bluetooth|Bluetooth]] {{rssi|RSSI}} cannot do this; [[wifi|Wi-Fi]] {{ftm|FTM}} ([[wifi|802.11mc / az]]) gets to ~1–2 m. [[uwb|UWB]]'s 499.2 MHz channel width is the physics that makes single-digit centimetres possible.

The {{spectrum|spectrum}} legacy: **the {{fcc|FCC}}'s First Report and Order on Valentine's Day 2002** authorised [[uwb|UWB]] under Part 15.519 at **−41.3 dBm/MHz** across **3.1–10.6 GHz** — an *unlicensed underlay* on top of every other service in those bands. The trick is the noise floor. [[uwb|UWB]] transmits at such a low average power that no incumbent licensee notices, but it spreads its energy across so much {{bandwidth|bandwidth}} that the cumulative pulse energy is detectable at the receiver. The two channels everyone actually uses are **Channel 5** (6489.6 MHz) and **Channel 9** (7987.2 MHz), each 499.2 MHz wide.`
						},
						{
							type: 'narrative',
...
The consumer story begins on **10 September 2019**, when {{apple|Apple}} shipped the **{{u1-chip|U1}} chip** in the iPhone 11. It went mass-market on **30 April 2021** with the $29 **AirTag**, whose Precision Finding feature swept the world's awareness of [[uwb|UWB]] into one product. Samsung followed with the **Galaxy SmartTag+** in April 2021. **{{ccc-digital-key|CCC Digital Key 3.0}}** (July 2021) made [[uwb|UWB]] the fine-ranging leg of phone-as-a-key for vehicles — BMW iX shipped first; Mercedes EQS, Hyundai/Kia, and VW {{id-identifier|ID}}.7 followed.

[[uwb|UWB]] silicon has consolidated to five suppliers: **{{apple|Apple}} captive** ({{u1-chip|U1}} 16 nm, **{{u2-chip|U2}}** 7 nm from iPhone 15), **{{qorvo|Qorvo}}** (acquired Decawave's DW1000/DW3000 line), **{{nxp|NXP}}** (Trimension SR040/SR150/SR250), **Samsung captive** (Exynos {{mqtt-connect|Connect}} U100), and a long tail (STMicroelectronics, Microchip, Spark Microsystems, Infineon's 2024-acquired 3db Access). ABI projects [[uwb|UWB]] phone penetration rising from **27% in 2025 to 52% by 2030** — [[uwb|UWB]] is on the same adoption curve [[nfc|NFC]] followed a decade earlier.`
						},
						{
							type: 'callout',
...
[[uwb|802.15.4a]] (2007), the original [[uwb|UWB]] standard, used a *public* preamble and {{sfd|SFD}} pattern for ranging. An attacker watching the radio could predict the next pulse and inject a small early copy that the receiver's correlator would lock onto — shortening the apparent distance by tens of metres. The family of attacks (Cicada, Early-Detect/Late-Commit) made pre-{{sts|STS}} [[uwb|UWB]] unsuitable for any security-critical application. This is the reason the early-2010s "[[uwb|UWB]] for digital keys" pitches went nowhere — the protocol was not secure enough.

**[[uwb|IEEE 802.15.4z]]** (ratified 31 August 2020) fixed it. The {{sts|STS}} block injects a **32–2048-chip pulse sequence whose positions are generated by {{aes-128|AES-128}} in Counter mode**, keyed by a per-session **STS_KEY** and a per-frame {{nonce|nonce}}. The receiver, holding the same key, generates the *expected* sequence locally and cross-correlates with a sharp autocorrelation peak. An attacker without the key sees noise. They cannot predict the next chip, cannot reliably early-replay, and cannot shorten the apparent distance.

The {{sts|STS}} block is the **distance commitment** that defeats the entire {{ble|BLE}}-{{rssi|RSSI}} {{replay-attack|relay}} class of attack. {{ccc-digital-key|CCC Digital Key 3.0}}, {{aliro|Aliro 1.0}}, AirTag Precision Finding, and every modern [[uwb|UWB]] product depend on it.`
						},
						{
							type: 'callout',
							title: 'Ghost Peak — even STS is not perfect',
							text: 'At **{{usenix-conf|USENIX}} Security 2022**, Patrick Leu, Giovanni Camurati, and colleagues at ETH Zürich published **Ghost Peak** — an attack that biased an {{sts|STS}} correlator\'s peak earlier with ~4% success using a $65 device. The trick was injecting {{sts|STS}}-like-but-random noise that happened to correlate enough to skew the receiver\'s timestamp by a few nanoseconds — enough to falsely shorten the measured distance into the "unlock allowed" zone. **The fix is {{ieee-802-15-4|IEEE}} 802.15.4ab** (Draft D03 September 2025, ratification expected early 2026), which adds **{{nba-mms|NBA-MMS}}** (narrowband-assisted multi-millisecond) ranging and a redesigned {{sts|STS}} receiver. The arc is the same one we have seen everywhere in this Part — the cryptography is sound, the negotiation/correlation logic gets patched generation after generation.'
						},
						{
							type: 'narrative',
							title: 'BLE bootstraps every UWB session — the indispensable on-ramp',
							text: `Almost no consumer [[uwb|UWB]] session ever opens *without* {{ble|BLE}} first. The reason is power and discovery: {{ble|BLE}} has ubiquitous always-on advertising and standardised pairing; [[uwb|UWB]] has neither. A [[uwb|UWB]] radio is power-hungry to keep listening, has no advertising channel, and its receiver needs to know exactly when to wake up to catch a sub-nanosecond pulse. The canonical bootstrap pattern in **{{ccc-digital-key|CCC Digital Key 3.0}}** and **{{aliro|Aliro 1.0}}** has five steps:

**(1)** {{ble|BLE}} GAP advertising with the application's service {{uuid|UUID}} (the lock advertises; the phone scans). **(2)** {{gatt|GATT}} {{service-discovery|service discovery}} + authentication — {{spake2|SPAKE2}}+/{{pake|PAKE}} for Digital Key, {{oob|OOB}} pairing for AirTag. **(3)** Session-key transport over the {{ble|BLE}} encrypted channel — phone and reader {{exchange|exchange}} the **STS_KEY** and ranging parameters via {{apdu|APDU-over-GATT}}. **(4)** {{ble|BLE}}-signalled [[uwb|UWB]] ranging start at a scheduled time slot — both radios power on their [[uwb|UWB]] stacks. **(5)** Ranging happens (Poll/Response/Final); results returned over {{ble|BLE}} for application-layer policy enforcement.

The Schlage {{fcc|FCC}} waiver (ET Docket 22-248, granted 2023) describes this exact sequence in regulator-vetted form. It is also why BLE {{channel-sounding|Channel Sounding}} — Bluetooth 6.0's cm-class ranging built into the BLE link itself — is interesting: it might collapse the five-step dance into one radio.`
						},
						{
							type: 'narrative',
							title: 'CCC Digital Key — the canonical unlock flow',
							text: `BMW iX shipped the first {{ccc-digital-key|CCC Digital Key 3.0}} vehicle in early 2022. The vehicle has multiple **[[uwb|UWB]] anchors** (typically one in each B-pillar and one per door handle) plus a {{ble|BLE}} radio. The phone has {{apple|Apple}} {{u1-chip|U1}}/{{u2-chip|U2}} or {{nxp|NXP}}/{{qorvo|Qorvo}} [[uwb|UWB]] silicon plus its own {{ble|BLE}}.

The unlock dance: {{ble|BLE}} advertising from the car → {{ble|BLE}} pairing + {{gatt|GATT}} authentication → {{apdu|APDU}} {{exchange|exchange}} where the car authenticates the Digital Key applet in the phone's {{ese|Secure Element}} and derives session keys → {{ble|BLE}} transfers the **STS_KEY** and ranging schedule → [[uwb|UWB]] {{ds-twr|DS-TWR}} ranging fires across multiple anchors simultaneously → the car computes {{tof-ranging|time-of-flight}} on each anchor, checks the distance is below threshold *and* the credential is valid → BLE returns Unlock granted/denied. The whole {{exchange|exchange}} takes well under a second. [[nfc|NFC]] remains the fallback when the phone's battery is dead.

**{{ccc-digital-key|CCC Digital Key 4.0}}** (announced July 2025, tested at the 13th Plugfest hosted by {{apple|Apple}}) adds cross-platform key sharing — you can send an {{android|Android}} friend a Digital Key to your BMW from an iPhone, and the friend's phone can unlock the car. **115 vehicle / module products were {{ccc-digital-key|CCC}}-certified in 2025**: BMW (first, late 2024 4.0), Mercedes, Hyundai/Kia, Audi (2025), Volvo, Porsche, GM, Ford, plus Chinese OEMs (NIO, XPENG, Geely group).`
						},
						{
							type: 'narrative',
					...
					sections: [
						{
							...
							text: `[[zigbee|Zigbee]] is the upper-layer protocol stack — {{zigbee-nwk|NWK}}, {{aps-layer|APS}}, {{zigbee-zdo|ZDO}}, {{zcl|ZCL}} — that the Connectivity Standards Alliance (founded August 2002 as the **Zigbee Alliance** by Invensys, Mitsubishi, Motorola, Philips, Samsung, and Honeywell) built on top of the {{ieee-802-15-4|IEEE 802.15.4}} {{phy|PHY}}/{{mac-address|MAC}}. The 2.4 GHz {{ism-band|ISM band}} offers 16 channels of 250 kbit/s O-QPSK; sub-GHz PHYs at 868 MHz (Europe) and 902–928 MHz (North America) give longer range at lower bit rates. Topologies are star, tree, and — the one everyone uses — **mesh**: every mains-powered device routes for its sleepy battery-powered neighbours, and the network heals itself when a router drops out. 127-byte {{phy|PHY}} {{payload|payloads}}, ~80–100 byte usable {{aps-layer|APS}} payloads, {{aes-128|AES-128}}-{{ccm-mode|CCM}}* security across both network and application layers.

The deployment story is unusually concentrated. **Philips Hue** launched on 29 October 2012 at {{apple|Apple}} Stores for $199, branded as *"Web-enabled lighting"* with the word Zigbee deliberately hidden, and grew to roughly **30 million bulbs lifetime** — the largest Zigbee installed base on Earth. **IKEA Trådfri** (2017), **SmartThings**, **Amazon Echo Plus**, **Hubitat**, and most commercial-lighting systems (Acuity nLight AIR, Eaton, Lutron Vive) all run Zigbee. The under-reported giant is **VusionGroup electronic shelf labels** — roughly 350 million units in the cumulative installed base to date, with **Walmart's June 2024 rollout** of Vusion ESLs beginning across ~2,300 US stores (with plans to expand further). More Zigbee-family devices in one retailer than Hue has ever sold.`
						},
						{
							type: 'callout',
							title: 'Zigbee is a brand the customer never sees',
							text: 'Hue\'s October 2012 launch is the canonical example of a successful protocol whose user-visible brand is the product, not the standard. Press releases mentioned *ZigBee LightLink* exactly once. In-store displays, packaging, and the iOS app strenuously avoided the term. Consumers bought "Web-enabled lighting", which is what they wanted. The {{sig-bluetooth-acronym|CSA}} renamed itself away from "Zigbee Alliance" in 2021 partly to acknowledge that the brand had been a marketing liability for a decade. Every successful low-power mesh protocol learned this lesson: name the *product*, not the wireless standard underneath.'
						},
						{
							type: 'narrative',
							title: 'The mesh, the trust center, and the install code',
							text: `A Zigbee network has three classes of device. **The Coordinator** (one per network) starts the mesh, holds the {{network-key|network key}}, and acts as the {{trust-center|Trust Center}} — the single trust root that authenticates every join. **Routers** are mains-powered devices (light bulbs, plugs, switches) that forward traffic for their neighbours and keep the mesh self-healing. **End devices** are battery-powered (sensors, buttons) that sleep most of the time and wake briefly to send a packet through their parent router.

At commissioning time, a new device joins by exchanging a 16-byte {{aes-128|AES-128}} **pre-configured link key** with the {{trust-center|Trust Center}}. The {{trust-center|Trust Center}} then sends the **{{network-key|network key}}** in an {{aps-layer|APS}} Transport-Key command encrypted under that link key. So far so good — except that for fifteen years the default link key was a publicly known sixteen-byte string: \`5A:69:67:42:65:65:41:6C:6C:69:61:6E:63:65:30:39\` = {{ascii|ASCII}} **ZigBeeAlliance09**. Generations of {{wireshark|Wireshark}} users memorised the hex; any attacker sniffing a join with that default key could read the {{network-key|network key}} in plaintext.

**{{install-code|Install codes}}** — a per-device 128-bit secret printed on the device's packaging or QR-encoded on the box, mandatory in Zigbee 3.0 commissioning — closed that hole. **{{dynamic-link-key|Dynamic Link Key}}** in **Zigbee PRO 2023 (R23)** went further: a SPEKE-over-{{curve25519|Curve25519}} {{handshake|handshake}} that derives a unique link key per device with no pre-shared secret at all. Zigbee's equivalent of the {{wpa3|WPA3}}/{{sae|SAE}} replacement of {{wpa2|WPA2}}-{{psk|PSK}}.`
						},
						{
							type: 'narrative',
							title: 'ZCL — a tiny REST API for light bulbs',
							text: `What makes [[zigbee|Zigbee]] a *consumer* protocol rather than a generic mesh is the **{{zcl|Zigbee Cluster Library}}** — an object model that gives every device a small, predictable, attribute-and-command vocabulary. Each **cluster** is a tiny object: **OnOff = 0x0006** has the boolean attribute OnOff and the commands On (0x00), Off (0x01), Toggle (0x02). **Level Control = 0x0008** has dim level and the commands Move-to-Level, Step, Stop. **Color Control = 0x0300** has hue, saturation, and the commands Move-to-Color, Move-to-Hue, and so on. **OTA Upgrade = 0x0019** lets the bulb pull a firmware {{bgp-update|update}} from the coordinator.
...
The deeper consequence is that {{matter|**Matter**}} reuses {{zcl|ZCL}} as its data model directly. **{{matter|Matter}} is essentially {{zcl|ZCL}} on [[ipv6|IPv6]].** Every cluster {{id-identifier|ID}}, every attribute, every command from Zigbee maps one-to-one to its {{matter|Matter}} equivalent. This is the reason the **Hue Bridge {{matter|Matter}} firmware** that Signify shipped on 19 September 2023 worked at all — it did not have to re-model Hue's behaviour for {{matter|Matter}}; it just had to translate the transport.`
						},
						{
							type: 'narrative',
							title: 'Thread — the IPv6-native successor',
							text: `{{thread|**Thread**}} is the sibling that took a different architectural {{turn|turn}}. Same {{ieee-802-15-4|802.15.4}} radio as Zigbee, same 2.4 GHz channels, same mesh topology — but where Zigbee carries application traffic in its own {{zigbee-nwk|NWK}} / {{aps-layer|APS}} stack, {{thread|Thread}} carries native **[[ipv6|IPv6]] over 6LoWPAN** {{header|header compression}} and uses **MLE** (Mesh Link Establishment) for routing. Every {{thread|Thread}} device gets a real [[ipv6|IPv6]] address and is reachable like any other host on your home network.

The {{thread|Thread}} Group was founded in **July 2014** by Nest (then a {{google|Google}} subsidiary), Samsung, ARM, Silicon Labs, {{nxp|NXP}}, and a few others. The pitch was: "Zigbee is a closed application stack with a non-{{ip-address|IP}} wire format; we want [[ipv6|IPv6]] mesh." Adoption was slow — the killer app that needed *another* low-power mesh was missing — until {{matter|**Matter**}} arrived in 2022 and made {{thread|Thread}} the radio of choice for sensors and switches that should not run Wi-Fi.

**{{thread|Thread}} 1.4** (December 2024) made **{{border-router|Thread Border Routers}}** a multi-vendor commodity. {{apple|Apple}}'s HomePod and {{apple|Apple}} TV 4K, {{google|Google}}'s Nest Hub, Amazon Echo Hub, Aqara M3, eero 6+, and others all act as {{thread|Thread}} Border Routers — bridging the 802.15.4 mesh to your home [[wifi|Wi-Fi]] network. The {{border-router|border router}} runs **{{mdns|DNS-SD}}** for {{service-discovery|service discovery}}, hands out [[ipv6|IPv6]] prefixes via {{slaac|SLAAC}} to the {{thread|Thread}} mesh, and forwards packets between the radios. From your phone, a {{thread|Thread}} sensor looks like any other [[ipv6|IPv6]] host — you can {{ping|ping}} it.

In 2026, **new device design has bifurcated**. Lighting and sensors are increasingly {{thread|Thread}} ({{matter|Matter}}-native, low power, IPv6-addressable). Cameras and high-{{bandwidth|bandwidth}} devices are [[wifi|Wi-Fi]] ({{matter|Matter}} 1.5 added camera streaming via RTSP). Zigbee is the *bridged legacy radio* — preserved for the installed base, no longer the default for new designs.`
						},
						{
							type: 'callout',
						...
						},
						{
							...
							text: `Zigbee and [[wifi|Wi-Fi]] both live on 2.4 GHz, and Zigbee always loses. Wi-Fi uses 20 MHz channels centred at 2412/2437/2462 MHz (channels 1/6/11). Zigbee channels are 2 MHz wide, centred at 2405 + 5·(k−11) MHz. **Zigbee channels 11–14 sit under Wi-Fi 1; 15 partially clears it; 20 sits in the gap between Wi-Fi 6 and 11; 25 and 26 sit above Wi-Fi 11.** A Wi-Fi network can fill several Zigbee channels with broadband noise loud enough that a {{zigbee-router|Zigbee router}} cannot win a CSMA-CA back-off.

The standard advice — choose channel 15, 20, 25, or 26 — works when you have one well-placed coordinator and a single Wi-Fi network to dodge. In dense apartment buildings with ~20 Wi-Fi APs visible, no single Zigbee channel is reliably clear. The R23 spec added **frequency-agility** so the coordinator can order the whole mesh to {{hop|hop}} to a new channel when interference exceeds a threshold. Newer multi-radio bridges (Aqara M3, {{apple|Apple}} HomePod gen 4, Amazon Echo Hub) put the Zigbee antenna **physically separated from the Wi-Fi antenna** in the case design, which sounds trivial but is the single biggest improvement in real-world reliability over the previous decade of "{{usb|USB}} stick plugged into the back of a router."

The single most common cause of unreliable Zigbee in any forum {{thread|thread}} you will ever read: **the coordinator dongle is plugged directly into the back of the Wi-Fi router**. Move the dongle to a {{usb|USB}} extension cable, three feet away. Half the complaints disappear.`
						},
						{
							type: 'narrative',
							title: 'The 350-million-unit shelf-label deployment nobody talks about',
							text: `Discussion of Zigbee in 2026 is usually about consumer smart-home — Hue, SmartThings, Aqara. The numbers say something else. **VusionGroup** (formerly SES-imagotag) has a cumulative installed base of roughly **350 million Zigbee-family ESL units**, and is rolling out across Walmart US stores (beginning ~2,300 in mid-2024, expanding through 2026). Carrefour, Tesco, Auchan, Mercadona, and most European hypermarket chains are also Vusion deployments at planetary scale.

Each ESL is a tiny e-paper display with a Zigbee-family radio (Vusion uses a proprietary variant on {{ieee-802-15-4|802.15.4}}) that updates its price text from a central server multiple times per day. The radio is always on, the display only when content changes — the e-paper holds its image without power. From the protocol's perspective, it is the same {{zigbee-nwk|NWK}} + {{aps-layer|APS}} + cluster-library shape as a Hue bulb; from the deployment's perspective, it is the largest production {{ieee-802-15-4|802.15.4}} network anywhere on Earth.
...
The dynamic-pricing implications are still controversial — Walmart's December 2024 announcement was met with consumer-pricing concerns about real-time price changes — but the *protocol* deployment proceeds either way. ESLs are the quiet, planet-scale, never-quite-mentioned-in-tech-press Zigbee success story.`
						},
						{
							...
							text: `New 2026 device design is not Zigbee — it is {{thread|Thread}} or [[wifi|Wi-Fi]] with {{matter|Matter}} on top. New Hue bulbs and IKEA Trådfri devices continue to ship Zigbee because the installed base is huge and the radios are cheap; **Aqara has been shipping {{thread|Thread}} + {{matter|Matter}}-native sensors since 2024**. Most major manufacturer roadmaps treat Zigbee as a *bridged legacy* — the radio that will be supported through 2030+, but not the radio future devices will default to.

The mass-scale exceptions remain commercial: ESLs, industrial lighting controls, and the Walmart-class deployments where Zigbee's per-device cost and zero-router-required architecture still beat anything else. The {{sig-bluetooth-acronym|CSA}}'s R23 work ({{dynamic-link-key|Dynamic Link Key}}, {{trust-center|Trust Center}} Swap-Out, Zigbee Direct, sub-GHz PHYs) is explicitly aimed at *preserving* the installed base, not growing it.

For the home-automation hobbyist, the **{{matter|Matter}} bridge** is the right framing in 2026. A Hue bulb is a Zigbee device; through the Hue Bridge's {{matter|Matter}} firmware, it appears as a {{matter|Matter}} accessory; from your iPhone or {{google|Google}} Home, it is "a light bulb." The Zigbee protocol is doing its job — exactly the job Hue's 2012 marketing copy promised: *web-enabled lighting*, with the word *Zigbee* still off the box.`
						},
						{
							type: 'image',
							...
							caption:
								'The **[[zigbee|Zigbee]]** logo. The Connectivity Standards Alliance (formerly Zigbee Alliance) was founded in August 2002; the spec sits on top of {{ieee-802-15-4|IEEE 802.15.4}} as {{zigbee-nwk|NWK}} + {{aps-layer|APS}} + {{zigbee-zdo|ZDO}} + {{zcl|ZCL}}. Despite the brand being deliberately hidden from consumers, the same {{zcl|ZCL}} data model now powers {{matter|Matter}} — {{matter|Matter}} is essentially {{zcl|ZCL}} on [[ipv6|IPv6]].',
					...
					sections: [
						{
							...
							text: `Every wireless protocol in this Part has been broken on stage in public, multiple times, by the same handful of researchers, with strikingly similar shapes of bug. **It is never the cryptography itself.** {{aes-128|AES-128}}, {{aes-gcm|AES-GCM}}, {{ecdsa|ECDSA}}, ChaCha20, {{curve25519|Curve25519}} — the primitives hold up. What gets broken is the **negotiation logic**, the **state machine**, the **roaming model**, the **trust assumptions between operators** — the *protocol surface around* the cryptography.

The lesson is the central one of this Part. Wireless is the only major networking category where the *physical layer is adversarial by default*. Wired networks fail when something breaks; wireless networks fail because the medium is shared with everything else operating in the same band — including, sometimes, an attacker. Every architectural choice in this category — frequency hopping, {{csma-ca|CSMA/CA}}, scheduled access, {{sts|STS}}, cryptographic distance bounds — exists to make a hostile medium predictable. The attacks in this chapter are the unfinished parts of that project.

Reading order: we start with the [[wifi|Wi-Fi]] arc because it is the most public, walk through [[bluetooth|Bluetooth]] {{br-edr|BR/EDR}} because it has been broken three times by the same author, look at [[nfc|NFC]]'s one canonical security-by-obscurity disaster ({{mifare|MIFARE}} Crypto1), the 2022 [[bluetooth|BLE]] relay attack and the [[uwb|UWB]] {{sts|STS}} response, and finish with the {{ss7|SS7}} / {{diameter|Diameter}} trust holdover that runs underneath the cellular network you are using to read this.`
						},
						{
							type: 'narrative',
							title: 'The Wi-Fi arc — KRACK, Dragonblood, FragAttacks, SSID Confusion',
							text: `**{{wpa2|WPA2}} was published in 2004** and looked solid for thirteen years. On **16 October 2017**, Mathy Vanhoef and Frank Piessens at KU Leuven released **{{krack|KRACK}}** — Key Reinstallation AttaCK. By replaying message 3 of {{wpa2|WPA2}}'s four-way {{handshake|handshake}}, an attacker tricked a client into reinstalling an already-used session key, resetting the per-packet {{iv|nonce}} counter and defeating CCMP integrity. **Every {{wpa2|WPA2}} client on Earth needed firmware updates.** The patches shipped within weeks, but the architectural fix took a full generation — {{wpa3|**WPA3**}} with {{sae|**SAE**}} (Simultaneous Authentication of Equals), which is the *dragonfly* {{handshake|handshake}} that derives a fresh Pairwise Master Key per session with {{pfs|forward secrecy}}.

The Vanhoef cadence has continued biennially since. **Dragonblood** (April 2019) found side channels in {{wpa3|WPA3}}-{{sae|SAE}}'s password-derivation function — fixed in {{wpa3|WPA3}}-Personal-revision-2. **FragAttacks** (May 2021) broke [[wifi|802.11]] *{{fragmentation|fragmentation}} and aggregation* design itself — not implementation bugs but spec flaws — and affected every Wi-Fi device shipped since 1997. **Framing Frames** (March 2023) abused Wi-Fi's power-save framing to inject malicious frames into a client's queue. **{{ssid|SSID}} Confusion / {{cve|CVE}}-2023-52424** (May 2024) showed the [[wifi|802.11]] standard does *not* require the {{ssid|SSID}} to enter {{pmk|PMK}} or session-key derivation in many config paths, enabling a downgrade-style trick against any client {{os|OS}}.

What every Wi-Fi attack since {{krack|KRACK}} has had in common: **the {{aes|AES}} primitive is fine**; the *{{handshake|handshake}} state machine around it* is the bug. Replay, downgrade, side channel, framing — variations on a theme. The Wi-Fi WG ships fixes; the next August Vanhoef finds the next attack; the cycle continues. This is the field's most reliable two-year clock.`
						},
						{
							type: 'narrative',
							title: 'Bluetooth — three breaks in five years, by the same author',
							text: `[[bluetooth|Bluetooth]] {{br-edr|BR/EDR}} has been broken at the spec level three times in five years, by the same author, each time on a different piece of the {{handshake|handshake}} state machine.

**{{knob-attack|KNOB}}** ({{cve|CVE}}-2019-9506, August 2019) downgraded the entropy of the {{br-edr|BR/EDR}} negotiated session key to **1 byte**. The {{encryption|Encryption}} Key Negotiation Protocol allowed a man-in-the-middle to coerce both peers into agreeing on an {{encryption|encryption}} key of 7 bytes' length (the minimum the spec permitted), then reduce that further during the negotiation. Brute-forceable in real time. Patched by tightening minimum-entropy checks; ~2 billion devices needed firmware updates.

**{{bias-attack|BIAS}}** ({{cve|CVE}}-2020-10135, May 2020) impersonated a previously-bonded {{peer|peer}} by abusing role-switch in Legacy Secure Connections. An attacker could re-establish the encrypted session of a device they had never bonded with, simply by claiming to be the bonded {{peer|peer}} and triggering a role switch that the older link manager did not authenticate. Patched in Core 5.2.

**{{bluffs-attack|BLUFFS}}** ({{cve|CVE}}-2023-24023, November 2023) broke {{forward-secrecy|forward secrecy}} by forcing reuse of session-key derivation across reconnections. Two reconnections to the same device produced the same session key — so an attacker who recorded one encrypted session and later compromised the long-term key could replay or decrypt traffic from earlier sessions. Patched by adding key-diversification across reconnections in Core 5.4 and 6.0.

Every {{br-edr|BR/EDR}} device shipped before mid-2024 is affected by at least one of these. {{ble|BLE Secure Connections}} uses different cryptography ({{ecdh|ECDH}} on Curve P-256) and a different pairing flow, and has held up better — but its own pairing-method downgrade (Just Works ↔ Numeric Comparison) has had its share of disclosures. The pattern matches Wi-Fi exactly: **{{aes|AES}} is fine; the {{handshake|handshake}} around it is where the bugs live**.`
						},
						{
							type: 'callout',
							title: 'MIFARE Crypto1 — the security-by-obscurity disaster',
							text: 'In December 2007 at 24C3, **[[pioneer:karsten-nohl|Karsten Nohl]]** and **[[pioneer:henryk-plotz|Henryk Plötz]]** ("Starbug") presented a result that became the field\'s textbook case for *why proprietary cryptography fails*. Philips\'s **Crypto1** stream cipher on {{mifare|MIFARE}} Classic cards — a 48-bit cipher kept secret as the entire security model — had been **decapped, photographed under a microscope, and reverse-engineered from the ~10,000 gates on the silicon die**. The cipher proved cryptographically weak on inspection; attacks reduced effective security to seconds of cloning time. **The Dutch OV-chipkaart kept shipping affected cards until 2024**, the London Oyster card variant was deprecated mid-2010s, but the world had to migrate billions of contactless cards to {{mifare|MIFARE}} DESFire (open {{aes-128|AES-128}}) over a decade. Every [[nfc|NFC]] standard since uses open {{peer|peer}}-reviewed cryptography. This is the canonical "security by obscurity does not scale" lesson in deployed wireless silicon.'
						},
						{
							type: 'narrative',
							title: 'The 2022 Tesla relay — and the rise of cryptographic distance',
							text: `On **15 May 2022**, Sultan Qasim Khan at NCC Group disclosed a **link-layer relay attack** against Tesla Model 3 phone-as-a-key. Two ~$25 dev boards (one near the phone, one near the car), a few hundred metres of cellular tunnel between them, and ~8 ms of added {{latency|latency}}. The Tesla believed the phone was in {{ble|BLE}} proximity when it was actually at the supermarket. CVSS 6.8. The attacker could unlock and drive away a parked Model 3 from 25 metres.

The flaw was not Tesla's. The flaw is that **{{ble|BLE}} {{rssi|RSSI}} is fundamentally untrustworthy** as a proximity primitive — signal strength can be amplified arbitrarily by a relay, and the link-layer round-trip-time check in classic {{ble|BLE}} was too coarse (~30 ms) to catch an 8 ms relay. Any {{ble|BLE}}-based digital-key product on the market in 2022 was vulnerable to the same class of attack.

The industry conclusion was structural: **proximity-by-radio-signal-strength is unfixable**. A relay with enough TX power and enough patience defeats any {{rssi|RSSI}} threshold. The only way to verify physical proximity is to measure **{{tof-ranging|time-of-flight}}**, because *the speed of light is the hard upper bound that no relay can shorten*. That insight pushed every credential standard since to mandate physics-based proximity: {{ccc-digital-key|CCC Digital Key 3.0}} ([[uwb|UWB]] ranging), {{aliro|Aliro 1.0}} ([[uwb|UWB]] or {{channel-sounding|Bluetooth Channel Sounding}}), and the Bluetooth 6.0 ranging mode that uses much tighter RTT than legacy {{ble|BLE}}.

The 2022 attack is the proximate cause of the entire 2024–2026 wave of [[uwb|UWB]] silicon, ranging-capable Bluetooth 6.0 chipsets, and the new "physics-as-proof" security architecture across the secure-access industry.`
						},
						{
							type: 'narrative',
							title: 'Ghost Peak — even STS is not perfect',
							text: `If the Tesla {{ble|BLE}} relay made the case for [[uwb|UWB]], **Ghost Peak** made the case that [[uwb|UWB]] itself was not finished. At **{{usenix-conf|USENIX}} Security 2022**, Patrick Leu, Giovanni Camurati, and colleagues at ETH Zürich published a result that bent {{sts|STS}} (the cryptographic distance commitment that 802.15.4z added in 2020) at ~4% success with a $65 device.

The attack injected {{sts|STS}}-like-but-random noise into the [[uwb|UWB]] channel. With enough trials, some of that noise happened to correlate with the legitimate {{sts|STS}} sequence well enough to bias the receiver's correlation peak earlier — falsely shortening the measured distance into the "unlock allowed" zone. The fix is **{{ieee-802-15-4|IEEE}} 802.15.4ab** (Draft D03, September 2025, ratification expected early 2026), which adds **{{nba-mms|NBA-MMS}}** (narrowband-assisted multi-millisecond ranging) and a redesigned {{sts|STS}} receiver that is much harder to bias.

Ghost Peak's lesson is the same one [[bluetooth|Bluetooth]] taught: **the cryptography is sound; the negotiation/correlation logic is where the bugs live**. {{aes-128|AES-128}}-CTR for {{sts|STS}} generation is fine; the receiver's peak-detection algorithm needed work. Every secure-ranging primitive since has been audited specifically for these correlation-bias attacks — and the next wave (802.15.4ab + {{channel-sounding|Channel Sounding}} + 11az) is being designed with those audits in mind from day one.`
						},
						{
							type: 'narrative',
							title: 'The cellular interconnect — SS7 / Diameter still trusts',
							text: `Inside one carrier's network, modern [[cellular|cellular]] security is strong. The cryptography is sound, the air interface is encrypted, the {{ims|IMS}} signalling runs over [[tls|TLS]]. **But the interconnect layer between carriers** — how a roaming visitor authenticates, how {{sms|SMS}} routes globally, how location is queried for billing, how voice calls cross borders — runs on **{{ss7|SS7}}** (designed 1975) and **{{diameter|Diameter}}** ({{rfc-doc|RFC}} 6733, 2012), both designed in an era of implicit trust between carrier peers.

Modern surveillance actors exploit this trust at scale. **Citizen Lab's 2024–25 disclosures** and **CISA's 2024 testimony to the {{fcc|FCC}}** document active commercial-grade surveillance using exactly this vector. Two threat actors (publicly labelled STA1 and STA2) operate {{ss7|SS7}}-routing services that silently track mobile users worldwide — *no malware on the target's phone is required*. An attacker who can rent access to an {{ss7|SS7}} endpoint can issue a routing query for any phone number on the planet and receive the device's approximate cell-tower location in seconds.

The [[cellular|cellular]] industry's response has been the **5GC service-based interconnect** ({{3gpp|3GPP}} {{ts-3gpp|TS}} 33.521) — authenticated SBI between carriers, with cryptographic identity for each network function. It is partially deployed inside {{5g-core|5G core}} networks but does not yet reach the global interconnect layer. The {{ss7|SS7}} layer below is still everywhere underneath, carrying {{sms|SMS}}, voice routing, and roaming handshakes for billions of subscribers. **It is the largest, oldest, most deeply-embedded "trust assumption that turned out to be wrong" anywhere on the public internet.**

The shape rhymes with every other story in this chapter. Wi-Fi: {{krack|KRACK}} in 2017, fix shipping by 2020. Bluetooth: {{knob-attack|KNOB}}/{{bias-attack|BIAS}}/{{bluffs-attack|BLUFFS}}, fixes in 5.4/6.0. [[nfc|NFC]]: Crypto1 in 2007, migration to {{aes|AES}} by 2020s. [[uwb|UWB]]: Ghost Peak in 2022, 802.15.4ab in 2026. Cellular: {{ss7|SS7}} exploitation public since the early 2010s, 5GC SBI partial fix arriving in the late 2020s. Every wireless protocol's cryptography has held; every wireless protocol's *architecture around the cryptography* has been broken and patched and broken and patched.`
						},
						{
							type: 'callout',
							title: 'The takeaway, in one paragraph',
							text: 'Across every wireless protocol in this Part: the **{{aes|AES}} primitives, {{ecdh|ECDH}} key exchanges, and signature algorithms have all held up under twenty years of public attack**. What gets broken on stage every other year is the *negotiation, state machine, roaming model, or trust assumption between operators* — the *protocol surface around* the cryptography. The fix is always the same: **make every negotiation step cryptographically authenticated, every key derivation forward-secret, every proximity claim physics-based, and every inter-operator interface mutually authenticated.** {{wpa3|WPA3}} did it for Wi-Fi; Bluetooth 5.4/6.0 is doing it for {{br-edr|BR/EDR}}; {{ccc-digital-key|CCC Digital Key}} 3.0 did it for proximity; 5GC {{sba|SBA}} is doing it for carrier interconnect. The job is half-finished, on a thirty-year clock.'
						},
						{
							type: 'narrative',
...
**Audit the state machine, not the cipher.** The crypto in your protocol is almost certainly fine — {{aes-128|AES-128}}, {{aes-gcm|AES-GCM}}, {{chacha20-poly1305|ChaCha20-Poly1305}}, {{curve25519|Curve25519}}, {{nist|NIST}} P-256 have all survived two decades of attack. The bugs live in *what your protocol does around the crypto* — key reinstallation, entropy negotiation, role switching, {{session-resumption|session resumption}}, downgrade handling. Spend most of your security review there.
...
**Public {{peer|peer}}-reviewed cryptography beats proprietary every time.** {{mifare|MIFARE}} Crypto1 lasted 14 years (1994–2008) before being decapped and broken. Open {{aes-128|AES-128}} has held up since 1998 without a known practical break. The migration cost is brutal; the survival cost of *not* migrating is brutaler.
...
Every chapter in this Part has security stories. This chapter ties them together as one arc — and the arc is **roughly the same arc as every other security-critical protocol family in this book**. The pattern is not unique to wireless; it is just *most visible* there, because the medium itself is adversarial.`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			synopsis:
				'The non-technical layer that decides where every wireless protocol may live — 6 GHz [[wifi|Wi-Fi]], {{mmwave|mmWave}}, {{cbrs|CBRS}}, {{wrc|WRC}}-27, [[wifi|Wi-Fi]] 8, {{ambient-iot|Ambient IoT}}, 6G targets, and {{starlink|Starlink}} {{direct-to-cell|Direct-to-Cell}}. The wireless frontier through 2030.',
			slots: [
				{
					kind: 'pull-quote',
...
The {{spectrum|spectrum}} layer is the *real* foundation of wireless. It is allocated globally by the **ITU-R** (the radio sector of the International Telecommunication Union, ~190 member states) on a treaty schedule called the **{{wrc|World Radiocommunication Conference}}** every 3–4 years. It is regionally harmonised by **CEPT** (Europe), **APT** (Asia-Pacific), and **CITEL** (the Americas). And it is *nationally* enforced by regulators like the {{fcc|FCC}} (US), Ofcom (UK), the Bundesnetzagentur (Germany), and the {{mic|MIC}} (Japan).

The result is that *the same physical radio* is legal in some countries, restricted in others, and banned outright in a few. **{{apple|Apple}}'s iPhone disables [[uwb|UWB]] Precision Finding in Japan, Indonesia, parts of the Middle East, and a few smaller markets** because of regional power-mask differences. The {{cbrs|CBRS}} band that powers Private 5G in the United States does not exist in Europe. The 6 GHz upper band that the {{fcc|FCC}} opened for Wi-Fi in 2020 may yet be reassigned to mobile carriers in the EU. Every protocol chapter has a wire-format story; this chapter is about the *policy* story underneath all of them.`
						},
						{
							type: 'narrative',
							title: 'The 6 GHz fight — the largest single bandwidth grant in 20 years',
							text: `**On 23 April 2020, the {{fcc|FCC}} freed 1,200 MHz of 6 GHz {{spectrum|spectrum}}** (5.925–7.125 GHz) for unlicensed use — the largest single {{bandwidth|bandwidth}} grant the United States has authorised in 20 years. Wi-Fi 6E and {{wifi-7|Wi-Fi 7}} were the immediate consumers. The decision essentially **tripled the unlicensed {{spectrum|spectrum}} available for [[wifi|Wi-Fi]]**, easing the 2.4 GHz and 5 GHz crowding that had bedevilled apartment buildings and convention centres for two decades.

The lower 6 GHz band (5.925–6.425 GHz) is **Low-Power Indoor (LPI)** — license-exempt for indoor use only, up to 30 dBm EIRP. The upper 6 GHz band (6.525–7.125 GHz) is **Standard Power** — up to 36 dBm EIRP outdoors, but arbitrated by an **{{afc|Automated Frequency Coordination}}** cloud service that checks the {{ap-access-point|AP}}'s location against a database of incumbent licensees (fixed microwave links, satellite services) before granting a channel. The {{fcc|FCC}} approved seven {{afc|AFC}} operators on 23 February 2024; the first {{afc|AFC}}-certified {{wifi-7|Wi-Fi 7}} {{access-point|access point}} (RUCKUS R770) shipped in April 2024.

**Then the EU went the other way.** On **12 November 2025**, the European Radio {{spectrum|Spectrum}} Policy Group recommended assigning the **upper 6 GHz band (6585–7125 MHz) to mobile/5G**, holding 6425–6585 MHz pending {{wrc|WRC-27}}. The [[wifi|Wi-Fi]] Alliance "strongly disagrees" and is lobbying hard. {{wrc|WRC-23}} (Dubai, November–December 2023) had already added 6 GHz to IMT-2020 (the [[cellular|cellular]] side) on a *co-primary* basis, opening the door to this exact contest.

**This is what {{spectrum|spectrum}} policy actually looks like** — the same physical band, same physical radios, allocated differently by different regional treaty bodies, with billion-dollar industry alignments fighting it out at {{wrc|WRC}} every four years. Wi-Fi shipping millions of 6 GHz devices in the US is one fact; whether those devices can use the upper-6-GHz band in Europe is a separate fact decided by a different process.`
						},
						{
							type: 'callout',
...
{{mmwave|mmWave}} is **line-of-sight only**, attenuates ~20 dB through a wet leaf, and is absorbed by glass-coated buildings, human bodies, and pretty much anything thicker than dry air. Carriers deployed it in dense urban hotspots (Verizon Manhattan, AT&T Manhattan, T-Mobile Stadium of America) but failed to make it the everyday 5G experience anyone advertised. The current honest assessment is that {{mmwave|mmWave}} is **wonderful in a stadium, useful in a dense business district, irrelevant in a suburb, and unworkable in a basement**.

Where {{mmwave|mmWave}} *has* won is **fixed wireless access (FWA)** — Verizon 5G Home, T-Mobile Home Internet, Starry — point-to-point links from a base station to a window-mounted antenna at a specific home or business. No mobility, no body shadowing, line-of-sight by design. FWA is now the fastest-growing residential broadband category in the US, eating into legacy cable and DSL share. The future of {{mmwave|mmWave}} is **fixed access plus dense urban capacity**, not the universal smartphone radio its marketing once promised.
...
The answer is **{{wifi-8|Wi-Fi 8}} / 802.11bn — Ultra High Reliability**. It is explicitly *not* a peak-speed upgrade: same bands as {{wifi-7|Wi-Fi 7}}, same 320 MHz max, same ~46 Gbit/s {{phy|PHY}} peak. The PAR objectives are quantitative: **+25% throughput at a given SINR, −25% 95th-percentile {{latency|latency}}, −25% MPDU loss across {{bss-coloring|BSS}} transitions.** Every {{wifi-8|Wi-Fi 8}} feature — **Multi-{{ap-access-point|AP}} Coordination** (Co-BF, Co-SR, Co-{{tdma|TDMA}}), **Seamless Roaming Domain** (SMD), **Enhanced Long Range PPDU**, **Distributed Resource Units**, **Non-Primary Channel Access** — is in service of {{tail-latency|tail latency}} and reliability, not headline throughput.

The pattern across the last two generations is the same: squeeze the existing speed budget for *consistency*, not for the marketing-friendly peak. **Wi-Fi 8 is targeted for ratification September 2028**. {{broadcom|Broadcom}} announced a Wi-Fi 8 chipset in October 2025; ASUS demoed a draft router at {{ces-show|CES}} 2026; consumer launches expected mid-to-late 2026. A "Wi-Fi 9" successor study group started January 2026 already.
...
**On 27 January 2025**, **T-Mobile + SpaceX {{starlink|Starlink}} launched the first commercial {{direct-to-cell|Direct-to-Cell}} service** in the United States: {{sms|SMS}}, MMS, and emergency messaging from ordinary phones, with the satellite acting as a base station in standard [[cellular|cellular]] bands n255/n256. **AT&T's AST SpaceMobile partnership** demonstrated 5G voice from a standard phone in mid-2023 (technical proof) and is rolling out commercial service through 2025–2026. **{{apple|Apple}}'s Globalstar-based Emergency SOS** has been live since the iPhone 14 (September 2022) — your phone, no special hardware, automatic fallback when terrestrial coverage drops.

The architectural change is the *band*. Older satellite phones (Iridium, Inmarsat) used dedicated satellite bands and required special handsets. {{direct-to-cell|Direct-to-Cell}} uses **standard terrestrial cellular bands**, so any phone with the right modem firmware can {{mqtt-connect|connect}}. The satellite is, from the phone's perspective, just another base station — albeit one orbiting at ~550 km and moving at ~7 km/s. {{harq|HARQ}} on a 5 ms terrestrial round-trip is one engineering problem; {{harq|HARQ}} on a 30 ms satellite round-trip with continuous Doppler shift is another. {{3gpp|3GPP}} Release 18 + 19 + 20 contain the timing-and-Doppler patches that make it work.
...
That is the right note to end Part V on. **Wireless is the only chapter in this book where the protocol stops at the silicon and starts again at the regulator.** Read every other Part top-down from the application. Read this one bottom-up from the {{spectrum|spectrum}} allocation chart — the one nobody on your team has framed on the wall.`
						},
						{
							...
							caption:
								'The **{{starlink|Starlink}} constellation** — 1,584 first-shell satellites in 72 orbital planes at 53° inclination. On 27 January 2025, T-Mobile + SpaceX launched the first commercial **{{direct-to-cell|Direct-to-Cell}}** service: {{sms|SMS}} and emergency messaging from ordinary phones to satellites acting as base stations in standard [[cellular|cellular]] bands n255/n256. *"No signal"* will increasingly mean *"no terrestrial signal — try walking outside"*. The frontier where the [[cellular|cellular]] network grows is upward.',
