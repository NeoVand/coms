...
The story did not end there. Vanhoef's group at KU Leuven has broken Wi-Fi roughly every two years since: **Dragonblood** (April 2019, side-channels in {{wpa3|WPA3}}-SAE), **FragAttacks** (May 2021, {{fragmentation|fragmentation}}/aggregation bugs in [[wifi|802.11]] design itself), **Framing Frames** (March 2023), **SSID Confusion / CVE-2023-52424** (May 2024 — the [[wifi|802.11]] standard does NOT require the SSID to enter PMK or session-key derivation in many config paths). The cadence is so reliable that the field plans security audits around his summer talks.
...
**{{ccc-digital-key|CCC Digital Key 3.0}}** uses BLE for proximity and then bootstraps a [[uwb|UWB]] session — the phone advertises a service UUID over BLE, the car authenticates, the encrypted {{gatt|GATT}} channel transfers the STS_KEY for the [[uwb|UWB]] ranging round, and only then does the [[uwb|UWB]] radio power on for the three-message DS-TWR {{exchange|exchange}}. **{{aliro|Aliro}} 1.0**, the access-control credential standard the CSA finalised on 26 February 2026, follows the same pattern for doors.
...
The {{auracast|Auracast}} accessibility story is the killer app, not "free wireless audio in the airport." For hard-of-hearing listeners, {{auracast|Auracast}} turns every public-address system into something their hearing aids can listen to directly — no analog loop, no battery-eating Bluetooth pair with a wall-mounted transmitter, just a scan-and-select. The SIG positions it as the *largest accessibility upgrade in consumer audio history*, and the deployment math (every hearing-aid manufacturer ships {{le-audio|LE Audio}} in 2026 hardware) backs that up.`
						},
						{
							type: 'narrative',
...
**{{thread|Thread}} 1.4** (December 2024) made **{{border-router|Thread Border Routers}}** a multi-vendor commodity. {{apple|Apple}}'s HomePod and {{apple|Apple}} TV 4K, {{google|Google}}'s Nest Hub, Amazon Echo Hub, Aqara M3, eero 6+, and others all act as {{thread|Thread}} Border Routers — bridging the 802.15.4 mesh to your home [[wifi|Wi-Fi]] network. The {{border-router|border router}} runs **{{mdns|DNS-SD}}** for {{service-discovery|service discovery}}, hands out [[ipv6|IPv6]] prefixes via {{slaac|SLAAC}} to the {{thread|Thread}} mesh, and forwards packets between the radios. From your phone, a {{thread|Thread}} sensor looks like any other [[ipv6|IPv6]] host — you can ping it.
					...
					sections: [
						{
							...
							text: `Every wireless protocol in this Part has been broken on stage in public, multiple times, by the same handful of researchers, with strikingly similar shapes of bug. **It is never the cryptography itself.** AES-128, {{aes-gcm|AES-GCM}}, ECDSA, ChaCha20, Curve25519 — the primitives hold up. What gets broken is the **negotiation logic**, the **state machine**, the **roaming model**, the **trust assumptions between operators** — the *protocol surface around* the cryptography.
...
**Audit the state machine, not the cipher.** The crypto in your protocol is almost certainly fine — AES-128, {{aes-gcm|AES-GCM}}, {{chacha20-poly1305|ChaCha20-Poly1305}}, Curve25519, NIST P-256 have all survived two decades of attack. The bugs live in *what your protocol does around the crypto* — key reinstallation, entropy negotiation, role switching, {{session-resumption|session resumption}}, downgrade handling. Spend most of your security review there.
