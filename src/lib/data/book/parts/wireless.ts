...
The cultural moment is **{{auracast|Auracast}}** — the SIG's brand for **{{broadcast|Broadcast}} Isochronous Streams** (BIS) over {{le-audio|LE Audio}} + LC3, one transmitter to unlimited listeners. Public venues replace analog hearing-loops with an {{auracast|Auracast}} {{broadcast|broadcast}}; nearby listeners scan, pick a stream, and tune in. **Frankfurt Airport became the first airport in the world to broadcast all gate announcements over {{auracast|Auracast}} on 28 January 2026**. Cinemas, theatres, gyms, lecture halls, and houses of worship are deploying similar setups through 2026 and 2027.
...
Whether {{channel-sounding|Channel Sounding}} actually displaces UWB for digital-key applications is still an open question in 2026. UWB has a five-year head start in {{ccc-digital-key|CCC Digital Key}}, a tighter timing precision (~30 cm at the silicon level versus ~5–10 cm cm-claimed for {{channel-sounding|Channel Sounding}} under good conditions), and dedicated cryptography ({{sts|STS}}) that was designed for adversarial environments from day one. Channel Sounding has the deployment advantage: it ships on every new Bluetooth 6.0 chip, which means every new smartphone. The next two years of car-key product announcements will tell.`
						},
						{
							type: 'callout',
...
In 2026, **new device design has bifurcated**. Lighting and sensors are increasingly Thread ({{matter|Matter}}-native, low power, IPv6-addressable). Cameras and high-{{bandwidth|bandwidth}} devices are [[wifi|Wi-Fi]] ({{matter|Matter}} 1.5 added camera streaming via RTSP). Zigbee is the *bridged legacy radio* — preserved for the installed base, no longer the default for new designs.`
						},
						{
							...
							text: 'The CSA launched **{{matter|Matter}} 1.0 on 4 October 2022** as an IP-native smart-home standard, running over [[wifi|Wi-Fi]], [[ethernet|Ethernet]], and {{thread|Thread}}. {{matter|Matter}} does **not** run over [[zigbee|Zigbee]] on the wire — but it reuses Zigbee\'s {{zcl|ZCL}} data model directly, which lets a *{{matter|Matter}} Bridge for non-{{matter|Matter}} devices* (Hue Bridge, Aqara Hub M3) translate Matter operations to Zigbee one-to-one. Now at **v1.5 (20 November 2025)**, with camera streaming via RTSP, removing the last category that previously required Zigbee bridging.'
						},
						{
							type: 'narrative',
...
For the home-automation hobbyist, the **{{matter|Matter}} bridge** is the right framing in 2026. A Hue bulb is a Zigbee device; through the Hue Bridge's {{matter|Matter}} firmware, it appears as a Matter accessory; from your iPhone or {{google|Google}} Home, it is "a light bulb." The Zigbee protocol is doing its job — exactly the job Hue's 2012 marketing copy promised: *web-enabled lighting*, with the word *Zigbee* still off the box.`
						},
						{
							type: 'image',
