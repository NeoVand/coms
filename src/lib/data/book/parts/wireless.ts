...
The {{spectrum|spectrum}} legacy: **the FCC's First Report and Order on Valentine's Day 2002** authorised [[uwb|UWB]] under Part 15.519 at **−41.3 dBm/MHz** across **3.1–10.6 GHz** — an *unlicensed underlay* on top of every other service in those bands. The trick is the noise floor. UWB transmits at such a low average power that no incumbent licensee notices, but it spreads its energy across so much {{bandwidth|bandwidth}} that the cumulative pulse energy is detectable at the receiver. The two channels everyone actually uses are **Channel 5** (6489.6 MHz) and **Channel 9** (7987.2 MHz), each 499.2 MHz wide.`
						},
						{
							type: 'narrative',
...
UWB silicon has consolidated to five suppliers: **{{apple|Apple}} captive** (U1 16 nm, **U2** 7 nm from iPhone 15), **Qorvo** (acquired Decawave's DW1000/DW3000 line), **NXP** (Trimension SR040/SR150/SR250), **Samsung captive** (Exynos Connect U100), and a long tail (STMicroelectronics, Microchip, Spark Microsystems, Infineon's 2024-acquired 3db Access). ABI projects UWB phone penetration rising from **27% in 2025 to 52% by 2030** — UWB is on the same adoption curve [[nfc|NFC]] followed a decade earlier.`
						},
						{
							type: 'callout',
...
The practical effect for a global consumer product: **(a)** geo-fence [[uwb|UWB]] features based on locale, **(b)** default to **Channel 5** in Japan and a handful of other restricted jurisdictions, **(c)** support a *no-UWB* mode entirely for countries where [[uwb|UWB]] is not permitted (a few small markets). This is one of the underappreciated reasons UWB consumer adoption is slower than {{ble|BLE}} or [[wifi|Wi-Fi]] — *the regulatory map is fragmented*, and every iPhone software release ships a different list of countries where Precision Finding works.
...
**{{thread|Thread}} 1.4** (December 2024) made **{{border-router|Thread Border Routers}}** a multi-vendor commodity. {{apple|Apple}}'s HomePod and {{apple|Apple}} TV 4K, {{google|Google}}'s Nest Hub, Amazon Echo Hub, Aqara M3, eero 6+, and others all act as {{thread|Thread}} Border Routers — bridging the 802.15.4 mesh to your home [[wifi|Wi-Fi]] network. The {{border-router|border router}} runs **{{mdns|DNS-SD}}** for {{service-discovery|service discovery}}, hands out [[ipv6|IPv6]] prefixes via {{slaac|SLAAC}} to the {{thread|Thread}} mesh, and forwards packets between the radios. From your phone, a Thread sensor looks like any other [[ipv6|IPv6]] host — you can ping it.
