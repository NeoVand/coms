...
[[rtp|RTP]] rides on top of [[udp|UDP]] because **late audio is worse than missing audio** — retransmitting a packet that arrives 200 ms late delivers something the receiver cannot use. RTP adds three things to a [[udp|UDP]] datagram: a **{{sequence-number|sequence number}}** so the receiver can detect loss and reorder packets, a **timestamp** so playback can be paced correctly, and a **{{payload|payload}} type** field that names the {{codec|codec}}.`
						},
						{
							type: 'narrative',
...
**RFC 8888 (2020)** finally unified per-packet RTCP feedback formats for Google {{congestion-control|Congestion Control}} (GCC), Cisco NADA (RFC 8698), and Ericsson SCReAM (RFC 8298). **L4S/ECN-marked feedback** is now live in libwebrtc behind the field trial \`WebRTC-RFC8888CongestionControlFeedback/Enabled\` — the bridge from RTP into the [[frontier:l4s-comcast-launch|L4S frontier]].`
						...
						},
						{
							...
							text: `Discord's homegrown C++ SFU forwards [[rtp|RTP]] for **2.5 million concurrent voice users across 850+ voice servers in 13 regions and 30+ data centers**. That is the largest publicly documented RTP fleet on the internet. To save CPU at that scale, Discord **swapped {{dtls|DTLS}}-{{srtp|SRTP}} for libsodium's xsalsa20_poly1305** in their custom libwebrtc fork — illustrating the field axiom that **the standard is the floor, not the ceiling**.
...
Asterisk had its own RTP security incident in 2017: **AST-2017-008/-012 ("Asterisk RTP Bleed")** showed that Asterisk's \`strictrtp\` port-latching had no authentication, so flooding the [[udp|UDP]] port range at 28-168 MB/s could "re-train" the media flow to an attacker. The first patch missed {{rtcp|RTCP}} and a race condition, requiring a second advisory.`
						},
						{
							type: 'narrative',
							title: 'RTP-over-QUIC — The Frontier',
							text: `**RTP-over-QUIC (RoQ)** — \`draft-ietf-avtcore-rtp-over-quic-14\` — entered Working Group Last Call in **July 2025**. {{alpn|ALPN}} token \`roq\`. Multiplexes [[rtp|RTP]] sessions over one [[quic|QUIC]] connection; preserves the entire RTP ecosystem while gaining [[quic|QUIC]]'s {{encryption|encryption}}, NAT-friendliness, and {{zero-rtt|0-RTT}}.

Active 2025-2026 work in the IETF AVTCORE WG: **RFC 9628 (2024)** finally promoted the VP9 RTP {{payload|payload}} format to Standards Track. Drafts in flight cover haptics, V3C volumetric video, JPEG XS 3rd edition, APV {{codec|codec}}, and an HEVC/H.265 [[webrtc|WebRTC]] profile (\`draft-ietf-avtcore-hevc-webrtc-08\`, March 2026). RTP keeps acquiring new payload formats forty years after Casner first audio-cast IETF San Diego.`
						}
					]
				},
...
The audio engine was bought, not built: in **May 2010 Google paid USD 68.2 million for Global IP Solutions (GIPS)** specifically because GIPS's NetEQ {{jitter|jitter}} buffer was already running on **800 million endpoints**. Google open-sourced it in 2011 as \`libwebrtc\`. By end-2018, libwebrtc reached **1.21 million lines of code** — three times the size of the Space Shuttle's onboard software (per Justin Uberti's 2019 figure).`
						},
						{
							type: 'narrative',
							title: 'NAT Traversal — The Hard Part',
							text: `Two browsers behind home routers cannot just open a connection to each other; their public-facing addresses are different from their private ones. [[webrtc|WebRTC]] uses **ICE** (Interactive Connectivity Establishment, RFC 8445) — a coordinated dance where each peer gathers candidate addresses from STUN servers (RFC 8489), sends them to the other peer via a signalling channel (the developer's choice — often WebSocket), and probes connectivity over each candidate pair. When direct [[udp|UDP]] fails, **TURN** (RFC 8656) relays through a third party.

The media itself is [[rtp|RTP]] wrapped in **{{srtp|SRTP}}** (Secure [[rtp|RTP]], RFC 3711), with keys established through **{{dtls|DTLS}}** (Datagram [[tls|TLS]]) over the same connection. WebRTC also offers **DataChannel** for arbitrary application data over the same connection — useful for in-game state sync alongside voice chat.

**The {{codec|codec}} wars ended in a draw**: [[rfc:9000|RFC 7742]] (March 2016) mandates *both* VP8 and H.264 Constrained Baseline as MTI video codecs. Cisco neutralized H.264 patents by open-sourcing OpenH264 and paying MPEG-LA royalties on its behalf — without that, WebRTC-in-Safari would have been impossible.`
						},
						{
							type: 'callout',
...
The 2021–2026 spec refresh shipped a generation of follow-ons: **RFC 9143 (Feb 2022) BUNDLE**, **RFC 9147 (April 2022) {{dtls|DTLS}} 1.3**, **RFC 9335 (Jan 2023) {{srtp|SRTP}} Cryptex**, **RFC 9429 (April 2024) JSEP-bis**, **[[rfc:9605|RFC 9605]] (Aug 2024) SFrame**, **RFC 9725 (March 2025) WHIP** (replacing a generation of ad-hoc WebSocket signalling for ingest).`
						},
						{
							type: 'narrative',
...
Plan B [[sdp|SDP]] is fully gone now: deprecation-warned in Chrome M89 (Feb 2021), removed in M93 (Aug 2021) with a Reverse Origin Trial through M96 (Jan 2022). All \`sdpSemantics\` flag handling was finally removed from Chromium in 2024. Unified Plan is the only remaining SDP semantics.`
						}
					]
				},
					...
					sections: [
						{
							...
							text: `When you place a VoIP call, two protocols work in tandem before any audio flows. **[[sip|SIP]]** (Session Initiation Protocol) is the signalling layer — text-based, request/response shaped like [[http1|HTTP]], with verbs like INVITE, ACK, BYE, REGISTER. SIP locates the callee (through registration servers and proxies that resolve \`sip:alice@example.com\`), negotiates capability, and sets up or tears down the session.

Both SIP and [[sdp|SDP]] — and [[rtp|RTP]] above them — were authored by **[[pioneer:henning-schulzrinne|Henning Schulzrinne]]**, a Columbia University professor who has authored more than 70 RFCs, served as FCC CTO three times, and was inducted into the Internet Hall of Fame in 2013.
...
Twenty-eight years in, the protocol-version line is still \`v=0\`. The SDP "offer/answer" {{exchange|exchange}} — one side offers their capability, the other answers with the subset they accept — became the standard pattern for media negotiation everywhere, including [[webrtc|WebRTC]].`
						},
						{
							type: 'callout',
...
The 2026 cryptographic milestone: **\`draft-pantos-hls-rfc8216bis-22\` (May 2026) added AES-256-GCM as a permissible HLS {{encryption|encryption}} method** — the most consequential cryptographic change in nearly a decade, and the bis draft also renamed the "master playlist" to "**Multivariant Playlist**."`
						},
						{
							type: 'narrative',
...
MoQT's data model is **{{pub-sub|publish/subscribe}} with relay caches**: media flows as **track > group > subgroup > object**, mapped onto QUIC streams or unreliable QUIC datagrams, and runs over either raw [[quic|QUIC]] or {{webtransport|WebTransport}} so it's reachable from browsers.`
						},
						{
							type: 'callout',
...
The spec has nonetheless attracted serious implementation effort. **NAB 2026 (28 April 2026)** demoed MoQ interop across **eleven vendors** — Ant Media, AWS, Bitmovin, Broadpeak, CacheFly, Cloudflare, Nomad Media, Oracle, Norsk, Synamedia, Red5 — under a new "OpenMOQ Software Consortium." **Cloudflare deployed an MoQ relay at every Cloudflare edge across 330+ cities in 2025** as a beta managed service — the first global MoQ relay network.`
						},
						{
							...
							text: `The browser story matured fast: **{{webtransport|WebTransport}} reached cross-browser Baseline status in March/April 2026** (Safari 26.4). Combined with WebCodecs, this is the first time JavaScript can implement non-RTP media transports natively. Safari first shipped early WebTransport in **18.4 (March/April 2025)**.

Twitch's MoQ heritage matters: Twitch's internal **"Warp"** QUIC-based replacement for [[hls|HLS]] (presented at Demuxed 2021) became the seed of the MoQ Working Group's WARP draft.

**End-to-end secure MoQ objects**: \`draft-ietf-moq-secure-objects-00\` (March 2026) brings application-layer {{encryption|encryption}} into the spec from day one, mirroring what SFrame retrofitted onto [[rtp|RTP]]. MoQ is being designed to learn from RTP's E2EE retrofit pain.
...
The conservative alternative: **RTP-over-QUIC (RoQ)**. \`draft-ietf-avtcore-rtp-over-quic-14\` entered Working Group Last Call July 2025, keeping the entire RTP/{{rtcp|RTCP}} ecosystem intact while swapping [[udp|UDP]] for [[quic|QUIC]]. The fork in the road is real.`
						}
					]
				},
