...
[[rtp|RTP]] rides on top of [[udp|UDP]] because **late audio is worse than missing audio** — retransmitting a packet that arrives 200 ms late delivers something the receiver cannot use. [[rtp|RTP]] adds three things to a [[udp|UDP]] datagram: a **{{sequence-number|sequence number}}** so the receiver can detect loss and reorder packets, a **timestamp** so playback can be paced correctly, and a **{{payload|payload}} type** field that names the {{codec|codec}}.`
						},
						{
							...
							text: `[[rtp|RTP]] requires both the {{sequence-number|sequence number}} and the timestamp to **start from random initial values**, not zero. This was a 1996 design decision — the spec authors thought it was good practice for synchronisation. It turned out to be **load-bearing for {{srtp|SRTP}}'s {{aes|AES}}-CTR nonces eight years later**: random sequence numbers are the entropy that keeps the per-packet {{aes|AES}} counter unique across reboots and re-keys.

The companion **{{rtcp|RTCP}}** ([[rtp|RTP]] Control Protocol) flows alongside, carrying receiver reports (loss rates, {{jitter|jitter}}), sender reports (mapping wall-clock time to [[rtp|RTP]] timestamps for cross-stream sync), and source descriptions ({{cname-record|CNAME}}, the canonical participant identifier). {{rtcp|RTCP}} is what lets a video conferencing client detect that 8% of audio packets are being dropped and adapt the {{codec|codec}} down.

**{{rfc-doc|RFC}} 8888 (2020)** finally unified per-packet {{rtcp|RTCP}} feedback formats for {{google|Google}} {{congestion-control|Congestion Control}} (GCC), {{cisco|Cisco}} NADA ({{rfc-doc|RFC}} 8698), and Ericsson SCReAM (RFC 8298). **{{l4s|L4S}}/{{ecn|ECN}}-marked feedback** is now live in libwebrtc behind the field trial \`WebRTC-RFC8888CongestionControlFeedback/Enabled\` — the bridge from [[rtp|RTP]] into the [[frontier:l4s-comcast-launch|L4S frontier]].`
						},
						{
							type: 'callout',
						...
						},
						{
							...
							text: `Discord's homegrown C++ {{sfu|SFU}} forwards [[rtp|RTP]] for **2.5 million concurrent voice users across 850+ voice servers in 13 regions and 30+ data centers**. That is the largest publicly documented [[rtp|RTP]] fleet on the internet. To save {{cpu|CPU}} at that scale, Discord **swapped {{dtls|DTLS}}-{{srtp|SRTP}} for libsodium's xsalsa20_poly1305** in their custom libwebrtc fork — illustrating the field axiom that **the standard is the floor, not the ceiling**.

The 2020 Citizen Lab **Zoom disclosure** revealed Zoom wrapped [[rtp|RTP]] in a custom transport over [[udp|UDP]]/8801 and encrypted it with **{{aes-128|AES-128}}-ECB** — leaking patterns in encrypted video frames. Industry pressure forced a switch to {{aes-gcm|AES-GCM}} and ultimately E2EE.
...
Active 2025-2026 work in the {{ietf|IETF}} AVTCORE WG: **{{rfc-doc|RFC}} 9628 (2024)** finally promoted the {{vp9|VP9}} [[rtp|RTP]] {{payload|payload}} format to Standards Track. Drafts in flight cover haptics, V3C volumetric video, JPEG XS 3rd edition, APV {{codec|codec}}, and an {{hevc|HEVC}}/H.265 [[webrtc|WebRTC]] profile (\`draft-ietf-avtcore-hevc-webrtc-08\`, March 2026). [[rtp|RTP]] keeps acquiring new {{payload|payload}} formats forty years after Casner first audio-cast {{ietf|IETF}} San Diego.`
						},
						{
							type: 'image',
...
The audio engine was bought, not built: in **May 2010 {{google|Google}} paid USD 68.2 million for Global [[ip|IP]] Solutions (GIPS)** specifically because GIPS's NetEQ {{jitter|jitter}} buffer was already running on **800 million endpoints**. {{google|Google}} open-sourced it in 2011 as \`libwebrtc\`. By end-2018, libwebrtc reached **1.21 million lines of code** — three times the size of the Space Shuttle's onboard software (per Justin Uberti's 2019 figure).`
						},
						{
							...
							text: `Two browsers behind home routers cannot just open a connection to each other; their public-facing addresses are different from their private ones. [[webrtc|WebRTC]] uses **{{ice|ICE}}** (Interactive Connectivity Establishment, [[rfc:8445|RFC 8445]]) — a coordinated dance where each {{peer|peer}} gathers candidate addresses from {{stun|STUN}} servers ([[rfc:8489|RFC 8489]]), sends them to the other {{peer|peer}} via a signalling channel (the developer's choice — often [[websockets|WebSocket]]), and probes connectivity over each candidate pair. When direct [[udp|UDP]] fails, **{{turn|TURN}}** ([[rfc:8656|RFC 8656]]) relays through a third party.

The media itself is [[rtp|RTP]] wrapped in **{{srtp|SRTP}}** (Secure [[rtp|RTP]], {{rfc-doc|RFC}} 3711), with keys established through **{{dtls|DTLS}}** (Datagram [[tls|TLS]]) over the same connection. [[webrtc|WebRTC]] also offers **DataChannel** for arbitrary application data over the same connection — useful for in-game state sync alongside voice chat.

**The {{codec|codec}} wars ended in a draw**: [[rfc:9000|RFC 7742]] (March 2016) mandates *both* VP8 and H.264 Constrained Baseline as MTI video codecs. {{cisco|Cisco}} neutralized H.264 patents by open-sourcing OpenH264 and paying {{mpeg-org|MPEG}}-LA royalties on its behalf — without that, [[webrtc|WebRTC]]-in-Safari would have been impossible.`
						},
						{
							type: 'callout',
							title: 'AV1 came from screen-share',
							text: '**{{av1|AV1}} went default-on for screen-share in {{google|Google}} Meet in 2024.** {{av1|AV1}} hardware encode shipped in Chrome M120 (Dec 2023). Firefox 125 added {{av1|AV1}}+EME in April 2024. But the royalty-free claim has a new asterisk: on **23 March 2026 Dolby filed {{av1|AV1}}+{{hevc|HEVC}} patent suits against Snap**, re-opening the question of whether {{av1|AV1}} is actually patent-clean. The {{codec|codec}} wars never end.'
						},
						{
							type: 'narrative',
							title: 'CVE-2023-7024 and the libwebrtc Blast Radius',
							text: `**{{cve|CVE}}-2023-7024 (December 2023)** — a heap buffer overflow in libwebrtc's \`WebRtcAudioSink::OnSetFormat\` was exploited in the wild as Chrome's 8th zero-day of 2023. Fixed in 120.0.6099.129. CISA added it to the Known Exploited Vulnerabilities catalog January 2024.
...
The 2021–2026 spec refresh shipped a generation of follow-ons: **{{rfc-doc|RFC}} 9143 (Feb 2022) BUNDLE**, **{{rfc-doc|RFC}} 9147 (April 2022) {{dtls|DTLS}} 1.3**, **RFC 9335 (Jan 2023) {{srtp|SRTP}} Cryptex**, **RFC 9429 (April 2024) JSEP-bis**, **[[rfc:9605|RFC 9605]] (Aug 2024) SFrame**, **RFC 9725 (March 2025) WHIP** (replacing a generation of ad-hoc [[websockets|WebSocket]] signalling for ingest).`
						},
						{
							type: 'narrative',
...
Plan B [[sdp|SDP]] is fully gone now: deprecation-warned in Chrome M89 (Feb 2021), removed in M93 (Aug 2021) with a Reverse Origin Trial through M96 (Jan 2022). All \`sdpSemantics\` flag handling was finally removed from Chromium in 2024. Unified Plan is the only remaining [[sdp|SDP]] semantics.`
		...
		// ────────────────────────────────────────────────────────────
		{
			...
			slots: [
				{
					kind: 'pull-quote',
					text: 'The `sips:` {{uri|URI}} scheme means [[tls|TLS]] hop-by-hop only, NOT end-to-end like `https:`. [[rfc:5630|RFC 5630]] (2009) explicitly warns against the padlock-icon intuition. Forty years in, this is still where engineers get burned.',
					attribution: 'Author'
				},
				{
					kind: 'prose',
...
Twenty-eight years in, the protocol-version line is still \`v=0\`. The [[sdp|SDP]] "offer/answer" {{exchange|exchange}} — one side offers their capability, the other answers with the subset they accept — became the standard pattern for media negotiation everywhere, including [[webrtc|WebRTC]].`
						...
						},
						{
							...
							text: `STIR/SHAKEN is the protocol family that lets carriers cryptographically attest "yes, this caller {{id-identifier|ID}} is real" — the long-running attempt to fix robocalls. It is not new: STIR ({{rfc-doc|RFC}} 8224) was published in 2018; SHAKEN was framework-level guidance from the [[sip|SIP]] Forum / ATIS in 2017.

The 2024-2026 milestone: **{{fcc|FCC}} Eighth Report & Order (adopted 21 Nov 2024) tightened STIR/SHAKEN compliance, taking effect 18 September 2025**. U.S. obligated providers must now sign with their *own* SPC certificates rather than a third party.
...
**ZRTP ({{rfc-doc|RFC}} 6189)** is the only protocol named for its inventor — **Phil "Z" Zimmermann** (PGP). Its **Short Authentication Strings** are read aloud between humans to detect MITMs: the two callers speak the same 4-digit hash; if they match, the channel is authenticated. ZRTP remains the standard against which other end-to-end voice security schemes are measured.

The cryptography is slowly tightening: **{{rfc-doc|RFC}} 8760 (March 2020)** finally deprecated {{md5|MD5}} in [[sip|SIP]] digest auth in favor of {{sha256|SHA-256}} / SHA-512-256. But PJSIP/Asterisk only added {{sha256|SHA-256}} outbound support in 2023-2024, so most real deployments still negotiate {{md5|MD5}}. The protocol is modern; the deployed installed base is two decades behind.`
						},
						{
							type: 'image',
							...
							caption:
								'A canonical **[[sip|SIP]]** session: {{sip-invite|INVITE}} → 100 Trying → 180 Ringing → 200 OK → {{ack|ACK}} → media flows → BYE. The same shape as an {{http-method|HTTP}} {{request-response|request/response}}, with a few extra status codes for telephony semantics. By 2025 {{gsma|GSMA}} reported **310+ commercial {{volte|VoLTE}} operators** in 140+ countries — the largest [[sip|SIP]] deployment on Earth runs inside [[cellular|cellular]] {{ims|IMS}} cores.',
...
{{apple|Apple}} changed that with **[[hls|HLS]]** ({{http-method|HTTP}} Live Streaming), which **shipped on 17 June 2009 with iPhone {{os|OS}} 3.0 / iPhone 3GS** — created at {{apple|Apple}} by Roger Pantos and William May Jr. The 2007/2008 iPhone had no Flash, and {{apple|Apple}} needed something that survived {{nat|NAT}}/firewalls on 3G. Reusing {{http-method|HTTP}}/443 was a deliberate {{firewall|firewall}}-traversal play.
...
The trick was breaking the stream into **2-10 second segments**, each a regular .ts (or later .mp4/CMAF) file accessible via plain [[http1|HTTP]]. A small **playlist** file (.m3u8) lists the segments in order. The client downloads the playlist, fetches segments, and plays them. To support multiple bitrates, the server publishes parallel playlists (240p, 480p, 1080p, 4K) and a master playlist; the client switches bitrates between segments based on observed {{bandwidth|bandwidth}}.`
						...
						},
						{
							...
							text: `**{{mpeg-org|MPEG}}-[[dash|DASH]] ({{iso-iec|ISO/IEC}} 23009-1)** was first published in **2012** as the standardised version of the same idea. The differences from [[hls|HLS]] are {{codec|codec}} restrictions, {{manifest|manifest}} format ({{xml|XML}} {{mpd|MPD}} vs M3U8), and licensing. The **5th edition (23009-1:2022)** is freely available via {{iso|ISO}} ITTF; the **6th edition (FDIS 23009-1)** reached stage 50.00 by April 2025 and adds **L3D-[[dash|DASH]]/SSR** for sub-second join times.
...
**CMAF ({{iso-iec|ISO/IEC}} 23000-19)** — the joint {{apple|Apple}}+{{microsoft|Microsoft}} fMP4 proposal at {{mpeg-org|MPEG}} #114 (San Diego, Feb 2016), first published 2018, 4th edition published 2024 — is what finally lets one set of fMP4 segments serve both [[hls|HLS]] and [[dash|DASH]]. **Disney+ runs 100% [[hls|HLS]]+CMAF end-to-end**.
...
The 2026 cryptographic milestone: **\`draft-pantos-hls-rfc8216bis-22\` (May 2026) added {{aes|AES}}-256-GCM as a permissible [[hls|HLS]] {{encryption|encryption}} method** — the most consequential cryptographic change in nearly a decade, and the bis draft also renamed the "master playlist" to "**Multivariant Playlist**."`
						},
						{
							type: 'narrative',
...
**Media over [[quic|QUIC]] (MoQ)** is the first {{ietf|IETF}} media transport that intentionally **is not [[rtp|RTP]]**. \`draft-ietf-moq-transport-17\` was published **March 2026**, with co-editors Suhas Nandakumar ({{cisco|Cisco}}), Victor Vasiliev ({{google|Google}}), Ian Swett ({{google|Google}}), and Alan Frindell ({{meta|Meta}}).
...
The spec has nonetheless attracted serious implementation effort. **NAB 2026 (28 April 2026)** demoed MoQ interop across **eleven vendors** — Ant Media, {{aws|AWS}}, Bitmovin, Broadpeak, CacheFly, {{cloudflare|Cloudflare}}, Nomad Media, Oracle, Norsk, Synamedia, Red5 — under a new "OpenMOQ Software Consortium." **{{cloudflare|Cloudflare}} deployed an MoQ relay at every {{cloudflare|Cloudflare}} edge across 330+ cities in 2025** as a beta managed service — the first global MoQ relay network.`
						},
						{
							type: 'narrative',
...
**End-to-end secure MoQ objects**: \`draft-ietf-moq-secure-objects-00\` (March 2026) brings application-layer {{encryption|encryption}} into the spec from day one, mirroring what SFrame retrofitted onto [[rtp|RTP]]. MoQ is being designed to learn from [[rtp|RTP]]'s E2EE retrofit pain.

The honest 2025 {{latency|latency}} landscape:
- Standard [[hls|HLS]]/[[dash|DASH]]: ~6-30 s
- LL-[[hls|HLS]] / LL-[[dash|DASH]]: 2-5 s
- [[webrtc|WebRTC]] {{sfu|SFU}}: ~50-200 ms
