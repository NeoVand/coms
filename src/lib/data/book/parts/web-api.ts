					...
					sections: [
						{
							...
							text: `[[http1|HTTP/1.1]] is the most successful application protocol ever shipped. It was originally specified in **RFC 2068 (January 1997)**, then revised through RFC 2616 (1999) and RFC 7230-7235 (2014). In **June 2022** the IETF replaced the entire 1997-2014 lineage in one big bang with a five-document set: **[[rfc:9110|RFC 9110]]** (HTTP Semantics), [[rfc:9112|RFC 9112]] (HTTP/1.1 Messaging), RFC 9111 (Caching), and the matching HTTP/2 ([[rfc:9113|RFC 9113]]) and HTTP/3 ([[rfc:9114|RFC 9114]]) updates. *The Register* called the 7 June 2022 cluster *"the day TCP stopped being assumed."*
...
**Persistent connections.** HTTP/1.0 (RFC 1945, 1996) opened a fresh TCP connection per request — disastrous as web pages grew. HTTP/1.1 reused connections for multiple requests by default, dropping latency dramatically. **Pipelining** (sending the next request before the first response arrives) was specified but rarely deployed because head-of-line blocking made it slower in practice than just opening more connections. Browsers settled on **6 parallel TCP connections per origin** as the operational compromise.`
						...
						},
						{
							...
							text: `Despite HTTP/2 (51%) and HTTP/3 (~21%) carrying the majority of modern web traffic, **HTTP/1.1 stubbornly persists at around 28%** of Cloudflare-observed requests in Q1 2026. Three reasons.

**APIs and CLI tools.** \`curl\`, \`wget\`, scripting clients, server-to-server REST traffic, internal microservice calls — many of these still default to HTTP/1.1 because the simplicity outweighs the multiplexing benefit. A single API call doesn't need binary framing or stream prioritisation.
...
**Debuggability.** When something is wrong with an HTTP exchange, the fastest diagnostic path is reading the bytes on the wire. HTTP/1.1 is the only version where you can do that without a protocol decoder. For internal tooling and developer-facing surfaces, "I can see the request" is a feature.`
						}
					]
				},
					...
					sections: [
						{
							...
							text: `By 2009, web pages averaged 90 requests across 15 origins. The 6-connection-per-origin browser cap meant every page paid TCP setup overhead repeatedly, and **head-of-line blocking** at the application layer was capping throughput. Google's **SPDY** experiment ([[pioneer:mike-belshe|Mike Belshe]] + Roberto Peon, 2009) proposed multiplexing many requests over a single connection, with binary framing and per-frame priority.

SPDY became the basis for [[http2|HTTP/2]] under HTTPbis WG chair Mark Nottingham. **[[rfc:9113|RFC 9113]]** (originally RFC 7540, May 2015) ships HTTP/2 as a binary-framed multiplexed protocol. The semantics — verbs, headers, status codes — are unchanged from HTTP/1.1; only the wire format moves from text to **9-byte frame headers** (Length 24-bit, Type 8-bit, Flags 8-bit, Reserved 1-bit, Stream ID 31-bit) plus payload. A single HTTP/2 connection carries any number of **streams**, each a logically independent request/response pair. Client streams use odd IDs; server-push streams (now extinct) used even ones.

**HPACK** (RFC 7541) compresses repeated headers (cookies, user-agent) by reference instead of resending them. It uses a 61-entry static table of common header fields, a per-connection dynamic table, and Huffman-coded literals. HPACK was designed in direct response to **CRIME/BREACH-style attacks** on gzip-compressed headers — the static table prevents the attacker from inferring secrets from compression ratios.`
						},
						{
							type: 'narrative',
							title: 'The Deployment Win, Then the Long Tail',
							text: `Real-world page loads dropped 30-40% with HTTP/2 enabled. CDNs adopted it within a year. By 2018, over 35% of all websites supported HTTP/2; by 2026, it carries about **51% of Cloudflare-observed web requests** — still the dominant version of HTTP on the internet, even as HTTP/3 grows.

The unsolvable structural flaw: HTTP/2 still runs over [[tcp|TCP]], and TCP retransmission stalls **all** streams on a connection when even one packet is lost. The very feature HTTP/2 added — multiplexing — turned a single dropped packet into a whole-connection stall. The fix had to wait for [[quic|QUIC]] and [[http3|HTTP/3]].`
						},
						{
							type: 'callout',
						...
						},
						{
							...
							text: `The most consequential changes in HTTP/2 over the last 24 months have been security disclosures.
...
But adoption has plateaued. As of Q1 2026, HTTP/3 carries roughly **21% of Cloudflare-observed web requests** — flat or slightly declining for several months. HTTP/2 still dominates at ~51%; HTTP/1.x persists near 28%.

The plateau correlates with **the 2024 ACM Web Conference paper "QUIC is not Quick Enough over Fast Internet"** (Zhang et al., doi:10.1145/3589334.3645323) showing **up-to-45.2% throughput regressions** vs HTTP/2 above ~500 Mbps. The cause: receiver-side userspace ACK and copy overhead — QUIC implementations live above the kernel and pay for every packet a context switch the kernel TCP stack does not.

The fix in flight is **in-kernel QUIC**. Xin Long posted the first ~9,000-line in-kernel QUIC patch series for Linux on **22 July 2025** (LWN coverage). The design uses \`IPPROTO_QUIC\` (mirroring \`IPPROTO_MPTCP\`) with the TLS handshake delegated to userspace via \`tlshd\`. Mainline merge is expected 2026 at earliest.`
						},
						{
							type: 'callout',
...
**[[frontier:multipath-quic|Multipath QUIC]]** (draft-ietf-quic-multipath) entered IESG Last Call in December 2025; the latest draft -21 is dated 17 March 2026. Alibaba and Apple have already deployed predecessors. Multipath QUIC inherits MPTCP's algorithmic ideas inside a transport that actually traverses middleboxes.

**HTTP Datagrams and Capsules** ([[rfc:9000|RFC 9297]], August 2022, D. Schinazi & L. Pardue) standardised unreliable datagrams over HTTP/3, enabling MASQUE and WebTransport.
...
**[[frontier:ech-rfc-9849|Encrypted Client Hello (ECH)]]** was approved by the TLS WG and entered the RFC editor queue in 2025 (RFC 9849-track, IANA registry allocated 2025-07-30). Cloudflare turned it on for ~70% of its zones; Russia began censoring ECH connections; major browsers ship ECH gated by HTTPS DNS records (RFC 9460).
					...
					sections: [
						{
							...
							text: `In **June 2000**, **[[pioneer:roy-fielding|Roy Thomas Fielding]]** filed his PhD dissertation at UC Irvine: *"Architectural Styles and the Design of Network-Based Software Architectures."* Chapter 5 described the architectural style behind [[http1|HTTP]]. He named it **REST** — Representational State Transfer.
...
[[rest|REST]]-style APIs became the default for web services because they inherited HTTP's caching, status codes, and tooling for free. By 2010, "REST API" was the answer to "how should two services on the internet talk to each other" for nearly every problem domain.`
						...
						},
						{
							...
							text: `The trouble with REST emerged at scale, specifically on mobile.

A mobile app loading a user profile might need to call \`/users/123\`, \`/users/123/posts\`, \`/posts/[ids]/comments\`, and \`/users/[ids]\` — four round-trips for a single screen. On a 4G connection with 100 ms RTT, that is 400 ms of latency before the app can render anything. Each individual REST endpoint was clean; the **chain of endpoints required to populate one screen** was the problem.

Facebook's mobile team hit this wall in 2012 and built [[graphql|GraphQL]] to solve it: a single endpoint where the client describes **exactly what data it wants** and the server returns exactly that, in one round-trip. Facebook open-sourced the spec in 2015. The current edition is **GraphQL September 2025** (https://spec.graphql.org/September2025/), maintained by the GraphQL Foundation under the Linux Foundation.

GraphQL deliberately *does not* mandate a transport. The de-facto binding is **GraphQL-over-HTTP** (https://graphql.github.io/graphql-over-http/draft/), typically POST \`/graphql\` for mutations and queries — though GET is allowed for cacheable, idempotent queries. JSON ([[rfc:9110|RFC 8259]]) is the default body format.`
						},
						{
							type: 'narrative',
							title: 'Subscriptions, and the Two Patterns',
							text: `GraphQL has three operation types: query, mutation, and **subscription**. Subscriptions need a long-lived connection — the server has to push events to the client over time. Two transports compete.

**graphql-ws over [[websockets|WebSocket]]** (https://github.com/enisdenjo/graphql-ws) — the original. Bidirectional WebSocket carries JSON-RPC-shaped messages. Works everywhere; doesn't compose well with HTTP/2 multiplexing or HTTP caches.

**[[sse|Server-Sent Events]]** (text/event-stream over HTTP) — increasingly preferred. Survives proxies that mangle WebSocket upgrades, composes with HTTP/2, inherits HTTP auth, supports auto-reconnection out of the box. Tools like \`gqlgen\` and \`graphql-yoga\` default to SSE for new projects.
...
The model lets you describe a service as Go-like methods (\`rpc GetUser(UserRequest) returns (User);\`) and compile clients and servers in any of a dozen languages. **Streaming methods** come in four flavours: unary (one request, one response), server-streaming (one request, many responses), client-streaming (many requests, one response), and bidirectional (many of both, interleaved on a single stream). Every binding is type-checked at compile time.`
						...
						},
						{
							...
							text: `gRPC was one of the implementations affected by **CVE-2024-27316 "HTTP/2 CONTINUATION Flood"** (April 2024) and **CVE-2025-8671 "MadeYouReset"** (August 2025). Because gRPC builds on HTTP/2, every HTTP/2 vulnerability ripples through every gRPC implementation.
...
Active 2024-2026 work in the gRPC working group includes: native HTTP/3 support (currently experimental, gated on widespread server-side QUIC); a clearer story for cancellation propagation across streaming methods; and improved interop with OpenTelemetry traces.`
						}
					]
				},
...
**[[websockets|WebSockets]]** ([[rfc:6455|RFC 6455]], December 2011, edited by Ian Fette of Google and Alexey Melnikov of Isode) hijack an HTTP/1.1 connection with an \`Upgrade: websocket\` request and switch the connection to a bidirectional binary frame protocol. After the upgrade, neither side waits for the other — both can send any time. **~99% of browsers support it since 2012.** Slack, Discord, Figma, and most live-collaboration apps use WebSockets. **Slack documents 5M+ concurrent WebSocket sessions** in production; **Cloudflare's WebSocket Hibernation API** for Durable Objects bills only when JS executes and survives idle WebSocket connections without server cost.

**[[sse|Server-Sent Events]]** is the simpler one-way version, specified by **Ian Hickson and first shipped in Opera 9 in September 2006**. The server holds open an HTTP response and writes \`data:\` frames over time. No protocol switch, no binary framing — just a long-lived response stream with \`Content-Type: text/event-stream\`. Currently defined in **§9.2 of the HTML Living Standard**.`
						},
						{
							type: 'narrative',
							title: 'The SSE Renaissance',
							text: `For most of its life SSE lost mindshare to WebSocket. The 2010s narrative was *"WebSockets are the future of real-time."*
...
- It is **HTTP**. CORS, auth, caching, proxies, CDNs all just work.
...
- It **composes with HTTP/2 multiplexing** — many SSE streams share one TCP connection.
...
WebSocket is the right answer when the client genuinely needs to send messages back over the same connection at high frequency: chat with interrupts, collaborative editing, multiplayer gaming. SSE is the right answer when the workload is server→client only.`
						...
						},
						{
							...
							text: `Both WebSocket and SSE were designed for HTTP/1.1. The HTTP/2 bootstraps for both ([[rfc:6455|RFC 8441]] for WebSocket, RFC 8442 for SSE) ship in Firefox 65+, Chrome, and major servers, but adoption is uneven (Mattermost documented Chrome 95+ failures behind HTTP/2 proxies that don't translate Extended CONNECT). The HTTP/3 bootstrap (RFC 9220) exists but is barely deployed.

The longer-term replacement is **WebTransport over HTTP/3** — bidirectional streams + unreliable datagrams over QUIC, exposed to JavaScript as a Promise-based API. Chrome and Edge ship implementations as of 2026; Safari has no support; Firefox has limited support. WebTransport is interesting for *some* workloads (gaming, low-latency bidirectional, datagrams for game state) but not a wholesale replacement for either WebSocket or SSE before 2027-2028.
...
**SSE is not gone in MCP** — it is now an *optional response mode* of Streamable HTTP. WebSocket transport is **not** an official MCP transport as of May 2026, but **SEP-1288** (August 2025) is an active proposal to add one; the controversy is around how to convey \`Mcp-Session-Id\` (browser WebSocket APIs cannot read post-handshake headers) and authentication.`
						},
						{
							type: 'callout',
						...
						},
						{
							...
							text: `[[a2a|A2A]] — the **Agent2Agent Protocol**, published by Google on **9 April 2025** — handles the case MCP did not address: collaboration **between agents** rather than between an agent and a tool. Capability discovery, task delegation, asynchronous event streams, agent-to-agent authentication.

A2A runs at L7 over HTTP(S) over TCP/IP, like MCP. It uses **JSON-RPC 2.0 + HTTP(S) + SSE** — explicitly not WebSocket. Custom HTTP headers like \`A2A-Version\` and \`A2A-Extensions\` carry control metadata. Authentication leans on **TLS 1.3 + OAuth 2.0** ([[rfc:8446|RFC 8446]] + RFC 6749) rather than defining its own scheme.
