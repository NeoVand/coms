...
**{{congestion-control|Congestion control}}** cuts your sending rate the moment a packet is lost, even if the loss was unrelated to congestion. On wireless networks where packets drop from radio interference rather than queue overflow, classic TCP overreacts — which is why **BBR** (Google, 2016) modelled the network instead of inferring from loss, and why **L4S** ([[frontier:l4s-comcast-launch|RFC 9330/31/32]], 2023) replaced loss with explicit ECN signalling for cooperating flows.`
						...
						},
						{
							...
							text: `The single most important change in [[tcp|TCP]]'s history is the **1988 {{congestion-avoidance|congestion avoidance}}** work by [[pioneer:van-jacobson|Van Jacobson]] and Mike Karels — {{slow-start|slow start}}, {{aimd|AIMD}}, fast retransmit, {{exponential-backoff|exponential backoff}} — that prevented the [[outage:nsfnet-1986-collapse|1986 collapse]] from repeating. The single most important change since is **{{cubic|CUBIC}}** ([[rfc:9438|RFC 9438]], default in Linux/Windows/Apple stacks since the late 2000s) and **BBR** (Google, 2016, [[frontier:bbrv3-default|now BBRv3]] for google.com and YouTube traffic).

The story of TCP, more than any other transport, is the story of {{congestion-control|congestion control}}. Everything else — {{flow-control|flow control}}, error recovery, connection state — settled by 1988. The active research has moved through Reno, NewReno, Vegas, CUBIC, Compound, BBR v1/v2/v3, and now [[frontier:l4s-comcast-launch|L4S]] with TCP Prague. Each generation refined the inference about network state from increasingly sparse signals.

The most recent paradigm shift: L4S replaces loss-as-signal with **explicit ECN marking**. Cooperating senders mark packets ECT(1); routers put them in a separate isolated queue and mark Congestion Experienced *early*, before the queue grows. The result is sub-millisecond queuing {{latency|latency}} at full link utilisation. **Comcast launched L4S in production January 2025**; Apple iOS 17 / macOS Sonoma defaulted L4S support for [[quic|QUIC]].`
						},
						{
							type: 'narrative',
							title: 'What Shipped in 2024-2026',
							text: `**Linux 6.7 (January 2024)** merged native **TCP-AO ([[rfc:9000|RFC 5925]])** — five thousand lines of new networking code finally giving Linux a modern replacement for the deprecated TCP-MD5 used by [[bgp|BGP]] and LDP. The same release added microsecond-resolution [[tcp|TCP]] timestamps.

**AccECN** (\`draft-ietf-tcpm-accurate-ecn-34\`, March 2025) is on the Standards Track path. It reallocates the old ECN-{{nonce|Nonce}} bit to deliver more than one congestion signal per RTT — the precondition L4S over TCP needs for fine-grained congestion response.

**Linux 6.15 (mid-2025)** landed **io_uring zero-copy receive** (\`io_uring zcrx\`) integrated with the kernel TCP stack. Single-flow throughput jumped from ~74 Gb/s (epoll) to **~106 Gb/s (io_uring zcrx)** — a ~40% throughput improvement for high-{{bandwidth|bandwidth}} servers without any application changes.

The vulnerability surface keeps producing CVEs. **CVE-2019-11477 ({{sack|SACK}} Panic)** was the canonical case — a single TCP packet, no authentication, would crash any vulnerable Linux host across kernels 2.6.29 through 5.1, ten years of unpatched code in the heart of every Linux server. Modern kernel networking is now fuzzed continuously by **syzkaller**; most TCP CVEs since 2020 have been found by fuzzing rather than by humans.`
						}
					]
				},
...
[[rfc:768|RFC 768]] is **three pages long**. It was published in August 1980 by [[pioneer:jon-postel|Jon Postel]], two months before the first version of [[tcp|TCP]] (RFC 793, September 1981). It has not been updated since. There has been nothing to update — [[udp|UDP]] works, and the entire weight of "what does this protocol need to do?" sits with the application above it.`
						},
						{
							type: 'narrative',
...
- **[[dns|DNS]]** — every lookup is one datagram each way. ~14 trillion queries per day on Google Public [[dns|DNS]] alone. The persistent connection model [[tcp|TCP]] gives you is overhead a recursive resolver does not need; the connection per query model UDP gives you scales to root-server volumes.
- **[[ntp|NTP]]** — time correction has to be precise to the microsecond. You cannot tolerate {{handshake|handshake}} delay; you cannot tolerate {{retransmission|retransmission}} timing variance. UDP delivers a packet in a few milliseconds and lets the protocol math figure out clock {{offset|offset}} from RTT.
- **[[dhcp|DHCP]]** — you do not have an {{ip-address|IP address}} yet. You cannot do TCP. UDP {{broadcast|broadcast}} is the only way a host without an address can ask the network for one.
- **[[rtp|RTP]]** — a dropped audio packet should be ignored, not retransmitted. Late audio is worse than missing audio.
- **[[quic|QUIC]]** — the entire next-generation transport runs *inside* UDP datagrams to escape kernel ossification. By 2026, [[quic|QUIC]] carries ~21% of Cloudflare-observed web traffic and >75% of Meta's internet bytes. UDP's role as the QUIC substrate has made it the fastest-growing protocol on the internet by relative volume.
...
The single thing UDP gives you above raw IP is **ports** — the 16-bit demux that picks which application receives the packet on a given host. That is the entire reason multiple applications can share a host's network adapter. It is most of what L4 needs to be.`
						...
						},
						{
							...
							text: `Almost all internet [[udp|UDP]] traffic growth in the last five years has been [[quic|QUIC]]. Where UDP used to be a niche transport ([[dns|DNS]], [[ntp|NTP]], [[rtp|RTP]]), it now carries the majority of [[http3|HTTP/3]] traffic plus the entire next generation of media transports — [[frontier:moq-transport|MoQ Transport]] over [[quic|QUIC]], RTP-over-QUIC (\`draft-ietf-avtcore-rtp-over-quic-14\` in WG Last Call July 2025), HTTP/3 datagrams (RFC 9297), {{masque|MASQUE}} (RFCs 9298/9484).
...
The longer arc: **in-kernel QUIC** (Xin Long's 9,000-line patch series, July 2025) puts QUIC into the kernel as \`IPPROTO_QUIC\`, mirroring \`IPPROTO_MPTCP\`. Mainline merge expected 2026. When that ships, QUIC will run alongside [[tcp|TCP]] at kernel speeds — and UDP will become an even larger share of the internet's transport mix.
...
**Message-orientation** — data is delivered as discrete messages, not a stream of bytes that the application has to re-frame. The application's send is the application's recv on the other side.`
						},
						{
							...
							text: `[[sctp|SCTP]] is, on paper, the better protocol. It powers SS7-over-IP and **Diameter** (the LTE/5G signalling stack) and a few specialised use cases. But it failed to displace [[tcp|TCP]] for general use because **NAT and {{firewall|firewall}} middleboxes do not understand it**.
...
The deeper lesson SCTP teaches is the lesson [[quic|QUIC]] applied: **if you want a new transport on the deployed internet, you must tunnel inside [[udp|UDP]]**. SCTP did not, and was confined to controlled networks. [[quic|QUIC]] did, and is rapidly becoming the default. Multipath QUIC ([[frontier:multipath-quic|IETF Last Call December 2025]]) brings SCTP-style multi-homing into a transport that actually traverses middleboxes.`
						},
						{
							type: 'callout',
							title: 'WebRTC Data Channels are SCTP under the hood',
							text: 'There is one place [[sctp|SCTP]] runs successfully on the open internet: **[[webrtc|WebRTC]] Data Channels**. [[rfc:8831|RFC 8831]] defines the data channel as **[[sctp|SCTP]] over {{dtls|DTLS}} over [[udp|UDP]]** — the SCTP-over-something-else trick [[quic|QUIC]] would later generalise. The browser implementations (libwebrtc, Firefox\'s networking stack) carry an [[sctp|SCTP]] stack in user space. [[webrtc|WebRTC]] is the largest production SCTP deployment by message count, even though almost nobody knows it.'
						},
						{
							type: 'narrative',
							title: 'What Survived',
							text: `Most of [[sctp|SCTP]]'s good ideas survived through descendants. Multi-streaming and {{connection-migration|connection migration}} are core to [[quic|QUIC]]. Multi-homing is what [[mptcp|MPTCP]] approximates for [[tcp|TCP]] and what [[frontier:multipath-quic|multipath QUIC]] is generalising. Message-orientation is the default in modern application protocols ([[http2|HTTP/2]] and [[http3|HTTP/3]] frame the bytes; [[grpc|gRPC]] adds length prefixes; WebSocket has explicit message boundaries).

The protocol itself remains specialised. It is the canonical example of a technically-superior transport that lost on deployment economics — and the canonical justification for [[quic|QUIC]]'s choice to tunnel inside [[udp|UDP]]. Knowing why SCTP failed makes every modern transport-design decision clearer.`
						}
					]
				},
...
That is the [[mptcp|Multipath TCP]] proposition. **[[mptcp|MPTCP]]** (RFC 6824 in January 2013, current RFC 8684 in March 2020) presents a normal [[tcp|TCP]] socket to applications, but underneath it negotiates **subflows** over multiple paths and aggregates their throughput. Sequence numbers and ACKs are coordinated at the MPTCP layer; {{congestion-control|congestion control}} runs per subflow but is coupled (RFC 6356, "LIA" — Linked Increases Algorithm) to prevent over-allocating capacity to the better path.

The application has no idea any of this is happening. The socket interface is identical to a regular [[tcp|TCP]] socket. The kernel does the multipath bookkeeping; the wire format uses TCP options that legacy middleboxes mostly forward unchanged.`
						},
						{
							type: 'narrative',
...
Multipath [[quic|QUIC]] inherits [[mptcp|MPTCP]]'s algorithmic ideas — subflows, coupled {{congestion-control|congestion control}}, packet scheduling across paths — but operates inside QUIC's much more deployable carrier ([[udp|UDP]]). Where MPTCP had to fight middleboxes that didn't understand [[tcp|TCP]] options, multipath QUIC encrypts everything except a handful of public bits inside the UDP envelope. Middleboxes see UDP; the multipath logic is invisible.

**Apple, Alibaba, and Tessares have already deployed predecessors** (gQUIC multipath at Google, Apple's iCloud sync, Alibaba's mobile e-commerce). Once multipath QUIC ships in mainline implementations (quiche, mvfst, quinn, msquic), it becomes the natural multipath transport for [[http3|HTTP/3]].
...
**MASQUE WG** (RFC 9298 — Proxying [[udp|UDP]] in HTTP, August 2022; RFC 9484 — Proxying IP in HTTP, October 2023) ships CONNECT-UDP and CONNECT-IP. Apple Private Relay and Cloudflare's WARP-related proxy services use these.

**[[frontier:moq-transport|MoQ Transport]]** (\`draft-ietf-moq-transport-17\`, March 2026) is the first IETF media transport that intentionally is not [[rtp|RTP]] — sub-second live streaming with one-to-many {{pub-sub|publish/subscribe}} at CDN scale.

**RTP-over-QUIC (RoQ)** (\`draft-ietf-avtcore-rtp-over-quic-14\`) entered Working Group Last Call in July 2025 — preserves the entire RTP ecosystem while gaining QUIC's {{encryption|encryption}}, NAT-friendliness, and {{zero-rtt|0-RTT}}.
