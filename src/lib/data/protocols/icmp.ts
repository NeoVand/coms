...
export const icmp: Protocol = {
	...
	rfc: 'RFC 792',
	oneLiner:
		'The diagnostic protocol behind {{ping|ping}} and {{traceroute|traceroute}} — how the network reports errors.',
...
[[ipv6|IPv6]] uses a separate specification called {{icmpv6|ICMPv6}} ({{rfc-doc|RFC}} 4443) with different type numbers and additional functionality. {{icmpv6|ICMPv6}} is more critical than its [[ip|IPv4]] counterpart because it incorporates {{ndp|Neighbor Discovery Protocol}} ({{ndp|NDP}}), which replaces [[arp|ARP]] for address resolution and handles router discovery, address autoconfiguration, and duplicate address detection.`,
	howItWorks: [
		...
		},
		{
			...
			description:
				"When a packet's {{ttl|TTL}} reaches zero, the router sends Type 11 back. {{traceroute|Traceroute}} exploits this by sending packets with incrementing {{ttl|TTL}} values (1, 2, 3...) to discover each {{hop|hop}}."
		},
		{
			title: 'Redirect',
			description:
...
def ping(host):
    ...
    print(f"Reply from {host}: time={rtt:.1f}ms")`,
		caption:
			'[[icmp|ICMP]] requires raw sockets — it operates at the network layer, below [[tcp|TCP]]/[[udp|UDP]]',
		alternatives: [
			{
				language: 'cli',
...
ICMP Echo Reply:
  ...
  Type: 8, Code: 0, ID: 0x1234, Seq: 1`
	...
	image: {
		...
		caption:
			'A DEC VT100 terminal — the type of terminal where early network administrators ran {{ping|ping}} and {{traceroute|traceroute}}, the quintessential [[icmp|ICMP]] diagnostic tools. [[icmp|ICMP]] was defined in 1981, and these terminals were the window into the network.',
