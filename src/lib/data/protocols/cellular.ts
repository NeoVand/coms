...
export const cellular: Protocol = {
	...
	oneLiner:
		'The 3GPP radio family that gets a phone an IP address from a base station 50 km away — every modern smartphone, every IoT cellular module, ~9 billion subscriptions, the largest wireless deployment on Earth.',
	overview: `**Cellular** in 2026 is two protocols braided into one ecosystem: **4G LTE** (3GPP Release 8, December 2008 — still the universal floor) and **5G NR** (Release 15, June 2018 — the current normative baseline). Both share the 3GPP standards body, the same air-interface design philosophy (OFDMA + flexible numerology + HARQ), the mandatory [[ipsec|IPsec]] envelope on every backhaul link, and an [[ipv6|IPv6]] mandate that has been quietly migrating every major carrier's user-plane to IPv6-only since ~2020. We treat them as one encyclopedia node the way [[bluetooth|Bluetooth Classic + BLE]] is one node — the SIG and the radio diverge but the consumer story is unified.

The radio stack is the headline. **PHY** (3GPP TS 38.211–214) carries OFDMA with five numerologies — subcarrier spacings of 15, 30, 60, 120, and 240 kHz — letting the same protocol address sub-6 GHz mid-band (FR1) and mmWave (FR2). **MAC** does hybrid ARQ over 8-process stop-and-wait. **RLC** handles segmentation and reassembly across 10/16-bit sequence numbers. **PDCP** does header compression (ROHC) and AES-CTR ciphering. **RRC** drives the connection state machine — \`RRC_IDLE → CONNECTED → INACTIVE\` for 5G — and **NAS** carries mobility, authentication, and session management end-to-end between the UE and the core. Above all that, the user plane is just [[ip|IP]] (almost always [[ipv6|IPv6]] now); above *that*, the application runs whatever protocols ordinary internet applications run.

The Core Network is where the architectural revolution between 4G and 5G actually lives. **EPC** (Evolved Packet Core, LTE) is a small zoo of named monolithic boxes — MME, SGW, PGW, HSS, PCRF, all glued together by GTP and Diameter. **5GC** (5G Core, NR-SA) is a **service-based architecture** where dozens of network functions (AMF, SMF, UPF, AUSF, UDM, PCF, NRF, NEF, NSSF, AF…) talk to each other over **[[http2|HTTP/2]] with JSON payloads protected by [[tls|TLS]]**. The control plane of every modern carrier on Earth is now an [[http2|HTTP/2]] microservice fabric. Every N2/N3 interface between the radio access network and the core is wrapped in [[ipsec|IPsec ESP]] per 3GPP TS 33.501 — the single largest enterprise [[ipsec|IPsec]] deployment on Earth runs inside this layer.

The frontier in 2026 is **5G-Advanced** (Release 18, frozen June 2024; Release 19 in progress; Release 20 study items for 6G already kicking off in 2025), **Open RAN** deployments (Vodafone UK, Deutsche Telekom, Rakuten Symphony, DISH on AWS Wavelength), and **satellite direct-to-cell** — T-Mobile + SpaceX Starlink launched commercial service in January 2025; AT&T's AST SpaceMobile partnership and Apple's Globalstar-based Emergency SOS are reshaping what "no signal" means.`,
	howItWorks: [
		{
			title: 'PHY — OFDMA with five numerologies',
			description:
				"5G NR carries data on **Orthogonal Frequency-Division Multiple Access** subcarriers spaced at 15, 30, 60, 120, or 240 kHz (3GPP TS 38.211). The choice is the *numerology* — smaller spacing = longer symbols = more robust at low frequencies; larger spacing = shorter symbols = required at mmWave. One framework, two very different deployment regimes (FR1 sub-6 GHz, FR2 mmWave 24–52 GHz)."
		},
		{
			title: 'MAC — Hybrid ARQ',
			description:
				"**HARQ** combines forward error correction with retransmission. The receiver stores soft-decoded LLRs from failed transmissions and combines them with the retransmitted copy (chase-combining or incremental-redundancy). 8 parallel stop-and-wait processes per UE (HARQ process ID is 3 bits) keep the pipe full without head-of-line blocking. Why cellular gets 99.999% reliability without [[tcp|TCP]]'s retransmit cost on the link."
		},
		{
			title: 'RLC + PDCP — sequence numbers, ciphering, header compression',
			description:
				'**RLC** runs in TM (transparent), UM (unacknowledged), or AM (acknowledged) mode with 10- or 16-bit sequence numbers (TS 38.322). **PDCP** above it (TS 38.323) does ROHC header compression (squashing the 40-byte IPv6+TCP/UDP header to 1–4 bytes), AES-CTR ciphering, and 32-bit anti-replay. The cipher is keyed off K_gNB, derived from the AKA authentication.'
		},
		{
			title: 'RRC — connection state machine',
			description:
				'The Radio Resource Control state machine (TS 38.331) has three states in 5G: \`RRC_IDLE\` (UE sleeps, only listens to paging), \`RRC_INACTIVE\` (5G-only; UE keeps security context for fast resume), \`RRC_CONNECTED\` (full bearer, scheduled). State transitions cost battery — careful RRC tuning is the difference between 6-hour and 24-hour battery life on an IoT module.'
		},
		{
			title: 'NAS — authentication, mobility, session management',
			description:
				"**Non-Access Stratum** signalling (TS 24.501) carries end-to-end between UE and core, transparently through the gNB. **Registration Request** (with **SUCI** — the public-key-encrypted SUPI) starts the AKA handshake; the UDM decrypts to SUPI, generates an authentication vector, and the UE's USIM verifies AUTN and computes RES*. After Security Mode Command, NAS is integrity-protected and ciphered with K_NASint / K_NASenc."
		},
		{
			title: '5GC service-based architecture',
			description:
				"The 5G Core is a microservice fabric. AMF, SMF, UPF, AUSF, UDM, PCF, NRF, NEF, NSSF, AF — each is a **network function** with an [[http2|HTTP/2]]+JSON API protected by [[tls|TLS]]. Service-based interfaces are named Nausf, Nudm, Namf, Nsmf, Npcf, Nnrf, Nnef, Nnssf, Naf. The control plane of every 5G carrier on Earth is now an [[http2|HTTP/2]] microservice fabric — and every backhaul hop is wrapped in [[ipsec|IPsec ESP]]."
		},
		{
			title: 'GTP-U on N3 — the user-plane tunnel',
			description:
				"User-plane packets between the gNB and the UPF travel over **GTP-U** (GPRS Tunnelling Protocol — User plane) on UDP/2152. Each PDU session gets a Tunnel Endpoint Identifier (TEID). The tunnel preserves the UE's [[ip|IP]] address as the inner packet's source/destination regardless of which gNB the UE is camping on — this is how a phone keeps its [[ip|IP]] across handovers between base stations."
		}
	],
	useCases: [
  ...
  ... whatever the UE is sending ...`
	...
	recentChanges: [
		...
		},
		{
			...
			description:
				'After 18 months of beta, **T-Satellite by T-Mobile** opened to the public in January 2025 — every modern phone with band n25/n26 can connect to a low-Earth-orbit Starlink satellite for SMS and emergency messaging without any app. AT&T + AST SpaceMobile and Apple\'s Globalstar Emergency SOS round out the satellite-direct-to-cell story. The first time "no signal" stops meaning *no signal*.',
			source: {
		...
		},
		{
			...
			description:
				'After being specified in Release 17 (2022), the first NR-NTN deployments shipped in late 2024 — the radio-layer protocol that lets standard 5G NR talk to a satellite at 600 km without proprietary modulation. Combined with Apple\'s announced iPhone 17 satellite-NR support (2025), this closes the gap between "phone radio" and "satellite radio" for the first time.',
			source: {
	...
	realWorldDeployments: [
		{
			...
			description:
				"The largest wireless protocol family by user count. Specified collectively by 3GPP — a partnership of ETSI (Europe), ARIB + TTC (Japan), ATIS (North America), CCSA (China), TSDSI (India), TTA (Korea). Every cellular phone on Earth runs 3GPP protocols at the radio layer."
		},
		{
			org: 'Reliance Jio',
		...
		},
		{
			...
			description:
				'The first commercial **satellite direct-to-cell** service. Standard band n25/n26 phones connect to Starlink satellites in low-Earth orbit for SMS and emergency. Apple\'s Globalstar partnership and AT&T\'s AST SpaceMobile follow similar patterns. Reshapes "coverage" as a concept.'
		},
		{
