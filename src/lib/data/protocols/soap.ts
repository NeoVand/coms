	...
	howItWorks: [
		{
			...
			description:
				'Client fetches the {{wsdl|WSDL}} document from the service endpoint (typically at ?{{wsdl|wsdl}}). The {{wsdl|WSDL}} describes all available operations, their input/output message schemas, data types, and the endpoint {{url|URL}} — everything needed to {{generate-ac|generate}} client code.'
		},
		{
		...
		},
		{
			title: 'HTTP POST',
			description:
				'The complete {{xml|XML}} envelope is sent as an {{http-method|HTTP}} POST request with {{content-type|Content-Type}}: text/xml ([[soap|SOAP]] 1.1) or application/soap+xml ([[soap|SOAP]] 1.2). Unlike [[rest|REST]], [[soap|SOAP]] always uses POST regardless of whether the operation reads or writes data.'
		},
		{
			title: 'Response or Fault',
			description:
				'Server processes the request and returns a [[soap|SOAP]] response envelope containing the result in the Body. If an error occurs, the server returns a [[soap|SOAP]] Fault element with a fault code, fault string, and optional detail — the [[soap|SOAP]] equivalent of {{http-method|HTTP}} error status codes.'
		}
	],
	useCases: [
...
curl https://example.com/service?wsdl`
	...
	image: {
		...
		caption:
			'The {{ibm|IBM}} System/360 at the USDA (1966). Mainframes like these laid the groundwork for enterprise computing — the world that [[soap|SOAP]] was built to serve, wrapping remote procedure calls in {{xml|XML}} for cross-platform interoperability.',
