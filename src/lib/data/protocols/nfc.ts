...
[[nfc|NFC]] supports **four operating modes**: *Reader/Writer* (phone reads a passive tag), *Card Emulation* (phone presents itself as a card via {{ese|Secure Element}} on {{apple|Apple}} Pay or via {{hce|Host Card Emulation}} on {{android|Android}} 4.4+ / iOS 17.4+ EEA), *{{peer-to-peer|Peer-to-Peer}}* (largely deprecated), and *Wireless Charging* (added 2020). On top of NFC's transport sit several full vertical stacks: ISO/IEC 7816-4 {{apdu|APDUs}} and **EMVCo Contactless** Books C-1 through C-7 + Book E (cards and payments); **ICAO Doc 9303 Part 11** BAC/PACE (e-passports); {{ccc-digital-key|CCC Digital Key}} (vehicles); {{aliro|Aliro 1.0}} (access control, finalised Feb 2026); NFC Forum WLC (wireless charging); and {{matter|Matter}} 1.3+ NFC commissioning.
...
reader.onreadingerror = () => console.warn('cannot read — tag empty or unsupported');`,
		caption:
			"The Web NFC API exposes only {{ndef|NDEF}} read/write — no card emulation, no raw 14443 — and only on {{android|Android}} Chromium. {{apple|Apple}} has not implemented it. The {{handshake|handshake}} (REQA → {{atqa|ATQA}} → SEL → SAK → RATS → ATS → READ {{ndef|NDEF}} TLV) happens inside the platform's NFC controller; the page sees only the {{ndef|NDEF}} abstraction.",
		alternatives: [
			{
				language: 'python',
...
Total airtime from field-on to ARQC: ~300–800 ms on a real terminal.`
	...
	recentChanges: [
		...
		},
		{
			...
			description:
				"On **26 February 2026** the Connectivity Standards Alliance — the same group that runs {{matter|Matter}} — finalised {{aliro|Aliro 1.0}}, a PKI-based access-control credential standard. {{ecdsa|ECDSA}} mutual authentication, support for NFC tap-to-access, BLE proximity, and BLE+[[uwb|UWB]] ranged unlock, with credentials provisioned into {{apple|Apple}}/{{google|Google}}/Samsung wallets. First certifications: {{apple|Apple}}, Allegion, Aqara, {{google|Google}}, HID, Kastle, Kwikset, Last Lock, Nordic, Nuki, NXP, Qorvo, Samsung, STMicro. Target verticals: corporate, hospitality, residential, multi-family, university.",
			source: {
