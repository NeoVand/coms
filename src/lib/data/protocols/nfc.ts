...
reader.onreadingerror = () => console.warn('cannot read — tag empty or unsupported');`,
		caption:
			"The Web NFC API exposes only {{ndef|NDEF}} read/write — no card emulation, no raw 14443 — and only on {{android|Android}} Chromium. {{apple|Apple}} has not implemented it. The {{handshake|handshake}} (REQA → ATQA → SEL → SAK → RATS → ATS → READ {{ndef|NDEF}} TLV) happens inside the platform's NFC controller; the page sees only the {{ndef|NDEF}} abstraction.",
		alternatives: [
			{
				language: 'python',
