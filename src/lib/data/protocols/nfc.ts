	...
	howItWorks: [
		{
			...
			description:
				"All NFC variants operate at a carrier of 13.56 MHz ± 7 kHz, the unlicensed {{ism-band|ISM}} allocation that is the legacy of ISO 14443. Coupling is *inductive* (magnetic) between two loop antennas — the magnetic field falls off as 1/r³, vs 1/r² for far-field radiative coupling. Effective range is ≤10 cm for ISO 14443 (NFC-A/B/F) and historically ≤1 m for ISO 15693 (NFC-V). The PCD (Proximity Coupling Device — the reader) energises its loop antenna; the passive PICC (Proximity Integrated Circuit Card) harvests power from the field and communicates back via {{load-modulation|load modulation}} — switching a resistor on its own antenna at an 847.5 kHz subcarrier (13.56 MHz/16), perceived by the reader as small amplitude/phase changes in its own resonant loop. Modern phones use *active load modulation* (ALM) instead, generating a small reflected carrier — which is why an iPhone can be read across a metal-backed case where a plain plastic card cannot."
		},
		{
		...
		},
		{
			...
			description:
				"When a PICC enters the field it begins in IDLE. The reader broadcasts a 7-bit REQA (0x26) or WUPA (0x52) short frame; the card responds with **ATQA** (2 bytes) declaring its UID size (4/7/10 bytes) and anti-collision support. The reader then runs the **bit-frame anti-collision** loop with SEL+NVB frames (SEL=0x93 for cascade level 1, 0x95 for CL2, 0x97 for CL3), converging on each byte of the UID a bit at a time when multiple cards are in the field. When the UID is complete the card answers with **SAK**; if bit 6 of SAK is set, the card supports ISO 14443-4 and the reader proceeds with RATS→ATS to negotiate frame size (FSCI) and timing (FWI). T2T cards skip RATS and go straight to T2T READ commands."
		},
		{
			title: 'Card Emulation: SELECT PPSE → SELECT AID → GET PROCESSING OPTIONS → GENERATE AC',
			description:
				"For card-emulation mode (Apple Pay, transit, access), once 14443-4 is established the reader speaks **ISO 7816-4 APDUs**: 4-byte header `CLA INS P1 P2` + optional command body. The first command is *SELECT PPSE* — the Proximity Payment System Environment AID `2PAY.SYS.DDF01` — and the card returns an FCI listing all supported {{aid|payment AIDs}} in priority order. The reader picks one (e.g. Mastercard `A0000000041010`), SELECTs it, gets back a PDOL listing parameters the card needs (amount, currency, country, terminal-type, unpredictable number), then sends *GET PROCESSING OPTIONS* with those parameters. The card returns AIP+AFL telling the reader which files to read; READ RECORDs pull the PAN, expiry, and public-key certificate chain. Finally *GENERATE AC* with CDOL1 data asks the card for an {{emv-cryptogram|Application Cryptogram}} — either an ARQC (online) or TC (offline). The cryptogram is signed in the eSE/HCE app and is what proves the transaction to the issuer."
		},
		{
			title: 'NDEF: the data format for everything that is not a payment',
			description:
				"NFC Forum **NDEF** (NFC Data Exchange Format) is the binary record container that lives in tags and rides over LLCP/SNEP. Each record begins with a 1-byte header — bits MB/ME (Message Begin/End), CF (Chunk Flag), SR (Short Record — 1-byte length vs 4), IL (ID Length present), and a 3-bit TNF (Type Name Format: 0=Empty, 1=Well-Known like `U` for URI or `T` for Text, 2=MIME media, 3=Absolute URI, 4=External, 5=Unknown). The URI Well-Known record uses a single-byte prefix shorthand — 0x03 for `https://` saves 8 bytes per record on tags as small as 48 bytes. NDEF was formally adopted as an IEC standard in **March 2026** alongside NFC-WLC."
		},
		{
			title: 'Three transports, one tap: Connection Handover to Bluetooth / Wi-Fi / Matter',
			description:
				"For higher-throughput sessions NFC is almost always a *bootstrap*. The **Connection Handover** spec (v1.5) defines NDEF records of TNF=0x02 with MIME `application/vnd.bluetooth.le.oob` carrying the [[bluetooth|Bluetooth]] MAC address, name, and Security Manager OOB key — a single tap replaces a discovery/pairing dialog. The parallel Wi-Fi handover record carries SSID/key/security mode — used for tap-to-join on printers and some smart-plug commissioning. **Matter 1.3+** adds NFC as one of the three permitted commissioning paths alongside QR and BLE. **CCC Digital Key 3.0/4.0** uses NFC to bootstrap a credential into a phone, then BLE for proximity and UWB for centimetre-accurate ranging. Aliro 1.0 likewise spans NFC tap-to-access + BLE proximity + BLE/UWB ranged — three transports, one credential."
		},
		{
...
reader.onreadingerror = () => console.warn('cannot read — tag empty or unsupported');`,
		caption:
			"The Web NFC API exposes only NDEF read/write — no card emulation, no raw 14443 — and only on Android Chromium. Apple has not implemented it. The handshake (REQA → ATQA → SEL → SAK → RATS → ATS → READ NDEF TLV) happens inside the platform's NFC controller; the page sees only the NDEF abstraction.",
		alternatives: [
			{
				language: 'python',
...
Total airtime from field-on to ARQC: ~300–800 ms on a real terminal.`
	...
	recentChanges: [
		...
		},
		{
			...
			description:
				"The Car Connectivity Consortium announced **Digital Key 4.0** in July 2025 and tested it at the 13th Plugfest, hosted by Apple. Headline: *cross-version compatibility* — a DK3 phone unlocks a DK4 car and vice versa. **115 vehicle/module products** were certified in 2025 alone, including the first Chinese OEMs (NIO, XPENG, Geely group brands). Devices must support at least one of NFC, BLE, or UWB; NFC remains the fallback that works even on a phone with a dead battery (>5 hours on iPhone reserve power).",
			source: {
		...
		},
		{
			...
			description:
				"On **26 February 2026** the Connectivity Standards Alliance — the same group that runs Matter — finalised {{aliro|Aliro 1.0}}, a PKI-based access-control credential standard. ECDSA mutual authentication, support for NFC tap-to-access, BLE proximity, and BLE+UWB ranged unlock, with credentials provisioned into Apple/Google/Samsung wallets. First certifications: Apple, Allegion, Aqara, Google, HID, Kastle, Kwikset, Last Lock, Nordic, Nuki, NXP, Qorvo, Samsung, STMicro. Target verticals: corporate, hospitality, residential, multi-family, university.",
			source: {
	...
	realWorldDeployments: [
		...
		},
		{
			...
			description:
				"Belgium issued the first ICAO Doc 9303-compliant eMRTD in **2004**. By 2026 ~1 billion are in circulation. The chip is an ISO 14443 PICC running ISO 7816-4 with a small ICAO file system (DG1 MRZ, DG2 photo, EF.SOD signature, EF.COM index, plus optional DG3 fingerprints / DG4 iris). The reader cannot get any data without **BAC** (legacy 3DES from MRZ) or **PACE** (modern ECDH from MRZ or 6-digit CAN). The Digital Travel Credential (DTC-PC/VC) — passport on phone — is in airport pilots in Finland, the Netherlands, and Singapore."
		},
		{
			org: 'NXP MIFARE family',
			scale: '>10 billion MIFARE ICs shipped cumulatively',
			description:
				"Philips (NXP from 2006) shipped MIFARE Classic in **1994**, DESFire EV1 in **2006**, MIFARE Plus in **2011**, DESFire EV2 in 2016, EV3 in the early 2020s. The Classic Crypto1 break in 2007 (Nohl/Plötz/Starbug at 24C3 Berlin) is the canonical 'security by obscurity does not buy two decades' lesson. DESFire EV3 is now standard for new transit and access deployments — full AES, Common Criteria EAL5+, side-channel hardened. Dutch OV-chipkaart's last Classic mainline cards were not retired until 2024 — 17 years after the original break."
		},
		{
			org: 'CCC Digital Key',
			scale: '115 vehicle/module products certified in 2025; BMW first to certify (late 2024); first Chinese OEMs in 2025',
			description:
				"The Car Connectivity Consortium's Digital Key turns a phone into a vehicle key. v1.0 was proprietary; v2.0 standardised NFC; v3.0 added BLE proximity + UWB ranging (so the car can tell *which side* of the door you are on); **v4.0** (announced July 2025) brings cross-version interoperability. BMW + NXP were the first to certify (late 2024). 115 products certified in 2025 alone; Mercedes, Hyundai/Kia/Genesis, Audi (new in 2025), Volvo, Porsche, GM, Ford, plus a wave of Chinese OEMs (NIO, XPENG, Geely group — Volvo, Polestar, ZEEKR, Lynk & Co., smart, Lotus)."
		}
	],
		...
		pitfalls: [
			{
				...
				text: "The single biggest source of 'NFC doesn't work' complaints is poor antenna tuning. The PCD and PICC each present an LC resonant tank around 13.56 MHz; small detuning (e.g. a metal sheet within 5 mm of the antenna) reduces the magnetic coupling factor *k* and starves the PICC's chip and load-modulation amplitude. **Rules of thumb:** target the PICC resonant frequency *1–3 MHz above* 13.56 MHz on the bench — the card-to-reader mutual inductance pulls it down to the design point. Treat any metal sheet within 5 mm of the antenna as catastrophic — phones with metal backs (post-2014) require either a ferrite separator (0.1–0.3 mm thick) or active load modulation in the controller (NXP PN553 onward). Q factor: lower Q broadens the peak for interoperability at the cost of lower peak induced voltage; readers typically Q ≈ 20–30, cards 25–40."
			},
			{
				title: 'HCE AID conflicts and cold-start latency on Android',
				text: "When two installed apps claim the same {{aid|AID}} (e.g. both a banking app and a transit app claiming `A000000003101001`), Android resolves via the `apduservice.xml` category ('payment', 'other') and the user-selected default for that category. Apps declaring a *wildcard AID prefix* can intercept others; Android tightened the checks but `aid-prefix-filter` is still a footgun. **Equally important:** HCE service activation has a non-trivial cold-start latency budget (~70–150 ms), and EMV terminals time out aggressively. Keep the `HostApduService` warm by avoiding heavy initialisation in `processCommandApdu`. Inspect routing with `adb shell dumpsys nfc` (look at the Routing Table / AID routing section) and `adb logcat -s NfcAdapter NfcService NfcDispatcher`."
			},
			{
				title: 'EMV "fast tap" and the AIP byte trap',
				text: "Mastercard's PayPass and Visa's qVSDC contactless flows have two execution profiles: **Mag-Stripe Mode** (legacy, deprecated in modern terminals) and **EMV Mode** (normal 'fast tap' with CDA). The card returns an AIP byte that signals whether DDA/CDA is supported. A common custom-HCE wallet bug: returning an AIP claiming DDA/CDA support but failing to produce a valid signature in `GENERATE AC` — the terminal aborts after ~600 ms with status word `6985`. **Cure:** match the AIP bits exactly to what your cryptogram code can produce, and test with terminals from at least three networks (Visa, Mastercard, Amex) before launch — kernels diverge in their corner cases."
			},
			{
				title: 'HCE relay attacks: the protocol does not know if the phone is in the room',
				text: "{{hce|HCE}} terminates {{apdu|APDUs}} in a normal application, which means a malicious app or an attacker phone close to the victim can relay the entire APDU stream over the internet to a confederate's phone at a terminal. TU Darmstadt's **NFCGate** (2015+) made this trivial. The protocol cannot tell the difference — an APDU stream tunnelled over IP is structurally indistinguishable from a local tap. **Mitigations:** EMV **Relay Resistance Protocol (RRP)** in Kernel 2 v2.6+ binds round-trip timing into the cryptogram (real NFC ≤ 5 ms, internet relay ≥ 50 ms); Apple Pay's eSE architecture sidesteps it entirely. If you ship HCE without RRP, your safety net is *back-end velocity and geo-anomaly monitoring* — not phone-side time bounds."
			}
		]
	}
};
