...
The tradeoff is {{latency|latency}}: buffering several segments means [[hls|HLS]] typically has 6-30 seconds of delay. Low-{{latency|Latency}} [[hls|HLS]] (LL-[[hls|HLS]]) reduces this to 2-4 seconds by using partial segments and preload hints (\`EXT-X-PRELOAD-HINT\`). [[hls|HLS]] is widely used across the streaming landscape — Disney+ relies on it as a primary format, while services like Netflix and YouTube primarily use [[dash|DASH]] but fall back to [[hls|HLS]] for {{apple|Apple}} device compatibility.`,
	howItWorks: [
		...
		},
		{
			...
			description:
				'Player downloads segments sequentially via {{http-method|HTTP}} GET. Segments are cached by CDNs worldwide. The player maintains a buffer of upcoming segments.'
		},
		{
...
#EXT-X-ENDLIST`
	...
	image: {
		...
		caption:
			'The [[hls|HLS]] streaming pipeline — video is encoded at multiple bitrates, segmented into small chunks, and served over standard {{http-method|HTTP}}. The player dynamically switches between quality levels based on network conditions, ensuring smooth playback.',
