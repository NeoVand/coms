...
export const wifi: Protocol = {
	...
	overview: `[[wifi|Wi-Fi]] brings [[ethernet|Ethernet]]-style networking to the airwaves. Instead of transmitting {{frame|frames}} over copper or fiber, [[wifi|802.11]] uses radio frequencies (2.4 GHz, 5 GHz, and now 6 GHz) to send data wirelessly. But air is a shared medium — everyone within range can hear everything — so [[wifi|Wi-Fi]] adds {{encryption|encryption}} ({{wpa2|WPA2}}/{{wpa3|WPA3}}), collision avoidance (CSMA/{{certificate-authority|CA}} instead of [[ethernet|Ethernet]]'s {{csma-cd|CSMA/CD}}), and a more complex frame format with up to four {{mac-address|MAC}} addresses (three typically used): the receiver address (RA), transmitter address (TA), and destination address (DA) — plus an optional fourth address used in wireless bridging (WDS) mode.

The differences from [[ethernet|Ethernet]] are deeper than just "no cable." Because wireless stations can't detect collisions while transmitting (the "hidden node" problem), [[wifi|Wi-Fi]] uses {{rts-cts|RTS/CTS}} {{handshake|handshakes}} and carrier sensing to avoid them. Every frame must be {{ack|acknowledged}} — if the sender doesn't get an {{ack|ACK}}, it {{retransmission|retransmits}}. The {{access-point|access point}} bridges wireless and wired worlds, translating between [[wifi|802.11]] and 802.3 frames so that [[arp|ARP]], [[ip|IP]], and everything above works seamlessly across both.
...
[[wifi|Wi-Fi]] shares the 2.4 GHz {{ism-band|ISM band}} with **[[bluetooth|Bluetooth]]** — the two are the universal coexistence pairing in the [[wifi|Wireless]] category. Modern combo chips ({{apple|Apple}} H-series, {{broadcom|Broadcom}}, Qualcomm) do time-division arbitration at the silicon level so [[wifi|Wi-Fi]] and [[bluetooth|Bluetooth]] don't starve each other. The escape to 5 GHz and 6 GHz on the [[wifi|Wi-Fi]] side has eased the crowding; [[bluetooth|Bluetooth]] stays at 2.4 GHz where every battery-powered consumer device already lives — and so do [[zigbee|Zigbee]] (channels 11–26) and {{thread|Thread}}, both of which use {{ieee-802-15-4|IEEE 802.15.4}} radios that intentionally dodge Wi-Fi's busy channels 1/6/11. Through the **{{matter|Matter}}** bridge, those low-power meshes appear to your home network as ordinary {{ip-address|IP}}-addressable devices over the same Wi-Fi you're already running. The 13.56 MHz [[nfc|NFC]] tap pairs Wi-Fi credentials onto printers and IoT devices via {{ndef|NDEF}} handover, and the 6–9 GHz [[uwb|UWB]] radio sits well above the Wi-Fi bands and never contends with it.`,
	howItWorks: [
		{
		...
		},
		{
			...
			description:
				'The client authenticates with the {{access-point|AP}} (Open System or {{sae|SAE}} for {{wpa3|WPA3}}), then sends an {{association-request|Association Request}} specifying desired parameters. The {{access-point|AP}} responds with an {{association-response|Association Response}}, assigning the client an Association {{id-identifier|ID}} and granting network access.'
		},
		{
			title: '4-way handshake (WPA2/WPA3)',
			description:
				'After association, the client and {{access-point|AP}} perform a 4-message EAPOL {{handshake|handshake}} to derive per-session {{encryption|encryption}} keys ({{ptk|PTK}}). Both sides prove they know the passphrase without revealing it. This produces the temporal keys used to encrypt all subsequent data frames.'
		},
		{
			title: 'Data frames with CSMA/CA',
			description:
				'Before transmitting, the sender listens for a clear channel (carrier sense) and waits a random backoff time (collision avoidance). Data frames carry up to four {{mac-address|MAC}} addresses (three typically used) — RA, TA, and DA — and are encrypted with the session keys. Every {{unicast|unicast}} frame must be acknowledged by the receiver.'
		},
		{
...
iw dev wlan0 station dump`
  ...
  Tagged Parameters:
	...
	image: {
		...
		caption:
			'The [[wifi|802.11]] [[wifi|Wi-Fi]] {{mac-address|MAC}} frame format — more complex than [[ethernet|Ethernet]] because wireless needs extra fields. Up to four {{mac-address|MAC}} addresses handle the receiver, transmitter, destination, and source, while the Frame Control field encodes the frame type, {{encryption|encryption}} status, and power management flags.',
