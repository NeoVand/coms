...
[[wifi|Wi-Fi]] shares the 2.4 GHz {{ism-band|ISM band}} with **[[bluetooth|Bluetooth]]** — the two are the universal coexistence pairing in the [[wifi|Wireless]] category. Modern combo chips (Apple H-series, Broadcom, Qualcomm) do time-division arbitration at the silicon level so [[wifi|Wi-Fi]] and [[bluetooth|Bluetooth]] don't starve each other. The escape to 5 GHz and 6 GHz on the [[wifi|Wi-Fi]] side has eased the crowding; [[bluetooth|Bluetooth]] stays at 2.4 GHz where every battery-powered consumer device already lives — and so do [[zigbee|Zigbee]] (channels 11–26) and Thread, both of which use {{ieee-802-15-4|IEEE 802.15.4}} radios that intentionally dodge Wi-Fi's busy channels 1/6/11. Through the **{{matter|Matter}}** bridge, those low-power meshes appear to your home network as ordinary IP-addressable devices over the same Wi-Fi you're already running. The 13.56 MHz [[nfc|NFC]] tap pairs Wi-Fi credentials onto printers and IoT devices via NDEF handover, and the 6–9 GHz [[uwb|UWB]] radio sits well above the Wi-Fi bands and never contends with it.`,
	howItWorks: [
		{
