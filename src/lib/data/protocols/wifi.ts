...
export const wifi: Protocol = {
	...
	overview: `[[wifi|Wi-Fi]] brings [[ethernet|Ethernet]]-style networking to the airwaves. Instead of transmitting {{frame|frames}} over copper or fiber, [[wifi|802.11]] uses radio frequencies (2.4 GHz, 5 GHz, and now 6 GHz) to send data wirelessly. But air is a shared medium — everyone within range can hear everything — so [[wifi|Wi-Fi]] adds {{encryption|encryption}} (WPA2/{{wpa3|WPA3}}), collision avoidance (CSMA/{{certificate-authority|CA}} instead of [[ethernet|Ethernet]]'s CSMA/CD), and a more complex frame format with up to four MAC addresses (three typically used): the receiver address (RA), transmitter address (TA), and destination address (DA) — plus an optional fourth address used in wireless bridging (WDS) mode.
