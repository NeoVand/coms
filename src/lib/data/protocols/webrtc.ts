...
Under the hood, [[webrtc|WebRTC]] is actually a bundle of protocols: {{ice|ICE}} for connectivity establishment, {{dtls|DTLS}} (based on [[tls|TLS]]) for {{encryption|encryption}}, {{srtp|SRTP}} (secured [[rtp|RTP]]) for media, and [[sctp|SCTP]] for data channels. The browser {{api|API}} abstracts all of this into a relatively simple JavaScript interface.`,
	howItWorks: [
		{
			title: 'Signaling',
			description:
				'Peers {{exchange|exchange}} "offers" and "answers" ([[sdp|SDP]] descriptions of their capabilities) through a {{signaling|signaling}} server ([[websockets|WebSocket]], {{http-method|HTTP}}, or even carrier pigeon — [[webrtc|WebRTC]] doesn\'t care how).'
		},
		{
			title: 'ICE candidate gathering',
			description:
				'Each {{peer|peer}} gathers network candidates: local IPs, {{stun|STUN}}-discovered public IPs, and {{turn|TURN}} relay addresses. These are exchanged via the {{signaling|signaling}} server.'
		},
		{
...
asyncio.run(main())`,
		caption:
			'Get camera access, create an offer, and signal — [[webrtc|WebRTC]] handles the rest {{peer-to-peer|peer-to-peer}}',
		alternatives: [
			{
				language: 'javascript',
...
STUN Response:
  ...
  Attributes:
	...
	image: {
		...
		caption:
			'[[webrtc|WebRTC]] made this possible directly in the browser — no plugins, no downloads. {{peer-to-peer|Peer-to-peer}} video calls, screen sharing, and data channels all run natively in Chrome, Firefox, and Safari.',
