	...
	howItWorks: [
		{
			...
			description:
				'The sending host wraps the transport-layer segment ([[tcp|TCP]] or [[udp|UDP]]) in an [[ip|IP]] header containing the source [[ip|IP]], destination [[ip|IP]], {{ttl|TTL}} (typically 64 or 128), protocol number, header {{checksum|checksum}}, and optional fields. This [[ip|IP]] packet is then passed down to the link layer for framing.'
		},
		{
		...
		},
		{
			...
			description:
				'The destination host reassembles any fragments using the Identification field and fragment offsets, verifies the header {{checksum|checksum}}, strips the [[ip|IP]] header, and delivers the {{payload|payload}} to the correct transport-layer protocol ([[tcp|TCP]], [[udp|UDP]], [[icmp|ICMP]]) based on the Protocol field.'
		}
	],
	useCases: [
...
print("Raw IP packet sent!")`,
		caption:
			'Crafting a raw [[ip|IP]] packet in Python — manually setting every header field from version to destination address',
		alternatives: [
			{
				language: 'javascript',
