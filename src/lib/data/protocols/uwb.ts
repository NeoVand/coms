...
export const uwb: Protocol = {
	...
	oneLiner:
		'Sub-nanosecond impulse-radio at 6–9 GHz that measures time-of-flight to 10–30 cm — AirTag Precision Finding, BMW Digital Key, and Aliro hands-free unlock all ride this.',
	overview: `**[[uwb|UWB]] is not a data radio — it is a clock.** Modern UWB transmits sub-nanosecond Gaussian-monocycle impulses across ≥500 MHz of spectrum so that two devices can measure the **time-of-flight of a radio pulse** and convert it to distance with 10–30 cm accuracy. [[bluetooth|Bluetooth]] RSSI and [[wifi|Wi-Fi]] RTT cannot match that — and the whole point in 2026 is the *security* of the measurement, not just the precision. The physical layer is **IEEE 802.15.4z-2020** (ratified 31 August 2020); the spectrum is unlicensed under FCC Part 15 Subpart F at **−41.3 dBm/MHz** across 3.1–10.6 GHz, the legacy of the FCC's First Report and Order on Valentine's Day 2002. The two channels everyone actually uses are **Channel 5** (6489.6 MHz) and **Channel 9** (7987.2 MHz), each 499.2 MHz wide.

The consumer story begins on **10 September 2019** when Apple shipped the **U1 chip** in the iPhone 11. It went mass-market on **30 April 2021** with the $29 **AirTag**, whose Precision Finding feature swept the world's awareness of UWB into one product. Samsung followed with the **Galaxy SmartTag+** in April 2021. **CCC Digital Key 3.0** (July 2021) made UWB the fine-ranging leg of phone-as-a-key for vehicles — BMW iX shipped first; Mercedes EQS, Hyundai/Kia, and VW ID.7 followed. On **26 February 2026** the CSA published **{{aliro|Aliro 1.0}}** — the *Matter for door locks* — with NFC tap + BLE proximity + BLE/UWB ranged as its three transports. UWB silicon has consolidated to five suppliers: Apple captive (U1 16 nm, **U2** 7 nm in iPhone 15+), **Qorvo** (ex-Decawave DW1000/DW3000), **NXP** Trimension SR040/SR150/SR250, Samsung captive (Exynos Connect U100), and a long tail of STMicroelectronics, Microchip, Spark Microsystems, and **3db Access** (Infineon acquired 2024). ABI projects UWB phone penetration rising from **27 % in 2025 to 52 % by 2030**.

The reason UWB matters more than its raw precision is that it can **prove proximity even when an attacker tries to lie about it**. The **2022 NCC Group Tesla Model 3 BLE relay** (CVSS 6.8) — Sultan Qasim Khan unlocking and driving away a parked Tesla from 25 m using $50 of dev boards — showed why RSSI-based proximity is fundamentally broken: a relay can shorten apparent distance with low-latency forwarding. Time-of-flight cannot be shortened by a relay; the speed of light is the hard upper bound. The cryptographic primitive that makes 802.15.4z secure is the **Scrambled Timestamp Sequence (STS)** — an AES-128-CTR-generated pulse pattern injected into the ranging frame as a distance commitment. Pre-STS UWB (802.15.4a, 2007) was vulnerable to early-detect/late-commit pulse-replay; even STS was shown attackable at ~4 % success in **Ghost Peak** (Leu et al., USENIX Security 2022), which motivated **IEEE 802.15.4ab** (Draft D03 September 2025, ratification expected early 2026) with **NBA-MMS** narrowband-assisted multi-millisecond ranging and a redesigned STS receiver.`,
	howItWorks: [
		{
			title: 'Impulse-radio physical layer — Gaussian monocycles, not a carrier',
			description:
				"Instead of modulating a continuous carrier, **IR-UWB** transmits sub-nanosecond Gaussian-monocycle (or doublet) pulses spaced across the available spectrum. A symbol is built from a burst of pulses at one of two time-slot positions (BPM) with chosen polarity (BPSK) — *Burst-Position Modulation + Binary Phase-Shift Keying*. The mean pulse-repetition frequency is **BPRF ≈ 64 MHz** in the default 802.15.4z mode or **HPRF 124.8 / 249.6 MHz** in higher-power modes. Data rates: 850 kbps, 6.81 Mbps, or 27 Mbps — most ranging deployments use **6.81 Mbps** because shorter frame air-time reduces clock-drift error. The 499.2 MHz channel bandwidth and the ≥0.2 fractional-bandwidth requirement is what makes a UWB transmitter *legally UWB* under FCC Part 15.519."
		},
		{
			title: 'The four ranging paradigms — TWR, DS-TWR, TDoA, AoA',
			description:
				"**SS-TWR (Single-Sided Two-Way Ranging):** Poll → Response. Initiator measures round-trip, subtracts the responder's reply delay (sent in the response payload), divides by 2. *Highly sensitive to relative clock drift* — a 20 ppm crystal with a 200 µs reply delay yields ~4 ns of error = ~1.2 m. **DS-TWR (Double-Sided Two-Way Ranging):** Three messages Poll → Response → Final. The cross-product `(T_round1 × T_round2 - T_reply1 × T_reply2) / (T_round1 + T_round2 + T_reply1 + T_reply2)` cancels clock drift to first order — the production method in 802.15.4z. **TDoA:** Tag chirps once, ≥3 time-synchronised anchors compute the difference of arrival — used for warehouse/hospital RTLS where anchors are wired. **AoA / PDoA:** Two antennas spaced ~λ/2 (≈ 1.9 cm at 8 GHz) compare phase to derive angle — AirTag Precision Finding combines DS-TWR distance with PDoA direction."
		},
		{
			title: 'STS — Scrambled Timestamp Sequence, the cryptographic distance bound',
			description:
				"The vulnerability that 802.15.4a (2007) shipped with: its preamble and SFD patterns are public, so an attacker can predict the next pulse and inject an early replica that the receiver locks onto, shortening apparent distance — the *Cicada* / *Early-Detect / Late-Commit* family. **STS in 802.15.4z** replaces the predictable preamble-derived timestamp with a 32–2048-chip pulse sequence whose positions are generated by **AES-128 in Counter mode** keyed by a per-session STS_KEY and a per-frame nonce. An attacker without the key sees noise; they cannot predict the next chip and cannot reliably early-replay. The receiver, holding the same key, generates the expected sequence locally and cross-correlates — sharp autocorrelation peak with a fresh code every frame. This is the **distance commitment** that defeats the BLE-RSSI relay class of attack."
		},
		{
			title: 'BLE bootstraps every UWB session — the indispensable on-ramp',
			description:
				"Almost no consumer UWB session is ever opened without [[bluetooth|BLE]] first. The reason is power and discovery: BLE has ubiquitous always-on advertising and standardised pairing; UWB has neither. The canonical pattern in **CCC Digital Key 3.0** and **Aliro 1.0**: **(1)** BLE GAP advertising with the application's service UUID; **(2)** GATT service discovery + authentication (SPAKE2+/PAKE for Digital Key; OOB pairing for AirTag); **(3)** session-key transport over the BLE encrypted channel — phone and reader exchange **STS_KEY** and ranging parameters via APDU-over-GATT; **(4)** BLE-signalled UWB ranging start at a scheduled time slot; **(5)** ranging happens, results returned over BLE. The FCC waiver Schlage filed (ET Docket 22-248, granted 2023) describes this exact sequence in regulator-vetted form."
		},
		{
			title: 'CCC Digital Key 3.0 — the canonical UWB unlock flow',
			description:
				"BMW iX shipped first in early 2022. The vehicle has multiple **UWB anchors** (typically B-pillar and one per door) plus a BLE radio; the phone has U1/U2 (Apple) or NXP/Qorvo silicon plus its BLE radio. The unlock sequence: BLE Advertising → BLE pairing + GATT auth → APDU exchange where the vehicle authenticates the Digital Key applet in the phone's {{ese|Secure Element}} and derives session keys → BLE transfers the UWB session key (STS_KEY) and ranging schedule → UWB Poll/Response/Final DS-TWR ranging fires across multiple anchors → vehicle computes ToF on each anchor, checks the distance is below threshold *and* the credential is valid → BLE returns Unlock granted/denied. NFC remains the fallback for battery-dead phones. **CCC Digital Key 4.0** (announced July 2025, tested at the 13th Plugfest hosted by Apple) adds cross-platform key sharing across Android and iOS."
		},
		{
			title: 'The competition — Wi-Fi FTM (802.11mc/11az) and the honest verdict',
			description:
				"**[[wifi|Wi-Fi]] Fine Timing Measurement (FTM)** in IEEE 802.11mc and the security-enhanced **802.11az** is the closest analogue to UWB ranging in mass-market silicon. *Native silicon:* Wi-Fi is already in every phone vs. a dedicated UWB chip. *Bandwidth:* Wi-Fi 80–160 MHz vs. UWB 499.2 MHz. *Accuracy:* Wi-Fi 1–2 m typical vs. UWB 10–30 cm. *Secure ranging:* Wi-Fi 11az TB/NTB with PASN vs. UWB STS. *Infrastructure:* Wi-Fi reuses existing APs (Cisco/Aruba support) vs. dedicated UWB anchors. **Honest verdict:** FTM is *good enough* for room-level indoor positioning; **UWB wins for proximity-based authentication** (car keys, smart locks) because cm-precision *and* the STS distance commitment combine to produce a hard distance upper-bound that FTM cannot match today. 802.11az narrows the gap. They will most likely coexist."
		},
		{
			title: 'Regional masks — design for the strictest you must support',
			description:
				"FCC Part 15.519 caps average PSD at **−41.3 dBm/MHz** across 3.1–10.6 GHz. ETSI EN 302 065 in Europe is similar with stricter Detect-and-Avoid requirements in some sub-bands. **Japan** applies a different mask with restrictions in **7.25–7.75 GHz**, overlapping Channel 9 — Apple's iPhone reduces or disables UWB features (Precision Finding, Find People) in Japan and a handful of other countries. Per-country radio law in Korea and individual jurisdictions further constrains commercial UWB. For a global product, design to: **(a)** geo-fence UWB features based on locale; **(b)** default to **Channel 5** in Japan; **(c)** support a *no-UWB* mode for countries where UWB is not permitted at all (consult Apple Support's UWB-availability list for the current jurisdictional state)."
		}
	],
	useCases: [
  ...
  func session(_ session: NISession, didUpdate nearbyObjects: [NINearbyObject]) {
    ...
    if let direction = obj.direction {
      print("Bearing vector: \\(direction)")              // 3D unit vector, AoA + ARKit
		...
		caption:
			"Apple's Nearby Interaction framework is the public API every iOS UWB app uses. The U1/U2 silicon handles DS-TWR + PDoA; the framework returns distance in metres and a 3D direction vector at ~10 Hz. AirTag Precision Finding, BMW Digital Key, and the AR experiences in Apple Vision Pro accessories all sit on top of this. The same NIDiscoveryToken-over-BLE pattern is what Aliro 1.0 generalises across Android and Samsung wallets.",
		alternatives: [
			{
				language: 'python',
...
Continuous Precision Finding refreshes at ~10 Hz on Apple silicon.`
	...
	recentChanges: [
		{
			...
			description:
				"Apple introduced the **U2 ultra-wideband chip** alongside the iPhone 15 family on **12 September 2023**. 7 nm (vs U1's 16 nm), internal codename T2024, part number 339M00298. **1.5× longer Precision Finding range** vs the U1. The U2 also shipped in Apple Watch Series 9 / Watch Ultra 2 — the first time an Apple Watch could itself act as a UWB initiator for Precision Finding. AirTag 2 (January 2026) carries the U2 alongside the anti-stalking hardware updates.",
			source: {
		...
		},
		{
			...
			description:
				"On **15 May 2022** NCC Group's **Sultan Qasim Khan** publicly disclosed a link-layer BLE relay against Tesla Model 3 / Y phone-as-a-key (CVSS 6.8 AV:A/AC:H/PR:N/UI:N/S:U/C:H/I:H/A:N). ~$50 of Bluetooth dev boards, ~8 ms relay latency, below Tesla's ~30 ms GATT threshold. Test: 2020 Model 3, iPhone 13 mini 25 m from the car, relay device 3 m from the car — vehicle unlocked and driven away. The same technique works against Kwikset/Weiser Kevo. Tesla recommended PIN-to-Drive as a stopgap; the deeper industry response was the adoption of **UWB-backed CCC Digital Key 3.0** — cryptographic distance bounds the speed of light cannot relay around.",
			source: {
		...
		},
		{
			...
			description:
				"Patrick Leu, Giovanni Camurati, Alexander Heinrich, Marc Roeschlin, Claudio Anliker, Matthias Hollick, and Srdjan Capkun published **Ghost Peak: Practical Distance Reduction Attacks Against HRP UWB Ranging** at **USENIX Security 2022**. First practical distance reduction against deployed **Apple U1 ⇄ U1** and **U1 ⇄ NXP / Qorvo** combinations. Reduces 12 m to 0 m at up to ~4 % success rate with a **$65 attacker device**. The technique exploits how STS receivers integrate correlation peaks — randomly injected STS-like signals are misread as a low-power early copy of the real signal. Triggered Apple's published response paper (arXiv:2312.03964) and the redesign work that eventually fed into IEEE 802.15.4ab.",
			source: {
		...
		},
		{
			...
			description:
				"The **Car Connectivity Consortium** announced Digital Key 4.0 in July 2025 and validated it at the **13th Plugfest hosted by Apple**. Adds **device-to-device key sharing across Android and iOS**, validates backward compatibility with 3.0 vehicles, and keeps the requirement that devices support at least one of NFC, BLE, or UWB. HRP UWB under 802.15.4z remains the secure default. **115 vehicle/module products** were certified across 2025 alone; BMW (first cert late 2024), Mercedes, Hyundai/Kia, Audi (new for 2025), VW, Porsche, GM, Ford, and Chinese OEMs (NIO, XPENG, Geely brands).",
			source: {
		...
		},
		{
			...
			description:
				"On **26 February 2026** the CSA published **{{aliro|Aliro 1.0}}**, a PKI-based access-control credential standard with **NFC (tap) + BLE (proximity) + BLE+UWB (ranged, hands-free)** as its three transports. ECDSA mutual authentication; credentials provisioned into Apple, Google, and Samsung wallets. First products: **Aqara U400** (first smart lock with UWB; Aliro coming via firmware update) and **Kwikset Halo Select Plus**. Launch partners include Apple, Google, Samsung, ASSA ABLOY, NXP, Infineon, STMicroelectronics, Silicon Labs, Nordic Semiconductor, plus smart-lock makers Schlage, Avia, Nuki, Xthings. Aliro generalises the Apple Home Key UX across Android — the most consequential UWB-related deployment of 2026.",
			source: {
	...
	realWorldDeployments: [
		...
		},
		{
			...
			description:
				"Announced **13 January 2021**; customer rollout from early 2022. **First production CCC Digital Key 3.0 UWB vehicle**. BMW jointly developed the spec with Apple and the CCC. BMW Group was the first OEM to receive CCC Digital Key Certification for its UWB-based digital vehicle access. The UX: walk up to the car, the doors unlock as you reach the handle (BLE detects you, UWB confirms which side of the door you are on); inside, set the phone down anywhere and start the car (no key-fob ritual)."
		},
		{
			org: 'Mercedes-Benz EQS / S-Class',
			scale: 'NFC+UWB CCC Digital Key 3.0 across the flagship lineup',
			description:
				"Mercedes-Benz partnered with Apple on the Wallet-based Digital Key for the EQS and S-Class from 2022. **NXP SR1xx** silicon family in the vehicle anchors. The Mercedes Digital Key was one of the first deployed cases of an NFC-fallback + UWB-precision unlock — battery-dead phones still get in via NFC tap; otherwise UWB walk-up authentication takes over."
		},
		{
		...
		},
		{
			...
			description:
				"**Sonitor** runs clinical-grade hospital asset tracking (patients, equipment, surgical instruments). **Zebra Technologies** runs warehouse forklift and pallet tracking at high anchor density. **Apex Locate** focuses on manufacturing. These are TDoA-anchor deployments (multiple time-synchronised receivers measure the time difference of arrival of a single tag's pulse) rather than peer-to-peer ranging — anchors connect over wired Ethernet with PTP-grade clock distribution. **Cisco partnered with Sewio in October 2019** to integrate UWB into wireless access points, but the strategy has been slow to mature."
		},
		{
			...
			description:
				"Apple shipped >1 billion iPhones from the iPhone 11 (10 September 2019) onward by mid-decade. Not every iPhone has UWB — **iPhone SE 1/2/3, iPhone 16e, and iPhone 17e** do not include a UWB chip — but the FiRa-compatible UWB device install base is the largest of any UWB ecosystem worldwide, dwarfing the Samsung Galaxy UWB-flagship base. The U1 cohort spans iPhone 11 through iPhone 14, AirTag (1st gen), HomePod mini, Apple Watch Series 6+, and the AirPods Pro 2 case. The U2 cohort: iPhone 15/16/17 (excluding 16e/17e), AirTag 2."
		}
	],
