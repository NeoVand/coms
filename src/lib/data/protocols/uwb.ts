...
export const uwb: Protocol = {
	...
	overview: `**[[uwb|UWB]] is not a data radio — it is a clock.** Modern UWB transmits sub-nanosecond Gaussian-monocycle impulses across ≥500 MHz of {{spectrum|spectrum}} so that two devices can measure the **{{tof-ranging|time-of-flight of a radio pulse}}** and convert it to distance with 10–30 cm accuracy. [[bluetooth|Bluetooth]] {{rssi|RSSI}} and [[wifi|Wi-Fi]] {{rtt|RTT}} cannot match that — and the whole point in 2026 is the *security* of the measurement, not just the precision. The physical layer is **{{ieee-802-15-4|IEEE 802.15.4z-2020}}** (ratified 31 August 2020); the spectrum is unlicensed under FCC Part 15 Subpart F at **−41.3 dBm/MHz** across 3.1–10.6 GHz, the legacy of the FCC's First Report and Order on Valentine's Day 2002. The two channels everyone actually uses are **Channel 5** (6489.6 MHz) and **Channel 9** (7987.2 MHz), each 499.2 MHz wide.
...
The reason UWB matters more than its raw precision is that it can **prove proximity even when an attacker tries to lie about it**. The **2022 NCC Group Tesla Model 3 BLE relay** (CVSS 6.8) — Sultan Qasim Khan unlocking and driving away a parked Tesla from 25 m using $50 of dev boards — showed why RSSI-based proximity is fundamentally broken: a {{replay-attack|relay}} can shorten apparent distance with low-{{latency|latency}} forwarding. {{tof-ranging|Time-of-flight}} cannot be shortened by a relay; the speed of light is the hard upper bound. The cryptographic primitive that makes 802.15.4z secure is the **{{sts|Scrambled Timestamp Sequence (STS)}}** — an AES-128-CTR-generated pulse pattern injected into the ranging frame as a distance commitment. Pre-STS UWB (802.15.4a, 2007) was vulnerable to early-detect/late-commit pulse-replay; even STS was shown attackable at ~4 % success in **Ghost Peak** (Leu et al., USENIX Security 2022), which motivated **IEEE 802.15.4ab** (Draft D03 September 2025, ratification expected early 2026) with **NBA-MMS** narrowband-assisted multi-millisecond ranging and a redesigned STS receiver.`,
	howItWorks: [
		...
		},
		{
			...
			description:
				"BMW iX shipped first in early 2022. The vehicle has multiple **UWB anchors** (typically B-pillar and one per door) plus a BLE radio; the phone has U1/U2 (Apple) or NXP/Qorvo silicon plus its BLE radio. The unlock sequence: BLE Advertising → BLE pairing + {{gatt|GATT}} auth → {{apdu|APDU}} {{exchange|exchange}} where the vehicle authenticates the Digital Key applet in the phone's {{ese|Secure Element}} and derives session keys → BLE transfers the UWB session key (STS_KEY) and ranging schedule → UWB Poll/Response/Final DS-TWR ranging fires across multiple anchors → vehicle computes ToF on each anchor, checks the distance is below threshold *and* the credential is valid → BLE returns Unlock granted/denied. [[nfc|NFC]] remains the fallback for battery-dead phones. **{{ccc-digital-key|CCC Digital Key}} 4.0** (announced July 2025, tested at the 13th Plugfest hosted by Apple) adds cross-platform key sharing across Android and iOS."
		},
		{
			title: 'The competition — Wi-Fi FTM (802.11mc/11az) and the honest verdict',
			description:
				"**[[wifi|Wi-Fi]] Fine Timing Measurement (FTM)** in IEEE 802.11mc and the security-enhanced **802.11az** is the closest analogue to UWB ranging in mass-market silicon. *Native silicon:* Wi-Fi is already in every phone vs. a dedicated UWB chip. *{{bandwidth|Bandwidth}}:* Wi-Fi 80–160 MHz vs. UWB 499.2 MHz. *Accuracy:* Wi-Fi 1–2 m typical vs. UWB 10–30 cm. *Secure ranging:* Wi-Fi 11az TB/NTB with PASN vs. UWB STS. *Infrastructure:* Wi-Fi reuses existing APs (Cisco/Aruba support) vs. dedicated UWB anchors. **Honest verdict:** FTM is *good enough* for room-level indoor positioning; **UWB wins for proximity-based authentication** (car keys, smart locks) because cm-precision *and* the STS distance commitment combine to produce a hard distance upper-bound that FTM cannot match today. 802.11az narrows the gap. They will most likely coexist."
		},
		{
...
Continuous Precision Finding refreshes at ~10 Hz on Apple silicon.`
	...
	realWorldDeployments: [
		...
		},
		{
			...
			description:
				"Mercedes-Benz partnered with Apple on the Wallet-based Digital Key for the EQS and S-Class from 2022. **NXP SR1xx** silicon family in the vehicle anchors. The Mercedes Digital Key was one of the first deployed cases of an NFC-fallback + UWB-precision unlock — battery-dead phones still get in via [[nfc|NFC]] tap; otherwise UWB walk-up authentication takes over."
		},
		{
	...
	funFacts: [
		...
		},
		{
			...
			text: "Through 2021–2022, accounts amassed followers posting *found an AirTag in my jacket pocket at the airport / in my car after a date / in my luggage* threads. These stories drove enormous public awareness of UWB's existence — most non-engineers had never heard of UWB before they heard of AirTags — and accelerated cross-industry safety standardisation. The result: the **{{ietf|IETF}} DULT (Detecting Unwanted Location Trackers)** working group, the cross-vendor *tracker has been moving with you* alerts that work across Apple's Find My and Google's Find My Device networks regardless of vendor, and the AirTag 2's hardware anti-tampering."
		},
		{
			title: 'Apple silicon engineers published their security response on arXiv',
			text: "**Secure Ranging with IEEE 802.15.4z HRP UWB** (Luo, Kalkanli, Zhou, Zhan, Cohen — arXiv:2312.03964, December 2023) is a rare instance of Apple silicon engineers publishing a {{peer|peer}}-reviewable response to academic security research — specifically the Singh / Leu / Capkun lineage from ETH Zurich. The paper specifies an STS receiver design and proves its security and asymptotic optimality under the documented attack model. The cultural significance is almost as large as the technical: Apple does not normally publish protocol-level threat analysis, and the act of doing so signalled that UWB security was sufficiently competitive to require open {{exchange|exchange}} with academia."
		},
		{
