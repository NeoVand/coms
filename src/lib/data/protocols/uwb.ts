...
The reason UWB matters more than its raw precision is that it can **prove proximity even when an attacker tries to lie about it**. The **2022 NCC Group Tesla Model 3 {{ble|BLE}} relay** (CVSS 6.8) — Sultan Qasim Khan unlocking and driving away a parked Tesla from 25 m using $50 of dev boards — showed why {{rssi|RSSI}}-based proximity is fundamentally broken: a {{replay-attack|relay}} can shorten apparent distance with low-{{latency|latency}} forwarding. {{tof-ranging|Time-of-flight}} cannot be shortened by a relay; the speed of light is the hard upper bound. The cryptographic primitive that makes 802.15.4z secure is the **{{sts|Scrambled Timestamp Sequence (STS)}}** — an {{aes-128|AES-128}}-CTR-generated pulse pattern injected into the ranging frame as a distance commitment. Pre-{{sts|STS}} UWB (802.15.4a, 2007) was vulnerable to early-detect/late-commit pulse-replay; even {{sts|STS}} was shown attackable at ~4 % success in **Ghost Peak** (Leu et al., {{usenix-conf|USENIX}} Security 2022), which motivated **{{ieee-802-15-4|IEEE}} 802.15.4ab** (Draft D03 September 2025, ratification expected early 2026) with **{{nba-mms|NBA-MMS}}** narrowband-assisted multi-millisecond ranging and a redesigned {{sts|STS}} receiver.`,
	howItWorks: [
		...
		},
		{
			...
			description:
				"**{{twr|SS-TWR}} (Single-Sided Two-Way Ranging):** Poll → Response. Initiator measures round-trip, subtracts the responder's reply delay (sent in the response {{payload|payload}}), divides by 2. *Highly sensitive to relative {{clock-drift|clock drift}}* — a 20 ppm crystal with a 200 µs reply delay yields ~4 ns of error = ~1.2 m. **{{twr|DS-TWR}} (Double-Sided Two-Way Ranging):** Three messages Poll → Response → Final. The cross-product `(T_round1 × T_round2 - T_reply1 × T_reply2) / (T_round1 + T_round2 + T_reply1 + T_reply2)` cancels {{clock-drift|clock drift}} to first order — the production method in 802.15.4z. **TDoA:** Tag chirps once, ≥3 time-synchronised anchors compute the difference of arrival — used for warehouse/hospital RTLS where anchors are wired. **AoA / PDoA:** Two antennas spaced ~λ/2 (≈ 1.9 cm at 8 GHz) compare phase to derive angle — AirTag Precision Finding combines {{ds-twr|DS-TWR}} distance with PDoA direction."
		},
		{
			title: 'STS — Scrambled Timestamp Sequence, the cryptographic distance bound',
			description:
				'The vulnerability that 802.15.4a (2007) shipped with: its preamble and {{sfd|SFD}} patterns are public, so an attacker can predict the next pulse and inject an early replica that the receiver locks onto, shortening apparent distance — the *Cicada* / *Early-Detect / Late-Commit* family. **{{sts|STS}} in 802.15.4z** replaces the predictable preamble-derived timestamp with a 32–2048-chip pulse sequence whose positions are generated by **{{aes-128|AES-128}} in {{aead|Counter mode}}** keyed by a per-session STS_KEY and a per-frame {{nonce|nonce}}. An attacker without the key sees noise; they cannot predict the next chip and cannot reliably early-replay. The receiver, holding the same key, generates the expected sequence locally and cross-correlates — sharp autocorrelation peak with a fresh code every frame. This is the **distance commitment** that defeats the {{ble|BLE}}-{{rssi|RSSI}} {{replay-attack|relay}} class of attack.'
		},
		{
			title: 'BLE bootstraps every UWB session — the indispensable on-ramp',
			description:
				"Almost no consumer UWB session is ever opened without {{ble|BLE}} first. The reason is power and discovery: {{ble|BLE}} has ubiquitous always-on advertising and standardised pairing; UWB has neither. The canonical pattern in **{{ccc-digital-key|CCC Digital Key 3.0}}** and **{{aliro|Aliro 1.0}}**: **(1)** {{ble|BLE}} GAP advertising with the application's service {{uuid|UUID}}; **(2)** {{gatt|GATT}} {{service-discovery|service discovery}} + authentication ({{spake2|SPAKE2}}+/{{pake|PAKE}} for Digital Key; {{oob|OOB}} pairing for AirTag); **(3)** session-key transport over the {{ble|BLE}} {{encryption|encrypted}} channel — phone and reader {{exchange|exchange}} **STS_KEY** and ranging parameters via {{apdu|APDU-over-GATT}}; **(4)** {{ble|BLE}}-signalled UWB ranging start at a scheduled time slot; **(5)** ranging happens, results returned over {{ble|BLE}}. The {{fcc|FCC}} waiver Schlage filed (ET Docket 22-248, granted 2023) describes this exact sequence in regulator-vetted form."
		},
		{
			title: 'CCC Digital Key 3.0 — the canonical UWB unlock flow',
			description:
				"BMW iX shipped first in early 2022. The vehicle has multiple **UWB anchors** (typically B-pillar and one per door) plus a {{ble|BLE}} radio; the phone has {{u1-chip|U1}}/{{u2-chip|U2}} ({{apple|Apple}}) or {{nxp|NXP}}/{{qorvo|Qorvo}} silicon plus its {{ble|BLE}} radio. The unlock sequence: {{ble|BLE}} Advertising → {{ble|BLE}} pairing + {{gatt|GATT}} auth → {{apdu|APDU}} {{exchange|exchange}} where the vehicle authenticates the Digital Key applet in the phone's {{ese|Secure Element}} and derives session keys → {{ble|BLE}} transfers the UWB session key (STS_KEY) and ranging schedule → UWB Poll/Response/Final {{ds-twr|DS-TWR}} ranging fires across multiple anchors → vehicle computes ToF on each anchor, checks the distance is below threshold *and* the credential is valid → BLE returns Unlock granted/denied. [[nfc|NFC]] remains the fallback for battery-dead phones. **{{ccc-digital-key|CCC Digital Key}} 4.0** (announced July 2025, tested at the 13th Plugfest hosted by {{apple|Apple}}) adds cross-platform key sharing across {{android|Android}} and iOS."
		},
		{
			title: 'The competition — Wi-Fi FTM (802.11mc/11az) and the honest verdict',
			description:
				'**[[wifi|Wi-Fi]] Fine Timing Measurement ({{ftm|FTM}})** in {{ieee-802-15-4|IEEE}} 802.11mc and the security-enhanced **802.11az** is the closest analogue to UWB ranging in mass-market silicon. *Native silicon:* Wi-Fi is already in every phone vs. a dedicated UWB chip. *{{bandwidth|Bandwidth}}:* Wi-Fi 80–160 MHz vs. UWB 499.2 MHz. *Accuracy:* Wi-Fi 1–2 m typical vs. UWB 10–30 cm. *Secure ranging:* Wi-Fi 11az TB/NTB with PASN vs. UWB {{sts|STS}}. *Infrastructure:* Wi-Fi reuses existing APs ({{cisco|Cisco}}/Aruba support) vs. dedicated UWB anchors. **Honest verdict:** FTM is *good enough* for room-level indoor positioning; **UWB wins for proximity-based authentication** (car keys, smart locks) because cm-precision *and* the {{sts|STS}} distance commitment combine to produce a hard distance upper-bound that FTM cannot match today. 802.11az narrows the gap. They will most likely coexist.'
		},
		{
...
Continuous Precision Finding refreshes at ~10 Hz on Apple silicon.`
	...
	recentChanges: [
		{
			...
			description:
				"{{apple|Apple}} introduced the **{{u2-chip|U2}} ultra-wideband chip** alongside the iPhone 15 family on **12 September 2023**. 7 nm (vs {{u1-chip|U1}}'s 16 nm), internal codename T2024, part number 339M00298. **1.5× longer Precision Finding range** vs the {{u1-chip|U1}}. The {{u2-chip|U2}} also shipped in {{apple|Apple}} Watch Series 9 / Watch Ultra 2 — the first time an {{apple|Apple}} Watch could itself act as a UWB initiator for Precision Finding. AirTag 2 (January 2026) carries the U2 alongside the anti-stalking hardware updates.",
			source: {
		...
		},
		{
			...
			description:
				"Patrick Leu, Giovanni Camurati, Alexander Heinrich, Marc Roeschlin, Claudio Anliker, Matthias Hollick, and Srdjan Capkun published **Ghost Peak: Practical Distance Reduction Attacks Against {{hrp-uwb|HRP}} UWB Ranging** at **{{usenix-conf|USENIX}} Security 2022**. First practical distance reduction against deployed **{{apple|Apple}} {{u1-chip|U1}} ⇄ {{u1-chip|U1}}** and **U1 ⇄ {{nxp|NXP}} / {{qorvo|Qorvo}}** combinations. Reduces 12 m to 0 m at up to ~4 % success rate with a **$65 attacker device**. The technique exploits how {{sts|STS}} receivers integrate correlation peaks — randomly injected {{sts|STS}}-like signals are misread as a low-power early copy of the real signal. Triggered {{apple|Apple}}'s published response paper (arXiv:2312.03964) and the redesign work that eventually fed into {{ieee-802-15-4|IEEE}} 802.15.4ab.",
			source: {
		...
		},
		{
			...
			description:
				'On **26 February 2026** the {{sig-bluetooth-acronym|CSA}} published **{{aliro|Aliro 1.0}}**, a {{pki|PKI}}-based access-control credential standard with **[[nfc|NFC]] (tap) + {{ble|BLE}} (proximity) + {{ble|BLE}}+UWB (ranged, hands-free)** as its three transports. {{ecdsa|ECDSA}} mutual authentication; credentials provisioned into {{apple|Apple}}, {{google|Google}}, and Samsung wallets. First products: **Aqara U400** (first smart lock with UWB; {{aliro|Aliro}} coming via firmware {{bgp-update|update}}) and **Kwikset Halo {{imap-select|Select}} Plus**. Launch partners include {{apple|Apple}}, {{google|Google}}, Samsung, ASSA ABLOY, {{nxp|NXP}}, Infineon, STMicroelectronics, Silicon Labs, Nordic Semiconductor, plus smart-lock makers Schlage, Avia, Nuki, Xthings. {{aliro|Aliro}} generalises the {{apple|Apple}} Home Key {{ux|UX}} across {{android|Android}} — the most consequential UWB-related deployment of 2026.',
			source: {
	...
	realWorldDeployments: [
		...
		},
		{
			...
			description:
				'Mercedes-Benz partnered with {{apple|Apple}} on the Wallet-based Digital Key for the EQS and S-Class from 2022. **{{nxp|NXP}} SR1xx** silicon family in the vehicle anchors. The Mercedes Digital Key was one of the first deployed cases of an NFC-fallback + UWB-precision unlock — battery-dead phones still get in via [[nfc|NFC]] tap; otherwise UWB walk-up authentication takes over.'
		},
		{
			org: 'Aqara U400 + Apple Home Key + Aliro',
			scale:
				'First smart lock shipped with UWB support (2025); Aliro 1.0 firmware update post-CES 2026',
			description:
				'**Aqara U400** is the first commercial smart lock to ship with UWB. Initially {{apple|Apple}}-Home-Key-only — meaning the experience worked only for iPhone owners. With **{{aliro|Aliro}} 1.0** ({{sig-bluetooth-acronym|CSA}}, 26 February 2026) coming via firmware {{bgp-update|update}} — announced at {{ces-show|CES}} 2026 — the same lock will work hands-free with {{android|Android}}, Samsung, and any {{aliro|Aliro}}-compliant wallet. The {{apple|Apple}} Home Key experience (tap or walk up; door unlocks if your iPhone is the authenticated bearer) is what {{aliro|Aliro}} generalises across vendors.'
		},
		{
		...
		},
		{
			...
			scale:
				'TDoA anchor deployments; typical anchor density 1 per ~20–30 m² for cm-class accuracy',
			description:
				"**Sonitor** runs clinical-grade hospital asset tracking (patients, equipment, surgical instruments). **Zebra Technologies** runs warehouse forklift and pallet tracking at high anchor density. **Apex Locate** focuses on manufacturing. These are TDoA-anchor deployments (multiple time-synchronised receivers measure the time difference of arrival of a single tag's pulse) rather than {{peer-to-peer|peer-to-peer}} ranging — anchors {{mqtt-connect|connect}} over wired [[ethernet|Ethernet]] with PTP-grade clock distribution. **{{cisco|Cisco}} partnered with Sewio in October 2019** to integrate UWB into wireless access points, but the strategy has been slow to mature."
		},
		{
			org: 'UWB-equipped iPhone install base',
			scale:
				'>1 B iPhones from iPhone 11 (Sep 2019) onward; ABI projects 27 % → 52 % UWB phone penetration 2025–2030',
			description:
				'{{apple|Apple}} shipped >1 billion iPhones from the iPhone 11 (10 September 2019) onward by mid-decade. Not every iPhone has UWB — **iPhone {{se-secure-element|SE}} 1/2/3, iPhone 16e, and iPhone 17e** do not include a UWB chip — but the FiRa-compatible UWB device install base is the largest of any UWB ecosystem worldwide, dwarfing the Samsung Galaxy UWB-flagship base. The {{u1-chip|U1}} cohort spans iPhone 11 through iPhone 14, AirTag (1st gen), HomePod mini, {{apple|Apple}} Watch Series 6+, and the AirPods Pro 2 case. The {{u2-chip|U2}} cohort: iPhone 15/16/17 (excluding 16e/17e), AirTag 2.'
		}
	],
	...
	practicalWisdom: {
		pitfalls: [
			...
			},
			{
				...
				text: "**802.15.4a UWB (pre-{{sts|STS}}) is exploitably vulnerable to distance-decrease attacks.** Even 4z {{sts|STS}}, naïvely implemented, was broken by Ghost Peak (2022) and Cicada++ at ~4 % success. For any 2024+ design: **(a)** require {{sts|STS}} in all production ranging modes; **(b)** use FiRa's current cipher-suite recommendations rather than rolling your own; **(c)** treat the {{sts|STS}} receiver as a security-critical component subject to the same review as cryptographic code ({{apple|Apple}}'s published reference receiver, arXiv:2312.03964, is the current academic state of the art); **(d)** plan for {{ieee-802-15-4|IEEE}} 802.15.4ab silicon support as it becomes available, particularly {{nba-mms|NBA-MMS}} which significantly improves both link budget and anti-injection robustness."
			},
			{
				title: 'AoA antenna geometry — get the spacing right',
				text: "Phase-difference-of-arrival angle estimation requires ≥2 antennas spaced **~λ/2**. At 6.5 GHz, λ/2 ≈ 2.3 cm; at 8 GHz, λ/2 ≈ 1.9 cm. **Spacing larger than λ/2 introduces angular ambiguity. Spacing significantly smaller degrades angular resolution.** Use the canonical ~λ/2 spacing. For 3D angle estimation (azimuth + elevation), use three antennas in an L-shape or 2D array. Beware of the antenna's near-field interaction with the phone body / car door — {{apple|Apple}}'s {{u1-chip|U1}} antennas are placed along the iPhone frame for a reason; door-handle UWB anchors typically use planar PCB patches with carefully controlled ground planes."
			},
			{
				title: 'BLE-bootstrap fragility — design for graceful failure',
				text: "Every consumer UWB session opens over [[bluetooth|BLE]]. {{ble|BLE}} pairing failures, {{gatt|GATT}} timeouts, or {{ble|BLE}} link loss during the UWB session derail the whole interaction. **(a)** Make UWB ranging {{idempotent|idempotent}} — a failed ranging round should not invalidate the {{ble|BLE}} session key; allow retries. **(b)** Set {{ble|BLE}} {{gatt|GATT}} response timers generously enough to allow STS_KEY transport before UWB starts (~100–300 ms typical). **(c)** Provide an [[nfc|NFC]] fallback for the unlock {{ux|UX}} ({{ccc-digital-key|CCC Digital Key}} 3.0 design pattern) — if {{ble|BLE}} {{handshake|handshake}} fails or the device's battery is critically low, the [[nfc|NFC]] tap-to-unlock path keeps the user in their car. **(d)** Log BLE/UWB transition failures separately for diagnostics — they look identical to the user but have different root causes."
			},
			{
