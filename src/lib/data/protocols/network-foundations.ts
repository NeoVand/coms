...
sudo tcpdump -i eth0 -v -c 5 ip`
  ...
  Payload: TCP segment (40 bytes)
	...
	},
	{
		...
		overview: `IPv6 is the successor to [[ip|IPv4]], designed to solve the internet's address exhaustion crisis. IPv4's 32-bit addresses provide roughly 4.3 billion unique addresses — a number that seemed inexhaustible in 1981 but was effectively depleted by 2011. IPv6 uses 128-bit addresses, providing 340 undecillion (3.4×10³⁸) addresses — enough to assign a unique address to every atom on the surface of the Earth and still have addresses left over.

But IPv6 isn't just "bigger addresses." The protocol was redesigned from scratch with decades of operational experience. The header was simplified: IPv4's variable-length header with a checksum and options field became a fixed 40-byte header with no checksum (upper layers handle integrity) and an elegant extension header chain for optional features. Fragmentation was removed from routers entirely — only the source host fragments, discovered through Path MTU Discovery using [[icmp|ICMPv6]].

IPv6 eliminates broadcast entirely, replacing it with multicast and anycast. Instead of [[arp|ARP]] broadcasts to resolve addresses, IPv6 uses Neighbor Discovery Protocol (NDP), which runs over ICMPv6 and uses solicited-node multicast — far more efficient than flooding every device on the network. NDP also handles stateless address autoconfiguration (SLAAC), where a device can configure its own globally unique address without a [[dhcp|DHCP]] server.
