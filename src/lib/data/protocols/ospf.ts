...
export const ospf: Protocol = {
	...
	overview: `[[ospf|OSPF]] is the dominant link-state {{igp|Interior Gateway Protocol}} on [[ip|IP]] networks. Where [[bgp|BGP]] is the *external* protocol that stitches the internet's {{autonomous-system|autonomous systems}} together, [[ospf|OSPF]] is what each AS uses *inside* to compute paths between its own routers — every enterprise core, every MPLS PE-CE link, every mid-tier carrier {{igp|IGP}}. The trick is elegant: every router floods a description of its own links to every other router in the area, every router holds an **identical** {{lsdb|Link State Database (LSDB)}}, and every router independently runs **Dijkstra's shortest-path-first algorithm** on that database to compute its {{routing-table|routing table}}. No router trusts another's path computation; they all derive the same answer from the same graph.

[[ospf|OSPFv2]] ([[rfc:2328|RFC 2328]], April 1998, edited by [[pioneer:john-moy|John Moy]] at Ascend) is **STD 54** — still the canonical [[ip|IPv4]] spec, 244 pages, unchanged at the level of {{frame|frame}} format. OSPFv3 ([[rfc:5340|RFC 5340]], 2008) carries [[ipv6|IPv6]] and — via {{rfc-doc|RFC}} 5838 — [[ip|IPv4]] as separate address families. Everything modern (Segment Routing, Flex-Algo, SRv6, {{bfd|BFD}} Strict-Mode) is layered on through Opaque {{lsa|LSAs}} and Router Information {{lsa|LSA}} TLVs, not by rewriting the protocol. [[ospf|OSPF]] runs directly on [[ip|IP]] as protocol number 89 — no [[tcp|TCP]], no [[udp|UDP]] — and uses {{link-local|link-local}} {{multicast|multicast}} \`224.0.0.5\` / \`FF02::5\` for {{adjacency|adjacency}}.

[[pioneer:radia-perlman|Radia Perlman]]'s parallel design **{{isis|IS-IS}}** dominates tier-1 {{isp|ISP}} backbones; hyperscaler data-center fabrics increasingly skip both in favour of EBGP everywhere ({{rfc-doc|RFC}} 7938). But for the campus, the branch, the enterprise {{wan|WAN}}, and the mid-tier provider — [[ospf|OSPF]] is the protocol that draws the map.`,
	howItWorks: [
		{
		...
		},
		{
			...
			description:
				'Neighbours progress through eight states: **Down → Init → 2-Way → ExStart → {{exchange|Exchange}} → Loading → Full** (with **Attempt** for NBMA). On {{broadcast|broadcast}} networks (e.g. [[ethernet|Ethernet]]) routers elect a **Designated Router ({{ospf-dr|DR}})** and Backup DR — the DR is the only neighbour every router on the segment becomes Full with, cutting the {{adjacency|adjacency}} mesh from O(n²) to O(n).'
		},
		{
		...
		},
		{
			title: 'Run Dijkstra',
			description:
				'Once the {{lsdb|LSDB}} is identical on every router, each runs the **shortest-path-first algorithm** ([[pioneer:edsger-dijkstra|Edsger Dijkstra]], 1956) on its own copy and installs the resulting tree into its forwarding table. Areas (Area 0 is the backbone) keep LSDBs scoped so the {{spf|SPF}} computation stays tractable — typical campus areas are 100–200 routers, {{lsdb|LSDB}} < 1 MB.'
		},
		{
			title: 'Stay converged — BFD, authentication, segment routing',
			description:
				'Modern deployments pair [[ospf|OSPF]] with **{{bfd|BFD}}** (300 ms × 3 multipliers for sub-second loss detection — {{rfc-doc|RFC}} 9355 Strict-Mode requires {{bfd|BFD}} up before {{adjacency|adjacency}} forms) and **{{hmac|HMAC}}-{{sha256|SHA-256}}** authentication ({{rfc-doc|RFC}} 5709 for v2, RFC 7166 Authentication Trailer for v3). Segment Routing (RFC 8665/8666) and SRv6 (RFC 9513) ride on top via Opaque LSAs; Flex-Algo ([[rfc:9350|RFC 9350]]) lets operators compute multiple parallel {{spf|SPF}} planes — by {{igp|IGP}} cost, by min-delay, by {{srlg|SRLG}} exclusion.'
		}
	],
	useCases: [
...
Total time on a tuned network: < 100 ms.`
	...
	image: {
		...
		caption:
			'The [[ospf|OSPF]] {{adjacency|adjacency}} process — eight states from `Down` to `Full`. Every {{adjacency|adjacency}} on every router walks this diagram every time a link comes up. Sticking in `ExStart` or `Loading` is the classic [[ospf|OSPF]] debugging signature.',
	...
	recentChanges: [
		{
			...
			description:
				'Lets operators compute **multiple parallel {{spf|SPF}} planes** per [[ospf|OSPF]]/{{isis|IS-IS}} area, each with its own metric ({{igp|IGP}} cost, TE cost, min-delay) and constraints (include/exclude admin groups, exclude SRLGs). Carried in the new Flexible Algorithm Definition {{tlv|TLV}} in the Router Information {{lsa|LSA}}.',
		...
		},
		{
			...
			description:
				'Targets [[ospf|OSPF]]/{{isis|IS-IS}} scaling in spine-leaf {{ai|AI}} fabrics: instead of every router flooding to every neighbour, the area elects a leader that picks a flooding *subgraph*. The control plane stops melting when you stack 1024 spines.',
		...
		},
		{
			...
			description:
				'A malformed Extended Prefix Opaque {{lsa|LSA}} could crash the [[ospf|OSPF]] daemon in `ospf_ext.c` ({{frr|FRR}} v4.0 through v10.4.1). Fixed upstream in PR #19480. Affects every {{linux|Linux}} distribution shipping {{frr|FRR}} — patch immediately if your edge routers run {{frr|FRR}}.',
	...
	realWorldDeployments: [
		...
		},
		{
			...
			description:
				'Both Cumulus {{linux|Linux}} and {{nvidia|NVIDIA}} SONiC ship [[ospf|OSPF]] via {{frr|FRR}} as the canonical alternative to "{{bgp-hijack|BGP}}-in-the-{{dc-domain-controller|DC}}" ({{rfc-doc|RFC}} 7938) — common in mid-scale fabrics where operators want a topology-aware {{igp|IGP}} without learning {{isis|IS-IS}}.'
		},
		{
			org: 'FRRouting',
		...
		},
		{
			...
			description:
				"Both ship [[ospf|OSPF]] with full Segment Routing ({{rfc-doc|RFC}} 8665/8666), Flex-Algo ([[rfc:9350|RFC 9350]]), and {{bfd|BFD}} Strict-Mode ({{rfc-doc|RFC}} 9355). {{cisco|Cisco}}'s {{ios-xr|IOS-XR}} has shipped Flex-Algo since 2019; {{junos|Junos}} {{os|OS}} Evolved caught up in 24.2R1 (Aug 2024)."
		}
	],
	...
	funFacts: [
		{
			...
			text: 'Common misreading: {{ospf-hello|OSPF}} is "first-fit" or "fastest path". In fact, **Shortest Path First** was the popular name in the 1980s for [[pioneer:edsger-dijkstra|Dijkstra]]\'s algorithm. The protocol is named after the algorithm, not the routing strategy.'
		},
		{
			title: 'RFC 2328 is the longest unmodified Internet Standard',
			text: '[[rfc:2328|RFC 2328]] (OSPFv2, April 1998) is **244 pages** — STD 54 — and has been the canonical spec for 28 years. The frame format and core algorithm have not changed. Everything new since (Segment Routing, Flex-Algo, SRv6, {{bfd|BFD}} Strict-Mode) is layered on through Opaque LSAs and Router Information TLVs.'
		},
		{
			title: 'OSPF has reliable delivery without TCP',
			text: '[[ospf|OSPF]] runs **directly on [[ip|IP]]** as protocol number 89. There is no [[tcp|TCP]] underneath. Reliability is implemented in [[ospf|OSPF]] itself: every {{lsu|LSU}} is explicitly acknowledged with an {{lsack|LSAck}}; LSAs carry sequence numbers and checksums; the {{dbd|DBD}} {{exchange|exchange}} uses an Init/More/Master-Slave bit. [[ospf|OSPF]] is one of the few production protocols to re-implement transport-layer guarantees above raw [[ip|IP]].'
		},
		{
			title: 'Radia Perlman designed the competitor, not OSPF',
			text: '[[pioneer:radia-perlman|Radia Perlman]] — the inventor of Spanning Tree and "Mother of the Internet" — designed **{{isis|IS-IS}}**, the link-state {{igp|IGP}} that competed with [[ospf|OSPF]] and won the tier-1 {{isp|ISP}} backbone. The Lemelson-{{mit|MIT}} Program credits her with developing the algorithms that make *both* {{isis|IS-IS}} and [[ospf|OSPF]] efficient and scalable.'
		}
	],
	...
	practicalWisdom: {
		pitfalls: [
			...
			},
			{
				...
				text: 'A 40 s DeadInterval means a link failure takes ~40 s to detect — unacceptable on any modern backbone. **Cure:** pair [[ospf|OSPF]] with **{{bfd|BFD}}** (default 300 ms × 3 multiplier = ~900 ms detection), or use {{rfc-doc|RFC}} 9355 {{bfd|BFD}} Strict-Mode which refuses {{adjacency|adjacency}} until {{bfd|BFD}} is up. Sub-second {{failover|failover}} is the modern baseline.'
			},
			{
