...
export const zigbee: Protocol = {
	...
	oneLiner:
		'IEEE 802.15.4-based low-power mesh networking — the radio under every Philips Hue bulb, IKEA Trådfri light, and Walmart shelf label on Earth.',
	overview: `[[zigbee|Zigbee]] is the upper-layer protocol stack — NWK, APS, ZDO, ZCL — that the Connectivity Standards Alliance (CSA, formerly the **Zigbee Alliance**, founded August 2002 by Invensys, Mitsubishi, Motorola, Philips, Samsung, and Honeywell) built on top of the IEEE 802.15.4 PHY/MAC. The 2.4 GHz band offers 16 channels of 250 kbit/s O-QPSK; 868 MHz (Europe) and 902–928 MHz (NA) sub-GHz PHYs give longer range at lower bit rates. Topologies are star, tree, and **mesh** — every mains-powered device routes for its sleepy neighbours, and the network heals itself when a router drops out. 127-byte PHY payloads, ~80–100 byte usable APS payloads, and AES-128-CCM* security across both the network and application layers.
...
The current spec is **Zigbee PRO 2023 (R23)**, document 05-3474-23, ratified 15 March 2023. Headline additions: **Dynamic Link Key** negotiation using SPEKE over Curve25519 (no more "ZigBeeAlliance09" default key as the only fallback), **Trust Center Swap-Out** so a hub failure no longer means re-joining every device, **Device Interview** for policy-gated joins, sub-GHz support, and **Zigbee Direct** (a phone-as-coordinator BLE bootstrap, mandatory in R23 Coordinators). Above the radio, the CSA has spent the last six years migrating Zigbee's application semantics — the **ZCL** data model, originally renamed "Dotdot" in 2017 — onto IP, producing **Matter** (October 2022). Matter does not replace Zigbee on the wire; it *bridges* it. Signify's September 2023 Hue Bridge Matter firmware turned ~30M Hue bulbs into Matter accessories overnight while keeping the Zigbee mesh underneath. The right framing in 2026: Zigbee is being preserved as a bridged legacy radio while new device design moves to [[wifi|Wi-Fi]] or Thread.`,
	howItWorks: [
		{
			title: 'IEEE 802.15.4 PHY — 2.4 GHz O-QPSK at 250 kbit/s + sub-GHz',
			description:
				"All Zigbee variants ride IEEE 802.15.4-2020. The dominant PHY is **2.4 GHz O-QPSK** with 16 channels (11–26), 5 MHz spacing, 250 kbit/s data rate. Sub-GHz PHYs at 868 MHz (Europe, 20 kbit/s BPSK) and 902–928 MHz (NA, 40 kbit/s BPSK) give better wall penetration and less [[wifi|Wi-Fi]] co-channel interference at lower bit rates; R23 adds FSK PHYs in those same bands. Frames are bounded at **127 octets PSDU** with a **16-bit FCS**. The MAC uses unslotted CSMA-CA in most deployments (FFDs as routers, RFDs as end-devices), with optional beacon-mode for low-power sensor networks."
		},
		{
			title: 'NWK layer — mesh routing with AODV',
			description:
				"Zigbee's Network layer sits on top of 802.15.4 MAC and runs **AODV-style on-demand mesh routing** (since R20, 2007). Every mains-powered router maintains a route table and can repair broken paths reactively; concentrators (typical: the Coordinator) use source routing for many-to-one traffic, eliminating per-route discovery on each upstream send. The 8-bit Radius field acts as a TTL. **Frequency-agility** lets the Coordinator order the whole network to hop to a new channel if interference exceeds threshold — invaluable for [[wifi|Wi-Fi]] coexistence on 2.4 GHz. Frames are addressed by 16-bit short addresses (locally unique) or 64-bit EUI-64 (globally unique)."
		},
		{
			title: 'APS layer — Application Support Sublayer with endpoints and clusters',
			description:
				"The APS layer multiplexes application traffic onto **endpoints** (1–240, like sub-addresses on a device — endpoint 11 might be a bulb, endpoint 1 a switch). Frames carry a **ProfileID** (Home Automation = 0x0104, Smart Energy = 0x0109), a **ClusterID** (OnOff = 0x0006, Level Control = 0x0008, Color Control = 0x0300, OTA Upgrade = 0x0019), and an **APSCounter** for replay protection. APS-level AES-128-CCM* encryption is layered on top of NWK encryption when sensitive (e.g. Trust Center Transport Key commands). The APS layer also handles binding tables — persistent destination-routing for groupcast lights and similar."
		},
		{
			title: 'ZCL — the Zigbee Cluster Library data model',
			description:
				"ZCL is what makes Zigbee a *consumer* protocol rather than a generic mesh. Each **cluster** is a small object with attributes (e.g. OnOff.OnOff is a boolean) and commands (OnOff.Toggle = 0x02). A modern Hue bulb implements OnOff (0x0006), Level Control (0x0008), Color Control (0x0300), Identify (0x0003), Groups (0x0004), Scenes (0x0005), OTA Upgrade (0x0019), and a dozen others. ZCL is the same data model that Matter uses on IP — **Matter is essentially ZCL on IPv6** — which is why a Hue bulb's behaviour translates one-to-one across the Matter Bridge that Signify shipped on 19 September 2023."
		},
		{
			title: 'Trust Center + install codes — securing the join',
			description:
				"At commissioning time a new device joins by exchanging a 16-byte AES-128 **pre-configured link key** with the **Trust Center** (usually the Coordinator). The Trust Center then sends the network key in an APS Transport-Key command encrypted under that link key. The default link key — `5A:69:67:42:65:65:41:6C:6C:69:61:6E:63:65:30:39` = ASCII *ZigBeeAlliance09* — is universally known, which is why sniffer-at-join attacks recover the network key from any default-key join. **Install codes** (a per-device 128-bit secret printed on the device, mandatory in Zigbee 3.0 commissioning) close that hole; **R23's Dynamic Link Key** with SPEKE over Curve25519 eliminates pre-shared secrets entirely."
		},
		{
			title: 'Matter bridge — how Zigbee gets to the rest of the building',
			description:
				"The CSA's current strategy is to keep Zigbee on the wire and bridge it to **Matter** at the gateway. The Hue Bridge translates Matter cluster commands into Zigbee ZCL groupcasts; Matter 1.3 (May 2024) added **command batching** specifically to fix the *popcorn effect* — bulbs lighting one-by-one — when a single Matter scene-set fanned out into per-bulb Zigbee writes. **Aqara Hub M3** (CES 2024, shipped 8 May 2024) is the canonical triple-radio bridge: Zigbee 3.0 + Thread border router + Matter controller, with 8 GB encrypted local storage and *no* microphone or camera. For the FOSS path the answer is **zigbee2mqtt** by Koen Kanters — 5,390 devices from 568 vendors supported as of May 2026 — bridging Zigbee onto MQTT for Home Assistant and friends."
		},
		{
	...
	useCases: [
		...
		'Phone-as-coordinator commissioning via Zigbee Direct (R23, BLE GATT bootstrap with SPEKE/Curve25519)',
		'Utility AMI — Zigbee Smart Energy 1.x in-home displays + meter endpoints (PG&E, CenterPoint, Itron)',
		'Bridged Matter accessories — Hue Bridge, Aqara Hub M3, SmartThings Station turn Zigbee into Matter'
	],
	codeExample: {
		language: 'python',
...
That entire on-the-wire dump fits in roughly 40 bytes — a single Hue bulb command.`
	...
	recentChanges: [
		...
		},
		{
			...
			description:
				"Public rollout **19/20 September 2023** of Matter firmware for the second-generation square Hue Bridge after a Q1 2023 target slipped. The update exposes existing Hue Zigbee bulbs to Apple Home, Google Home, Amazon Alexa, and SmartThings over IP, while preserving the Zigbee mesh underneath. This single update made the Hue Bridge by far the largest Matter Bridge installation in the world. Signify has continued to ship Bridge and Bridge Pro firmware updates through 2024–26.",
			source: {
		...
		},
		{
			...
			description:
				"Released **11 August 2025**. Matter 1.4.2 adds **Wi-Fi-only commissioning** — devices can be set up without a Bluetooth Low Energy radio, reducing BOM cost for cheaper Matter-bridged accessories. Also adds **Quieter Reporting** (battery-saving data-model change) and admin verification. For the Zigbee story, this means a Matter Bridge for Zigbee no longer needs BLE silicon — accelerating low-cost bridges from secondary brands.",
			source: {
		...
		},
		{
			...
			description:
				"Released **20 November 2025**. Matter 1.5 adds **camera streaming** via an RTSP side-channel — the long-awaited piece for Matter to fully displace Zigbee in new home-security deployments. For Zigbee, this is the moment new device categories (cameras, video doorbells) no longer have a reason to go through a Zigbee bridge — they can go directly on [[wifi|Wi-Fi]] or Thread with Matter from day one.",
			source: {
	...
	realWorldDeployments: [
		...
		},
		{
			...
			description:
				"Shenzhen-based, originally an Apple HomeKit ecosystem partner. The **Aqara Hub M3** (announced CES 8 January 2024, shipping 8 May 2024) is the textbook modern multi-protocol bridge: Zigbee 3.0 + Thread border router + Matter controller and bridge, dual-band [[wifi|Wi-Fi]] (2.4/5 GHz, WPA3), PoE, USB-C, 8 GB encrypted local storage, **no microphone or camera**. The M3 supersedes older Aqara hubs and runs automations locally — an Edge-Hub model that does not depend on the Aqara cloud."
		},
		{
			org: 'Samsung SmartThings',
			scale: 'Tens of millions of hubs across Hub v1–v3 and Station',
			description:
				"SmartThings shipped the original Hub in 2013; Samsung acquired the company in August 2014. Carries Zigbee + Z-Wave from day one; later generations added Thread and Matter. SmartThings is the most common single Zigbee Coordinator outside of Hue. Samsung sits on the CSA board."
		},
		{
	...
	funFacts: [
		...
		},
		{
			...
			text: "Sixteen bytes of ASCII — `5A:69:67:42:65:65:41:6C:6C:69:61:6E:63:65:30:39` — baked into the specification of one of the most widely deployed wireless protocols on Earth, used as the fallback when a device has no install code. Generations of Wireshark users have memorised that hex string. The fact that Zigbee 3.0 requires install codes for proper commissioning while simultaneously retaining this key for compatibility is the protocol's most-quoted security cautionary tale. R23's Dynamic Link Key with SPEKE/Curve25519 finally retires the pre-shared default."
		},
		{
	...
	practicalWisdom: {
		pitfalls: [
			...
			},
			{
				...
				text: "Until R23's Trust Center Swap-Out, losing the Coordinator usually meant rebuilding the entire network. Even with R23 you should: **(a)** take regular backups of the coordinator's NV (zigbee2mqtt writes `coordinator_backup.json` automatically); **(b)** keep the Coordinator on UPS or PoE; **(c)** use a USB extension cable to keep the dongle away from USB 3.0 ports and SSDs, which emit 2.4 GHz noise; **(d)** consider redundant routing density so a single dropped device doesn't partition the mesh."
			},
			{
