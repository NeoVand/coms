...
The current spec is **Zigbee PRO 2023 (R23)**, document 05-3474-23, ratified 15 March 2023. Headline additions: **Dynamic Link Key** negotiation using SPEKE over Curve25519 (no more "ZigBeeAlliance09" default key as the only fallback), **{{trust-center|Trust Center}} Swap-Out** so a hub failure no longer means re-joining every device, **Device Interview** for policy-gated joins, sub-GHz support, and **Zigbee Direct** (a phone-as-coordinator {{ble|BLE}} bootstrap, mandatory in R23 Coordinators). Above the radio, the CSA has spent the last six years migrating Zigbee's application semantics — the **{{zcl|ZCL}}** data model, originally renamed "Dotdot" in 2017 — onto IP, producing **{{matter|Matter}}** (October 2022). Matter does not replace Zigbee on the wire; it *bridges* it. Signify's September 2023 Hue Bridge Matter firmware turned ~30M Hue bulbs into Matter accessories overnight while keeping the Zigbee mesh underneath. The right framing in 2026: Zigbee is being preserved as a bridged legacy radio while new device design moves to [[wifi|Wi-Fi]] or Thread.`,
	howItWorks: [
		{
