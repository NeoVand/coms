...
export const nfcTap: SimulationConfig = {
	...
	steps: [
		...
		},
		{
			...
			layers: [
				createNFCALayer({
					...
					direction: 'PICC → PCD',
					payload: '0x04 0x00 (ATQA — 4-byte UID, std anti-collision)',
					crc: '(none)'
				})
			]
		},
		{
			...
			layers: [
				createNFCALayer({
					...
					direction: 'PCD ↔ PICC (multi-frame exchange)',
					payload: 'SEL=0x93 NVB=0x20 → UID=04 1A 2B 3C BCC=4D → SEL=0x93 NVB=0x70 ... → SAK=0x28',
		...
		},
		{
			...
			layers: [
				createNFCALayer({
					frame: 'Full frame (CRC_A protected)',
					direction: 'PCD ↔ PICC',
					payload: 'RATS=E0 80 → ATS=06 75 33 81 02 80 (FSCI=5, FWI=3, CID supported)',
					crc: 'CRC_A'
				})
			]
		},
		...
		},
		{
			...
			layers: [
				createAPDULayer({
					header: '80 A8 00 00',
					lc: '23',
					data: '83 21 ...PDOL filled: amount £28.50, currency GBP, country UK, TVR, UN...',
					le: '00',
		...
		},
		{
			...
			layers: [
				createAPDULayer({
					header: '80 AE 80 00',
					lc: '1F',
					data: '...CDOL1 data: amount £28.50, currency GBP, TVR, UN 0x9C2A3B4D...',
					le: '00',
					response:
		...
		},
		{
			...
			layers: [
				createAPDULayer({
					header: '(issuer round-trip via acquirer + payment network — not in-field)',
					lc: '—',
					data: 'ARQC → issuer HSM → ARPC (APPROVED, ARC=00)',
					le: '—',
	...
	steps: [
		{
			...
			layers: [
				createNFCALayer({
					frame: 'Short → standard frames',
					direction: 'PCD → PICC → PCD',
					payload: 'REQA 0x26 → ATQA 0x00 0x44 (7-byte UID)',
					crc: '(none / short)'
				})
			]
		},
		{
			...
			layers: [
				createNFCALayer({
					...
					direction: 'PCD ↔ PICC',
					payload: 'CL1 → UID-CL1 (with CT=0x88) + BCC → CL2 → UID-CL2 + BCC → SAK 0x00 (T2T)',
					crc: 'CRC_A'
				})
			]
		},
		{
			...
			layers: [
				createNFCALayer({
					...
					direction: 'PCD ↔ PICC',
					payload: 'READ 0x30 0x03 → 16 bytes: E1 10 6D 00 (CC) + first NDEF page',
					crc: 'CRC_A'
				})
			]
		},
		...
		},
		{
			...
			layers: [
				createNDEFLayer({
					...
					payloadLen: 21,
					type: "'U' (URI)",
					payload: 'https://transit.example.com/qr → handed to default browser'
				})
			]
		}
	]
