...
- Distinct pioneers mentioned without `[[pioneer:…]]` wrap: **29** (of 61 catalogued)
- Distinct concepts mentioned without `{{…}}` wrap: **116** (of 328 catalogued)
...
- L215:180 — `[[pioneer:jim-kardach|Jim Kardach]]`  · _ho united Denmark and Norway. Jim Kardach at Intel proposed the name as_
...
**`src/lib/data/book/parts/async-iot.ts`** — 2 term(s)
...
**`src/lib/data/book/parts/famous-outages.ts`** — 4 term(s)
...
- L304 — `{{matter|matter}}`  · _is encrypted, so what does it matter if a third party sees it?" Tw_

**`src/lib/data/book/parts/frontier.ts`** — 7 term(s)
...
- L294 — `{{spectrum|Spectrum}}`  · _cs in Quantum-X Photonics and Spectrum-X Photonics.', attributi_

**`src/lib/data/book/parts/how-to-learn-more.ts`** — 1 term(s)
- L50 — `{{arpanet|ARPANET}}`  · _t_1974.svg.png', alt: 'ARPANET in 1974 — the network the fir_

**`src/lib/data/book/parts/layer-2-3.ts`** — 6 term(s)
...
**`src/lib/data/book/parts/patterns-failures.ts`** — 5 term(s)
...
**`src/lib/data/book/parts/realtime-av.ts`** — 2 term(s)
...
**`src/lib/data/book/parts/story-of-the-internet.ts`** — 3 term(s)
...
**`src/lib/data/book/parts/transport.ts`** — 3 term(s)
...
**`src/lib/data/book/parts/utilities-security.ts`** — 5 term(s)
...
**`src/lib/data/book/parts/web-api.ts`** — 8 term(s)
...
**`src/lib/data/book/parts/wireless.ts`** — 20 term(s)
...
- L112 — `{{airtime|airtime}}`  · _t; the delivered number is an airtime budget.', attribution: '_
- L168 — `{{wpa3|WPA3}}`  · _Dragonblood, and the road to WPA3', text: `WPA2's four-w_
...
- L566 — `{{airtime|airtime}}`  · _s, retries rise, which raises airtime utilisation, which raises ret_

**`src/lib/data/category-stories/network-foundations.ts`** — 3 term(s)
...
**`src/lib/data/category-stories/realtime-av.ts`** — 2 term(s)
...
**`src/lib/data/category-stories/web-api.ts`** — 1 term(s)
- L141 — `{{hpack|HPACK}}`  · _title: 'Co-creator of SPDY & HPACK', org: 'Google', co_

**`src/lib/data/category-stories/wireless.ts`** — 4 term(s)
...
**`src/lib/data/comparison/pairs.ts`** — 29 term(s)
...
**`src/lib/data/concept-foundations.ts`** — 16 term(s)
...
**`src/lib/data/diagram-definitions.ts`** — 46 term(s)
...
**`src/lib/data/frontier.ts`** — 3 term(s)
...
**`src/lib/data/journeys.ts`** — 10 term(s)
...
- L511 — `{{codec|codec}}`  · _EVC, VP9, AV1, and any future codec without protocol changes. It_
- L187 — `{{broadcast|broadcast}}`  · _o subsequent packets skip the broadcast entirely. You can see your ow_
...
- L615 — `{{matter|matter}}`  · _ncy\|latency}} and type safety matter more than human readability._

**`src/lib/data/outages.ts`** — 4 term(s)
...
**`src/lib/data/protocols/bgp.ts`** — 3 term(s)
...
**`src/lib/data/protocols/bluetooth.ts`** — 11 term(s)
...
- L42 — `{{broadcast|broadcast}}`  · _nd for BIS-based public-venue broadcast — airports, theatres, gyms, h_
...
**`src/lib/data/protocols/cellular.ts`** — 5 term(s)
...
- L24 — `{{mmwave|mmWave}}`  · _t regimes (FR1 sub-6 GHz, FR2 mmWave 24–52 GHz)." }, { titl_
...
**`src/lib/data/protocols/dns.ts`** — 4 term(s)
...
**`src/lib/data/protocols/grpc.ts`** — 2 term(s)
...
**`src/lib/data/protocols/hls.ts`** — 1 term(s)
...
**`src/lib/data/protocols/ipsec.ts`** — 5 term(s)
...
**`src/lib/data/protocols/kerberos.ts`** — 4 term(s)
...
**`src/lib/data/protocols/mdns-dns-sd.ts`** — 9 term(s)
...
- L224 — `{{thread|Thread}}`  · _e to a registrar (typically a Thread {{border-router\|Border Router_
...
**`src/lib/data/protocols/nat-traversal.ts`** — 8 term(s)
...
**`src/lib/data/protocols/nfc.ts`** — 7 term(s)
...
**`src/lib/data/protocols/oauth2.ts`** — 1 term(s)
- L223 — `{{latency|latency}}`  · _} ] }, performance: { latency: 'Authorization flow: 1-3_

**`src/lib/data/protocols/ospf.ts`** — 2 term(s)
...
**`src/lib/data/protocols/sdp.ts`** — 2 term(s)
...
**`src/lib/data/protocols/tcp.ts`** — 7 term(s)
...
**`src/lib/data/protocols/tls.ts`** — 5 term(s)
...
- L22 — `{{bandwidth|bandwidth}}`  · _dth}} and the ≥0.2 fractional-bandwidth requirement is what makes a U_
...
**`src/lib/data/protocols/webrtc.ts`** — 4 term(s)
...
**`src/lib/data/protocols/wireguard.ts`** — 5 term(s)
...
- L37 — `{{matter|Matter}}`  · _nslates one-to-one across the Matter Bridge that Signify shipped o_
