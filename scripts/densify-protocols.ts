 ...
 * in "IPv6").
 */
function applyAlias(text: string, id: string, alias: string, stat: WrapStat): string {
...
function densifyString(src: string, stats: Map<string, WrapStat>): string {
	...
	for (const entry of ALIASES) {
		...
		for (const alias of entry.aliases) {
			...
			if (lastIdx < work.length) out.push(applyAlias(work.slice(lastIdx), entry.id, alias, stat));
			work = out.join('');
		}
	}
	return work;
...
function parseArgs(argv: string[]): { apply: boolean; files: string[] } {
	...
	for (const a of argv.slice(2)) {
		if (a === '--apply') args.apply = true;
			...
			console.log(`Usage: npx tsx scripts/densify-protocols.ts [--apply] [--files=path,path]`);
			process.exit(0);
		}
	}
	return args;
...
function main() {
	...
	for (const f of targets) {
		...
		if (result.changed) {
			...
			for (const s of top) {
				console.log(
					`  ${s.id.padEnd(10)} ${String(s.count).padStart(4)}   e.g. ${s.samples[0] ?? ''}`
				);
			}
			if (apply) {
